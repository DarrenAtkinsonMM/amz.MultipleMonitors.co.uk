jQTubeUtil = function (h) {
	gdataPrefix = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'gdata.youtube.com';
	googlePrefix = ('https:' == document.location.protocol ? 'https://' : 'http://') + 'suggestqueries.google.com';
	...
	var s = function () { },
			...
			l = 10,
			u = 1,
			w = gdataPrefix + "/feeds/api/videos";
	MostPopular = gdataPrefix + "/feeds/api/standardfeeds/most_popular";
	MostRecent = gdataPrefix + "/feeds/api/standardfeeds/most_recent";
	TopRated = gdataPrefix + "/feeds/api/standardfeeds/top_rated";
	TopFavs = gdataPrefix + "/feeds/api/standardfeeds/top_favorites";
	RecentlyFeatured = gdataPrefix + "/feeds/api/standardfeeds/recently_featured";
	SuggestURL = googlePrefix + "/complete/search";
	...
	f.video = function (a, b) {
		return n(gdataPrefix + "/feeds/api/videos/" + a + "?alt=json", b)
	};
	f.mostViewed =
			function (a, b) {
				return i(gdataPrefix + "/feeds/api/standardfeeds/most_viewed", j(a, true), b)
			};
