...
Class Uploader
	...
	Public Function IsAllowed(ext)
		...
		If(InStr(1, "|" & lcase(Me.allowedTypes) & "|", "|" & lcase(ext) & "|", 1)<>0) then
			IsAllowed = True
		End If
	...
	Public Sub Save(path)
		...
		For Each fileItem In UploadedFiles.Items
			...
			'// ProductCart Mod: Add fileItem.FileName for filename
			If Not IsUploadAllowed(fileItem.FileName) Then
				UploadErrorCode = "FILENAME"
				UploadErrorFile = fileItem.FileName
				Exit Sub
			End If
    ...
    set fs = Server.CreateObject("Scripting.FileSystemObject")
		...
		if UploadedFiles.Count > 0 then
			...
			'// ProductCart Mod: Add sanitation for filename
			If Not IsUploadAllowed(outFileName) Then
				UploadErrorCode = "FILENAME"
				UploadErrorFile = fileItem.FileName
				Exit Sub
			End If
	...
	Private Function ConvertBinaryToString(start, length)
		...
		strTmp = objStream.Read(length)
		objStream.Close
		Set objStream = Nothing
		
		outStr = ""
		For i = 1 to length
			outStr = outStr & Chr(AscB(MidB(strTmp,i,1)))
		Next
		
		ConvertBinaryToString = outStr
	End Function
