...
session("redirectPage")="https://www.multiplemonitors.co.uk/shop/pc/manpay2.asp"
 pcStrPageName = "manpay2"
...
If scSSL="" OR scSSL="0" Then
	tempURL=replace((scStoreURL&"/"&scPcFolder&"/pc/manpay2.asp"),"//","/")
	tempURL=replace(tempURL,"https:/","https://")
	tempURL=replace(tempURL,"http:/","http://") 
Else
	tempURL=replace((scSslURL&"/"&scPcFolder&"/pc/manpay2.asp"),"//","/")
	tempURL=replace(tempURL,"https:/","https://")
	tempURL=replace(tempURL,"http:/","http://")
End If
...
<%
 ...
 pcIdCustomer=1
pcBillingFirstName=request("mpBillFirstName")
pcBillingLastName=request("mpBillLastName")
pcBillingPhone=request("mpBillTel")
pcCustomerEmail=request("mpBillEmail")
pcBillingAddress=request("mpBillAddress1")
pcBillingAddress2=request("mpBillAddress2")
pcBillingAddress3=request("mpBillAddress3")
pcBillingCity=request("mpBillCity")
pcBillingPostalCode=request("mpBillPCode")
pcBillingStateCode="FL"
pcBillingCountryCode=request("mpBillCountry")
 
pcShippingFirstName=request("mpDelFirstName")
pcShippingLastName=request("mpDelLastName")
pcShippingPhone=request("mpDelTel")
pcShippingAddress=request("mpDelAddress1")
pcShippingAddress2=request("mpDelAddress2")
pcShippingAddress3=request("mpDelAddress3")
pcShippingCity=request("mpDelCity")
pcShippingPostalCode=request("mpDelPCode") 
pcShippingStateCode="FL"
pcShippingCountryCode=request("mpDelCountry")

pcBillingTaxAmount=0
pcBillingTotal=request("mpAmount")
pcPaymentDesc=request("mpDesc")
...
'SB E
 ...
 'Session Variables
  Session("mpBillFirstName") = mpBillFirstName
  Session("mpBillLastName") = mpBillLastName
  Session("mpBillTel") = mpBillTel
  Session("mpBillEmail") = mpBillEmail
  Session("mpBillAddress1") = mpBillAddress1
  Session("mpBillAddress2") = mpBillAddress2
  Session("mpBillAddress3") = mpBillAddress3
  Session("mpBillCity") = mpBillCity
  Session("mpBillPCode") = mpBillPCode
  Session("mpBillCountry") = mpBillCountry
  
  Session("mpDelFirstName") = mpDelFirstName
  Session("mpDelLastName") = mpDelLastName
  Session("mpDelTel") = mpDelTel
  Session("mpDelAddress1") = mpDelAddress1
  Session("mpDelAddress2") = mpDelAddress2
  Session("mpDelAddress3") = mpDelAddress3
  Session("mpDelCity") = mpDelCity
  Session("mpDelPCode") = mpDelPCode
  Session("mpDelCountry") = mpDelCountry
   
   'Build Card Address for Display
	strCardBillAdd = "<p>" & pcBillingFirstName & " " & pcBillingLastName &"</p>"
	strCardBillAdd = strCardBillAdd & "<p>" & pcBillingAddress & "</p>"
	if pcBillingAddress2 <> "" Then
		strCardBillAdd = strCardBillAdd & "<p>" & pcBillingAddress2 & "</p>"
	end if 
	if pcBillingAddress3 <> "" Then
		strCardBillAdd = strCardBillAdd & "<p>" & pcBillingAddress3 & "</p>"
	end if 	
	strCardBillAdd = strCardBillAdd & "<p>" & pcBillingCity & ", " & pcShippingPostalCode & "</p>"
	if pcBillingCountryCode <> "GB" Then
		strCardBillAdd = strCardBillAdd & "<p>" & pcBillingCountryCode & "</p>"
	end if 	
...
If request.QueryString("crypt")<>"" then
	...
	If pcv_StrStatus="OK" then
		...
		Response.redirect "manpay-ok.asp?payid=" & request.querystring("pay")
	Else
		if pcv_strStatus = "ABORT" then 
			   StatusOutput = "You elected to cancel your online payment<BR>Any credit/debit card details you entered have not been sent to the bank. You will not be charged for this transaction. Press the Back To Payment Page button to try again."
		elseif pcv_strStatus = "NOTAUTHED" then 
			   StatusOutput = "SagePay was unable to authorise your payment<BR>Your bank would not authorise your selected method of payment. You will not be charged for this transaction. Press the Back To Payment Page button to try again."
		else
			   StatusOutput = "An error has occurred at SagePay<br>Because an error occurred in the payment process, you will not be charged for this transaction, even if an authorisation was given by the bank. Press the Back To Payment Page button to try again."
		end if

        call closeDb()
        Session("message") = StatusOutput
        Session("backbuttonURL") = "https://www.multiplemonitors.co.uk/shop/pc/manpayForm.asp?amount=" & request.querystring("amount") & "&pay=" & request.querystring("pay")
...
tempURL=replace(tempURL,"http:/","http://")
tempURL=replace(tempURL,"https:/","https://")
tempURL=tempURL & "?amount=" & pcBillingTotal & "&pay=" & pcPaymentDesc
...
if len(DescString)>0 then
	...
	DescString=pcPaymentDesc
end if
pcvPostString = pcvPostString + "Description="& DescString &"&"
pcvPostString = pcvPostString + "SuccessURL=" & Replace(tempURL, " ","-") & "&"
pcvPostString = pcvPostString + "FailureURL=" & Replace(tempURL, " ","-") & "&"
...
pcvPostString = pcvPostString + "BillingAddress3=" & left(pcBillingAddress3,100) & "&"
...
pcvPostString = pcvPostString + "DeliveryAddress3=" & left(pcShippingAddress3,100) & "&"
...
If Ucase(pcShippingCountryCode)="US" then
				...
				<div class="row">
					<div class="col-xs-12 pagetitle">
						<div class="wow fadeInDown" data-wow-offset="0" data-wow-delay="0.1s">
							<h3 class="color marginbot-0 h-semi" title="Card Payment Details">Quick Payment Page - Step: 2 / 3</h3>
						</div>
					</div>				
				</div>		
			</div>		
...
<div id="pcMain">
    ...
    <%= strCardBillAdd %>
    <p><a href="#" onclick="history.back(-1)">Edit</a></p>
</div>

          <div class="pcFormItem"> 
        <div class="daOPCCCPaySecTitle"><%=dictLanguage.Item(Session("language")&"_GateWay_4")%></div>
              <div class="pcFormField"><%= scCurSign & money(pcBillingTotal)%></div> 
          </div>

          <div class="pcAttention"><br />
			  <strong>NOTE:</strong> When you click on the 'Continue To Payment' button, you will temporarily leave our Web site and will be taken to a secure payment page on the SagePay Web site. SagePay are leaders in secure Internet payment processing, ensuring your transactions are processed securely and reliably.<br /><br />
			  By making a payment you agree to our <a href="/pages/terms/">terms and conditions</a> of sale.<br /><br />
          </div>
