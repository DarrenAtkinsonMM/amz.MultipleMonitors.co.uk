...
<div id="pcMain">
	<div class="pcMainContent">
		...
		<%
		query="SELECT x_Type,x_Login,x_Password,x_Curcode,x_AIMType,x_CVV,x_testmode,x_secureSource,x_accountType,x_eCheck FROM authorizeNet Where id=1"
		...
		If Request.Form("PaymentGWCentinel")="Go" OR request.QueryString("Centinel")<>"" Then %>
			...
			If pcPay_Cent_Active=1 AND pcPay_CentByPass=0 Then %>
				...
				If Not IsEmpty(centinelResponse) Then
				...
				Else
					...
					Else
						...
						If CStr(DEBUGOUTPUT) = "True" Then
							Response.redirect ("gwAuthorizeAIM.asp?centinel=Y")
						Else
							Session("message") = Session("Centinel_Message")
			...
			if pcBillingTotal > 0 then
				If request.QueryString("centinel")="Y"	Then
					...
					<div class="pcFormItem">
						<div class="pcFormLabel">Enrolled : </div>
						<div class="pcFormField"><%=Session("Centinel_Enrolled")%></div>
					</div>
					<div class="pcFormItem">
						<div class="pcFormLabel">PAResStatus :</div>
						<div class="pcFormField"><%=Session("Centinel_PAResStatus")%></div>
					</div>
					<div class="pcFormItem">
						<div class="pcFormLabel">SignatureVerification : </div>
						<div class="pcFormField"><%=Session("Centinel_SignatureVerification")%></div>
					</div>
					<div class="pcFormItem">
						<div class="pcFormLabel">Transaction Id :</div>
						<div class="pcFormField"><%=Session("Centinel_TransactionId")%></div>
					</div>
					<div class="pcFormItem">
						<div class="pcFormLabel">Order Id :</div>
						<div class="pcFormField"><%=Session("Centinel_OrderId")%></div>
					</div>
					<div class="pcFormItem">
						<div class="pcFormLabel">Error No :</div>
						<div class="pcFormField"><%=Session("Centinel_ErrorNo")%></div>
					</div>
					<div class="pcFormItem">
						<div class="pcFormLabel">Error Desc : </div>
						<div class="pcFormField"><%=Session("Centinel_ErrorDesc")%></div>
					</div>
					<div class="pcFormItem">
						<div class="pcFormLabel">Reason Code :</div>
						<div class="pcFormField"><%=Session("Centinel_ReasonCode#")%></div>
					</div>
					<div class="pcFormItem">
						<div class="pcFormLabel">Reason Desc : </div>
						<div class="pcFormField"><%=Session("Centinel_ReasonDesc#")%></div>
					</div>
				    <br /><br />
                ...
                If x_testmode="1" Or x_testmode="-1" Then
                ...
                stext=stext & "&x_first_name=" & replace(pcBillingFirstName,",","||")
                stext=stext & "&x_last_name=" & replace(pcBillingLastName,",","||")
                stext=stext & "&x_company=" & replace(pcBillingCompany,",","||")
                stext=stext & "&x_address=" & replace(pcBillingAddress,",","||")
                stext=stext & "&x_city=" & replace(pcBillingCity,",","||")
                stext=stext & "&x_state=" & replace(pcBillingState,",","||")
                stext=stext & "&x_zip=" & replace(pcBillingPostalCode,",","||")
                stext=stext & "&x_country=" & pcBillingCountryCode
                stext=stext & "&x_phone=" & replace(pcBillingPhone,",","||")
                ...
                stext=stext & "&x_Ship_To_First_Name=" & replace(pcShippingFirstName,",","||") '?
                stext=stext & "&x_Ship_To_Last_Name=" & replace(pcShippingLastName,",","||") '?
                    stext=stext & "&x_ship_to_company=" & replace(pcShippingCompany,",","||") '?
                stext=stext & "&x_Ship_To_Address=" & replace(pcShippingAddress,",","||")
                stext=stext & "&x_Ship_To_City=" & replace(pcShippingCity,",","||")
                stext=stext & "&x_Ship_To_State=" & replace(pcShippingState,",","||")
                stext=stext & "&x_Ship_To_Zip=" & replace(pcShippingPostalCode,",","||")
                ...
                'SB S
                if x_accountType = 1 Then
                    xml.open "POST", "https://test.authorize.net/gateway/transact.dll?"& stext & "", false
                else
                    xml.open "POST", "https://secure2.authorize.net/gateway/transact.dll?"& stext & "", false
                end if
                'SB E
                ...
                session("x_first_name") = replace(strArrayVal(13),"||",",")
                session("x_last_name") = replace(strArrayVal(14),"||",",")
                session("x_company") = replace(strArrayVal(15),"||",",")
                session("x_address") = replace(strArrayVal(16),"||",",")
                session("x_city") = replace(strArrayVal(17),"||",",")
                session("x_state") = replace(strArrayVal(18),"||",",")
                session("x_zip") = replace(strArrayVal(19),"||",",")
                session("x_country") = strArrayVal(20)
                session("x_phone") = replace(strArrayVal(21),"||",",")
                session("x_fax") = strArrayVal(22)
                session("x_email") = strArrayVal(23)
                session("x_ship_to_first_name") = replace(strArrayVal(24),"||",",")
                session("x_ship_to_last_name") = replace(strArrayVal(25),"||",",")
				session("x_ship_to_company") = replace(strArrayVal(26),"||",",")
				session("x_ship_to_address ") = replace(strArrayVal(27),"||",",")
				session("x_ship_to_city") = replace(strArrayVal(28),"||",",")
				session("x_ship_to_state") = replace(strArrayVal(29),"||",",")
				session("x_ship_to_zip") = replace(strArrayVal(30),"||",",")
				...
				Else
					...
					If (session("x_invoice_num")="") OR (IsNull(session("x_invoice_num"))) OR (session("x_invoice_num")<"0") then
						Response.Write "An error occurred during processing. Cannot get OrderID#. Please try again later."
						Response.end
					End if
					If Not(IsNumeric(session("x_invoice_num"))) then
						Response.Write "An error occurred during processing. Cannot get OrderID#. Please try again later."
						Response.end
					End if
					pcTrueOrdnum=(CLng(session("x_invoice_num"))-scpre)
					...
					ElseIf session("x_response_code")<>1 Then
						...
						ElseIf session("x_response_code") = "4" Then
							session("x_response_reason_text") = "This transaction has been declined. Please contact your credit card company for details."
						End If
                        ...
                        call closeDb()
                        Session("message") = "<b>Error code " & session("x_response_code") & ": " & session("x_response_reason_text")
                        Session("backbuttonURL") = tempURL & "?psslurl=" & session("redirectPage") & "&idCustomer=" & session("idCustomer") & "&idOrder=" & session("GWOrderId") & "&ordertotal=" & pcBillingTotal
                        response.redirect "msgb.asp?back=1"
					End If
				End If
		...
		Else
            ...
            <% if x_eCheck="1" then %>
            	<div><a href="gwAuthorizeCheck.asp" class="pcButton">Pay by Check</a></div>
            <% end if %>
            ...
            %>
                <div class="pcFormItem">
                    ...
                    <div class="pcFormField"><%= scCurSign & money((pcBillingTotal + pcBillingSubScriptionTotal))%></div>
                </div>
            <% Else %>
                <div class="pcFormItem">
            ...
            <div class="pcFormItem">
                ...
                <div class="pcFormField"><!--#include file="inc_sb_widget.asp"--></div>
            </div>
            <% End If
            'SB E %>
            ...
            If pcIsSubscription AND scSBaymentPageText <>"" Then %>
                ...
                <div class="pcFormItem">
                    ...
                    <div class="pcFormField"><%=scSBaymentPageText%></div>
                </div>
            ...
            <% If pcIsSubscription AND pcv_intIsTrial AND scSBPaymentPageTrialText <> "" Then %>
                ...
                <div class="pcFormItem">
                    ...
                    <div class="pcFormField"><%=scSBPaymentPageTrialText%></div>
                </div>
            <% 
            End if 
            'SB E
