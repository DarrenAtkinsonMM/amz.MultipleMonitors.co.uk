<%'Start SDBA - Send Low Inventory Notification
	...
	query="SELECT DISTINCT idproduct,sku,serviceSpec,pcProd_ReorderLevel,description,pcprod_ParentPrd FROM Products WHERE removed=0 AND ((Products.active<>0) OR ((Products.active=0) AND (Products.pcProd_ParentPrd>0) AND (Products.pcProd_SPInActive=0))) AND configOnly=0 AND pcProd_NotifyStock=1 AND stock<pcProd_ReorderLevel AND pcProd_SentNotice=0 AND noStock=0;"
	...
	IF NOT rs.eof THEN
		...
		For i=0 to intCount
			...
			pcprod_ParentPrd=pcArray(5,i)

			strURL=""
			strMsgBody=""

			if Clng(pcprod_ParentPrd)=0 then
				...
				strURL=strPathInfo & scAdminFolderName & "/login_1.asp?RedirectURL=" & Server.URLEnCode("app-subPrdsMngAll.asp?idproduct=" & pcprod_ParentPrd)
				strMsgBody=strMsgBody & pcv_prdName & " (product ID#" & pcv_idproduct & " - SKU #" & pcv_sku & ") needs to be restocked. The inventory level of sub-product(s) has dropped below the notification level of " & pcv_ReorderLevel & " units." & vbcrlf & vbcrlf
			end if
			...
			if Clng(pcprod_ParentPrd)>0 then
				query="UPDATE products SET pcProd_SentNotice=1 WHERE idproduct=" & pcprod_ParentPrd
				set rs=server.CreateObject("ADODB.RecordSet")
				set rs=connTemp.execute(query)
				set rs=nothing
			end if
