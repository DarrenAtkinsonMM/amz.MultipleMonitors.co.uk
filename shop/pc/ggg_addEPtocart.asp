...
For dd=1 to Count
	...
	geRemain=getUserInput(request("remain" & dd),0)
	if geRemain="" then
		geRemain="0"
	end if
	...
	IF (geID<>"") AND (clng(geadd)>0) then
		...
		lineNumber=0
		dim iNextIndex, iCapturedNext, iTempStockTotal
		iNextIndex=cint(0)
		iCapturedNext=cint(0)
		iTempStockTotal=cint(0)
		...
		if request.Form("imode")="updOrd" then
			...
			for f=1 to ppcCartIndex
				...
				if (pcCartArray(f,10)=0) and (pcCartArray(f,0)=int(trim(pIdProduct))) then
					...
					'// Check if cart quantity is greater than desired quantity
					iTempStockTotal=Int(pcCartArray(f,2))+Int(pQuantity)
					if Int(iTempStockTotal)>Int(geRemain) then
						call closeDb()
						'Set session variables to handle error on msg.asp
						session("pcErrStrPrdDesc") = pDescription
						session("pcErrIntRemain") = geRemain
						response.redirect "msg.asp?message=319"
					end if
					
					if scOutofstockpurchase=-1 AND pNoStock=0 AND pcv_intBackOrder=0 then
						for g=1 to ppcCartIndex
							if (pcCartArray(g,10)=0) and (pcCartArray(g,0)=int(trim(pIdProduct))) then
