...
Dim ItemsDiscounts, Charges, DefaultPrice, pIdOrder
...
IF request("OrderRepeat")<>"haveto" then
	...
	do while not rstemp.eof
		...
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		query="SELECT DISTINCT products.pcprod_ParentPrd,Products.serviceSpec,Products.stock,Products.noStock,Products.pcprod_minimumqty,Products.pcprod_qtyvalidate,pcProd_multiQty,Products.pcProd_BackOrder "
		...
		tmp3=tmp3 & " AND ((categories_products.idproduct=products.idproduct) OR (categories_products.idproduct=products.pcprod_ParentPrd)) AND categories.idcategory=categories_products.idcategory AND categories.iBTOhide=0 "
		...
		query = query & tmp1 & " from Products" & tmp2 & " WHERE products.idproduct=" & pidProduct & " AND products.removed=0 AND ((products.active<>0) or ((products.active=0) and (products.pcProd_SPInActive=0) and (products.pcProd_ParentPrd<>0))) " & queryNFSO & tmp3
		...
		IF not rs.eof THEN
			...
			tmpParentPrd=rs("pcprod_ParentPrd")
			...
			if (scOutofStockPurchase=-1 AND CLng(pStock)<1 AND pserviceSpec=0 AND pNoStock=0 AND pcv_BackOrder=0) OR (pserviceSpec<>0 AND scOutofStockPurchase=-1 AND iBTOOutofstockpurchase=-1 AND CLng(pStock)<1 AND pNoStock=0 AND pcv_BackOrder=0) then
				...
				if tmpParentPrd>"0" then
					
					query="SELECT formQuantity FROM Products WHERE idproduct=" & tmpParentPrd & ";"
					set rsQ=connTemp.execute(query)
					if not rsQ.eof then
						if rsQ("formQuantity")<>"0" and NotForSaleOverride(session("customerCategory"))=0 then
							pcv_OrdHaveOutStock=1
						else
							pcv_OrdHaveStock=1
						end if
...
'/////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	do while not rstemp.eof
		...
		'  Start: New Query with "rstemp". Do for each product!
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		query="SELECT DISTINCT products.pcprod_ParentPrd,products.serviceSpec,products.stock,products.noStock,products.pcprod_minimumqty,products.pcprod_qtyvalidate,products.pcProd_multiQty,Products.pcProd_BackOrder,Products.active "
		...
		IF not rs.eof THEN '// Start: Add and calculate products
			...
			if pcv_intActive<>-1 AND session("pcv_intAdminPreview")<>1 then
				set rs=nothing
				call closeDb()
				response.redirect "msg.asp?message=317"
			end if


			If statusAPP="1" Then

				SUBquery="SELECT Products.pcProd_Relationship from Products WHERE products.idproduct=" & pidProduct &";"
				Set SubQueryObj=Server.CreateObject("ADODB.RecordSet")
				Set SubQueryObj=connTemp.execute(SUBquery)
				pcv_Relationship=SubQueryObj("pcProd_Relationship")
				set SubQueryObj=nothing
			...
			IF (pcv_OrdHaveStock=0) OR (scOutofStockPurchase=-1 AND CLng(pStock)<1 AND pserviceSpec=0 AND pNoStock=0 AND pcv_BackOrder=0) OR (pserviceSpec<>0 AND scOutofStockPurchase=-1 AND iBTOOutofstockpurchase=-1 AND CLng(pStock)<1 AND pNoStock=0 AND pcv_BackOrder=0) THEN

				'// Do not do anything, its out of stock.
          ...
          if err.number<>0 then
			...
			if lineNumber=0 then
				...
				If statusAPP="1" Then
					if pcv_Relationship<>"" then
						pcv_HaveApparel=1
						pcApp=split(pcv_Relationship,"_")
						tmpStr=""
						For mk=1 to ubound(pcApp)
							tmpStr=tmpStr & pcApp(mk) & "$$"
						Next
						pcCartArray(ppcCartIndex,32)=tmpStr & pcApp(0)
						pcCartArray(ppcCartIndex,11) = ""
						pcCartArray(ppcCartIndex,12) = ""
					else
						pcCartArray(ppcCartIndex,32)=""
					end if
                    
                    '// Parent Product Id                   
                    pcCartArray(ppcCartIndex,43)=pcf_GetParentId(pIdProduct)
