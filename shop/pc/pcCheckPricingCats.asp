...
Public Function GetDefaultQty(pIdProduct,tempVarCat,tmpID)
Dim query,rs,tmp1

	query="SELECT products.idproduct,products.pcprod_minimumqty FROM products INNER JOIN configSpec_products ON products.idproduct=configSpec_products.configProduct WHERE configSpec_products.configProductCategory="&tempVarCat&" AND configSpec_products.specProduct="&pIdProduct&" AND configSpec_products.cdefault<>0 AND configSpec_products.configProduct=" & tmpID & ";"
	set rs=conntemp.execute(query)
	if rs.eof then
		query="SELECT products.idproduct,products.pcprod_minimumqty FROM products INNER JOIN configSpec_products ON products.idproduct=configSpec_products.configProduct WHERE configSpec_products.configProductCategory="&tempVarCat&" AND configSpec_products.specProduct="&pIdProduct&" AND configSpec_products.cdefault<>0;"
		set rs=conntemp.execute(query)
	end if
