...
Dim pcCartArray, f, pNewQuantity, pIdProduct, tempQty
	...
	if pcCartArray(f,10)=0 then
		...
		if int(tempQty)<>int(pcCartArray(f,2)) then 
			...
			if scOutofstockpurchase=-1 then
				...
				if not rsC.eof then
					...
					if CheckOFS(tmpID,pNewQuantity,tmpiStock,f)=1 then
						call closedb()
						response.Clear()
						Session("message") = "The quantity of "&tmpiDesc&" that you are trying to order is greater than the quantity that we currently have in stock. We currently have "&tmpiStock&" unit(s) in stock."
			...
			if pIdProduct<>"" and pcCartArray(f,16)="" then
				...
				if rstemp.eof then
          			call closedb()
					Session("message") = "Items do not exist."
					...
					IF pcCartArray(f,16)<>"0" and pcCartArray(f,16)<>"" then
						...
						if Pstring<>"" and uCase(Pstring)<>"NA" then
							...
							For m=lbound(ArrProduct) to (UBound(ArrProduct)-1)
                                ...
                                pcv_strApparelQuery = ""
								If statusAPP="1" Then
									query="SELECT pcProd_ParentPrd FROM Products WHERE idProduct=" & ArrProduct(m) & " AND pcProd_ParentPrd>0;"
									set rstemp=connTemp.execute(query)
									if not rstemp.eof then
										pcv_ParentPrd = rstemp("pcProd_ParentPrd")
                                        pcv_strApparelQuery = " OR IdProduct=" & pcv_ParentPrd 
									end if
									set rstemp=nothing									
								End If
								
                                query="SELECT quantityFrom,quantityUntil,discountperUnit,percentage,discountperWUnit FROM discountsPerQuantity WHERE IDProduct=" & ArrProduct(m) & pcv_strApparelQuery 
								set rstemp=connTemp.execute(query)
