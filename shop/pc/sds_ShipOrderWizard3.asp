...
IF request("action")="add" THEN
	...
	if pcv_shippedDate<>"" then
		query="INSERT INTO pcPackageInfo (idOrder,pcPackageInfo_ShipMethod,pcPackageInfo_ShippedDate,pcPackageInfo_TrackingNumber,pcPackageInfo_Comments, pcPackageInfo_MethodFlag) VALUES (" & pcv_IdOrder & ",N'" & pcv_method & "','" & dtShippedDate & "','" & pcv_tracking & "',N'" & pcv_AdmComments & "',1);"
	else
		query="INSERT INTO pcPackageInfo (idOrder,pcPackageInfo_ShipMethod,pcPackageInfo_TrackingNumber,pcPackageInfo_Comments, pcPackageInfo_MethodFlag) VALUES (" & pcv_IdOrder & ",N'" & pcv_method & "','" & pcv_tracking & "',N'" & pcv_AdmComments & "', 1);"
	end if
	...
	query="INSERT INTO pcAdminComments (idorder,pcACom_ComType,pcACom_Comments,pcDropShipper_ID,pcACom_IsSupplier,pcPackageInfo_ID) VALUES (" & qry_ID & ",2,N'" & pcv_AdmComments & "'," & session("pc_idsds") & "," & session("pc_sdsIsDropShipper") & "," & pcv_PackageID & ");"
	set rstemp=connTemp.execute(query)
...
<div id="pcMain">
	<div class="pcMainContent">
		<h1><%= dictLanguage.Item(Session("language")&"_sds_viewpast_1c")%> - <%= dictLanguage.Item(Session("language")&"_sds_shiporderwizard_1")%> <%=(scpre+int(pcv_IdOrder))%></h1></td>
    ...
    <ul class="pcShipWizardHeader">
      ...
      <li class="pcShipWizardStep3 active">
        <img src="<%=pcf_getImagePath("images","step3a.gif")%>">
    ...
    <div class="pcSuccessMessage"><%= dictLanguage.Item(Session("language")&"_sds_shiporderwizard_20")%></div>

		<div class="pcSpacer"></div>
    
		<a class="pcButton pcButtonBackToOrder" href="sds_viewPastD.asp?idOrder=<%= (scpre+int(pcv_IdOrder))%>">
    	<img src="<%=pcf_getImagePath("",rslayout("pcLO_backtoOrder"))%>" alt="<%= dictLanguage.Item(Session("language")&"_css_pcLO_backtoorder") %>">
