...
if request("PaymentSubmitted")="Go" then
	...
	If objHTTP.Status = 200 Then  ' HTTP_STATUS_OK=200 
		...
		Elseif strstatus = "E" Then
            ...
            call closeDb()
            Session("message") = "Error code " & errCode&":&nbsp;"&(errString)
            Session("backbuttonURL") = tempURL & "?psslurl=gwEMerchant.asp&idCustomer=" & session("idCustomer") & "&idOrder=" & session("GWOrderId")
	...
	Else   
        ...
        call closeDb()
        Session("message") = errCode&"&nbsp;due to&nbsp;"&lcase(errString)
        Session("backbuttonURL") = tempURL & "?psslurl=gwEMerchant.asp&idCustomer=" & session("idCustomer") & "&idOrder=" & session("GWOrderId")
        response.redirect "msgb.asp?back=1"
