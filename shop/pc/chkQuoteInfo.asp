...
for i=lbound(ArrProduct) to (UBound(ArrProduct)-1)
	...
	query="select price,Wprice,multiSelect from configSpec_products where configProductCategory=" & ArrCategory(i) & " and configProduct=" & pcv_intIdProduct & " and specProduct=" & pcv_Midproduct

	set rsB=conntemp.execute(query)
	if not rsB.eof then    
        pcPriceConf = rsB("price")
        pcWPriceConf = rsB("Wprice")

        If cdbl(pcv_intIdProduct)<>cdbl(ArrProduct(i)) Then
        
            '// If BTO Override Price is $0, then use correct Apparel Pricing
            If ((Session("customerType")=1) And (pcWPriceConf=0)) OR ((Session("customerType")<>1) And (pcPriceConf=0)) Then
            
                query="SELECT btoBPrice, price from Products where idproduct=" & ArrProduct(i)
                set rsC = connTemp.execute(query)
                If Not rsC.Eof Then
                    pcPriceConf = rsC("price")
                    pcWPriceConf = rsC("btoBPrice")
                End If
                set rsC=nothing
        ...
        If (Session("customerType")=1) and (pcWPriceConf<>0) Then
            NewPrice=cdbl(pcWPriceConf)
        else
            NewPrice=cdbl(pcPriceConf)
        end if
...
'End Recalculate Default Price
	...
	for i=lbound(ArrProduct) to (UBound(ArrProduct)-1)
		...
		set rsQ=connTemp.execute(query)
		if not rsQ.eof then
			tmpBTOParentPrd=" OR IDProduct=" & rsQ("pcProd_ParentPrd")
		else
			tmpBTOParentPrd=""
		end if
		set rsQ=nothing
		query="select * from discountsPerQuantity where IDProduct=" & ArrProduct(i) & tmpBTOParentPrd
