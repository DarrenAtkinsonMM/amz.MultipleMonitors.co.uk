...
IF HaveSecurity=0 THEN
	...
	if request("action")="add" then
		...
		pcCustomerEmail=getUserInput(request.Form("email"),0)
		...
		Else
			UpdateSuccess="0"
			pcv_strErrorMessage = objSB.pcf_GetNode(result, "Reason", "//OneTimePaymentResponse")
            If len(pcv_strErrorMessage)=0 Then
                pcv_strErrorMessage = "We could not process your payment. Please contact support for assistance. "
            End If
		End If  
...
<table class="pcMainTable">
    ...
    <%ELSE%>
		...
		<%ELSE%>            
				...
				<% If pcv_strErrorMessage <> "" Then %>
                <tr>
                    <td colspan="2">
                        <div class="pcErrorMessage"><%=pcv_strErrorMessage%></div>
                    </td>
                </tr>
				<% End If %>
                <tr>
				...
				<tr>
					<td colspan="2">
                        ...
                        <table class="pcShowContent">
                              ...
                              %>
 <input type="hidden" name="email" value="<%=pcStrCustomerEmail%>" />
                ...
                <tr>
                    <td colspan="2"> 
                        <input type="image" id="DSubmit" name="DSubmit" value="DSubmit" src="<%=pcf_getImagePath("images/sample","pc_button_update.gif")%>" class="clearBorder">
                    </td>
                </tr>
        <%END IF%>
    <%END IF%>
