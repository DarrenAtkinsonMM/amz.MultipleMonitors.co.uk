...
if pcv_PNREF<>"" then
	...
	if session("GWOrderId")="" then
		...
		if session("GWOrderId")<>"" then
			session("GWOrderId")=clng(session("GWOrderId"))+clng(scpre)
		end if
	end if
	session("GWSessionID")=Session.SessionID
...
if CartOrderID="" then
	CartOrderID = getUserInput(request("merchant_order_id"),0)
end if	
if CartOrderID<>"" then
	gwTransID=getUserInput(request("order_number"),0)
...
select case rt_gateway
	...
	'// PayPal Express Checkout (all flavours)
	case "PayPalExp"
		if session("GWTransType")="2" or session("GWTransType")="A" then
			pPaymentStatus=1
		else
			pPaymentStatus=2
		end if
	...
	'// PayPal Payments Pro (Direct Payments)
	case "PayPalDP"
		...
		'SB E
	'// Payflow Editions (PayPal Payments Advanced, PayPal Payments Pro, Payflow Link, Payflow Pro)
	case "PayPalAdvanced", "PayPalPro", "PFL", "PFP"
		if session("GWTransType")="A" then 'Pending PayPal order
			pPaymentStatus=1
		else
			pPaymentStatus=2
		end if
	case "AIM"
		...
		gwAVSCode = session("AVSCode")
		gwCVV2Code = session("CVV2Code") & "||" & session("x_cav_response")
		...
		'SB E
	case "DPM"
		if ucase(Request("type"))="AUTH_ONLY" then
			pPaymentStatus=1
		else
			pPaymentStatus=2
		end if
		paymentCode="AuthorizeDPM"
		gwAVSCode = Request("avs")
		gwCVV2Code = Request("cvv") & "||" & Request("cav")
		gwAuthCode = Request("auth")
		gwTransId = Request("trans")
		session("GWOrderTotal") = ""
		If session("idCustomer") = "" then
			session("idCustomer") = Request("idCust")
		end if
		'SB S
		if NOT request("s")="true" then
			session("GWOrderDone")="YES"
...
if (session("pcSFIdPayment")<>"") AND (session("pcSFIdPayment")<>"0") then
	if session("pcSFIdPayment")=999999 then
		'PayPal Advanced Express Checkout
		If session("ExpressPayPPA") = "YES" then
			query="SELECT pcPayTypes_processOrder,pcPayTypes_setPayStatus FROM PayTypes WHERE gwCode=80"
		Else
			'PayPal Flow Link Express Checkout
			If session("ExpressPayPPL") = "YES" Then
				query="SELECT pcPayTypes_processOrder,pcPayTypes_setPayStatus FROM PayTypes WHERE gwCode=99"
			'Normal PayPal Express Checkout
			Else
...
if trim(pIdOrder)="" then
	call closedb()
    Session("message") = dictLanguage.Item(Session("language")&"_updOrdStats_1")
...
pVATID=rsObjOrder("pcCust_VATID")
...
if pord_DeliveryDate <> "" then
	...
	pord_DeliveryDate=showdateFrmt(pord_DeliveryDate) & ", " & pord_DeliveryTime
end if
...
ptaxAvalaraOrder = rsObjOrder("pcOrd_Avalara")
...
'	set rs = nothing
	...
	if (Session("SBEditOrder")<>"") AND (Session("SBEditOrderID")<>"") then
		EditSB=1
		objSB.Guid = Session("SBEditOrder")
		result = objSB.UpdateSubscriptionRequest(Setting_APIUser, Setting_APIPassword, Setting_APIKey)
	else
	...
	If len(SB_ErrMsg)>0 Then	
		...
		Select Case getUserInput(request("gw"),0)
			...
			Case "PayPalDP" :  pcv_strPaymentPage="gwPayPal.asp"
	...
	IF EditSB=1 THEN
		...
		query="UPDATE ProductsOrdered SET pcPO_SubActive=3 WHERE idOrder=" & pIdOrder & ";"
		set rs=connTemp.execute(query)
		set rs=nothing
        
		'Hide SB Edited Order
		query="UPDATE Orders SET orderStatus=0 WHERE idOrder=" & pIdOrder & ";"
		set rs=connTemp.execute(query)
		set rs=nothing
        
		query="SELECT SB_Terms FROM SB_Orders WHERE SB_GUID like '" & Session("SBEditOrder") & "' ORDER BY idOrder DESC;"
		set rs=connTemp.execute(query)
		SBTerms=""
		if not rs.eof then
			SBTerms=rs("SB_Terms")
		...
		query="INSERT INTO SB_Orders (idOrder,SB_GUID,SB_Terms) VALUES (" & pIdOrder & ",'" & Session("SBEditOrder") & "',N'" & SBTerms & "');"
		set rs=connTemp.execute(query)
		set rs=nothing

		response.redirect "sb_CustViewSubs.asp?u=1"
	END IF
End if
'SB E
...
if rs.eof then
	set rs=nothing
	call closeDb()
    Session("message") = dictLanguage.Item(Session("language")&"_updOrdStats_4")
    response.redirect "msgb.asp?back=1" 
end if
...
do while not rs.eof
	...
	query="SELECT noStock, stock, sales, description FROM products WHERE idProduct="&pIdProduct
...
'// Order is processed when placed -> Commit Avalara transaction
If pOrderStatus="3" AND ptaxAvalaraOrder=1 Then
	call pcs_CommitAvalara(pIdOrder, "commit")
End If
		...
		DO while not rstemp.eof
			...
			if not rs.eof then
				...
				IF (pGCGenFile<>"") THEN
						...
						'Test if the store is using HTTPS and build the path to the license file appropriately.'
...
if ppStatus=0 then
	...
	if pcOrderKey<>"" then
		storeAdminEmail=storeAdminEmail & "<br>"
		storeAdminEmail=storeAdminEmail & FixedField(100, "R", "----------------------------------------------------------------------------------------------------") & "<br>"
		storeAdminEmail=storeAdminEmail & "ORDER CODE: " & pcOrderKey & "<br>"
		storeAdminEmail=storeAdminEmail & FixedField(100, "R", "----------------------------------------------------------------------------------------------------") & "<br>"
	end if
	...
	If NOT rs.eof then
		If rs("idaffiliate")<>1 then
			...
			if ppStatus=0 then
				...
				call pcs_hookAffOrderEmailSent(AffiliateEmail)
			end if
		End If
		set rs=nothing
	End If
...
'// START - Order is processed when placed -> Update Apparel Parent Product Sales
IF pOrderStatus="3" THEN
	If statusAPP="1" Then 
	%>
	<!--#include file="app-updsales.asp"-->
	<%
	End If
END IF
