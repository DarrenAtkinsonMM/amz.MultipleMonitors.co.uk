...
if DateDiff("d", Month(Now)&"/"&Year(now), session("reqExpMonth")&"/20"&session("reqExpYear"))<=-1 then
	Session("message") = "<b>Error</b>: "& dictLanguage.Item(Session("language")&"_paymntb_o_6")
...
If pcPay_Cent_Active=1 AND pcPay_CentByPass=0 Then
	...
	If xmlResponse <> "" Then
		...
		If strErrorNo = "0" AND strCardEnrolled = "Y" Then 
			...
			<IFRAME src="pcPay_Cent_Ecauth.asp" width="100%" height="450" frameALLOWTRANSPARENCY="true"> </IFRAME> 
			</body>
			</html>
			<% 
			conlayout.Close 
		...
		Else 
				...
				Else
					...
					Session("message") = "<b>Errors&nbsp;</b>:&nbsp;"&session("Message2")
					response.redirect "msgb.asp?back=1"
				End if
		End If
	Else
