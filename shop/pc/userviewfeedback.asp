...
<div id="pcMain">
	<div class="pcMainContent">
		...
		if (request("action")="add") and (request("rewrite")="0") then
			...
			query="Insert Into pcComments (pcComm_IDOrder,pcComm_IDParent,pcComm_IDUser,pcComm_CreatedDate,pcComm_EditedDate,pcComm_FType,pcComm_FStatus,pcComm_Priority,pcComm_Description,pcComm_Details) values (" & LngIdOrder & "," & LngIDFeedback & "," & session("IDCustomer") & ",'" & dtComDate & "','" & dtComDate & "',0,0,0,'',N'" & NewComments & "')" 
			set rs=connTemp.execute(query)
      ...
      <div class="pcSpacer"></div>
			<div class="pcFormItem"><%= dictLanguage.Item(Session("language")&"_viewFeedback_c")%><b>#<%=LngIDFeedback%></b></div>
    </div>
    
    <div class="pcSpacer"></div>
		
    <div class="pcSectionTitle">
      <%= dictLanguage.Item(Session("language")&"_viewFeedback_d")%><strong><%=CheckDate(createdDate)%></strong> <%= dictLanguage.Item(Session("language")&"_viewFeedback_x")%> 
  	...
  	<div class="pcShowContent">  
        ...
        <div class="pcFormField">
          ...
          if not rs.eof then
            ...
            if TypeImage=1 then
              if PImg<>"" then%>
                <img src="<%=pcf_getImagePath("images",PImg)%>" alt="<%=PName%>">
              <%end if
            else%>
              <%=ucase(PName)%>
            <%end if
      ...
      <% 'Short Description %>
      <div class="pcFormItem">
      	<div class="pcFormLabelRight"><%= dictLanguage.Item(Session("language")&"_viewFeedback_j")%></div>
        <div class="pcFormField"><%= FDesc %></div>
      </div>
      
      <% 'Details %>
      <div class="pcFormItem">
      	<div class="pcFormLabelRight"><%= dictLanguage.Item(Session("language")&"_viewFeedback_k")%></div>
        <div class="pcFormField"><%= pcf_FixHTMLContentPaths(FDetails) %></div>
      </div>
      ...
      <div class="pcFormItem">
        ...
        <div class="pcFormField">
          ...
          if (session("UserType")=3) then%><%= dictLanguage.Item(Session("language")&"_viewFeedback_m")%>
        ...
        if not rs.eof then
        %>
        <div class="pcFormItem pcSmallText">
          <div class="pcFormLabelRight"><%= dictLanguage.Item(Session("language")&"_viewFeedback_s")%></div>
          <div class="pcFormField">
            <% Do until rs.eof %>
              <a href="userdownload.asp?IDFile=<%=rs("pcUpld_IDFile")%>" target="_blank"><img src="<%=pcf_getImagePath("images","DownLoad.gif")%>" style="height: 19px; width: 18px" alt="Download File"></a>&nbsp;<b><%
              Filename= rs("pcUpld_FileName")
...
if not rs.eof then
	%>
	<div class="pcSectionTitle"><strong><%= dictLanguage.Item(Session("language")&"_viewFeedback_t")%></strong></div>
<%end if
...
Do while not rs.eof
	...
	<div class="pcShowContent">
		<p>
			<%= dictLanguage.Item(Session("language")&"_viewFeedback_u")%><%if (createdDate & "") = (editedDate & "") then%><%= dictLanguage.Item(Session("language")&"_viewFeedback_v")%><strong><%=CheckDate(createdDate)%></strong><%else%><%= dictLanguage.Item(Session("language")&"_viewFeedback_w")%><strong><%=CheckDate(editedDate)%></strong><%end if%><%= dictLanguage.Item(Session("language")&"_viewFeedback_x")%><strong><%
			if (intIDUser<>"") and (intIDUser<>"0") then
    		query="SELECT name,lastname FROM Customers WHERE IDCustomer=" & intIDUser
	...
	<div class="pcShowContent">    
  	<div class="pcFormItem">
    	<div class="pcFormLabelRight"><%= dictLanguage.Item(Session("language")&"_viewFeedback_z")%></div>
      <div class="pcFormField"><%=FDetails%></div>
    </div>
      ...
      <div class="pcSmallText">
      	<%= dictLanguage.Item(Session("language")&"_viewFeedback_s")%><br>
				<%Do until rstemp.eof%>
  				<a href="userdownload.asp?IDFile=<%=rstemp("pcUpld_IDFile")%>" target="_blank"><img src="<%=pcf_getImagePath("images","DownLoad.gif")%>" style="height: 19px; height: 18px" alt="Download File"></a>&nbsp;<b><%
...
Loop%>
	...
	<br/>
		...
		<form name="hForm" method="post" action="userviewfeedback.asp?action=add" onSubmit="return Form1_Validator(this)" class="pcForms">
			<div class="pcShowContent">
				<div class="pcSectionTitle">
					<%= dictLanguage.Item(Session("language")&"_viewFeedback_1")%>
				</div>
				<%if AllowUpload="1" then%>
					<div class="pcFormItem">
						<ol>
							<li><%= dictLanguage.Item(Session("language")&"_viewFeedback_2")%></li>
							<li><%= dictLanguage.Item(Session("language")&"_viewFeedback_3")%></li>
						</ol>
					</div>
				<%else%>
					<div class="pcFormItem">
						<%= dictLanguage.Item(Session("language")&"_viewFeedback_4")%>
					</div>
				<%end if%>
				<%if AllowUpload="1" then%>
          <div class="pcFormItem">
            <div class="pcFormLabelRight"><%= dictLanguage.Item(Session("language")&"_viewFeedback_5")%></div>
            <div class="pcFormField">
              <%query="SELECT pcUpld_IDFile,pcUpld_FileName FROM pcUploadFiles WHERE pcUpld_IDFeedback=0"
              set rs=connTemp.execute(query)
              if rs.eof then%>
                <%= dictLanguage.Item(Session("language")&"_viewFeedback_6")%><br>
              <%else
              ACount=0
              do while not rs.eof
              ACount=ACount+1
              ...
              <br><%= dictLanguage.Item(Session("language")&"_viewFeedback_7")%><a href="#" onClick="javascript:newWindow1('userfileuploada_popup.asp?IDFeedback=0&amp;ReLink=<%=Server.URLencode("userviewfeedback.asp?IDOrder=" & clng(scpre)+clng(LngIdOrder) & "&amp;IDFeedback=" & LngIDFeedback)%>','window2')"><%= dictLanguage.Item(Session("language")&"_viewFeedback_8")%></a>.
            </div>
          </div>
          <div class="pcSpacer"></div>
        <%end if%>
      	...
      	<div class="pcFormItem">
					<div class="pcFormLabelRight">
						<%= dictLanguage.Item(Session("language")&"_viewFeedback_9")%>
          </div>
					<div class="pcFormField">
            <input type="hidden" name="IDOrder" value="<%=clng(scpre)+LngIdOrder%>">
            <input type="hidden" name="IDFeedback" value="<%=LngIDFeedback%>">
            <textarea class="htmleditor" name="comments" id="comments" cols="56" rows="8"><%=request("comments")%></textarea>
          </div>
				</div> 
				<div class="pcSpacer"></div>
      	...
      	<div class="pcFormItem">
        	...
        	<div class="pcFormField">
          	<div class="pcFormButtons">
          		<button class="pcButtonAddComment" type="submit" name="Submit" onClick="document.hForm.rewrite.value='0';">Add Comment</button>
            	<button class="pcButtonOtherMessages" onClick="location='userviewallposts.asp?IDOrder=<%=clng(scpre)+clng(LngIdOrder)%>';"><%= dictLanguage.Item(Session("language")&"_viewFeedback_10")%></button>
    ...
    <iframe name="downloadwindow" src="about:blank" style="margin: 0px; padding: 0px; width: 0px; height: 0px;"></iframe>
