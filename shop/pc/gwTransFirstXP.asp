...
if request("PaymentSubmitted")="Go" then
	...
	txpSecCode = GetUserInput(Request.Form("CVV"),0)
	...
	TXP_PostData=TXP_PostData&"<v1:SendTranRequest xmlns:v1=""http://postilion/realtime/merchantframework/xsd/v1/"">"&vbcrlf
		...
		TXP_PostData=TXP_PostData&"<v1:contact>"&vbcrlf
			...
			TXP_PostData=TXP_PostData&"<v1:state>" & left(pcBillingStateCode,2) &"</v1:state>"&vbcrlf
			TXP_PostData=TXP_PostData&"<v1:zipCode>" & pcBillingPostalCode &"</v1:zipCode>"&vbcrlf
			TXP_PostData=TXP_PostData&"<v1:ctry>" & pcBillingCountryCode &"</v1:ctry>"&vbcrlf
		TXP_PostData=TXP_PostData&"</v1:contact>"&vbcrlf
	...
	If TXPTestMode="1" Then
		TXPURL ="https://ws.cert.processnow.com/portal/merchantframework/MerchantWebServices-v1"
	Else
		TXPURL ="https://ws.processnow.com/portal/merchantframework/MerchantWebServices-v1"
	End If
	...
	Else
		...
		sErrorName = ""
		If responseError ="Unknown Error" Then
			sErrorName = objGenXMLClass.ReadResponseNode("//detail","SystemFault:SystemFault/name")
			sErrorMessage = objGenXMLClass.ReadResponseNode("//detail","SystemFault:SystemFault/message")
			sErrorCode = objGenXMLClass.ReadResponseNode("//detail","SystemFault:SystemFault/errorCode")
		End If
		if sErrorName <> "" then
			response.write "There has been a server/system error - payment was not completed."
			response.write "<hr>Name: "&sErrorName &"<hr>"
			response.write "Message: "&sErrorMessage &"<hr>"
			response.write "Code: "&sErrorCode &"<hr>"
			response.End()
		end if	

        call closeDb()
        Session("message") = responseError
        Session("backbuttonURL") = tempURL & "?psslurl=gwTransFirstXP.asp&idCustomer=" & session("idCustomer") & "&idOrder=" & session("GWOrderId")
