...
Dim strCol, Count, HTMLResult, intPicsPerRow, PopFile,ShowSub,ShowPic,pShowButton,pName
HTMLResult=""
Count = 0
DCnt=0
strCol = ""
...
HTMLResult=HTMLResult & "<div class=""pcInfoMessage"">Click on an image or image name to view its actual size. Images are automatically resized to better fit into this window, and therefore <b>may appear distorted</b>.</div>" & vbcrlf
HTMLResult=HTMLResult & "<table class=""table table-condensed"">" & vbcrlf
    ...
    for x = 1 to intPicsPerRow
        ...
        if pshowimage="YES" then
	        HTMLResult=HTMLResult & "<div>" & vbcrlf
   	        HTMLResult=HTMLResult & "<a href=""Javascript:openGalleryWindow('"&PopFile&"?ShowPic="&ShowSub&"/"&ShowPic&"')""><img src='"& pcf_getImagePath(ShowSub,pName)&"' style=""max-width: 100px; max-height: 100px;""></a>" & vbcrlf
	        HTMLResult=HTMLResult & "</div>" & vbcrlf
	    end if
	    ...
	    if pShowButton <> 0 then 
	        HTMLResult=HTMLResult & "<INPUT TYPE=""SUBMIT"" name=""UPD"&DCnt&""" VALUE=""Select"" class=""btn btn-default btn-sm"">" & vbcrlf
	    end if 
...
IF session("admin")=-1 then
    IF DCnt>0 THEN
        HTMLResult=HTMLResult & "<br /><br /><div align=""center""><form name='image1' method='post' class='pcForms'>"&vbCrlf
            ...
            HTMLResult=HTMLResult & "<input type='submit' name='del' value='Delete Checked Images' onclick=""return(confirm('You are about to delete the images you have selected. This action cannot be undone. Would you like to continue?'));"" class=""btn btn-default"">"&vbCrlf
            For i=1 to DCnt
