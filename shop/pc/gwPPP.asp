...
session("redirectPage")="gwPPP.asp"
...
stext = stext & "&CSCREQUIRED="&pcPay_PayPal_CVC
stext = stext & "&CSCEDIT="&pcPay_PayPal_CVC
stext = stext & "&CREATESECURETOKEN=Y"
stext = stext & "&RETURNURL="&PPPURL&"gwPPPResults.asp"
stext = stext & "&CANCELURL="&PPPURL&"gwPPPResults.asp"
stext = stext & "&ERRORURL="&PPPURL&"gwPPPResults.asp"
...
dim ppmode
ppmode = ""
if pcPay_PayPal_Sandbox = "1" then ppmode = "MODE=TEST&"
	...
	<div class="pcMainContent">
		...
		<% if request("Message")&""<>"" then
			...
			if lcase(myMsg)="session" then
				myMsg = session("ppp_message")
			end if %>
      <div class="pcErrorMessage"><%=myMsg%></div>
		<% end if %>

		<div class="pcPayPalFrameContainer">
      <iframe class="pcPayPalFrame" id="PPPFrame" name="PPPFrame" src="https://payflowlink.paypal.com/?<%=ppmode%><%=iframeString%>" width="490" height="565" scrolling="no"></iframe>
		</div>
	</div>
</div>
