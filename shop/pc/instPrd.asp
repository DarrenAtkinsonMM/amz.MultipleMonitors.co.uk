...
if ((len(pCSCnt)>0) AND (validNum(pCSCnt))) then
	...
	if pcv_Apparel="1" then
		'// check for multiple sub-products
		tmpArr=GetUserInput(request("SavedList"),0)
		tmpArr1=GetUserInput(request("SavedQtyList"),0)
		if tmpArr<>"" then
			...
			if NOT validNum(tmpArrSPID(0)) Then
				response.redirect "msg.asp?message=207"
			end if
			query="select pcprod_ParentPrd from Products where idproduct=" & tmpArrSPID(0)
			...
			query="select pcprod_ParentPrd from Products where idproduct=" & getUserInput(request("idproduct"),10)
			set rs=connTemp.execute(query)
			if not rs.eof then
				pcv_ParentPrd=rs("pcprod_ParentPrd")
			else
...
'// Unlock shopping cart in SB Edit Order mode
if (request("index")<>"") AND (request("index")<>"0") AND (Session("SBEditOrder")<>"") AND (Session("SBEditOrderID")<>"") then
	pcv_sbLockCart=False
	pcv_sbIsCartLockable=False
else
	if ((request("index")="") OR (request("index")="0")) AND (Session("SBEditOrder")<>"") AND (Session("SBEditOrderID")<>"") AND (pSubscriptionID="0") then
		pcv_sbLockCart=True
	end if
end if
'SB E
...
<div id="pcMain">
	<div class="pcMainContent">
		...
		if tmpSPCount>"0" then
			if pCSCnt>0 then
				tmpTotal=pcv_strProductsQuantity+clng(tmpSPCount)
			else
				tmpTotal=pcv_strProductsQuantity+clng(tmpSPCount)-1
			end if
		else
			tmpTotal=pcv_strProductsQuantity
		end if
		...
		for iAddM=tmp_start to tmpTotal

			pIsAccessory=0  '// Cross Sell - Accessory flag (0=false, -1=true)

			tmp_notapparel=0

			IF (pcv_strProductsQuantity>1) and ((iAddM<=pCSCnt+1) or ((piCnt>0) and (iAddM<=piCnt))) Then '// If there are multiple items to be added
			'// There are Cross Sell products
			if (pCSCnt > 0) and (iAddM<=pCSCnt+1) then
	        ...
	        if (iAddM=1) and (tmpSPCount=0) then
		        '--> Product ID
	        ...
	        elseif (iAddM<>1) then
	        	tmp_notapparel=1
					...
					If NOT rstemp.EOF Then
						...
						End If
					End If
					If (pcv_sbLockCart=True AND pSubscriptionID>0) OR (pcv_sbLockCart=False AND pSubscriptionID>0 AND pcv_BoundQty>0 AND pcv_sbIsCartLockable=True AND IsTaxOrShipping) OR (pcv_sbLockCart=True AND pSubscriptionID=0 AND pcv_BoundQty>0 AND pcv_sbIsCartLockable=True AND IsTaxOrShipping) Then
			...
			Else '// IF pcv_strProductsQuantity>1 Then
				...
				if (pcv_Apparel="1") and (tmpSPCount>0) and (tmpSPIndex<tmpSPCount) then
					tmpSPIndex=tmpSPIndex+1
					'--> Product ID
					pIdProduct=tmpArrSPID(tmpSPIndex-1)
					'--> Quantity
					pQuantity=tmpArrSPQty(tmpSPIndex-1)
			...
			if pQuantity>0 then
				...
				if pcv_Apparel="1" then
					...
					if IsNull(pcv_intOptionGroupCount) OR pcv_intOptionGroupCount="" OR (Not IsNumeric(pcv_intOptionGroupCount)) then
						pcv_intOptionGroupCount = 0
					end if
		...
		if (pcv_Apparel="1") and (tmp_notapparel=0) then
			...
			query="SELECT iRewardPoints,description, price, bToBPrice, sku, emailText, weight, deliveringTime, pcSupplier_ID, cost, stock, notax, noshipping, OverSizeSpec,noStock,pcprod_QtyToPound,pcProd_BackOrder,pcProd_ShipNDays, products.pcProd_Surcharge1, products.pcProd_Surcharge2,pcProd_AvalaraTaxCode FROM products WHERE idproduct=" & pIdProduct & queryNFSO & " AND active=-1"
		end if
		...
		pcv_AvalaraProductTaxCode = rstemp("pcProd_AvalaraTaxCode")
        ...
        Else
		    query = "SELECT pcCats_AvalaraTaxCode FROM categories c JOIN categories_products cp ON cp.idCategory = c.idCategory WHERE cp.idProduct = " & pIdProduct & " AND pcCats_AvalaraTaxCode <> ''"
        End If
		...
		'If statusAPP="1" Then
			...
			'// Ensure that the parent product is active and for sale			
			if HasSub=1 then
				Dim rsActive, pcIntEligible, pcIntNotForSale
				pcv_tmpParent=rstemp("pcprod_ParentPrd")
				pcIntEligible=1
				query="SELECT formQuantity FROM products WHERE idproduct=" & pcv_tmpParent & " AND active=-1"
				set rsActive=Server.CreateObject("ADODB.Recordset")
				set rsActive=connTemp.execute(query)
				if rsActive.eof then ' The parent product is not active
					pcIntEligible=0
				else
					pcIntNotForSale=rsActive("formQuantity")
				end if
				set rsActive = nothing
				
				if pcIntNotForSale="-1" and NotForSaleOverride(session("customerCategory"))=0 then
					pcIntEligible=0
				end if
				
				if pcIntEligible=0 then
					call closeDb()
					response.redirect "msg.asp?message=41"
				end if
			else
				pcv_tmpParent = pIdProduct
			end if		
						
			'// Retrieve parent product's Surcharges
			query = "SELECT products.pcProd_Surcharge1, products.pcProd_Surcharge2 FROM products WHERE idproduct=" & pcv_tmpParent & " AND active=-1" 
			set rsSCObj=Server.CreateObject("ADODB.Recordset")
			set rsSCObj=connTemp.execute(query)
			pcv_Surcharge1 = rsSCObj("pcProd_Surcharge1")
			pcv_Surcharge2 = rsSCObj("pcProd_Surcharge2")
		
			'// Retrieve parent product's Reward Points
			if (iRewardPoints="" OR IsNull(iRewardPoints) OR iRewardPoints="0") AND (HasSub=1) then
				query="SELECT iRewardPoints FROM products WHERE idproduct=" & pcv_tmpParent & " AND active=-1"
				set rsRP=connTemp.execute(query)
				if not rsRP.eof then
					iRewardPoints=rsRP("iRewardPoints")
					if iRewardPoints="" OR IsNull(iRewardPoints) then
						iRewardPoints=0
					end if
				end if
				set rsRP = nothing
			...
			if (pWeight="" OR IsNull(pWeight) OR pWeight="0") AND (pcv_QtyToPound=0) AND (HasSub=1) then
				query="SELECT weight, pcprod_QtyToPound FROM products WHERE idproduct=" & pcv_tmpParent & " AND active=-1"
				set rsQTP=connTemp.execute(query)
				if not rsQTP.eof then
					pWeight=rsQTP("weight")
					if pWeight="" OR IsNull(pWeight) then
						pWeight=0
					end if
					pcv_QtyToPound=rsQTP("pcprod_QtyToPound")
					if pcv_QtyToPound="" OR IsNull(pcv_QtyToPound) then
						pcv_QtyToPound=0
					end if
				end if
				set rsQTP = nothing
	    ...
	    Next
			...
			if (XFCount<>"") and (IsNumeric(XFCount)) then
				if Clng(XFCount)>=1 then
					For iq=1 to XFCount
						pxfield=getUserInput(request("xfield" & iq),0)
						if pxfield<>"" then
							pxfield=replace(pxfield,vbCrlf,"<BR>")
						end if
						pxf=getUserInput(request("xf" & iq),10)
						if pxfield<>"" then
		...
		if (scOutofstockpurchase=-1) AND (pNoStock=0) AND (pcv_intBackOrder=0) then
			...
			if CheckOFS(pIdProduct,pQuantity,pStock,tmpIdx)=1 then
				call closedb()
				response.Clear()
				Session("message") = "The quantity of "&pDescription&" that you are trying to order is greater than the quantity that we currently have in stock. We currently have "&pStock&" unit(s) in stock."
		...
		'SB S
		queryQ="SELECT SB_PackageID FROM SB_Packages WHERE idProduct=" & pcCartArray(ppcCartIndex,0) & ";"
		set rsQ=connTemp.execute(queryQ)
		if not rsQ.eof then
			pSubscriptionID=rsQ("SB_PackageID")
		else
			pSubscriptionID=0
		end if
		set rsQ=nothing
		...
		if ( ((pCSCnt=0) OR (pCSCnt > 0 and iAddM=1)) AND xfieldsCnt>0 ) then
			pcCartArray(ppcCartIndex,21)=tmpXFStr
		end if
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		pcCartArray(ppcCartIndex,27)=0 '// Relationship - Parent/Child Index (-1=Parent,0=Non-Cross Sell,>0= Child's Parent Index)
		...
		if (pcv_Apparel="0") or (tmp_notapparel=1) then
			pcCartArray(ppcCartIndex,32)=""
		else
			pcCartArray(ppcCartIndex,11)=""
			pcCartArray(ppcCartIndex,12)=0
			tmpStr=""
            
            '// Array of option ids
			query="SELECT pcprod_Relationship FROM products WHERE idproduct=" & pIdProduct
			set rsT=connTemp.execute(query)
			if not rsT.eof then
				tmpStr1=rsT("pcprod_Relationship")
                If len(tmpStr1)>0 Then
                    tmpStr2=split(tmpStr1,"_")
                    For mk=1 to ubound(tmpStr2)
                        tmpStr=tmpStr & tmpStr2(mk) & "$$"
                    Next
                End If
			end if
			set rsT=nothing			
			tmpStr=tmpStr & pcv_ParentPrd
			pcCartArray(ppcCartIndex,32)=tmpStr
            
            '// Parent Product Id
            pcCartArray(ppcCartIndex,43)=pcv_ParentPrd
            
		end if
		
		'Avalara Product Tax Code
		pcCartArray(ppcCartIndex,44) = pcv_AvalaraProductTaxCode
		
		'Avalara Category Tax Code
		pcCartArray(ppcCartIndex,45) = pcv_AvalaraCategoryTaxCode
	...
	if Session("customerType")=1 OR session("customerCategory")<>0 then
		pcCartArray(CurrentProductsIndex,18)=1
	else
		pcCartArray(CurrentProductsIndex,18)=0
	end if
...
<% if (pcv_Apparel="1") then %>
<!--#include file="app-UpdSPDiscounts.asp"-->
<% end if
...
'QV-S
if Request("FromQuickView")="1" then
	response.Clear()
...
if scATCEnabled="1" then 
	Response.redirect pcf_GetCurrentPage()
else
	response.redirect "viewCart.asp?cs=1" '// cs = Check Session. Initializes the session check script.
end if
...
%>		
		...
		<p>Please wait while we process your items...</p>
