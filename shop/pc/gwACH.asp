	...
	if err.number<>0 then
		...
		Sub WriteReturnData(strMsg)
			...
			FOR i=0 TO UBOUND(arrValues)
				IF arrValues(i) > "" THEN
					Response.Write "<div class=""pcFormItem"">"
                    Response.Write "<div class=""pcFormLabel"">Array #: "&i&" :" & arrNames(i) & ":&nbsp;</div>"
					Response.Write "<div class=""pcFormField"">" & arrValues(i) & "</div>"
                    Response.Write "</div>"
				END IF
			NEXT
			...
			Session("backbuttonURL") = tempURL & "?psslurl=gwACH.asp&idCustomer=" & session("idCustomer") & "&idOrder=" & session("GWOrderId")
			ACH_rd_resultfailurl="msgb.asp?back=1"
