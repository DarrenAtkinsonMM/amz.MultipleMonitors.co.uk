...
pcStrShippingStateCode=URLDecode(getUserInput(request("shipstate"),4))
...
'// Override if "Wholesale" and "Commercial with Wholesale" enabled
If session("customerType")="1" And scComResShipAddress="3" Then
	pcIntShippingResidential="0"
	session(pcShipOpt)=pcIntShippingResidential
End If
					...
					if CDate(pcSFDF1)<Date() then
						...
						if DTCheck=1 then
							if pcSFTF1="" then
								...
								pcv_dtAllowedDateTime = DateAdd("h",24,now())
								pcv_dtCheckedDateTime = CDate(pcSFDF1) & " " & pcSFTF1
								if CDate(pcv_dtCheckedDateTime) < CDate(pcv_dtAllowedDateTime) Then
									pcErrMsg=pcErrMsg & "<li>"&dictLanguage.Item(Session("language")&"_opc_81")&"</li>"
								end if
							end if
						end if
...
pcv_boolIsShippingAddressValidated = getUserInput(request("IsShippingAddressValidated"), 5)
If (pcv_boolIsShippingAddressValidated = "") Or (pcv_boolIsShippingAddressValidated = "false") Then
    pcs_ValidateShippingAddress
End If
...
if pcErrMsg="" then
	...
	if pcShipOpt="-1" then
		...
		query="SELECT [name], lastName, email, phone, fax, customerCompany, address,address2, city, state, stateCode, zip, countryCode, shippingAddress, shippingAddress2 FROM customers WHERE idcustomer="& session("idCustomer") &" AND pcCust_Guest=" & session("CustomerGuest") & ";"
		...
		if not rs.eof then
			...
			pcStrShippingFirstName=getUserInput(rs("name"),0)
			pcStrShippingLastName=getUserInput(rs("lastName"),0)
			pcStrShippingCompany=getUserInput(rs("customerCompany"),0)
			pcStrShippingPhone=getUserInput(rs("phone"),0)
			pcStrShippingEmail=getUserInput(rs("email"),0)
			pcStrShippingAddress=getUserInput(rs("address"),0)
			pcStrShippingPostalCode=getUserInput(rs("zip"),0)
			pcStrShippingStateCode=getUserInput(rs("stateCode"),0)
			pcStrShippingProvince=getUserInput(rs("state"),0)
			pcStrShippingCity=getUserInput(rs("city"),0)
			pcStrShippingCountryCode=getUserInput(rs("countryCode"),0)
			pcStrShippingAddress2=getUserInput(rs("address2"),0)
			pcStrShippingFax=getUserInput(rs("fax"),0)
			
			'// Check if shipping address is empty, then make the shipping address same as billing address
			tmpShippingAddress=rs("shippingAddress")
			tmpShippingAddress2=rs("shippingAddress2")
			if (IsNull(tmpShippingAddress) OR tmpShippingAddress="") AND (IsNull(tmpShippingAddress2) OR tmpShippingAddress2="") then
				query="UPDATE Customers SET shippingCompany=N'" & pcStrShippingCompany & "',shippingAddress=N'" & pcStrShippingAddress & "',shippingAddress2=N'" & pcStrShippingAddress2 & "',shippingCity=N'" & pcStrShippingCity & "',shippingState=N'" & pcStrShippingProvince & "',shippingStateCode='" & pcStrShippingStateCode & "',shippingZip='" & pcStrShippingPostalCode & "',shippingCountryCode='" & pcStrShippingCountryCode & "',shippingPhone='" & pcStrShippingPhone & "',shippingEmail='" & pcStrShippingEmail & "',shippingFax='" & pcStrShippingFax & "' WHERE idcustomer=" & session("idCustomer") & " AND pcCust_Guest=" & session("CustomerGuest") & ";"
				set rs=connTemp.execute(query)
				if err.number<>0 then
					call LogErrorToDatabase()
					set rs=nothing
...
If DeliveryZip = "1" And pcErrMsg="" Then
