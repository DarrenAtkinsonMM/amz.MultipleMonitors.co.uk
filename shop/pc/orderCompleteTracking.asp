...
'// 3.) There are two sections in this page "TRACKING" and "LANDING": see below
	...
	IF pcv_intOrderComplete=1 THEN	
		...
		'// Read Google Conversion Tracking code
		if scEnableGCT="1" then			
            pcStrFolder = "theme/_common"
            pcStrGCTCode = pcf_OpenUTF8(pcStrFolder & "\GCT.inc", pcStrFolder & "\GCT.inc")
            pcStrGCTCode = replace(pcStrGCTCode,"<ORDER_TOTAL>",ptotal)            			
			Response.Write pcStrGCTCode
		End If
