...
'Clear cart before repeating order
session("pcCartIndex")=Cint(0)	
Dim pcCartArrayTMP(100,45)
session("pcCartSession")=pcCartArrayTMP
HaveNewSession=1
...
if not rsQ.eof then
	Session("idcustomer")=rsQ("idCustomer")
end if
set rsQ=nothing
...
query=query&",products.description,products.sku, products.weight,products.pcprod_QtyToPound,products.emailText, products.deliveringTime, products.idSupplier, products.cost, products.stock, products.notax, products.noshipping, products.iRewardPoints, products.pcProd_Surcharge1, products.pcProd_Surcharge2 FROM ProductsOrdered, products, orders WHERE orders.idorder=ProductsOrdered.idOrder AND ProductsOrdered.idProduct=products.idProduct AND orders.idCustomer=" &Session("idcustomer")& " AND orders.idOrder=" &pIdOrder&" AND ProductsOrdered.pcSubscription_ID>0;"
...
'*******************************************************************'
' START - Check Product Inventory Levels before Repeating Order
'*******************************************************************'
IF ReErr=0 THEN
...
'*******************************************************************'
' END - Check Product Inventory Levels before Repeating Order
'*******************************************************************'

IF (ReErr=0) OR (ReErr=5) THEN

do while ((not rstemp.eof) AND ((ReErr=0) OR (ReErr=5)))
	pidProductOrdered=rstemp("idProductOrdered")
	pidProduct=rstemp("idProduct")
	pquantity=rstemp("quantity")
	...
	tmp3=""
	'tmp2=",categories,categories_products"
	'tmp3=" AND categories_products.idproduct=products.idproduct AND categories.idcategory=categories_products.idcategory "
	'tmp3=tmp3 & " AND products.idproduct IN (SELECT DISTINCT categories_products.idproduct FROM categories,categories_products WHERE categories_products.idproduct=" & pidProduct & " AND categories.idcategory=categories_products.idcategory "
