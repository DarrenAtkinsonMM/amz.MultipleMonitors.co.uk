...
'SB-S
if request("sbskip")="1" then
	session("SB_SkipPayment")="" '// "1"
else
	session("SB_SkipPayment")=""
end if
'SB-E
...
ptaxAvalaraOrder = rs("pcCustSession_Avalara")
...
If pShippingReferenceId="0" Then '// This is a new address location, or the existing default location.
	...
	if NOT rs.eof then
		...
		if (pcv_tmpShippingCity="") OR (isNULL(pcv_tmpShippingCity)=True) then
			query="UPDATE customers SET shippingAddress=N'" & pShippingAddress & "', shippingCity=N'" & pShippingCity & "', shippingState=N'" & pShippingState & "', shippingStateCode='" & pShippingStateCode & "', shippingZip='" & pShippingZip & "', shippingCountryCode='" & pShippingCountryCode & "', shippingCompany=N'" & pShippingCompany & "', shippingAddress2=N'" & pShippingAddress2 & "', shippingEmail='" &pShippingEmail& "', shippingPhone='" &pShippingPhone& "',shippingFax='" & pShippingFax & "' WHERE IDCustomer=" & session("idCustomer") &";"
...
If session("OPCstep")="" then 'The OPC no need this step
	If pShippingNickName<>"" Then
		If pShippingNickName <> dictLanguage.Item(Session("language")&"_CustSAmanage_10") Then
		...
		if rs.eof then
			'// Insert new record
			query="INSERT INTO recipients (idCustomer, recipient_FullName,recipient_FirstName,recipient_LastName,recipient_Company,recipient_Address,recipient_Address2,recipient_City,recipient_StateCode,recipient_State,recipient_Zip,recipient_CountryCode,recipient_phone,recipient_NickName, recipient_Fax, recipient_Email) VALUES ("&session("idCustomer")&",N'"&pShippingFirstName&" "&pShippingLastName&"', N'"&pShippingFirstName&"',N'"&pShippingLastName&"',N'"&pShippingCompany&"',N'"&pShippingAddress&"',N'"&pShippingAddress2&"',N'"&pShippingCity&"','"&pShippingStateCode&"',N'"&pShippingState&"','"&pShippingZip&"','"&pShippingCountryCode&"','"&pShippingPhone&"',N'"&pShippingNickName&"','"&pShippingFax&"','"&pShippingEmail&"');"
		else
			'// Update
			query="UPDATE recipients SET recipient_FullName=N'"&pShippingFirstName&" "&pShippingLastName&"', recipient_FirstName=N'"&pShippingFirstName&"',recipient_LastName=N'"&pShippingLastName&"',recipient_Company=N'"&pShippingCompany&"',recipient_Address=N'"&pShippingAddress&"',recipient_Address2=N'"&pShippingAddress2&"',recipient_City=N'"&pShippingCity&"',recipient_StateCode='"&pShippingStateCode&"',recipient_State=N'"&pShippingState&"',recipient_Zip='"&pShippingZip&"',recipient_CountryCode='"&pShippingCountryCode&"',recipient_phone='"&pShippingPhone&"',recipient_NickName=N'"&pShippingNickName&"',recipient_Fax='"&pShippingFax&"', recipient_Email='"&pShippingEmail&"' WHERE recipient_NickName='"&pShippingNickName&"' AND idCustomer="&session("idCustomer")&";"
		end if
...
if chkPayment="FREE" then
	...
	set rsCPP = Nothing
	if (pgwCode=999999) OR ((session("ExpressCheckoutPayment")="YES") AND (session("PayWithAmazon")<>"YES")) then
		...
		Select Case Session("ExpressPayMethod")
		Case "PPP":
			pSslUrl="gwExpressPPPDo.asp"
		Case "PPA":
			pSslUrl="gwExpressPPADo.asp"
		Case "PFL":
			pSslUrl="gwExpressPFLDo.asp"
		Case Else
		...
		if rs.eof then
			set rs=nothing
            call closeDb()
            Session("message") = "Could not locate selected paytypes in database. "
...
strUpdateQuery=strUpdateQuery&"idCustomer=" & int(Session("idCustomer"))& ", details='" &pDetails &"', total=" &DecimalFormatter(pTotal)& ", taxAmount=" &DecimalFormatter(pTaxAmount)& ", comments=N'" &pComments& "', address=N'" &paddress & "', zip='" &pzip& "',state=N'" &pState& "',stateCode='" &pStateCode& "',city=N'" &pCity& "',CountryCode='" &pCountryCode& "',shippingAddress=N'" &pShippingAddress & "',shippingZip='" &pShippingZip& "',shippingState=N'" &pShippingState& "',shippingStateCode='" &pShippingStateCode& "', shippingCity=N'" &pShippingCity& "', shippingCountryCode='" &pShippingCountryCode& "',shipmentDetails='" &pShipmentDetails& "', paymentDetails='" &replace(pPaymentDetails,"'","''")& "',discountDetails='" &replace(pDiscountDetails,"'","''")& "',randomNumber=" &session("pcSFIdDbSession")& ",orderStatus=1,pcOrd_shippingPhone=' " &pShippingPhone& "',idAffiliate=" &pIdAffiliate& ", affiliatePay=" &DecimalFormatter(pAffiliatePay)&",shippingFullName=N'"&pShippingFullName&"', iRewardPoints="&pcUseRewards&",iRewardValue= " &piRewardValue&", iRewardPointsCustAccrued=" & pcCartRewards & ", address2=N'" &paddress2 & "', shippingCompany=N'" &pShippingCompany & "', shippingAddress2=N'" &pShippingAddress2 & "',taxDetails='"&ptaxDetailsString&"',SRF="&pSRF&",ordShipType="&pOrdShipType&", ordPackageNum="&pOrdPackageNum&", ord_OrderName=N'"&pord_OrderName&"'"
...
'// Is Sub has Shipping or Tax, then it is safe to assume nothing else in the cart does. Otherwise, all tax or shipping apply to other items.
for f=1 to ppcCartIndex 
...
Else
	...
	If pSBTaxAmount>0 Then
		strUpdateQuery=strUpdateQuery&",pcOrd_SubTax=" & DecimalFormatter(0) '// Tax
		strUpdateQuery=strUpdateQuery&",pcOrd_SubTrialTax=" & DecimalFormatter(0) '// Trial Tax
	Else
		strUpdateQuery=strUpdateQuery&",pcOrd_SubTax=" & DecimalFormatter(0) '// Tax
		strUpdateQuery=strUpdateQuery&",pcOrd_SubTrialTax=" & 0 '// Trial Tax
	End If
	strUpdateQuery=strUpdateQuery&",pcOrd_SubShipping=" & cdbl(0) + cdbl(0) '// Shipping
	strUpdateQuery=strUpdateQuery&",pcOrd_SubTrialShipping=" & cdbl(0) + cdbl(0) '// Trial Shipping (same as standard shipping)
...
strUpdateQuery=strUpdateQuery&",ord_VAT="&DecimalFormatter(pVATTotal)&",pcord_CatDiscounts=" & pcv_CatDiscounts & ",pcOrd_DiscountsUsed='"&pDiscountUsed&"',pcOrd_GcCode='" & pDiscountCode & "',pcOrd_GcUsed=" & GCAmount & ",pcOrd_GCs=0,pcOrd_IDEvent=" & gIDEvent & ",pcOrd_GWTotal=" & pGWTotal & ",pcOrd_GcReName=N'" & pcv_GcReName & "',pcOrd_GcReEmail='" & pcv_GcReEmail & "',pcOrd_GcReMsg=N'" & pcv_GcReMsg & "',pcOrd_shippingFax='"&pShippingFax&"',pcOrd_ShippingEmail='"&pShippingEmail&"',pcOrd_ShipWeight="&pShipWeight&",pcOrd_Avalara="&ptaxAvalaraOrder&" WHERE idOrder="&session("idOrderSaved")&";"
...
strInsertQuery=strInsertQuery&",ord_VAT,pcord_CatDiscounts,pcOrd_DiscountsUsed,pcOrd_GcCode,pcOrd_GcUsed,pcOrd_GCs,pcOrd_IDEvent,pcOrd_GWTotal,pcOrd_GcReName,pcOrd_GcReEmail,pcOrd_GcReMsg,pcOrd_shippingFax,pcOrd_ShippingEmail,pcOrd_ShipWeight,pcOrd_Avalara) VALUES ('" & replace(GCDetails,"'","''") & "'," & GCAmount & "," & pcShowShipAddr & ",'" & pcOrderKey & "'," & IDrefer & ","

strInsertQuery=strInsertQuery&"'" & pDateOrder  & "'"

strInsertQuery=strInsertQuery&"," & int(Session("idCustomer"))& ",'" &pDetails &"'," &DecimalFormatter(pTotal)& "," &DecimalFormatter(pTaxAmount)& ",N'" &pComments& "',N'" &paddress & "','" &pzip& "',N'" &pState& "','" &pStateCode& "',N'" &pCity& "','" &pCountryCode& "',N'" &pShippingAddress & "','" &pShippingZip& "',N'" &pShippingState& "','" &pShippingStateCode& "',N'" &pShippingCity& "','" &pShippingCountryCode& "','" &pShipmentDetails& "','" &replace(pPaymentDetails,"'","''")& "','" &replace(pDiscountDetails,"'","''")& "'," &session("pcSFIdDbSession")& ",1,' " &pShippingPhone& "'," &pIdAffiliate& ", " &DecimalFormatter(pAffiliatePay)&",N'"&pShippingFullName&"', "&pcUseRewards&", " &piRewardValue&", " &pcCartRewards&", N'" &paddress2 & "', N'" &pShippingCompany & "', N'" &pShippingAddress2 & "','"&ptaxDetailsString&"',"&pSRF&","&pOrdShipType&","&pOrdPackageNum&",N'"&pord_OrderName&"'"
...
strInsertQuery=strInsertQuery&","&DecimalFormatter(pVATTotal)&"," & pcv_CatDiscounts & ",'"&pDiscountUsed&"','" & pDiscountCode & "'," & GCAmount & ",0," & gIDEvent & "," & pGWTotal & ",N'" & pcv_GcReName & "','" & pcv_GcReEmail & "',N'" & pcv_GcReMsg & "','"&pShippingFax&"', '"&pShippingEmail&"',"&pShipWeight&","&ptaxAvalaraOrder&")"
...
if rs.eof then
	set rs=nothing
	call closeDb()
    Session("message") = "Could not locate order."
    response.redirect "msgb.asp?back=1"   
end if
...
'// Save the Avalara Order
If ptaxAvalaraOrder = 1 Then

    query = "SELECT TOP 1 idOrder FROM Avalara_orders WHERE idOrder = " & pIdorder
    set rs = server.CreateObject("ADODB.RecordSet")
    set rs = conntemp.execute(query)
    If Not rs.Eof Then
        query = "UPDATE Avalara_orders SET status = 'Uncommitted', updatedDate = '" & Now() & "' WHERE idOrder = " & pIdorder
    Else
        query = "INSERT INTO Avalara_orders (idOrder, idOrderCounter, status, updatedDate) VALUES (" & pIdorder & ", " & pIdorder & ", 'Uncommitted', '" & Now() & "')"
    End If
...
'// Update Avalara transaction status from "SalesOrder" to "SalesInvoice"
If ptaxAvalaraOrder = 1 And len(pcIsOPC) = 0 Then   
    call pcs_CommitAvalara(pIdOrder, "update")
End If
...
if pcvStrReasonNoRPF<>"" then
	...
	query="UPDATE orders SET adminComments=N'"&pcvStrReasonNoRPF&"' WHERE idOrder="& pIdOrder
	Set rs=Server.CreateObject("ADODB.Recordset")
	Set rs=conntemp.execute(query)
	set rs=nothing
end if
