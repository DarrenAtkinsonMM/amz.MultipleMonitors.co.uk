...
If request("OrderRepeat")<>"haveto" Then
    ...
    do while not rstemp.eof
        ...
        tmp3=" AND ((categories_products.idproduct=products.idproduct) OR (categories_products.idproduct=products.pcprod_ParentPrd)) AND categories.idcategory = categories_products.idcategory AND categories.iBTOhide = 0 "
        
        'tmp3=tmp3 & " AND products.idproduct NOT IN (SELECT DISTINCT products.idproduct FROM products,categories,categories_products WHERE products.idproduct=" & pidProduct & " AND ((categories_products.idproduct=products.idproduct) OR (categories_products.idproduct=products.pcprod_ParentPrd)) AND categories.idcategory=categories_products.idcategory "
        ...
        query = "SELECT DISTINCT products.serviceSpec, products.stock, products.noStock, products.pcprod_minimumqty, products.pcprod_qtyvalidate, products.pcProd_BackOrder "
        query = query & "FROM Products " 
        query = query & tmp2
        query = query & "WHERE products.idproduct=" & pidProduct & " "
        query = query & "AND products.removed=0 "
        query = query & "AND ( "
        query = query & "(products.active<>0) OR ((products.active=0) AND (products.pcProd_SPInActive=0) AND (products.pcProd_ParentPrd>0)) "
        query = query & ")" & queryNFSO & tmp3 & ";"
...
do while not rstemp.eof
	...
	query="SELECT DISTINCT Products.serviceSpec,Products.stock,Products.noStock,Products.pcprod_minimumqty,Products.pcprod_qtyvalidate,Products.pcProd_BackOrder,cast(Products.pcProd_Relationship as varchar(8000)) pcProd_Relationship FROM Products" & tmp2 & " WHERE Products.idproduct=" & pidProduct & " AND Products.removed=0 AND ((Products.active<>0) OR ((Products.active=0) AND (Products.pcProd_SPInActive=0) AND (Products.pcProd_ParentPrd>0)))" & queryNFSO & tmp3 & ";"
	set rs = connTemp.execute(query)	
    If Not rs.Eof Then	
        ...
        pcv_qtyvalid=rs("pcprod_qtyvalidate")
        pcv_BackOrder=rs("pcProd_BackOrder")
        pcv_Relationship=rs("pcProd_Relationship")
        ...
        ELSE
            ...
            if len(pcv_strSelectedOptions)>0 then
				...
				if not rs.eof then
					if rs("pcProdOpt_Required")=1 then
						Session("message")= dictLanguage.Item(Session("language")&"_alert_22")
            ...
            '// Start Reward Points
            If RewardsActive = 1 And IsNumeric(iRewardPoints) Then
        ...
        END IF '// IF (scOutofStockPurchase=-1 AND CLng(pStock)<1 AND pserviceSpec=0 AND pNoStock=0 AND pcv_BackOrder=0) OR (pserviceSpec<>0 AND scOutofStockPurchase=-1 AND iBTOOutofstockpurchase=-1 AND CLng(pStock)<1 AND pNoStock=0 AND pcv_BackOrder=0) THEN
