...
if (pcShippingFullName<>"") then
	if Instr(pcShippingFullName," ")>0 then
		pcShippingFirstName=Left(pcShippingFullName,Instr(pcShippingFullName," ")-1)
		pcShippingLastName=Mid(pcShippingFullName,Instr(pcShippingFullName," ")+1,Len(pcShippingFullName))
	else
		pcShippingFirstName=pcShippingFullName
		pcShippingLastName="N/A"
	end if
else
	pcShippingFirstName="N/A"
	pcShippingLastName="N/A"
end if
...
if pcIsSubscription Then
	...
	pcBillingTotal = ccur(pcBillingTotal)-ccur(pcBillingSubScriptionTotal)
...
'// Check for failed payments and lock the customer after 'N' attempts
call pcs_checkFailedPaymentAttempts(pcIdCustomer)

    
Call pcs_hookPrePayment(pcGatewayDataIdOrder)
%>