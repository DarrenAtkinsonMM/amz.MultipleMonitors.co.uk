			...
			If pcPay_Cent_Active=1 AND pcPay_CentByPass=0 Then %>
				...
				Else
					...
					If CStr(DEBUGOUTPUT) = "True" Then
						Response.redirect ("gwUSAePay.asp?centinel=Y")
					Else
			...
			if pcBillingTotal > 0 then
				...
				'save info in pcPay_Uep_Orders if AUTH_CAPTURE
					...
					Else
                        ...
                        call closeDb()
                        Session("message") = URLDecode(UMerror)
                        Session("backbuttonURL") = tempURL & "?psslurl="&session("redirectPage")&"&idCustomer=" & session("idCustomer") & "&idOrder=" & session("GWOrderId")
                        response.redirect "msgb.asp?back=1"
