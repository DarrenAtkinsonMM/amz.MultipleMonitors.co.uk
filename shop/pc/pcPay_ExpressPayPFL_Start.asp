...
returnURL	= url & "pcPay_ExpressPayPFL_Start.asp?currencyCodeType=" &  currencyCodeType & "&paymentAmount=" & OrderTotal & "&paymentType=" &PaymentAction 
...
If  Request.QueryString("token")&""="" Then
	...
	'***********************************************************************
		...
		If instr(ack,"APPROVED")>0 Then
				...
				'pcv_PayPalPPA_URL = pcv_PayPal_URL & token
...
Else
	...
	If ack="APPROVED" Then
		...
		query="SELECT payTypes.idPayment FROM payTypes WHERE payTypes.active = - 1 AND payTypes.gwCode = 99;"
		set rs=conntemp.execute(query)
		if not rs.eof then
