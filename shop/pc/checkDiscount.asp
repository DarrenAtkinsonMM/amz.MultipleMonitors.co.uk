...
if rsDisObj.eof then
	...
	If ponetime=true Then
		...
		query="SELECT A.idcustomer FROM used_discounts A,customers B WHERE A.iddiscount=" & piddiscount & " AND A.idcustomer=B.idcustomer AND B.email IN (SELECT C.email FROM customers C WHERE C.idcustomer="&pcv_IDCustomer&");"
...
'************ FILTERS
	...
	IF pcv_IDDiscount<>"" THEN	
								...
								query="select pcFCust_IDCustomer from PcDFCusts where pcFCust_IDDiscount=" & pcv_IDDiscount & " and pcFCust_IDCustomer=" & pcv_IDCustomer
								set rsF=server.CreateObject("ADODB.RecordSet")
								set rsF=connTemp.execute(query)
								if err.number<>0 then
									call LogErrorToDatabase()
								...
								query="select pcDFCustPriceCats.pcFCPCat_IDCategory from pcDFCustPriceCats, Customers where pcDFCustPriceCats.pcFCPCat_IDDiscount=" & pcv_IDDiscount & " and pcDFCustPriceCats.pcFCPCat_IDCategory = Customers.idCustomerCategory and Customers.idcustomer=" & pcv_IDCustomer
								set rsF=server.CreateObject("ADODB.RecordSet")
								set rsF=connTemp.execute(query)
								if err.number<>0 then
									call LogErrorToDatabase()
