...
'// Order number is valid
		...
		pcv_GCDetails=rs("pcOrd_GCDetails")
		if isNULL(pcv_GCDetails) OR pcv_GCDetails="" then
			pcv_GCDetails="0"
		end if
		'GGG Add-on start
		...
		Dim pSku, pcDPs, ptotal, ppaymentDetails,ptaxamount,pshipmentDetails, pdiscountDetails, pQtyDiscounts
		...
		do while not rsOrdObj.eof
			...
			pItemTotal = pItemTotal + pRowPrice1
			pQtyDiscounts = pQtyDiscounts + QDiscounts
			...
			if NOT pcv_strPayPalmethod="PayPalStandard" then
				objPayPalClass.AddNVPLineItem "NAME"&count, replace(pdescription, "&quot;", "")
				objPayPalClass.AddNVPLineItem "NUMBER"&count, pSku
				objPayPalClass.AddNVPLineItem "QTY"&count, pquantity				
				objPayPalClass.AddNVPLineItem "AMT"&count, tmpFinalRowPrice
		...
		if PayCharge>0 then
			...
			if NOT pcv_strPayPalmethod="PayPalStandard" then
				...
				objPayPalClass.AddNVPLineItem "NAME"&count, dictLanguage.Item(Session("language")&"_PayPal_7")
				objPayPalClass.AddNVPLineItem "NUMBER"&count, ""
				objPayPalClass.AddNVPLineItem "QTY"&count, 1				'
				objPayPalClass.AddNVPLineItem "AMT"&count, tmpFinalPayCharge
		...
		CouponArr = split(pdiscountDetails, ",")
		For i = 0 To UBound(CouponArr)
			CouponDetails = split(CouponArr(i),"||")
			
			'$1 off (1off) - || 1
			
			If UBound(CouponDetails) >= 1 Then
			
				CouponDesc=Replace(CouponDetails(0), " - ", "")
				
				'// Get the Coupon Name and Code
				CouponDescArr=Split(CouponDesc, "(")
				If UBound(CouponDescArr) >= 1 Then
					CouponName=CouponDescArr(0)
					CouponCode=Replace(CouponDescArr(1), ")", "")
				Else
				...
				If Len(CouponCode) > 0 Then
					query = "select pcDFShip.pcFShip_IDShipOpt FROM pcDFShip WHERE pcDFShip.pcFShip_IDDiscount IN (SELECT iddiscount FROM discounts WHERE discountcode = '" & CouponCode & "')"
					set rs = conntemp.execute(query)
				...
				If CouponAmt > 0 Then				
					tmpFinalCouponAmt = "-" & pcf_CurrencyField(money(CouponAmt))
						
					If hasFreeShipping Then
						pcv_strFinalShipDiscount = tmpFinalCouponAmt
					Else									
						pcv_strFinalCouponAmt = pcv_strFinalCouponAmt + CCur(CouponAmt)  '// Add the coupon amount to the total

						If Not pcv_strPayPalmethod="PayPalStandard" Then
							objPayPalClass.AddNVPLineItem "NAME"&count, CouponDesc
							objPayPalClass.AddNVPLineItem "NUMBER"&count, ""
							objPayPalClass.AddNVPLineItem "QTY"&count, 1				'
							objPayPalClass.AddNVPLineItem "AMT"&count, tmpFinalCouponAmt
						Else				
							%>
		...
		pcv_strFinalCouponAmt = money(pcv_strFinalCouponAmt)
		...
		'**************************************************************
		Dim pcv_strRewardDiscount
		if piRewardPoints<>"0" then
		
			'// Adjust the reward points discount value in case it's greater than the item total
			If piRewardValue > pItemTotal Then
				piRewardValue = pItemTotal	
			End If
			
			pcv_strRewardDiscount = money(piRewardValue) '// Use the money function to synch with the Invoice.
			tmpRewardDiscount = pcv_strRewardDiscount
			tmpRewardDiscount = pcf_CurrencyField(tmpRewardDiscount)
			tmpRewardDiscount = "-" & tmpRewardDiscount
			
			if NOT pcv_strPayPalmethod="PayPalStandard" then
				
				objPayPalClass.AddNVPLineItem "NAME"&count, dictLanguage.Item(Session("language")&"_PayPal_10") & " (" & piRewardPoints & " " & RewardsLabel & ")"
				objPayPalClass.AddNVPLineItem "NUMBER"&count, ""
				objPayPalClass.AddNVPLineItem "QTY"&count, 1
				objPayPalClass.AddNVPLineItem "AMT"&count, tmpRewardDiscount
		...
		'**************************************************************
		' START: Product Quantity Discounts
		'**************************************************************
		Dim pcv_strFinalQtyDiscounts
		if pQtyDiscounts>0 then
			pcv_strFinalQtyDiscounts = money(pQtyDiscounts) '// Use the money function to synch with the Invoice.
			tmpFinalQtyDiscounts = pcv_strFinalQtyDiscounts
			tmpFinalQtyDiscounts = pcf_CurrencyField(tmpFinalQtyDiscounts)
			tmpFinalQtyDiscounts = "-" & tmpFinalQtyDiscounts
			
			if NOT pcv_strPayPalmethod="PayPalStandard" then
				
				objPayPalClass.AddNVPLineItem "NAME"&count, dictLanguage.Item(Session("language")&"_PayPal_12")
				objPayPalClass.AddNVPLineItem "NUMBER"&count, ""
				objPayPalClass.AddNVPLineItem "QTY"&count, 1
				objPayPalClass.AddNVPLineItem "AMT"&count, tmpFinalQtyDiscounts
			...
			count=count+1
		Else
			pcv_strFinalQtyDiscounts = money(0)			
		end if
		'**************************************************************
		' END: Product Quantity Discounts
		'**************************************************************
		...
		if pCatDiscounts>0 then
			...
			if NOT pcv_strPayPalmethod="PayPalStandard" then
				...
				objPayPalClass.AddNVPLineItem "NAME"&count, dictLanguage.Item(Session("language")&"_PayPal_8")
				objPayPalClass.AddNVPLineItem "NUMBER"&count, ""
				objPayPalClass.AddNVPLineItem "QTY"&count, 1
				objPayPalClass.AddNVPLineItem "AMT"&count, tmpFinalCatDiscounts
		...
		'**************************************************************
		' START: Gift Certificate Discounts
		'**************************************************************
		If pcv_GCDetails <> "0" Then
			GCArr = Split(pcv_GCDetails, "|g|")
			For i = 0 To UBound(GCArr)
			
				'$1 off (1off) - || 1
				GCDetails = split(GCArr(i), "|s|")
				
				If UBound(GCDetails) >= 1 Then
					GCCode=GCDetails(0)
					GCDesc=GCDetails(1)
					If GCDetails(2)="" Then
						GCAmt=0
					...
					if GCAmt > 0 then
						...
						pcv_strFinalGiftCertAmt = pcv_strFinalGiftCertAmt + CCur(GCAmt)  '// Add the coupon amount to the total
						tmpFinalGiftCertAmt = "-" & pcf_CurrencyField(money(GCAmt))							
						
						if NOT pcv_strPayPalmethod="PayPalStandard" then		
							objPayPalClass.AddNVPLineItem "NAME"&count, GCDesc
							objPayPalClass.AddNVPLineItem "NUMBER"&count, ""
							objPayPalClass.AddNVPLineItem "QTY"&count, 1				'
							objPayPalClass.AddNVPLineItem "AMT"&count, tmpFinalGiftCertAmt
						else				
							%>
								<input type="hidden" name="item_name_<%=count%>" value="<%= GCDesc %>">
								<input type="hidden" name="quantity_<%=count%>" value="1"> 				
								<% if scDecSign = "," then %>
								<input type="hidden" name="amount_<%=count%>" value="<%=tmpFinalGiftCertAmt%>">
								<% else %>
								<input type="hidden" name="amount_<%=count%>" value="<%=tmpFinalGiftCertAmt%>">
								<% end if %>
							<%	
						...
						count=count+1
					Else
						pcv_strFinalGiftCertAmt = money(0)
						tmpFinalGiftCertAmt = money(0)			
					End If
				End If
			Next
		Else
		...
		End If
		'**************************************************************
		' END: Gift Certificate Discounts
		'**************************************************************
		...
		if pGWTotal>0 then		
			...
			if NOT pcv_strPayPalmethod="PayPalStandard" then
				...
				objPayPalClass.AddNVPLineItem "NAME"&count, dictLanguage.Item(Session("language")&"_PayPal_9")
				objPayPalClass.AddNVPLineItem "NUMBER"&count, ""
				objPayPalClass.AddNVPLineItem "QTY"&count, 1				'
				objPayPalClass.AddNVPLineItem "AMT"&count, tmpFinalGiftWrapping
		...
		pcv_strRewardDiscount=cCur(pcf_CurrencyField(pcv_strRewardDiscount))
		pcv_strFinalShipDiscount=cCur(pcf_CurrencyField(pcv_strFinalShipDiscount))
		pcv_strFinalGiftCertAmt=cCur(pcf_CurrencyField(pcv_strFinalGiftCertAmt))
		
		'// Perform the "ItemTotal" Calculation
		ItemTotal = pcv_strFinalTotal - ((pcv_strFinalShipCharge+pcv_strFinalShipDiscount)+pcv_strFinalServiceCharge+pcv_strFinalTax)
