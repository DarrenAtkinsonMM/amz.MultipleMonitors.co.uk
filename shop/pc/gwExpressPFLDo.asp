...
If ack=0 Then
	...
	if session("GWTransId") <> "" then
		...
		query="INSERT INTO pcPay_PFL_Authorize (idOrder, orderDate, amount, paySource, paymentmethod, transtype, authcode, captured, gwCode) VALUES ("&pcGatewayDataIdOrder&", '" & Date() & "', "&pcBillingTotal&", 'PayPalExp', '" & paymentMethod & "', '" & PaymentAction & "', '"&session("GWTransId")&"', 0, 99);"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=connTemp.execute(query)				
		if err.number<>0 then
			call LogErrorToDatabase()
