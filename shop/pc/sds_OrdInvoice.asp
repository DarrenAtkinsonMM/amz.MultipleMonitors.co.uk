	...
	<div id="pcMain">
		...
		Dim pidcustomer, porderdate, pAddress, pAddress2, pcity, pstateCode, pstate, pzip, pCountryCode, ppaymentDetails, pshipmentDetails, pshippingCompany, pshippingAddress, pshippingAddress2, pshippingCity, pshippingStateCode, pshippingState,pshippingZip, pshippingCountryCode, pshippingPhone, pidAffiliate, paffiliatePay, pdiscountDetails, ptaxAmount, ptotal, pcomments, porderStatus, pprocessDate, pshipDate, pshipvia, ptrackingNum, preturnDate, preturnReason,ptaxDetails,pSRF, pord_DeliveryDate, pord_OrderName, pcgwAuthCode, pcgwTransId, pcpaymentCode
				...
				' Show message is the customer is trying to view an order that is not his/hers
						...
						<div class="pcTableRow">
							<div style="width:25%;">
								<% If Len(scCompanyLogo) Then %>
									<img src="<%=pcf_getImagePath("../pc/catalog",scCompanyLogo)%>" alt="<%=scCompanyName%>">
								<% End If %>
							</div>
							...
							<div style="width:23%;float:right;"> 
								<div class="invoice">
									<%response.write dictLanguage.Item(Session("language")&"_custOrdInvoice_1")%>  
									<%
									if porderdate <> "" then
						...
						<div class="pcTableRow">
							<div style="width:50%;padding-left:0;">	
								<div class="invoice">
									...
									<div>&nbsp;</div>
									<div class="invoice">
										<strong><%response.write dictLanguage.Item(Session("language")&"_custOrdInvoice_17")%></strong>:
										<br>
										<% 
								...
								<div style="width:48%;float:right;padding-right:0;">
									<div class="invoice">
										<b><%response.write dictLanguage.Item(Session("language")&"_custOrdInvoice_5") & (scpre+int(qry_ID))%></b>
									</div>
									...
									%>
									<div class="invoice">
										<%response.write dictLanguage.Item(Session("language")&"_custOrdInvoice_6") & pcCustomerNumber%>
									</div>

									<%	if scOrderName="1" then
										if trim(pord_OrderName) <> "" Then%>
											<div class="invoice">
												<%response.write dictLanguage.Item(Session("language")&"_custOrdInvoice_7") & pord_OrderName %>
											</div>
										<% 	end If
									end if %>
									<% If trim(pord_DeliveryDate) <> "1/1/1900" and trim(pord_DeliveryDate) <> "" Then
										...
										%>
										<div class="invoice">
											<%response.write dictLanguage.Item(Session("language")&"_custOrdInvoice_8") & pord_DeliveryDate & ", " & pord_DeliveryTime%>
										</div>
									<% End If %>
									<div class="invoice">
									...
									<div class="invoice">
										<%response.write dictLanguage.Item(Session("language")&"_sds_custviewpastD_16")%>
										<% 
										Shipper=""
										Service=""
										Postage=""
      			...
      			<div class="pcTableRow">
							<%
								col_qtyClass			= "pcCol-1"	
								col_skuDescClass	= "pcCol-7"	
								col_right1Class		= "pcCol-2"	
								col_right2Class		= "pcCol-2"	
							%>
							<div class="pcTable invoice" style="padding: 0 0 8px 0;">
								<div class="pcTableHeader"> 
									<div class="<%= col_qtyClass %>"><%= dictLanguage.Item(Session("language")&"_custOrdInvoice_18")%></div>
									<div class="<%= col_skuDescClass %>"><%= dictLanguage.Item(Session("language")&"_custOrdInvoice_19")%></div>
									<div class="<%= col_right1Class %>"></div>
									<div class="<%= col_right2Class %>"></div>
								</div>
								<% 
									...
									query="SELECT ProductsOrdered.idProduct, ProductsOrdered.quantity, ProductsOrdered.pcPrdOrd_SelectedOptions, ProductsOrdered.pcPrdOrd_OptionsPriceArray, ProductsOrdered.pcPrdOrd_OptionsArray, ProductsOrdered.unitPrice, ProductsOrdered.xfdetails,ProductsOrdered.QDiscounts,ProductsOrdered.ItemsDiscounts"
										...
										Do until rsTemp.eof
											...
											<div class="pcTableRow"> 
												<div class="<%= col_qtyClass %>"><%=pquantity%></div>
												<div class="<%= col_skuDescClass %>"><%=psku%> - <%=pDescription%></div>
												<div class="<%= col_right1Class %>"></div>
												<div class="<%= col_right2Class %>"></div>
											</div>
											<% 'CONFIGURATOR ADDON-S
							...
							%>
											...
											<div class="pcTableRow">
												...
												%>
												<div class="<%= col_skuDescClass %>"><%=pcArray_strOptions(pcv_intOptionLoopCounter) %></div>
										...
										<% if pxdetails<>"" then %>
											<div class="pcTableRow"> 
												<div class="<%= col_qtyClass %>">&nbsp;</div>
												<div class="<%= col_skuDescClass %>"><%=pxdetails%></div>
												<div class="<%= col_right1Class %>">&nbsp;</div>
												<div class="<%= col_right2Class %>">&nbsp;</div>
											</div>
										<% end if %>
