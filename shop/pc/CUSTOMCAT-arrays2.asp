...
%>
<%
'DA Edit - Jump logic for array breadcrumb modification
if request.querystring("mid") = "" then
	bunBCpagechk = 0
else
	bunBCpagechk = request.querystring("mid")
end if

if not bunBCpagechk = 0 then
'We are on the choose monitor page and have a monitor selection already
	if not request.querystring("sid") = "" then
	'we have a monitor and stand selection so jump array final page
		response.redirect("/display-systems-3/?sid=" & request.querystring("sid") & "&mid=" & request.querystring("mid"))
	else
	'go back to stand selection page
		response.redirect("/display-systems/?mid=" & request.querystring("mid"))
	end if
end if
%>
...
'*******************************
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
' DA EDIT - Check for querystring value for stand id, if not there redirect back to bundles step 1	
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
if request.querystring("sid") = "" then
	Response.Status="301 Moved Permanently"
	Response.AddHeader "Location","/display-systems/"
	Response.End
end if
...
If validNum(pTempIntSubCategory) and pTempIntSubCategory<>1 then
	...
	if rs.EOF then
		set rs=nothing
		call closeDb()
		response.redirect "msg.asp?message=867"
	end if	
...
pcv_PageName = "Monitor Display Systems &mdash; Choose Your Monitors | Multiple Monitors"
pcv_DefaultDescription = "Step 2 of the monitor display array builder. Choose the monitors to pair with your selected stand. Matched bezels for clean multi-screen displays."
%>
...
<!--#include file="array-breadcrumb.asp"-->
    ...
    <%
        ...
        <% End If %>
			...
			<h5 class="text-uppercase color-med h-semi bundle-sub bundle-sub1 marginbot-20"><a style="text-decoration: underline; line-height: 1.5;" href="/pages/monitor-resolutions/">Unsure on monitor resolutions and sizes?<br />View our quick guide here</a></h3>
			</div>
	</section>
    <!-- /Section: Welcome -->
