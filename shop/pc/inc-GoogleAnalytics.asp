...
IF pcGAorderComplete=1 AND scGAType<>"2" AND (trim(scGoogleAnalytics)<>"" AND NOT IsNull(scGoogleAnalytics)) THEN ' 1
		...
		'// STEP 1 - START
			...
			IF pGetItems<>0 THEN ' 2 - If database errors, skip everything
				...
				IF scGAType="1" THEN '//Google Universal Analytics
					...
					pcGAtransaction = pcGAtransaction & "'id': '" & scpre+int(pIdOrder) & "'," & VbCrLf
					pcGAtransaction = pcGAtransaction & "'affiliation': '" & pcf_GoogleEscape(paffiliateName) & "'," & VbCrLf
			...
			IF pGetItems<>0 THEN ' 3 - If database errors, skip everything
				...
				Do While Not rs.eof
					...
					IF scGAType="1" THEN '//Google Universal Analytics
						...
						pcGAtransactionItems = pcGAtransactionItems & "'id': '" & scpre+int(pIdOrder) & "'," & VbCrLf
						pcGAtransactionItems = pcGAtransactionItems & "'name': '" & pcf_GoogleEscape(pName) & "'," & VbCrLf
						pcGAtransactionItems = pcGAtransactionItems & "'sku': '" & pcf_GoogleEscape(pSKU) & "'," & VbCrLf
						pcGAtransactionItems = pcGAtransactionItems & "'category': '" & pcf_GoogleEscape(pCategory) & "'," & VbCrLf
