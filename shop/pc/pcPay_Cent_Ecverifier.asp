...
If (pares <> "") Then
	...
	if intDebug=1 then
		...
		If Session("Centinel_ErrorNo") = "0" AND Session("Centinel_SignatureVerification") = "N" Then
            ...
            Session("message") = "Your Credit Card Issuer has indicated that the transaction was not able to be authenticated. To protect against unauthorized use, this card cannot be used to complete your purchase. Please provide another form of payment to complete your purchase."
            Session("backbuttonURL") = tempURL & "?psslurl=gwAuthorizeAIM.asp&idCustomer=" & session("reqCustomerID") & "&idOrder=" & session("reqOrderID") & "&ordertotal=" & session("AIMOrderTotal") & "&billingFirstName=" & session("reqBillFirstName") & "&billingLastName=" & session("reqBillLastName") & "&billingAddress=" & session("reqBillAddress") & "&billingZip=" & session("reqBillZip") & "&billingEmail=" & session("reqBillEmail") & "&idDbSession=" & pIdDbSession & "&randomKey=" & pRandomKey & "&billingCompany=" & session("reqBillCompany") & "&password=&billingPhone=" & session("reqBillPhone") & "&billingStateCode=" & session("reqBillState") & "&billingState=" & session("reqBillState") & "&billingCity=" & session("reqBillCity") & "&billingCountryCode=" & session("reqBillCountry") & "&shippingFullName=" & session("reqShipFirstName") & " " & session("reqShipLastName") & "&shippingCompany=" & x_ship_to_company & "&shippingAddress=" & session("reqShipAddress") & "&shippingcity=" & session("reqShipCity") & "&shippingState=" & session("reqShipState") & "&shippingStateCode=" & session("reqShipState") & "&shippingZip=" & session("reqShipZip") & "&shippingCountryCode=" & session("reqShipCountry")
        ...
        Else
            ...
            Session("message") = "Your Credit Card transaction was not able to be authenticated. Please provide another form of payment."
            Session("backbuttonURL") = tempURL & "?psslurl=gwAuthorizeAIM.asp&idCustomer=" & session("reqCustomerID") & "&idOrder=" & session("reqOrderID") & "&ordertotal=" & session("AIMOrderTotal") & "&billingFirstName=" & session("reqBillFirstName") & "&billingLastName=" & session("reqBillLastName") & "&billingAddress=" & session("reqBillAddress") & "&billingZip=" & session("reqBillZip") & "&billingEmail=" & session("reqBillEmail") & "&idDbSession=" & pIdDbSession & "&randomKey=" & pRandomKey & "&billingCompany=" & session("reqBillCompany") & "&password=&billingPhone=" & session("reqBillPhone") & "&billingStateCode=" & session("reqBillState") & "&billingState=" & session("reqBillState") & "&billingCity=" & session("reqBillCity") & "&billingCountryCode=" & session("reqBillCountry") & "&shippingFullName=" & session("reqShipFirstName") & " " & session("reqShipLastName") & "&shippingCompany=" & x_ship_to_company & "&shippingAddress=" & session("reqShipAddress") & "&shippingcity=" & session("reqShipCity") & "&shippingState=" & session("reqShipState") & "&shippingStateCode=" & session("reqShipState") & "&shippingZip=" & session("reqShipZip") & "&shippingCountryCode=" & session("reqShipCountry")
            response.redirect "msgb.asp?back=1"
