...
if pcv_HavePrdPromotions=1 then
	For k=1 to pcTmpIndex '// For each product in cart array
		For m=0 to PrdPromoCount '// For each promo available
			...
			if clng(pcf_ProductIdFromArray(pcTmpArr, k))=clng(PrdPromoArr(1,m)) then
				call CalPrdPromo(k,m) '// Apply Promo
				pcv_strUsedPrdPromo = pcv_strUsedPrdPromo & pcf_ProductIdFromArray(pcTmpArr, k) & "_" & m & "," '// Keep track for cleaning sessions
			end if
		Next
	Next
end if
...
Function GetAvailableUnits(tmpC,tmpP)
	...
	pcv_intBaseID = pcf_ProductIdFromArray(pcTmpArr, tmpC) 
	For UnitCount=1 to pcTmpIndex
		If cdbl(pcv_intBaseID) = cdbl(pcf_ProductIdFromArray(pcTmpArr, UnitCount)) Then
...
Sub CalPrdPromo(tmpC,tmpP)
	...
	IF pcv_Filters=pcv_FResults THEN
		...
		'Have Trigger
		if (tmpProductQty-tmpQtyTrigger>=0) AND (Session("UA_" & pcf_ProductIdFromArray(pcTmpArr, tmpC) & "_" & tmpP) <> "DONE") then
			...
			IF pcv_HavePrdTargets=1 OR (pcv_HavePrdTargets=0 AND pcv_HaveCatTargets=0) THEN
				...
				Do while (tmpProductQty>0) AND (clng(tmpProductQty)-clng(tmpQtyTrigger)>=0)					
					...
					For j=1 to pcTmpIndex '// For each item in cart array
						...
						if pcTmpArr(j,10)="0" AND tmpProductQty>"0" AND ((tmpPrdTargetStr="") OR (InStr(tmpPrdTargetStr,"**" & pcf_ProductIdFromArray(pcTmpArr, j) & "**")>0)) then	
							if tmpAll=1 then
								
								'// APPLY TO ALL
								tmpTotal=tmpTotal+(tmpProductQty*Session("UAP_" & pcf_ProductIdFromArray(pcTmpArr, tmpC) & "_" & tmpP))
								...
								if tmpU-clng(tmpProductQty)>=0 then '// If all "n" units are available
									...
									tmpTotal=tmpTotal+(tmpU*Session("UAP_" & pcf_ProductIdFromArray(pcTmpArr, tmpC) & "_" & tmpP))
					...
					Session("UA_" & pcf_ProductIdFromArray(pcTmpArr, tmpC) & "_" & tmpP) = "DONE" '// UA_ProductID_PromoID
