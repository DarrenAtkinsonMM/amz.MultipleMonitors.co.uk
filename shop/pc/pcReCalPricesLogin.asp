...
If (NeedReCalculate=1 OR Session("customerType")=1 OR session("customerCategory")<>0) Then
	
	'---start recalculations
	for t=1 to ppcCartIndex 'Start Loop
		if (pcCartArray(t,10)=0) AND (Instr(session("sf_FQuotes"),"****" & pcCartArray(t,0) & "****")=0) then 'if product not remove and not a Finalized Quote
			pcCartArray(t,18)=0
					...
					IF pcCartArray(t,16)<>"0" and pcCartArray(t,16)<>"" then
						...
						if Pstring<>"" and uCase(Pstring)<>"NA" then
							...
							For m=lbound(ArrProduct) to (UBound(ArrProduct)-1)
								...
								pcv_intIdProduct = pcf_GetParentId(ArrProduct(m))
								pcProd_ParentPrd = pcv_intIdProduct
																
								tmpIDeQty=GetItemDefaultQty(pcv_intIdProduct)
							
								if Clng(ArrQuantity(m))<Clng(tmpIDeQty) then 'Check item Quantity
									ArrQuantity(m)=tmpIDeQty
								end if
								
								query="SELECT price, Wprice,cdefault FROM configSpec_products WHERE specProduct="&pSIdProduct&" AND configProduct=" &ArrProduct(m) & " AND configProductCategory=" & ArrCategory(m)
								set rstemp=server.CreateObject("ADODB.RecordSet")
								set rstemp=conntemp.execute(query)
								...
								if NOT rstemp.eof then	 
									...
									cdefault=rstemp("cdefault")

								else

									query="SELECT price, BtobPrice,pcProd_ParentPrd FROM Products WHERE IDProduct=" &ArrProduct(m) & " AND pcProd_ParentPrd>0;"
									set rstemp=server.CreateObject("ADODB.RecordSet")
									set rstemp=conntemp.execute(query)
									if NOT rstemp.eof then	 
										tmpItemWPrice_A=rstemp("BtobPrice")
										tmpItemPrice_A=rstemp("price")
										pcProd_ParentPrd=rstemp("pcProd_ParentPrd")
									end if
									
									queryQ="SELECT price,btoBPrice FROM Products WHERE idProduct=" & pcv_intIdProduct & ";"
									set rsQ=connTemp.execute(queryQ)
									if not rsQ.eof then
										pcParentPrice=rsQ("price")
										pcParentWPrice=rsQ("btoBPrice")										

										queryQ="SELECT price,Wprice,cdefault FROM configSpec_Products WHERE specProduct=" & pSIdProduct & " AND configProduct=" & pcProd_ParentPrd  & " AND configProductCategory=" & ArrCategory(m) & ";"
										set rsQ2=connTemp.execute(queryQ)
										if not rsQ2.eof then
											pcPriceConf=rsQ2("price")
											pcWPriceConf=rsQ2("Wprice")
											cdefault=rsQ2("cdefault")
											set rsQ2=nothing
											
											if ccur(pcPriceConf)<>ccur(pcParentPrice) then
												tmpItemPrice_A=pcPriceConf
												tmpItemWPrice_A=pcWPriceConf
												HaveDiffPrice=1
											else
								...
								ELSE
									tmpItemPrice=tmpItemPrice_A
									tmpItemWPrice=tmpItemWPrice_A
								END IF
								...
								'if cdbl(ArrValue(m))<>0 then
									...
									tmpDefaultItemPrice=GetDefaultPrice(pSIdProduct,ArrCategory(m),pcv_intIdProduct)*GetDefaultQty(pSIdProduct,ArrCategory(m),pcProd_ParentPrd)
									tmpDef=ChkMultiSelectDef(pSIdProduct,ArrCategory(m),pcv_intIdProduct)
							
									'Radio/Dropdown Item
									if tmpDef=2 then
										'Default item
										if clng(cdefault)<>0 then
											BTODefaultPrice=BTODefaultPrice+tmpDefaultItemPrice
											ArrValue(m)=0
											AllItemPrices=AllItemPrices+(clng(ArrQuantity(m))-clng(tmpIDeQty))*cdbl(ArrPrice(m))
										else
											BTODefaultPrice=BTODefaultPrice+tmpDefaultItemPrice
											ArrValue(m)=((ArrPrice(m)*clng(tmpIDeQty))-cdbl(tmpDefaultItemPrice))
											AllItemPrices=AllItemPrices+cdbl(ArrPrice(m))*clng(tmpIDeQty)-cdbl(tmpDefaultItemPrice)+(clng(ArrQuantity(m))-clng(tmpIDeQty))*cdbl(ArrPrice(m))
										end if
										
									else
										'Not default checkbox
										if tmpDef=1 then
											tmpDefaultItemPrice=0
											ArrValue(m)=cdbl(ArrPrice(m))*clng(tmpIDeQty)-cdbl(tmpDefaultItemPrice)
											AllItemPrices=AllItemPrices+cdbl(ArrPrice(m))*clng(tmpIDeQty)-cdbl(tmpDefaultItemPrice)+(clng(ArrQuantity(m))-clng(tmpIDeQty))*cdbl(ArrPrice(m))
										else
											'Default checkbox
											if tmpDef=0 then
								...
								' get items discounts
								'====================
								pcv_IdProduct = pcf_GetParentId(ArrProduct(m))

								query="SELECT quantityFrom,quantityUntil,discountperUnit,percentage,discountperWUnit FROM discountsPerQuantity WHERE IDProduct=" & ArrProduct(m) & pcv_IdProduct 
								set rstemp=connTemp.execute(query)
	...
	If session("customerType")=1 AND ppcCartIndex>0 Then
		if (calculateCartTotal(pcCartArray, ppcCartIndex)<scWholesaleMinPurchase) and (Session("SFStrRedirectUrl")="") then
			'Session("message") = dictLanguage.Item(Session("language")&"_checkout_2")& scCurSign &money(scWholesaleMinPurchase)&dictLanguage.Item(Session("language")&"_techErr_3")&"<BR><BR><a href=""viewCart.asp"">"&dictLanguage.Item(Session("language")&"_titles_3")&"</a> : <a href=""default.asp"">"&dictLanguage.Item(Session("language")&"_titles_5")&"</a></b>"
