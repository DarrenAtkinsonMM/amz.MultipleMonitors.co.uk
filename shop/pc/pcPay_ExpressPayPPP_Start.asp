...
returnURL	= url & "pcPay_ExpressPayPPP_Start.asp?currencyCodeType=" &  currencyCodeType & "&paymentAmount=" & OrderTotal & "&paymentType=" &PaymentAction 
...
Else
	...
	If ack="APPROVED" Then
		...
		query="SELECT payTypes.idPayment FROM payTypes WHERE payTypes.active = - 1 AND payTypes.gwCode = 53;"
		set rs=conntemp.execute(query)
		if not rs.eof then
