...
<div class="pcShowCategoryImage">
	<%if NotImg<>"1" then %>
		...
		<% else%>
			<a href="<%=pcStrCatLink%>" data-idCategory="<%= intIdCategory %>" title="<%=pcStrCategoryDesc%>"><img src="<%=pcf_getImagePath("catalog","no_image.gif")%>" width="50" height="50" alt="<%=pcStrCategoryDesc%>"></a>
		<%end if%>
    <%end if %>
</div>
<div class="pcShowCategoryInfo">
	<div class="pcShowCategoryName">
  	<a href="<%=pcStrCatLink%>" data-idCategory="<%= intIdCategory %>" title="<%=pcStrCategoryDesc%>"><%=pcStrCategoryDesc%></a>
 	</div>
