...
if request.ServerVariables("CONTENT_LENGTH") > 0 then
	...
	if session("DF1")<>"" then
		If Not IsDate(session("DF1")) then
			Session("message") = dictLanguage.Item(Session("language")&"_catering_2")
	...
	If DTCheck="1" then
		if session("DF1")<>"" then
			...
			if DF2-Date()<=0 then
				Session("message") = dictLanguage.Item(Session("language")&"_catering_3")
				response.redirect "login.asp"
			else
				if (DF2-Date()=1) then
					if session("TF1")<>"" then
...
If request("msg") = "" Then
	if session("DF1")<>"" and IsDate(session("DF1")) Then
		...
		if icounter > 0 then
			call closeDb()
			Session("message") = dictLanguage.Item(Session("language")&"_catering_5")
			response.redirect "login.asp"
		end if
...
If DeliveryZip = "1" Then
	...
	if rsZipCodeObj.eof then
		...
		call closeDb()
		Session("message") = dictLanguage.Item(Session("language")&"_Custmoda_23")
		response.redirect "login.asp"
	end if
