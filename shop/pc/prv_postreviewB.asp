...
IF (FCount>0) and (request("action")="add") THEN
	...
	IF scSecurity=1 THEN
		if scReview=1 then
			...
			if pcv_Test=0 then
				if (InStr(ucase(Request.servervariables("HTTP_REFERER")),ucase(strSiteURL & "prv_ViewOrder.asp"))<>1) AND (InStr(ucase(Request.servervariables("HTTP_REFERER")),ucase(strSiteURL & "viewPrd.asp"))<>1) AND (InStr(ucase(Request.servervariables("HTTP_REFERER")),ucase("p" & tmpTrueID & ".htm"))<1) then
					Session("store_ReviewReg")=""
					Session("store_ReviewRegpostnum")=""
					Session("store_ReviewRegnum")=""
					pcv_Test=1
			...
			if pcv_Test=0 then
                ...
                If not blnCAPTCHAcodeCorrect then				
					Session("store_ReviewReg")=""
					pcv_Test=1
					response.redirect "prv_postreview.asp?IDPRoduct="&pcv_IDProduct&"&msg=1"
				end if
			end if
	...
	query="INSERT INTO pcReviews (pcRev_IDProduct,pcRev_Active,pcRev_IP,pcRev_Date,pcRev_MainRate,pcRev_MainDRate, pcRev_IDCustomer,pcRev_IdOrder) VALUES (" & pcv_intIdProduct & "," & RevActive & ",'" & pcv_IPAddress & "'," & dtTodaysDate & "," & pcv_Feel & "," & pcv_Rate & "," & TestRegCust(fnZeroIfNull(pIdCustomer)) & "," & tmpIdOrder & ")"
	' PRV41 end
	set rs=connTemp.execute(query)
	...
	query="SELECT pcRev_IDReview FROM pcReviews WHERE pcRev_IDProduct=" & pcv_intIdProduct & " order by pcRev_IDReview desc"
	set rs=connTemp.execute(query)
		if err.number<>0 then
			call LogErrorToDatabase()
			set rs=nothing
	...
	For m=0 to FCount-1
		...
		query="INSERT INTO pcReviewsData (pcRD_IDReview,pcRD_IDField,pcRD_Feel,pcRD_Rate,pcRD_Comment) VALUES (" & pcv_IDReview & "," & Rev_IDField & "," & Rev_Feel & "," & Rev_Rate & ",N'" & Rev_Com & "')"
		set rs=connTemp.execute(query)
	...
	If Not rs.Eof Then
	   If rs("pcRS_NeedCheck")=0 then
	     	...
	     	query = "UPDATE Products SET pcProd_AvgRating = " & GetOverallProductRating(pcv_intIdProduct) & " WHERE idProduct=" & pcv_intIdProduct
	      	set rs2 = connTemp.execute(query)
			set rs2 = Nothing
			call pcs_hookProductModified(pcv_intIdProduct, "")
			  ...
			  '// Thank You message to customer - START
				...
				if pcIntSendMessage=1 then
					...
					'// Send message
					 ...
					 call pcs_hookProductReviewEmailSent(pcStrCustEmail)
	...
	SPathInfo=SPathInfo&scPcFolder&"/"&scAdminFolderName&"/prv_ManageReviews.asp?IDProduct=" & pcv_intIdProduct & "&nav=1"	
	strNewOrderSubject=dictLanguage.Item(Session("language")&"_prv_17") & pcv_intIdProduct
	
	storeAdminEmail="" & dictLanguage.Item(Session("language")&"_prv_18") & vbcrlf & vbcrlf

	storeAdminEmail=storeAdminEmail&dictLanguage.Item(Session("language")&"_prv_19") & pcv_intIdProduct & vbcrlf
