<%if IsNull(pcv_IsSupplier) or pcv_IsSupplier="" then
	...
	If pcv_strShowSection = -1 Then
		...
		If pcv_DS_NoticeM=0 OR pcv_ManualSend=1 Then
			...
			pcv_PrdList=""
			query="SELECT DISTINCT Products.idproduct,Products.Description,Products.SKU,ProductsOrdered.quantity,ProductsOrdered.idconfigSession,ProductsOrdered.pcPrdOrd_SelectedOptions,ProductsOrdered.pcPrdOrd_OptionsArray,ProductsOrdered.xfdetails FROM pcDropShippersSuppliers INNER JOIN (Products INNER JOIN ProductsOrdered ON products.idproduct=ProductsOrdered.idproduct) ON (((pcDropShippersSuppliers.idproduct=ProductsOrdered.idproduct) OR (pcDropShippersSuppliers.idproduct = products.pcprod_ParentPrd)) AND (pcDropShippersSuppliers.pcDS_IsDropShipper=" & pcv_IsSupplier & ")) WHERE ProductsOrdered.idorder=" & qry_ID & " AND ProductsOrdered.pcDropShipper_ID=" & pcv_DropShipperID
			set rsQ1=connTemp.execute(query)
						...
						if pIdConfigSession<>"0" then
							...
							if stringProducts<>"na" then
								for i=lbound(ArrProduct) to (UBound(ArrProduct)-1)
									...
									query="SELECT displayQF FROM configSpec_Products WHERE configProduct="& pcv_intIdProduct &" AND specProduct=" & pcv_IDProduct
									set rsQ2=Server.CreateObject("ADODB.Recordset")
									set rsQ2=conntemp.execute(query)
									if err.number<>0 then
										call LogErrorToDatabase()
