...
IF tmpCartHasPrds=1 THEN
	...
	IF tmpGUID<>"" THEN
		...
		if SaveCustLogin=1 then
			query="SELECT SavedCartID,SavedCartName FROM pcSavedCarts WHERE SavedCartGUID like '" &  tmpGUID & "';"
		else
			query="SELECT SavedCartID,SavedCartName FROM pcSavedCarts WHERE SavedCartGUID like '" &  tmpGUID & "' AND IDCustomer=" & tmpIDCust & ";"
		end if
		set rsQ=connTemp.execute(query)
		if not rsQ.eof then
			IDSC=rsQ("SavedCartID")
			tmpName=rsQ("SavedCartName")
			'//Only update Auto Saved Cart
			if (Instr(tmpName,"Saved shopping cart ")=1) OR (Instr(tmpName,"Auto saved shopping cart ")=1) then
				HasSavedCart=1
			else
				HasSavedCart=0
			end if
		...
		if HasSavedCart=1 then
			...
			query="UPDATE pcSavedCarts SET SavedCartDate='" & dtTodaysDate & "',IDCustomer=" & tmpIDCust & ",SavedCartQuotes='" & session("sf_FQuotes") & "' WHERE SavedCartID=" & IDSC & ";"
			set rsQ=connTemp.execute(query)
			set rsQ=nothing
		end if
	END IF
	IF HasSavedCart=0 THEN
		...
		query="INSERT INTO pcSavedCarts (SavedCartGUID,SavedCartDate,SavedCartName,IDCustomer,SavedCartQuotes) VALUES ('" & tmpGUID & "','" & dtTodaysDate & "',N'" & tmpSaveName & "'," & tmpIDCust & ",'" & session("sf_FQuotes") & "');"
		set rsQ=connTemp.execute(query)
		set rsQ=nothing
		query="SELECT SavedCartID FROM pcSavedCarts WHERE SavedCartGUID like '" &  tmpGUID & "';"
		set rsQ=connTemp.execute(query)
	...
	IF HasSavedCart=1 THEN
		...
		query=query& "N'"& fixstring(pcCartArray(f,0)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,1)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,2)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,3)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,4)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,5)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,6)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,7)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,8)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,9)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,10)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,11)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,12)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,13)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,14)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,15)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,16)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,17)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,18)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,19)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,20)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,21)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,22)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,23)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,24)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,25)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,26)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,27)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,28)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,29)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,30)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,31)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,32)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,33)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,34)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,35)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,36)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,37)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,38)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,39)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,40)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,41)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,42)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,43)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,44)) &"', "
		query=query& "N'"& fixstring(pcCartArray(f,45)) &"');"	
		'SB E        
