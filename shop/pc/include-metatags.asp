...
' use this file. See: http://wiki.productcart.com/productcart/seo-meta-tags
'
' ******************************************************************************
'//Schema.org URL
Dim scmURL, intIdCategory
scmURL=""
...
' ******************************************************************************
' Get Product and Category and Content ID
' ******************************************************************************
GMidproduct = session("idProductRedirect")
if GMidproduct = "" then
	GMidproduct = getUserInput(request("idproduct"), 0)
end if
session("idProductRedirect") = ""

GMidcategory = session("idCategoryRedirect")
if GMidcategory = "" then
	GMidcategory = getUserInput(request("idcategory"), 0)
end if
if GMidcategory = "" then
...
GMidcontent = session("idParentContentPageRedirect")
if GMidcontent = "" then
	GMidcontent = session("idContentPageRedirect")
end if
if GMidcontent = "" then
	GMidcontent = getUserInput(request("idpage"), 0)
end if
if GMidcontent = "" then
	GMidcontent = pcv_IDPage
end if

GMidbrand = session("idBrandRedirect")
if GMidbrand = "" then
	GMidbrand = getUserInput(request("idbrand"), 0)
end if
session("idBrandRedirect") = ""


'// Check for Valid Numbers, but only if something exists
if len(GMidproduct)>0 then
	if not validNum2(GMidproduct) then
        call closeDb()
        response.redirect "msg.asp?message=207"
	end if
end if
if len(GMidcategory)>0 then
	if not validNum2(GMidcategory) then
        call closeDb()
        response.redirect "msg.asp?message=86"
	end if
end if

if len(GMidcontent)>0 then
	if not validNum2(GMidcontent) then
        call closeDb()
        response.redirect "msg.asp?message=300"
	end if
end if

if len(GMidbrand)>0 then
	if not validNum2(GMidbrand) then
        call closeDb()
        response.redirect "msg.asp?message=300"
	end if
end if
' DA Edit
GMidproduct=pIdProduct
'GMidcategory=daIdCategory
...
' ******************************************************************************
GMpcCartIndex=getUserInput(request("pcCartIndex"),0)
...
if validNum2(GMidproduct) then
	...
	query="select description,details,sDesc,pcprod_MetaTitle,pcprod_MetaDesc,pcprod_MetaKeywords,pcUrl from Products where idProduct=" & GMidproduct
	...
	if not rsTagObj.eof then
		...
		'// Create canonical URL
			...
			tempCanonicalURL=replace(tempCanonicalURL,"https:/","https://")
			pcStrCanonicalURL=tempCanonicalURL
			
			pcStrCanonicalURL = "https://www.multiplemonitors.co.uk/products/" & mtPUrl & "/"

		'// Create image URL
			tempImageURL=replace((scStoreURL&"/"&scPcFolder&"/pc/catalog/"&pimageUrl),"//","/")
			tempImageURL=replace(tempImageURL,"http:/","http://")
			tempImageURL=replace(tempImageURL,"https:/","https://")
...
' ******************************************************************************
' ******************************************************************************
' BRAND PAGE-specific Meta Tags
' ******************************************************************************
if (GMTags=False) and (validNum2(GMidbrand)) then
	Set conn=Server.CreateObject("ADODB.Connection")
	conn.Open scDSN

	'// Title, description, and keywords are set on viewbrands.asp

	'// Create canonical URL
		pcIntBrandID=GMidbrand
		BrandName=parentBrandName
		
		'// Call SEO Routine
		pcGenerateSeoLinks
		'//
		tempCanonicalURL=replace((scStoreURL&"/"&scPcFolder&"/pc/"&pcStrBrandLink),"//","/")
		tempCanonicalURL=replace(tempCanonicalURL,"http:/","http://")
		tempCanonicalURL=replace(tempCanonicalURL,"https:/","https://")
		pcStrCanonicalURL=tempCanonicalURL

	'// Create image URL
		query = "SELECT BrandLogo FROM Brands WHERE idBrand = " & GMidbrand
		SET rsPageImg=Server.CreateObject("ADODB.RecordSet")
		SET rsPageImg=conn.execute(query)
		if not rsPageImg.eof then
			pcStrPageImgTemp=rsPageImg("BrandLogo")
		end if
		SET rsPageImg=nothing
		if trim(pcStrPageImgTemp)<>"" then
			tempImageURL=replace((scStoreURL&"/"&scPcFolder&"/pc/catalog/"&pcStrPageImgTemp),"//","/")
...
' ******************************************************************************
' END BRAND PAGE-specific Meta Tags
' ******************************************************************************
...
'// START - Adjust Meta Tags for certain storefront pages
	...
	'// Canonical URL
	if pcStrPageName = "home.asp" or pcStrPageName = "showbestsellers.asp" or pcStrPageName = "showfeatured.asp" or pcStrPageName = "showspecials.asp" or pcStrPageName = "shownewarrivals.asp" or (pcStrPageName = "viewcategories.asp" and GMidcategory=1) or (pcStrPageName = "viewbrands.asp" and GMidbrand="") then
...
'// START - Write Meta Tags
	if trim(Title)<>"" then
		Response.Write "<TITLE itemprop=""name"">" & Title & " | Multiple Monitors</TITLE>" & vbcrlf
	end if
    ...
    If (pcStrPageName = "search.asp") OR (pcStrPageName = "msg.asp") OR (pcStrPageName = "msgb.asp") OR (pcStrPageName = "checkout.asp") OR (pcStrPageName = "onepagecheckout.asp") OR (pcStrPageName = "viewcart.asp") Then
	...
	'// DA Code to create canonical link tag
	If Instr(request.ServerVariables("http_x_rewrite_url"), "?") <> 0 then
		response.write vbcrlf & "<link rel=""canonical"" href=""https://www.multiplemonitors.co.uk" & Left(request.ServerVariables("http_x_rewrite_url"), (InStr(request.ServerVariables("http_x_rewrite_url"), "?")-1)) & """ />" & vbcrlf
	else
		response.write vbcrlf & "<link rel=""canonical"" href=""https://www.multiplemonitors.co.uk" & request.ServerVariables("http_x_rewrite_url") & """ />" & vbcrlf
	end if

	'//Schema URL
	scmURL=pcStrCanonicalURL
	...
	if (pcStrImageURL<>"") AND (tmpHasImg=1) then
		...
		'	pcStrImageURL=replace(pcStrImageURL,"http://","https://")
		'end if
...
if pcStrPageName = "viewprd.asp" then
	Response.Write "<meta property=""og:title"" content=""" & mtPName & """ />" & vbcrlf
	Response.Write "<meta property=""og:type"" content=""website"" />" & vbcrlf
	if psDesc<>"" then
		Response.Write "<meta property=""og:description"" content=""" & ClearHTMLTags2(psDesc,0) & """ />" & vbcrlf
	end if
	Response.Write "<meta property=""og:url"" content=""" & pcStrCanonicalURL & """ />" & vbcrlf
	if IsNull(pImageUrl) OR pImageUrl="" then
		ogImageUrl="no_image.gif"
	else
		ogImageUrl=pImageUrl
	end if
	ogImageUrl=Left(pcStrCanonicalURL,InStrRev(pcStrCanonicalURL,"/")) & "catalog/" & ogImageUrl
	'if UCase(Request.ServerVariables("HTTPS"))="ON" then
	'	ogImageUrl=replace(ogImageUrl,"http://","https://")
	'end if
	Response.Write "<meta property=""og:image"" content=""" & ogImageUrl & """ />" & vbcrlf
	if scCompanyName<>"" then
		Response.Write "<meta property=""og:site_name"" content=""" & scCompanyName & """ />" & vbcrlf
	end if
end if
