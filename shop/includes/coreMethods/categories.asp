...
Dim query,rsC
pcv_havecats1=0
query="SELECT idCategory,categoryDesc,idParentCategory,pccats_BreadCrumbs FROM categories ORDER BY categories.categoryDesc ASC;"
...
Sub pcs_CatList()
    ...
    IF IsEmpty(pcv_cats3) then
        response.write "Categories containing products not found."
    ELSE
        %>
        <select class="form-control" name="<%=cat_DropDownName%>" <%if cat_MultiSelect="1" then%>multiple<%end if%> <%if cat_EventAction<>"" then%><%=cat_EventAction%><%end if%> <%If cat_DropDownTabIndex<>"" Then Response.Write "tabindex='" & cat_DropDownTabIndex & "'" %>>
            ...
            <%For k=lbound(pcv_cats3,2) to ubound(pcv_cats3,2)
                if trim(pcv_cats3(0,k))<>"" then
                    ...
                    Next
                    if cat_ExcCircle>"0" then
                        if pcf_FindCatInBC(pcv_cats3(0,k),pcv_cats3(2,k),pcv_cats3(3,k))=true then
                            tmp1=1
                        end if
                    end if
                    if tmp1=0 then
                        if pcf_FindHiddenParent(pcv_cats3(0,k))=0 then
                            tmp2=0
...
Function pcf_FindHiddenParent(pcv_parentCategory)
	Dim k,pcv_end,pcv_back,tmpR,tmp1
	pcv_end=ubound(pcv_cats1,2)
	tmpR=0
	tmp1=0
	...
	For k=lbound(pcv_cats1,2) to ubound(pcv_cats1,2)
		...
		if pcv_parentCategory=pcv_cats1(0,k) then
			tmp1=1
			tmpR=pcf_FindHiddenParent(pcv_cats1(2,k))
			exit for
		end if
		if pcv_parentCategory=pcv_cats1(0,pcv_back) then
			tmp1=1
			tmpR=pcf_FindHiddenParent(pcv_cats1(2,pcv_back))
			exit for
		end if
	Next
	end if
	if (pcv_parentCategory>"1") AND (tmp1=0) then
		tmpR=1
	end if
	pcf_FindHiddenParent=tmpR
End Function
...
Sub pcf_FindParentExc(ExcCat,pcv_parentCategory)
    Dim k,pcv_end,pcv_back
	if pcv_tmpParent="true" then
		exit sub
	end if
	pcv_end=ubound(pcv_cats1,2)
	For k=lbound(pcv_cats1,2) to ubound(pcv_cats1,2)
		pcv_back=pcv_end-k
		if pcv_parentCategory=pcv_cats1(0,k) then
			if pcv_cats1(2,k)<>"1" then
				if Clng(ExcCat)=Clng(pcv_cats1(2,k)) then
					pcv_tmpParent="true"
					exit Sub
				end if
				call pcf_FindParentExc(ExcCat,pcv_cats1(2,k))
			end if
			exit for
		end if
		if pcv_parentCategory=pcv_cats1(0,pcv_back) then
			if pcv_cats1(2,pcv_back)<>"1" then
				if Clng(ExcCat)=Clng(pcv_cats1(2,pcv_back)) then
					pcv_tmpParent="true"
					exit Sub
				end if
				call pcf_FindParentExc(ExcCat,pcv_cats1(2,pcv_back))
			end if
			exit for
		end if
	Next
...
Function pcf_FindCatInBC(pcv_idcategory,pcv_parentCategory,pcv_BreadCrumbs)
    Dim tmp_C,tmp_D,p
	IF trim(pcv_BreadCrumbs)<>"" THEN
		tmp_C=split(pcv_BreadCrumbs,"|,|")
		For p=lbound(tmp_C) to ubound(tmp_C)
			if trim(tmp_C(p))<>"" then
			tmp_D=split(tmp_C(p),"||")
			if (clng(tmp_D(0))=clng(cat_ExcCircle)) then
				pcf_FindCatInBC=true
				exit function
			end if
			end if
		Next
	ELSE
		if IsNull(pcv_cats1) then
			call pcs_getCatsList()
		end if
		pcv_tmpParent=""
		if pcv_parentCategory<>"1" then
			if Clng(cat_ExcCircle)=Clng(pcv_parentCategory) then
				pcf_FindCatInBC=true
				exit function
			end if
			call pcf_FindParentExc(cat_ExcCircle,pcv_parentCategory)
		end if
		if pcv_tmpParent<>"" then
			pcf_FindCatInBC=true
			exit function
	...
	END IF
	pcf_FindCatInBC=false
End Function

Function CheckUpdateParent(tmpCID,tmpNID)
	Dim query, rstemp, pcv_tmpCID, pcv_tmpParentID, pcv_tmpBC

	cat_ExcCircle=tmpCID
	pcv_tmpCID=tmpNID
	pcv_tmpParentID=1
	pcv_tmpBC=""
	query="SELECT idParentCategory,pccats_BreadCrumbs FROM Categories WHERE IDCategory=" & pcv_tmpCID & ";"
	set rstemp=connTemp.execute(query)
	if not rstemp.eof then
		pcv_tmpParentID=rstemp("idParentCategory")
		if IsNull(pcv_tmpParentID) OR pcv_tmpParentID="" then
			pcv_tmpParentID="1"
		end if
		pcv_tmpBC=rstemp("pccats_BreadCrumbs")
	end if
	set rstemp=nothing
	if pcf_FindCatInBC(catID,pcv_tmpParentID,pcv_tmpBC)=true then
		CheckUpdateParent=0
	else
		CheckUpdateParent=1
	end if
