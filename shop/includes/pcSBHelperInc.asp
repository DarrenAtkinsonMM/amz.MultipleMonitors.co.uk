...
Function writeFile(pageName,pText)	
    ...
    if fso.FileExists(findit) then
        ...
        if Err.number>0 then
            Session("message") = "Error Updating File " & pageName &  "." & err.description
    ...
    Else
        ...
        f.Write(pText)
        if Err.number>0 then
            Session("message") = "Error Writing File " & pageName &  "."
            response.redirect "msgb.asp?back=1" 
        end if			
    End if 
    f.close
...
Function removeFile(pageName)
		...
		if fso.FileExists(findit) then			
			...
			if Err.number>0 then
                Session("message") = "Error Removing File " & pageName &  "."
                response.redirect "msgb.asp?back=1" 
			end if
...
Function getFile(pgName)
    ...
    if fso.FileExists(findit) then		
        Set f=fso.OpenTextFile(findit, 1, false,0)
         ...
         set f=nothing
        if Err.number>0 then			  
            Session("message") = "Error Reading File " & pageName &  "."
            response.redirect "msgb.asp?back=1" 
        end if			
    End if 
    set fso = nothing
