...
%>
		...
		<tr id="zone1<%=pcv_strCountryBox%>" style="display: none;"> 
			...
			<td>
				...
				%>
				<select class="pcStateProvinceSelect" name="<%=pcv_strTargetBox%>">
					<option value=""></option>
					<% 
					do while not rs.eof
		...
		<tr id="zone2<%=pcv_strCountryBox%>" style="display:;"> 
			...
			<td>										
			<p>
			<input class="pcStateProvinceInput" type="text" name="<%=pcv_strProvinceBox%>" id="<%=pcv_strProvinceBox%>" size="20" value="<% =pcf_FillFormField (pcv_strProvinceBox, pcv_isProvinceCodeRequired) %>">
...
Public Sub pcs_CountryDropdown
	...
	%>
	<tr> 
		<td><p><%= dictLanguage.Item(Session("language")&"_CustAddModShip_8")%></p></td>
		<td>
		<p>
		<%
		...
		%>
				...
				<select class="pcCountrySelect" name="<%=pcv_strCountryBox%>" onchange="SwitchStates(this.options.selectedIndex, '<%=pcv_strCountryBox%>', '<%=pcv_strTargetBox%>', '<%=pcv_strProvinceBox%>', '<%=pcv_strTargetBoxValue%>', '<%=pcv_strFormInstance%>');">
					<option></option>
					<% 
					For i = pcv_intLBound To pcv_intUBound
						'// get nested array
...
' START: State/ Province
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
Public Sub pcs_StateProvinceMobile
%>

		<div id="zone1<%=pcv_strCountryBox%>" style="display: none;"> 
			<div><p ID="Label<%=pcv_strCountryBox%>"><%=pcv_strSPFieldLabel%></p></div>
			<div class="pcMobileDiv">									
				<% 
				query="SELECT stateCode,stateName FROM states ORDER BY stateName ASC"
				set rs=server.CreateObject("ADODB.RecordSet")
				set rs=conntemp.execute(query)
		...
		<div id="zone2<%=pcv_strCountryBox%>" style="display:;"> 
			<div><p ID="Label2<%=pcv_strCountryBox%>"><%=pcv_strSPFieldLabel%></p></div>
			<div class="pcMobileDiv">
...
' START: Display the Country Dropdown
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
Public Sub pcs_CountryDropdownMobile

	if Session(pcv_strSessionPrefix&pcv_strCountryBox)="" then
		Session(pcv_strSessionPrefix&pcv_strCountryBox)=scShipFromPostalCountry
	end if
	%>
	<div><%response.write dictLanguage.Item(Session("language")&"_CustAddModShip_8")%></div>
	<div class="pcMobileDiv">
		<%
		pcv_strTargetBoxValue=Session(pcv_strSessionPrefix&pcv_strTargetBox)
		%>
		
				<select name="<%=pcv_strCountryBox%>" onchange="SwitchStates(this.options.selectedIndex, '<%=pcv_strCountryBox%>', '<%=pcv_strTargetBox%>', '<%=pcv_strProvinceBox%>', '<%=pcv_strTargetBoxValue%>', '<%=pcv_strFormInstance%>');">
					<option></option>
					<% 
					For i = pcv_intLBound To pcv_intUBound
						'// get nested array
