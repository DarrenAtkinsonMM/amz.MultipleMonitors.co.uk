...
(this.o.weekStart-e.getUTCDay()-7)%7*864E5);l=new Date(Number(l)+(11-l.getUTCDay())%7*864E5);var q=new Date(Number(q=r(l.getUTCFullYear(),0,1))+(11-q.getUTCDay())%7*864E5);c.push('<td class="cw">'+((l-q)/864E5/7+1)+"</td>")}l=this.getClassNames(e);l.push("day");if(this.o.beforeShowDay!==d.noop){var p=this.o.beforeShowDay(this._utc_to_local(e));p===u?p={}:"boolean"===typeof p?p={enabled:p}:"string"===typeof p&&(p={classes:p});!1===p.enabled&&l.push("disabled");p.classes&&(l=l.concat(p.classes.split(/\s+/)));
p.tooltip&&(t=p.tooltip)}l=d.unique(l);c.push('<td class="'+l.join(" ")+'"'+(t?' title="'+t+'"':"")+">"+e.getUTCDate()+"</td>");e.getUTCDay()===this.o.weekEnd&&c.push("</tr>");e.setUTCDate(e.getUTCDate()+1)}this.picker.find(".datepicker-days tbody").empty().append(c.join(""));var x=this.picker.find(".datepicker-months").find("th:eq(1)").text(b).end().find("span").removeClass("active");d.each(this.dates,function(a,c){c.getUTCFullYear()===b&&x.eq(c.getUTCMonth()).addClass("active")});(b<a||b>f)&&x.addClass("disabled");
...
a;if(!f){var f=D(this,"date"),e=d.extend({},z,f,g),e=E(e.language),g=d.extend({},z,e,f,g);h.is(".input-daterange")||g.inputs?(f={inputs:g.inputs||h.find("input").toArray()},h.data("datepicker",f=new C(this,d.extend(g,f)))):h.data("datepicker",f=new v(this,g))}if("string"===typeof a&&"function"===typeof f[a]&&(c=f[a].apply(f,b),c!==u))return!1});return c!==u?c:this};var z=d.fn.datepicker.defaults={autoclose:!1,beforeShowDay:d.noop,calendarWeeks:!1,clearBtn:!1,daysOfWeekDisabled:[],endDate:Infinity,
...
30,31][b]},validParts:/dd?|DD?|mm?|MM?|yy(?:yy)?/g,nonpunctuation:/[^ -\/:-@\[\u3400-\u9fff-`{-~\t\n\r]+/g,parseFormat:function(a){var b=a.replace(this.validParts,"\x00").split("\x00");a=a.match(this.validParts);if(!b||!b.length||!a||0===a.length)throw Error("Invalid date format.");return{separators:b,parts:a}},parseDate:function(a,b,c){function h(){var a=this.slice(0,g[k].length),b=g[k].slice(0,a.length);return a===b}if(!a)return u;if(a instanceof Date)return a;"string"===typeof b&&(b=n.parseFormat(b));
...
d.inArray(b,t)}).toArray());if(g.length===q.length){var p;k=0;for(p=q.length;k<p;k++){l=parseInt(g[k],10);b=q[k];if(isNaN(l))switch(b){case "MM":l=d(m[c].months).filter(h);l=d.inArray(l[0],m[c].months)+1;break;case "M":l=d(m[c].monthsShort).filter(h),l=d.inArray(l[0],m[c].monthsShort)+1}f[b]=l}for(k=0;k<t.length;k++)b=t[k],b in f&&!isNaN(f[b])&&(c=new Date(a),e[b](c,f[b]),isNaN(c)||(a=c))}return a},formatDate:function(a,b,c){if(!a)return"";"string"===typeof b&&(b=n.parseFormat(b));c={d:a.getUTCDate(),
