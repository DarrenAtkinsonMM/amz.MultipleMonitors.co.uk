...
'// 2026 redesign — friendly-URL slug validator. Allowed: a-z A-Z 0-9 - _
...
Public Function getUserInput(input, stringLength)
    Dim definition, pcv_boolIsSQLInjection
    
    pcv_boolIsSQLInjection = False
    pcv_boolShowFilters = False
    pcv_boolLoadCustomFilters = False
    ...
    If len(tempStr) > 0 Then
        ...
        Dim securityObj
        Set securityObj = pcf_LoadDefinitions()
        If (len(Session("DefinitionsObj"))<250) Or (pcv_boolLoadCustomFilters = True) Then
            Set securityObj = pcf_LoadDefaultDefinitions()
        End If
        ...
        '// Filter On All Definitions By Priority
        Dim defenderObj : Set defenderObj = securityObj.definitions
        For Each definition In defenderObj.keys()        
            
            pcv_strTitle = definition
            pcv_strDesc = defenderObj.Get(definition).description
            pcv_strPattern = defenderObj.Get(definition).pattern
            pcv_strReplaceString = defenderObj.Get(definition).replaceString
            pcv_strIgnoreCase = defenderObj.Get(definition).ignoreCase
            pcv_strIsGlobal = defenderObj.Get(definition).isGlobal
            pcv_strContinueOnError = defenderObj.Get(definition).continueOnError
            pcv_strType = defenderObj.Get(definition).type

            If pcv_strType="regEx" Then
                Dim objRegExp : Set objRegExp = New RegExp
                With objRegExp
                    .Pattern = pcv_strPattern
                    .IgnoreCase = pcv_strIgnoreCase
                    .Global = pcv_strIsGlobal
                End With
                tempStr = objRegExp.Replace(tempStr, pcv_strReplaceString)            
                Call pcs_DebugGetUserInput(pcv_strTitle, pcv_strDesc, tempStr, pcv_strPattern, pcv_strReplaceString, pcv_boolShowFilters, pcv_boolLoadCustomFilters)
                Set objRegExp = Nothing
            End If
            
            If pcv_strType="replace" Then
                If (instr(1, tempStr, pcv_strPattern, vbTextCompare)<>0) Then
                    tempStr	= replace(tempStr, pcv_strPattern, pcv_strReplaceString, 1, -1, 1)
                    If Not pcv_strContinueOnError Then
                        pcv_strMatchingPhrase = pcv_strPattern
                        pcv_boolIsSQLInjection = True
                    End If 
                End If
                Call pcs_DebugGetUserInput(pcv_strTitle, pcv_strDesc, tempStr, pcv_strPattern, pcv_strReplaceString, pcv_boolShowFilters, pcv_boolLoadCustomFilters)
            End If
        ...
        Next
        Set defenderObj = Nothing
        Set securityObj = Nothing
        
        If pcv_boolIsSQLInjection Then
            Session("message") = "You have entered illegal text into an input field."
...
Public Sub pcs_DebugGetUserInput(definition, pcv_strDesc, tempStr, pcv_strFilter, pcv_strReplaceString, pcv_boolShowFilters, pcv_boolLoadCustomFilters)
    If pcv_boolShowFilters Then       
        response.Write("<strong>" & pcv_strDesc & "</strong>" & ":  " & "<br />")
        response.Write("Filter = " & pcv_strFilter & "<br />")
        response.Write("ReplaceString = " & pcv_strReplaceString & "<br />")
        response.Write("Input = " & tempStr & "<br />") 
        response.Write("Input Escaped = " & escape(tempStr) & "<br />")          
        response.Write("<hr />" & "<br />")
    End If
End Sub
'// END: NEW FILTER MODULE
...
Public Function pcf_PaymentTypes(GatewayName)
		...
		'SB S	
		if session("customerType")=1 then
			query="SELECT idPayment FROM paytypes WHERE active=-1 AND gwCode<>50 AND gwCode<>999999 AND gwCode<>88 " & strAndSub & ";"
		else
			query="SELECT idPayment FROM paytypes WHERE active=-1 AND Cbtob=0 AND gwCode<>50 AND gwCode<>999999 AND gwCode<>88" & strAndSub & ";"
		end if			
		'SB E		
		set rs=server.CreateObject("ADODB.RecordSet")
		...
		query="SELECT idPayment FROM paytypes WHERE active=-1 and gwCode=88 " & strAndSub & ";"
		set rsAmz=Server.CreateObject("ADODB.Recordset")     
		set rsAmz=conPaymentTypes.execute(query)	
		if rsAmz.eof then
			pcv_intAmazonPaymentsActive=0
		else
			pcv_intAmazonPaymentsActive=-1
		end if
		...
		Select Case GatewayName
			...
			Case "PayWithAmazon":pcv_strPayTypeFilter=(pcv_intAmazonPaymentsActive=-1)
			Case "":pcv_strPayTypeFilter=(pcv_intPayPalExpressActive=-1 OR pcv_intGoogleActive=-1 OR pcv_intAmazonPaymentsActive=-1)
			Case Else:pcv_strPayTypeFilter=(pcv_intPayPalExpressActive=-1 OR pcv_intGoogleActive=-1 OR pcv_intAmazonPaymentsActive=-1)
		End Select
...
'// To Database From HTML Editor Field
...
Function pcf_AddToCart(IdProduct)
	...
	ELSE '// Feature is active
		...
		If IdProduct<>"" AND NOT isNULL(IdProduct) Then
			...
			'// 2.) PRODUCT FOR SALE:  Verify level is 0 OR is 1 with a custmer type of 1	
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			'// NOTES	
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			...
			'// 3.) STOCK CONSIDERATIONS:  If out of stock AND out of stock purchase is allowed show button.
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			'// NOTES	
			'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			...
			'// 4.) Product is an Apparel Product 	
			'//			Does not apply to stores not running the APP Add-on	
			If statusAPP="1" Then
				query="SELECT pcprod_Apparel FROM products WHERE idProduct=" & IdProduct & " AND configOnly=0 AND removed=0 "
				set rsApparel=server.CreateObject("ADODB.RecordSet")		
				set rsApparel=conntemp.execute(query)		
				If NOT rsApparel.EOF Then
					pcv_Apparel=rsApparel("pcprod_Apparel")
					set rsApparel=nothing
			...
			'// 6.) Product has required input fields 
			pcv_ReqInputFields=pcf_CheckForReqInputFields(IdProduct)
			If pcv_ReqInputFields=1 Then
				pcf_AddToCart=False
				Exit Function
			End If
			...
			'// 8.) Product has required options (if options are not required, then visiting the product details page is not mandatory) 
			pcv_ReqOptions=pcf_CheckForReqOptions(IdProduct)
			If pcv_ReqOptions=1 Then
				pcf_AddToCart=False
				Exit Function
			End If
	
	
			'// 9.) Product has required accessories 
			If pcf_CheckForReqAccessories(IdProduct,pcv_ReqOptions,pcv_ReqInputFields)=1 Then
				pcf_AddToCart=False
				Exit Function
			End If		
...
'//////////////////////////////////////////////////////////////////////////////////////////////////////////
'// String filter, to remove all but 26 characters of the Latin Alphabet and the 10 digit Numeric System //
'// plus replaces spaces with hyphens                                                                    //
'//////////////////////////////////////////////////////////////////////////////////////////////////////////

    Function RegExpFilter(strtoclean)
...
'// DA - Edit Return Shipping Estimate Functions
'*****************************************************************************************************
function daFunDelBlockDate()
	'Sets a date which no deliveries will occur before
	daFunDelBlockDate = CDate("January 06, 2026")
end function

function daFunDelDateBlockTest(numPC,numDaysAdd)
	'Check if the standard delivery estimate date is before the block date, return true if so
	if InStr(daFunDelDate(numPC,numDaysAdd),"XXX") > 0 then
		daFunDelDateBlockTest = True
	else
		daFunDelDateBlockTest = False
	End if
end function

function daFunDelDateReturn(numPC,numDaysAdd)
	'Simply returns the friendly delivery date
	daFunDelDateReturn = Replace(daFunDelDate(numPC,numDaysAdd),"XXX","")
end function

function daFunDelDate(numPC, numDaysAdd)
	'Main delivery date calculation function
	
	'numPC checks for PC, if there is one we add extra build time on before working out output
	'numDaysAdd adds some extra days on to the delivery for international lead times
	'if numDaysAdd = 999 then we need to find the next Saturday delivery spot after the standard delivery date
	boolSatDel = False
	if numDaysAdd = 999 then
		numDaysAdd = 0
		boolSatDel = True
	end if
	
	'Gets block date set in above function
	dtBlockUntil = daFunDelBlockDate
	
	'Set boolean based on PC in estimate
	If numPC > 0 then
		boolPC=true
	End if
	
	'Work out delivery dates based on today.
	dtNow = Now()
	CurrentDay = weekday(dtNow)
	CurrentDayName = weekdayname(CurrentDay)
	
	'Check if we are past the 2:30pm cut off
	dtTestHour = hour(dtNow)
	dtTestMinute = minute(dtNow)
	
	If dtTestHour < 14 Then
		intDateAdd = 0
	End if
	...
	'Do select to fine tune delivery options
	Select Case CurrentDay
		'Sunday
		Case 1
			if boolPC then
				'PC: Deliver following Monday
				dtEstFull = DateAdd("d", (8), dtNow) 'NORMALLY 8	
			Else
				'Other: Deliver Wed / Thur
				dtEstFull = DateAdd("d", (3), dtNow)
			End if
		'Monday
		Case 2
			if boolPC then
				'PC Deliver Monday or Tuesday
				dtEstFull = DateAdd("d", (7 + intDateAdd), dtNow)	'NORMALLY 7		
			else
				'Other: Deliver Wed / Thur
				dtEstFull = DateAdd("d", (2 + intDateAdd), dtNow)
			end if
		'Tuesday
		Case 3
			if boolPC then
				'PC Deliver Tuesday or Wednesday
				dtEstFull = DateAdd("d", (7 + intDateAdd), dtNow)	'NORMALLY 7		
			else
				'Other: Deliver Thur / Fri
				dtEstFull = DateAdd("d", (2 + intDateAdd), dtNow)
			end if
		'Wednesday
		Case 4
			if boolPC then
				'PC Deliver Wed or Thur
				dtEstFull = DateAdd("d", (7 + intDateAdd), dtNow) 	'NORMALLY 7
			else
				'Other: Deliver Fri / Mon
				If intDateAdd = 0 then
					dtEstFull = DateAdd("d", 2, dtNow)
				Else
					dtEstFull = DateAdd("d", 5, dtNow)
				End If			
			end if
		'Thursday
		Case 5
			if boolPC then
				'PC Deliver Thursday or Friday
				dtEstFull = DateAdd("d", (7 + intDateAdd), dtNow)	'NORMALLY 7		
			else
				'Other: Deliver Mon / Tue
				dtEstFull = DateAdd("d", (4 + intDateAdd), dtNow)
			end if
		'Friday
		Case 6
			if boolPC then
				'PC Deliver Friday or Monday
				If intDateAdd = 0 Then
					dtEstFull = DateAdd("d", (7), dtNow)	'NORMALLY 7
				Else
					dtEstFull = DateAdd("d", (10), dtNow)	'NORMALLY 10		
				End If			
			else
				'Other: Deliver Tue / Wed
				dtEstFull = DateAdd("d", (4 + intDateAdd), dtNow)
				'Work out cut off time to display
			end if
		'Saturday
		Case 7
			if boolPC then
				'PC Deliver following Monday
				dtEstFull = DateAdd("d", (9), dtNow)	'NORMALLY 9			
			else
				'Other: Deliver Wed / Thur
				dtEstFull = DateAdd("d", (4), dtNow)
			end if			
	End Select	
	
	'Check if we are past the block day, if not show block until date, otherwise show calculated date
	boolBlock = 0
	
	if dtEstFull < dtBlockUntil then
		dtEstFull = dtBlockUntil
		boolBlock = 1
	End if
	
	'Add in extra delivery days for international
	dtEstFull = DateAdd("d",(numDaysAdd),dtEstFull)
	
	'Check if we need a saturday delivery if so find next Saturday after delivery date
	if boolSatDel then
		'Work out delivery date day of week then add on correct number to make it 7 which is a saturday
		dtEstFull = DateAdd("d",(7 - WeekDay(dtEstFull)),dtEstFull)
	Else
		'We don't need saturday delivery
		'Check if del date is a saturday or sunday, if so add in some extra days to make it Monday
		Select Case WeekDay(dtEstFull)
			'Sunday
			Case 1
				dtEstFull = DateAdd("d",(1),dtEstFull)
			'Saturday
			Case 7
				dtEstFull = DateAdd("d",(2),dtEstFull)
		End Select	
	end if	
	...
	dtEstMonth = Month(dtEstFull)
	dtEstDay = weekday(dtEstFull)
	Select Case CStr(Day(dtEstFull))
		Case "1","21","31"
			delDateEnding = "st"
		Case "2","22"
			delDateEnding = "nd"
		...
		Case Else
			delDateEnding = "th"
	End Select
	delEst = WeekDayName(dtEstDay) & " " & CStr(Day(dtEstFull)) & delDateEnding & " " & MonthName(dtEstMonth)
	
	'Edit to append XXX to end of string output to show we have a blocked date
	if boolBlock then
		daFunDelDate = delEst & "XXX"
	else
		daFunDelDate = delEst
	end if
...
function daFunDelCutOff()
	'Separate function to work out order cut off time
	...
	Select Case CurrentDay
		'Sunday
		Case 1
			dtCutOff = "2:30pm Monday"
		'Monday
		Case 2
			If intDateAdd = 0 then
				dtCutOff = "2:30pm Today"
			Else
				dtCutOff = "2:30pm Tomorrow"
			End If			
		'Tuesday
		Case 3
			If intDateAdd = 0 then
	...
	daFunDelCutOff = dtCutOff

end function

function CheckVAT(boolEUState, strShipCountryCode, strCustomerVATID)
'// Function to check if VAT is charged or not. Return 1 for VAT Charge and 0 for No VAT Charge

	boolChargeVAT = 0

	If boolEUState > 0 Then
		'We are in the EU, if shipping is GB always charge VAT
		if strShipCountryCode = "GB" Then
			boolChargeVAT = 1
		Else
			'Non GB Shipping, if we have a VAT ID then remove VAT Charge.
			if strCustomerVATID <> "" Then
				boolChargeVAT = 0
			Else
				boolChargeVAT = 1	
			End If
		End If
	Else
		'Outside of EU so remove VAT Charge
		boolChargeVAT = 0
	End If
	
	CheckVAT = boolChargeVAT
