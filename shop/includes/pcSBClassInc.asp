...
gv_EndPointGateway				= gv_RootURL & "Gateway/Service4.svc"
...
Class pcARBClass
	...
	Private	sb_Reason
	Public Property Let Reason(strReason)
			sb_Reason   = strReason  
	End Property	
	...
	Function WebRequest(strMethod, strXML, strEndPoint, APIUser, APIPass, APIKey)
		...
		'response.Write(strEndPoint)
		'response.End()

		WebRequest = methodCall(strMethod, strXML, strEndPoint)

	End Function
	
	Public Function GetToken(APIUser, APIPass, APIKey)

		Dim to_hash, result, currentTime
		
		result = GetCurrentTime(APIUser, APIPass, APIKey)
		...
		GetToken = hex_hmac_sha1(APIKey, to_hash)	
	...
	Function RegisterAcc(APIUser, APIPass, APIKey)
		...
		pcv_token = GetToken(APIUser, APIPass, APIKey)
		result = WebRequest("ActivationRequest", RegisterAccXML, gv_EndPoint, APIUser, APIPass, APIKey)
	...
	Function GetPackages(APIUser, APIPass, APIKey)
		pcv_username = APIUser
		pcv_token = GetToken(APIUser, APIPass, APIKey)
		GetPackages = WebRequest("GetPackagesRequest", GetPackagesXML, gv_EndPoint, APIUser, APIPass, APIKey)
	End Function
	'//////////////////////////////////////////////////////////////////////////////
	'// END:  Get Packages
	'//////////////////////////////////////////////////////////////////////////////
	...
	'//////////////////////////////////////////////////////////////////////////////
	Function GetCurrentTime(APIUser, APIPass, APIKey)
		Dim xmlStr, results
	...
	Function GetTerms(LinkID)
		...
		'// 2.) Send Requesst		
		result = WebRequest("GetTermsRequest",xmlStr, gv_EndPoint, APIUser, APIPass, APIKey)	
	...
	Public Function SubscriptionRequestXML()
		Dim xmlStr				
				...
				If len(ShippingFirstName)>0 Then
					...
					'xmlStr = xmlStr & 			"<Password>" & removeInvalidXML(CustomerPassword) & "</Password>"
	...
	Public Function SubscriptionRequest(APIUser, APIPass, APIKey)	
		...
		result = WebRequest("SubscriptionRequest", SubscriptionRequestXML, gv_EndPoint, APIUser, APIPass, APIKey)
		...
		if strStatus="Success" then
			...
			pcv_strReason = pcf_GetNode(result, "ErrorDetail", "//Error")
            If len(pcv_strReason)=0 Then
                pcv_strReason = pcf_GetNode(result, "Reason", "//SubscriptionResponse")
            End If            
            SB_ErrMsg = "<div align=""left"">" & pcv_strReason & "</div>"
	...
	'//////////////////////////////////////////////////////////////////////////////
	'// START:  Update Subscription Request
	'//////////////////////////////////////////////////////////////////////////////
	Public Function UpdateSubscriptionRequestXML()
		Dim xmlStr				
				xmlStr = 			"<?xml version=""1.0"" encoding=""utf-8""?>"
				...
				If len(PayInfoCardNumber)>0 Then
					xmlStr = xmlStr & 		"<CreditCard>"
					xmlStr = xmlStr & 			"<CardNumber>" & PayInfoCardNumber & "</CardNumber>"
					xmlStr = xmlStr & 			"<CardType>" & PayInfoCardType & "</CardType>"
					xmlStr = xmlStr & 			"<ExpMonth>" & PayInfoExpMonth & "</ExpMonth>"
				...
				If len(Request("x_TrialTotalBillingCycles"))>0 Then
					'xmlStr = xmlStr & 		"<Package>"
					'xmlStr = xmlStr & 			"<Plan>"
					'xmlStr = xmlStr & 				"<Profile>"
					'xmlStr = xmlStr & 					"<TrialTotalBillingCycles>" & Request("x_TrialTotalBillingCycles") & "</TrialTotalBillingCycles>"
					'xmlStr = xmlStr & 				"</Profile>"
					'xmlStr = xmlStr & 			"</Plan>"
					'xmlStr = xmlStr & 		"</Package>"
				End If

				If len(CartRegularAmt)>0 Then
					xmlStr = xmlStr & 		"<Subscription>"
					xmlStr = xmlStr & 			"<BillingDetails>"
					...
					xmlStr = xmlStr & 				"<ShipAmount>" & CartRegularShipping & "</ShipAmount>"
					'if CartTrialShipping>0 OR IsTrial then
					'	xmlStr = xmlStr & 			"<TrialShipping>" & CartTrialShipping & "</TrialShipping>"
					'end if
					
					xmlStr = xmlStr & 				"<TaxAmount>" & CartRegularTax & "</TaxAmount>"
					'if CartTrialTax>0 OR IsTrial then	
					'	xmlStr = xmlStr & 			"<TrialTax>" & CartTrialTax & "</TrialTax>"
					'end if
		...
		UpdateSubscriptionRequestXML=xmlStr
	End Function
	
	Public Function UpdateSubscriptionRequest(APIUser, APIPass, APIKey)		
		pcv_username = APIUser
		pcv_token = GetToken(APIUser, APIPass, APIKey)	
		
		query = "SELECT pcPO_SubAmount, pcPO_SubTrialAmount FROM ProductsOrdered WHERE idOrder="& sb_IdOrder&" AND pcSubscription_id > 0;"
		set rs=Server.CreateObject("ADODB.RecordSet")
		set rs=connTemp.execute(query)		
		If NOT rs.EOF Then
		...
		result = WebRequest("UpdateSubscriptionRequest", UpdateSubscriptionRequestXML, gv_EndPointManagement, APIUser, APIPass, APIKey)

		strStatus = pcf_GetNode(result, "Ack", "//UpdateSubscriptionResponse")
		...
		elseif strStatus="Failure" then
			...
			tmpReason = pcf_GetNode(result, "Reason", "//UpdateSubscriptionResponse")
			
			SB_ErrMsg = "<div align=""left""><ul><li>We could not update your order due to the following reason: " & tmpReason & "</li></ul></div>"
	...
	End Function 
	'//////////////////////////////////////////////////////////////////////////////
	'// END:  Update Subscription Request
	'//////////////////////////////////////////////////////////////////////////////



	'//////////////////////////////////////////////////////////////////////////////
	'// START:  Update Credit Card Request
	'//////////////////////////////////////////////////////////////////////////////
	Public Function UpdateCreditCardRequestXML()
		Dim xmlStr				
				xmlStr = 			"<?xml version=""1.0"" encoding=""utf-8""?>"
				xmlStr = xmlStr & 	"<UpdateCreditCardRequest>"	
				xmlStr = xmlStr & 		"<Username>" & pcv_username & "</Username>"
				xmlStr = xmlStr & 		"<Token>" & pcv_token & "</Token>"
				xmlStr = xmlStr & 		"<Guid>" & GUID & "</Guid>"
				xmlStr = xmlStr & 		"<CreditCard>"
				...
				'xmlStr = xmlStr & 		"</Customer>"
				xmlStr = xmlStr & 	"</UpdateCreditCardRequest>"
		UpdateCreditCardRequestXML=xmlStr
	End Function
	
	Public Function UpdateCreditCardRequest(APIUser, APIPass, APIKey)	
		pcv_username = APIUser
		pcv_token = GetToken(APIUser, APIPass, APIKey)
		UpdateCreditCardRequest = WebRequest("UpdateCreditCardRequest", UpdateCreditCardRequestXML, gv_EndPointManagement, APIUser, APIPass, APIKey)
	End Function 
	'//////////////////////////////////////////////////////////////////////////////
	'// END:  Update Credit Card Request
	'//////////////////////////////////////////////////////////////////////////////
	...
	Public Function OneTimePaymentRequest(APIUser, APIPass, APIKey)	
		pcv_username = APIUser
		pcv_token = GetToken(APIUser, APIPass, APIKey)
		OneTimePaymentRequest = WebRequest("OneTimePaymentRequest", OneTimePaymentRequestXML, gv_EndPointManagement, APIUser, APIPass, APIKey)
	End Function 
	'//////////////////////////////////////////////////////////////////////////////
	...
	'//////////////////////////////////////////////////////////////////////////////
	Public Function TransactionRequestXML()
		Dim xmlStr				
				xmlStr = 			"<?xml version=""1.0"" encoding=""utf-8""?>"
				xmlStr = xmlStr & 	"<TransactionRequest>"	
				xmlStr = xmlStr & 		"<Username>" & pcv_username & "</Username>"
				xmlStr = xmlStr & 		"<Token>" & pcv_token & "</Token>"
				xmlStr = xmlStr & 		"<Amount>" & CartRegularAmt & "</Amount>"
				xmlStr = xmlStr & 		"<GUID>" & GUID & "</GUID>"
				...
				xmlStr = xmlStr & 		"</Customer>"
				xmlStr = xmlStr & 	"</TransactionRequest>"
		TransactionRequestXML=xmlStr
	End Function
	
	Public Function TransactionRequest(APIUser, APIPass, APIKey)	
		pcv_username = APIUser
		pcv_token = GetToken(APIUser, APIPass, APIKey)
		TransactionRequest = WebRequest("TransactionRequest", TransactionRequestXML, gv_EndPointGateway, APIUser, APIPass, APIKey)
	End Function 
	'//////////////////////////////////////////////////////////////////////////////
	...
	Public Function CancellationRequest(APIUser, APIPass, APIKey)	
		pcv_username = APIUser
		pcv_token = GetToken(APIUser, APIPass, APIKey)	
		CancellationRequest = WebRequest("CancellationRequest", CancellationRequestXML, gv_EndPointManagement, APIUser, APIPass, APIKey)
	End Function 
	'//////////////////////////////////////////////////////////////////////////////
	'// END:  Cancel Request
	'//////////////////////////////////////////////////////////////////////////////
	...
	Public Function GetSubscriptionDetailsRequest(APIUser, APIPass, APIKey)	
		pcv_username = APIUser
		pcv_token = GetToken(APIUser, APIPass, APIKey)
		GetSubscriptionDetailsRequest = WebRequest("GetSubscriptionDetailsRequest", GetSubscriptionDetailsRequestXML, gv_EndPointManagement, APIUser, APIPass, APIKey)
	End Function 
	'//////////////////////////////////////////////////////////////////////////////
