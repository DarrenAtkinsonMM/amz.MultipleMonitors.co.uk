...
if scEmailComObj="Bamboo" then
	...
	If session("News_MsgType")="1" Then
	objMail.ContentType = "text/html; charset=UTF-8"
	else
	objMail.ContentType = "text/plain; charset=UTF-8"
	End if
...
Public Function pcf_HtmlEmailWrapper(strRegEmail, pcv_HTMLEmailFontFamily)

    Dim pcv_strHTMLOut

    if session("News_MsgType")="1" then
        pcv_strHTMLOut = "<html>"
        pcv_strHTMLOut = pcv_strHTMLOut & "<body style=""font-family: " & pcv_HTMLEmailFontFamily & "; font-size: " & scEmailFontSize & ";""><div>" 
        pcv_strHTMLOut = pcv_strHTMLOut & strRegEmail
        pcv_strHTMLOut = pcv_strHTMLOut & "</div></body> "
        pcv_strHTMLOut = pcv_strHTMLOut & "</html> "
    else
        pcv_strHTMLOut=replace(strRegEmail, "<br>"&vbCrLf, vbCrLf)
        pcv_strHTMLOut=replace(pcv_strHTMLOut, "<br>", vbCrLf)
    end if

    pcf_HtmlEmailWrapper = pcv_strHTMLOut

End Function

Function FixedField(ByVal Width, ByVal Justify, ByVal Text)

	TmpText = Text
	TmpText = Replace(TmpText, "&reg;", "®")
	TmpText = Replace(TmpText, "&copy;", "©")
	TmpText = Replace(TmpText, "&trade;", "™")
    
    '// WIDTH ADJUSTMENTS
    If Width = 100 Then Width = 80
    If Width = 60 Then Width = 40
    If Width = 85 Then Width = 65
    If Width = 10 Then Width = 10
    If Width = 15 Then Width = 15

	Select Case True
		Case Width < Len(TmpText)
			Select Case True
				Case Justify="L"
					FixedField=Left(TmpText, Width)
				Case Justify="R"
					FixedField=Right(TmpText, Width)
				Case Else
			End Select
		...
		Case Width > Len(TmpText)
			Select Case True
				Case Justify="L"
					FixedField=TmpText & String(Width - Len(TmpText), " ")
				Case Justify="R"
					FixedField=String(Width - Len(TmpText), " ") & TmpText
				Case Else
			End Select

	End Select

	FixedField = Replace(FixedField, "®", "&reg;")
	FixedField = Replace(FixedField, "©", "&copy;")
	FixedField = Replace(FixedField, "™", "&trade;")
    FixedField = Replace(FixedField, " ", "&nbsp;")
