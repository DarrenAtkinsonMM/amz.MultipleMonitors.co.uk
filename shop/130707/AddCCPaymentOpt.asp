...
response.redirect "techErr.asp?error="& Server.Urlencode("Error in AddCCPaymentOpt: "&Err.Description) 
			...
			Else
				query="UPDATE payTypes SET active=-1, priceToAdd="&priceToAddCC&", percentageToAdd="&percentageToAddCC&", cvv="&cvv&", paymentNickName=N'"&paymentNickName&"',pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & " WHERE gwcode=6"
				set rstemp=Server.CreateObject("ADODB.Recordset")     
				rstemp.Open query, conntemp
				if err.number <> 0 then
					set rstemp=nothing
        ...
        <tr> 
            <td valign="top"> 
                <table class="pcCPcontent">
                    ...
                    <tr>
                        <td>
                            <input type="submit" value="Add" name="SubmitB" class="btn btn-primary">&nbsp;
