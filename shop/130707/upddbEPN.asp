...
response.redirect "upddbEPN.asp?mode=1"
	...
	query="ALTER TABLE [pcPay_EPN] ADD [pcPay_EPN_TranType] [nvarchar] (50) NULL"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
	...
	query="SELECT [pcPay_EPN_ID] FROM [pcPay_EPN] WHERE [pcPay_EPN_ID] = 1;"
	set rs=conntemp.execute(query)
	if rs.eof then
		query="INSERT INTO [pcPay_EPN] ([pcPay_EPN_ID], [pcPay_EPN_Account], [pcPay_EPN_RestrictKey], [pcPay_EPN_TranType]) VALUES (1,'','','');"
	...
	query="CREATE TABLE [dbo].[pcPay_EPN_Authorize] ("
	query=query&"[idAuthOrder] [INT] IDENTITY (1, 1) PRIMARY KEY CLUSTERED NOT NULL,"
	query=query&"[idOrder] [int] NULL,"
	query=query&"[idCustomer] [int]  DEFAULT(0) NULL,"
	query=query&"[authcode] [nvarchar](250) NULL,"
	query=query&"[captured] [bit] NULL,"
	query=query&"[transtype] [nvarchar](250) NULL,"
	query=query&"[AuthorizedDate] [datetime] NULL,"
	query=query&"[amount] [money] NULL"
	query=query&") ON [PRIMARY];"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		call TrapSQLError("pcPay_EPN_Authorize")	
	end if
	on error resume next
...
if strMode="complete" then
	call closeDb()
response.redirect "pcConfigurePayment.asp?mode="&tmp_mode&"&id="&tmp_id&"&gwchoice=42"
	response.end
else %>
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			...
			<% if scDB="Access" then %>
					<tr>
						<td align="center">
						  <input type="button" class="btn btn-default"  name="access" value="Update Your ProductCart MS Access Database" onClick="location='upddbEPN.asp?action=access&mode=<%=tmp_mode%>&id=<%=tmp_id%>';" class="btn btn-primary">
						</td>
					</tr>
			<% 
				else
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" class="btn btn-default"  name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbEPN.asp?action=sql&mode=<%=tmp_mode%>&id=<%=tmp_id%>';" class="btn btn-primary">
						<% end if %>
					</td>
			</tr>
			<% end if %>
