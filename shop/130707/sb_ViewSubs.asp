...
query="SELECT orders.idOrder, orders.orderDate, orders.total, orders.ord_OrderName, ProductsOrdered.idProductOrdered,ProductsOrdered.UnitPrice,ProductsOrdered.quantity, ProductsOrdered.pcSubscription_ID, ProductsOrdered.pcPO_SubAmount, ProductsOrdered.pcPO_SubActive, ProductsOrdered.pcPO_IsTrial, ProductsOrdered.pcPO_SubTrialAmount, ProductsOrdered.pcPO_SubStartDate, ProductsOrdered.pcPO_SubType, productsordered.pcPO_LinkID "
query=query&"FROM orders "
query=query&"INNER JOIN productsordered ON productsordered.idOrder = orders.idOrder "
query=query&"INNER JOIN SB_Orders ON SB_Orders.idOrder = orders.idOrder "
	
If pcv_intIDMain="0" Then

	query=query&"WHERE orders.OrderStatus>1  And orders.idOrder = ProductsOrdered.idOrder and ProductsOrdered.pcSubscription_ID >0  ORDER BY ProductsOrdered.idProductOrdered DESC"
	
Else

	query=query&"WHERE "
	If len(pcv_intIDMain)>0 Then
		query=query&"productsordered.pcPO_LinkID='" & pcv_intIDMain &"' AND "
	End If
	If len(pcv_strGUID)>0 Then
		query=query&"SB_Orders.SB_GUID='" & pcv_strGUID &"' AND "
	End If
	query=query&"orders.OrderStatus>1  And orders.idOrder = ProductsOrdered.idOrder and ProductsOrdered.pcSubscription_ID >0  ORDER BY ProductsOrdered.idProductOrdered DESC"
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error in sb_CustViewSubs.asp: "&err.description) 
		...
		<tr> 
            <td>
            	<% If len(pcv_strGUID)>0 Then %>
            		<input type="button" class="btn btn-default"  value=" Go Back " onClick="location.href='Orddetails.asp?id=<%=pcv_idOrder%>'">
                <% Else %>
            		<input type="button" class="btn btn-default"  value=" Main Menu " onClick="location.href='sb_Default.asp'">
                	<input type="button" class="btn btn-default"  value=" View Packages " onClick="location.href='sb_ViewPackages.asp'">
                <% End If %>
            </td>
        </tr>
    </table>
