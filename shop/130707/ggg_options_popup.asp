...
if request("action")="upd" then
	...
	if request("Apparel")="1" then
		if request("ParentPrd")="0" then
			pcv_ParentPrd=request("idproduct")
		else
			pcv_ParentPrd=request("ParentPrd")
		end if
		pcv_RelationShip=pcv_ParentPrd & "_" & replace(pcv_strSelectedOptions,"|","_")

		query="SELECT idproduct,price FROM products WHERE pcProd_ParentPrd=" & pcv_ParentPrd & " AND pcProd_Relationship like '" & pcv_RelationShip & "' AND removed=0 and pcProd_SPInActive=0;"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		pcv_NewID=0
		if not rs.eof then
			pcv_NewID=rs("idproduct")
			pcv_Price=rs("price")
		end if
		set rs=nothing
			
		if pcv_NewID<>0 then
			query ="Update pcEvProducts SET pcEP_OptionsArray='',pcEP_IDProduct=" & pcv_NewID & ",pcEP_Price=" & pcv_Price & " WHERE pcEP_ID=" & epID & " AND pcEP_IDProduct=" & pidproduct & " AND pcEP_IDEvent=" & gIDEvent & ";"
		end if
	else
...
<%if msg<>"" then%>
	...
	<tr>
		<td align="center">
			<input type="button" class="btn btn-default"  name="Back" value="Close Window" onClick="opener.location.reload(); self.close();">
		</td>
	</tr>
</table>
<%else%>
		...
		<td>		
			<table width="98%" border="0" align="center" cellpadding="4" cellspacing="0">
				...
				<tr class="normal">
					...
					<input type="hidden" name="ParentPrd" value="<%=request("ParentPrd")%>">
					<input type="hidden" name="Apparel" value="<%=request("Apparel")%>">
