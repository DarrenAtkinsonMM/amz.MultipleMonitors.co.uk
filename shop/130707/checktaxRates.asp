...
<body style="background-image: none;">
	<table class="pcCPMainTable" width="100%" border="0" cellspacing="1" cellpadding="3">
		<tr>
			<td>
				<table class="pcCPcontent">
					<tr> 
						<th width="71%">Tax Rate</th>
						<th width="29%"></th>
					</tr>
					<%
                    If ptaxfile=1 then
                        ...
                        If pCountryCode="US" then
                            ...
                            do While NOT TextStream.AtEndOfStream
                                ...
                                'ignore first line
                                if instr(ucase(Line), "ZIP_CODE") then
                                    iArray=split(Line,",")
                                    pcv_PostalCodeColumnFlag=False
                                    
                                    'loop to find correct array for each
                                    for q=0 to ubound(iArray)
                                        if trim(iArray(q))="ZIP_CODE" then
                                            ZIP_CODE_NUM=q
                                            pcv_PostalCodeColumnFlag=True
                                            'response.write q&"<BR>"
                                        end if
                                        if trim(iArray(q))="COUNTY_NAME" then
                                            COUNTY_NAME_NUM=q
                                            'response.write q&"<BR>"
                                        end if
                                        if trim(iArray(q))="CITY_NAME" then
                                            CITY_NAME_NUM=q
                                            'response.write q&"<BR>"
                                        end if
                                        if trim(iArray(q))="TOTAL_SALES_TAX" then
                                            TOTAL_SALES_TAX_NUM=q
                                            'response.write q&"<BR>"
                                        end if
                                        if trim(iArray(q))="TAX_SHIPPING_ALONE" then
                                            TAX_SHIPPING_ALONE_NUM=q
                                            'response.write q&"<BR>"
                                        end if
                                        if trim(iArray(q))="TAX_SHIPPING_AND_HANDLING_TOGETHER" then
                                    ...
                                    if instr(Line, pZip) then						
                                        ...
                                        if pcv_PostalCodeColumnFlag=False OR pcv_ValidPostalCode=True then
                                            ...
                                            optionStr=optionStr&"<tr><FORM action='checktaxRates.asp' NAME='inputForm"&zipCnt&"' onSubmit='return setForm"&zipCnt&"();'><td width='71%'>"&CITY_NAME&" - "&COUNTY_NAME&": "&TOTAL_SALES_TAX*100&"%<INPUT NAME='inputField"&zipCnt&"' TYPE='hidden' VALUE='"&TOTAL_SALES_TAX*100&"'></td><td width='29%'><INPUT TYPE='SUBMIT' name='UPD' VALUE='Select Rate' onSubmit='return setForm();'></td></form></tr>"
                                        end if
                                    end if
                                end if
                            loop
                        ...
                        if optionStr="" then
                            response.write "<tr><td colspan=""2""><p>ProductCart cannot find any applicable Sales Tax Rates for this Order based on the Tax Settings you have designated. If you are using a Tax File or fall back rates, then the Zip Code or State Code is not a match.</p><p>You can manually add Sales Tax by entering the correct amount in the box on the right and clicking the <strong>Update Order</strong> button at the bottom of the page.</p></td></tr>"
                        else
                            response.write optionStr & vbCrlf
...
response.redirect "techErr.asp?error="&Server.Urlencode("Error in tax 1: "&err.description)
                        ...
                        do until rs.eof 
                            ...
                            taxCnt=taxCnt+1 %>
                            <tr>
                            <FORM action="checktaxRates.asp" NAME="inputForm<%=taxCnt%>" onSubmit="return setForm<%=taxCnt%>();">
