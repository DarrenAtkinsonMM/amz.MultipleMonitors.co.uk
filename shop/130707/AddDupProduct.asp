...
if pcSCID>"0" then
	...
	if not rsS.eof then
		...
		<div class="pcCPmessageInfo">
			You are cloning an "On Sale" product. This product is currently included in the running Sale: 
			<a href="javascript:winSale('sm_showdetails.asp?id=<%=pcSCID%>')"><%=pcSCName%></a>
			<br/ >
			<% If statusAPP="1" OR scAPP=1 Then %> 
				Please remember the product price and sub-product prices are not the normal prices.
			<% Else %>
				Please remember the product price is not the normal price.
			<% End If %>
		</div>
		</td>
		</tr>
...
'SM-END%>
        ...
        <tr>
            <td colspan="2">
		        ...
		        <tr>
				<td>
				<form action="AddDupProduct.asp" id="FormAddCat" class="pcForms">
...
<form method="post" name="hForm" action="addProductB.asp" onSubmit="return Form1_Validator(this)" class="pcForms">
	  	...
	  	function LoadCategoryList()
		{
			...
			$pc.ajax({
				type: "GET",
				data: "idRootCategory=<%=pcv_IdRootCategory%>&strCats=<%=cat_SelectedItems%>",
			...
			<ul class="nav nav-tabs tabs-left">
				...
				<li>
					<div style="height:40px; margin-top:10px; text-align: center">
						...
						<input type="submit" name="Submit" value="Add Product" class="btn btn-primary" disabled>
					</div>
				</li>
			</ul>
		</div>
				...
				<div id="tab-1" class="tab-pane active">
					...
					<table class="pcCPcontent">				
						...
						<tr> 
							...
							<td>  
								<textarea class="htmleditor" id="details" name="details" rows="6" cols="60" tabindex="103"><%=pDetails%></textarea>
							</td>
						</tr>
						...
						<tr> 
							...
							<td valign="top">
								<textarea name="sdesc" rows="6" cols="60" tabindex="104"><%=psDesc%></textarea>
							</td>
						</tr>
						<% end if %>
            <tr>
				...
				<div id="tab-5" class="tab-pane">
					...
					<table class="pcCPcontent">
						...
						<tr>
							...
							<td>Yes<input type="checkbox" name="noStock" id="noStock" value="-1" <% if pNoStock<>0 then response.write "checked" %> class="clearBorder" tabindex="501"></td>
						</tr>
						<tr> 
							<td>Stock:</td>
							<td>  
						...
						<tr> 
							...
							<td><input name="minimumqty" type="text" value="<%=pcv_lngMinimumQty%>" size="5" maxlength="10" tabindex="503">
								&nbsp;&nbsp;&nbsp;&nbsp;          
									<input type="checkbox" name="qtyvalidate" value="1" <%if pcv_intQtyValidate=1 then%>checked<%end if%> class="clearBorder" tabindex="504"> Force purchase of multiples of:&nbsp;<input name="multiQty" type="text" value="<%=pcv_multiQty%>" size="5" maxlength="10">
							</font></td>
						</tr>
						<%'End SDBA%>
						<tr>
                        ...
                        <tr> 
							...
							<td>Yes
							    <input type="checkbox" name="noshipping" id="noShipping" value="-1" <% if pnoshipping="-1" then response.write "checked" %> class="clearBorder" tabindex="708">
                                &nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=449">
							</td>
						</tr>
                        <%
                            pcv_strDisplayStyle = ""
                        ...
                        %>
						<tbody id="noShippingSettings" <%= pcv_strDisplayStyle %>>
						    <tr>
							    <td>Display Non-Shipping Text:</td>
							    <td>Yes
								    <input type="checkbox" name="noshippingtext" value="-1" <% if pnoshippingtext="-1" then response.write "checked" %> class="clearBorder" tabindex="709">
							    </td>
						    </tr>
						</tbody>
						<tr>
						...
						<tr>
							<td colspan="2">This product will be shipped as an oversized product.
								<input name="OverSizeSpec" type="radio" value="YES" <% if pOverSizeSpec="YES" then response.write "checked" %> class="clearBorder" tabindex="608">&nbsp;Yes 
								<input name="OverSizeSpec" type="radio" value="NO" <% if pOverSizeSpec="NO" then response.write "checked" %> class="clearBorder" tabindex="609">&nbsp;No 
								<br>
							...
							<td colspan="2">
								<table class="pcCPcontent">
									<tr> 
                                        <td>Length:</td>
                                        <td width="15%"> 
                                            <input name="os_length" type="text" id="os_length" size="3" maxlength="3" value="<%=tOS_length%>" tabindex="612">
                                        </td>
                                    ...
                                    <tr> 
										...
										<td width="15%"> 
											<input name="os_height" type="text" id="Text1" size="3" maxlength="3" value="<%=tOS_height%>" tabindex="610">
										</td>
                                    </tr>
                                </table>
                            </td>
					...
					<table class="pcCPcontent">
						...
						<tr>
							...
							<tr>
								<td>Page Layout:</td>
								<td>
										<select name="displayLayout" id="displayLayout" tabindex="706" onChange="javascript:if (this.value=='t') {document.hForm.UseCustomPage.checked=true} else {document.hForm.UseCustomPage.checked=false}">
											...
											<option value="t" <%if pDisplayLayout="t" then%>selected<%end if%>>Customized Layout</option>
										</select>
								</td>
							</tr>
							<%
						...
						<table class="pcCPcontent">
							...
							<tr>
								...
								<table id="show_21" <%if (pcv_Apparel="0") or (pcv_Apparel="") then%>style="display:none"<%end if%> class="pcCPcontent">
									<tr>
										<td width="30%" nowrap>
											<input type="checkbox" name="dupsubprd" value="1" style="float: right" <%if pcv_Apparel="1" then%>checked<%end if%> class="clearBorder">
										</td>
										<td width="70%" nowrap><b>Also duplicate Sub-Products</b></td>
									</tr>
									<tr>
