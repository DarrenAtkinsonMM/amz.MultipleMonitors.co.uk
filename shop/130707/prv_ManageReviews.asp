...
IF request("action")="resetrating" and pcv_IDProduct>0 then
	...
	call pcs_hookProductModified(pcv_IDProduct, "")
	...
	<table class="pcCPcontent">
		...
		<tr>
			...
			' PRV41 end %>
				...
				' PRV41 end
				%>>&nbsp;
				<input type="button" class="btn btn-default"  value="Reset Rating" onclick="javascript:if (confirm('You are about to reset the rating for this product. All customer reviews for this product will be deleted from the database. Are you sure you want to complete this action?')) location='prv_ManageReviews.asp?action=resetrating&IDProduct=<%=pcv_IDProduct%>&nav=<%=pcv_RevName%>'">&nbsp;
				<input type="submit" value="Delete selected" name="submit2" class="btn btn-default" <%
				' PRV41 begin
			...
			<tr>
                ...
                <hr>
				<%if DataEmpty=1 then%>
				<input type="button" class="btn btn-default"  value="Other Pending Reviews" onClick="location='prv_ManageRevPrds.asp?nav=1';">&nbsp;
				<%else%>
				<input type="button" class="btn btn-default"  value="Bad Words List" onClick="location='prv_ManageBadWords.asp';">&nbsp;
				<%end if%>
			<%
			' PRV41 begin
			End If
			' PRV41 end
			%>
			<input type="button" class="btn btn-default"  value="Back" onClick="location='prv_ManageRevPrds.asp?nav=<%=pcv_RevName%>';">
			</td>
		</tr>
	</table>
