...
pcs_JavaTextField	"pShipFromEmail", pcv_isemailRequired, dictLanguage.Item(Session("language")&"_NewCust_3"), ""
...
IF request.Form("Submit")<>"" THEN
	...
	pcs_ValidateEmailField	"pShipFromEmail", pcv_isemailRequired, 250
	...
	pcs_ValidateTextField	"UseShipMap", false, 0
...
ELSE
	...
	if msg="" then 
		...
		Session("pcAdminpShipFromEmail")=scOriginEmailAddress
...
<form name="fromShipper" method="post" action="<%=pcStrPageName%>" onSubmit="return Form1_Validator(this)" class="pcForms">  
	...
	<table class="pcCPcontent">
		...
		<%	'// Email Custom Error
		if session("ErrpShipFromEmail")<>"" then %>
			<tr> 
				<td>&nbsp;</td>
				<td><img src="<%=pcf_GenerateIconURL("images/sample/pc_icon_next.gif")%>" width="10" height="10">You must enter a valid email address</td>
			</tr>
			<% session("ErrpShipFromEmail") = ""
		end if %>
		<tr> 
                        <td><p>Email:</p></td>
                        <td>
                        <p>
                        <input name="pShipFromEmail" type="text" value="<% =pcf_FillFormField ("pShipFromEmail", pcv_isEmailRequired) %>" size="20" maxlength="50">
                        <% pcs_RequiredImageTag "pShipFromEmail", pcv_isEmailRequired %>
                        </p>
                        </td>
                    </tr>
		<%	'// Phone Custom Error
        ...
        <tr>
            <td colspan="2">
                <p>Enter the Maximum Weight per Package (if any): <input name="packageWeightLimit" type="text" id="packageWeightLimit" size="4" maxlength="4" value="<%=scPackageWeightLimit%>"> <%=scShipFromWeightUnit%> &nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=474"></a></p>
            </td>
        </tr>
	<tr> 
		<td colspan="2" class="pcCPspacer"></td>
      ...
      <%
				...
				if intShipTypeCnt>1 then %>
					<tr> 
						<td colspan="2" valign="top">
							<p>Select a default shipping provider:&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=426"></a></p>
						</td>
					</tr>
                    <tr> 
						<td colspan="2" valign="top">
				...
				<tr valign="top">
					<td align="right" valign="top">
                        <%
                        pcv_boolHasMapping = pcf_HasShipMapping()
                        %>
                        <% If (scUseShipMap="0") And (Not pcv_boolHasMapping) Then %>
                            <input type="checkbox" name="UseShipMap" value="" disabled class="disabled clearBorder">
                        <% Else %>
                            <input type="checkbox" name="UseShipMap" value="1" <% If (scUseShipMap="1") Then %>checked<% End If %> class="clearBorder">
                        <% End If %>
                    </td>
					<td>
                        Filter shipping results into a single listing (no tab groups).
                        
                        <% If Not pcv_boolHasMapping Then %>
                            <% If (scUseShipMap="1") Then %>
                                <div class="pcSmallText">This feature is enabled, but no rates will be filtered until you <a href="manageShipMap.asp" target="_blank">setup shipping filters</a>.</div>
                            <% Else %>
                                <div class="pcSmallText">This feature cannot be enabled until you <a href="manageShipMap.asp" target="_blank">setup shipping filters</a>.</div>
                            <% End If %>
                        <% Else %>
                            <span class="pcSmallText"> - <a href="manageShipMap.asp" target="_blank">Manage Filters</a></span>
                        <% End If %>
                        
                    </td>
				</tr>
                -->
                <input type="hidden" name="UseShipMap" value="">
                ...
                <tr> 
                    <td colspan="2">Commercial vs. Residential &nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=475"></a></td>
                </tr>
				<tr> 
				...
				<tr> 
                    ...
                    <div class="pcSmallText">You can let customers decide whether to receive one or multiple shipments using the <a href="AdminSettings.asp?tab=3" target="_blank">Allow Separate Shipments</a> setting.</div></td>
				</tr>
				<tr> 
				...
				<tr> 
					...
					<td>Display 'Estimated Shipping Charges' widget on the View Shopping Cart page.</td>
				</tr>
				<tr> 
	...
	<tr> 
		...
		&nbsp;
		<input type="button" class="btn btn-default"  value="View/Edit Shipping Options" onClick="location.href='viewShippingOptions.asp'">
		</td>
	</tr>
</table>
</form>
