...
If request("submit1")<>"" OR request("submit2")<>"" OR request("submit3")<>"" then	
	...
	If request("submit1")<>"" then
		...
		'query="DELETE FROM configSessions WHERE (idconfigSession IN (SELECT SCArray16 FROM pcSavedCartArray WHERE pcSavedCartArray.SavedCartID IN (SELECT SavedCartID FROM pcSavedCarts WHERE SavedCartDate<" &strDtDelim&dateadd("d",ndays,pcCustSession_Date)&strDtDelim& ")));"
		'set rstemp=connTemp.execute(query)
		'set rstemp=nothing
	...
	If request("submit2")<>"" then
		...
		'query="DELETE FROM configSessions WHERE (idconfigSession IN (SELECT SCArray16 FROM pcSavedCartArray WHERE pcSavedCartArray.SavedCartID IN (SELECT SavedCartID FROM pcSavedCarts" & query1 & ")));"
		'set rstemp=connTemp.execute(query)
		'set rstemp=nothing
...
<% else %>
	<form action="PurgeSavedCarts.asp" method="post" name="form" id="form" class="pcForms">
		<table class="pcCPcontent">
			...
			<tr>
				<td colspan="2">
					<input name="submit1" type="submit" id="submit1" value=" Clean Up Database " class="btn btn-primary">
        		</td>
       		</tr>				
          	<tr> 
              	<td colspan="2" class="pcCPspacer"></td>
			...
			<tr>
				<td colspan="2">
					<input name="submit2" type="submit" id="submit2" value=" Clean Up Database " class="btn btn-primary">
        		</td>
       		</tr>
          	<tr> 
            	<td colspan="2" class="pcCPspacer"></td>
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error in PurgeSavedCarts: "&Err.Description) 
                    ...
                    <br /><br />
					<input name="submit3" type="submit" id="submit3" value=" Clean Up Database " class="btn btn-primary">
        		</td>
       		</tr>				
          	<tr> 
              	<td colspan="2" class="pcCPspacer"></td>
