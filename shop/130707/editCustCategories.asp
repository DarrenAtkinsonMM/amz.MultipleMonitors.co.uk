			...
			<td width="100%">
				<table id="FindProducts" class="pcCPcontent">
                    <tr>
                        ...
                        src_Button3=" Back "
                        src_PageSize=15
						'Avoid Pricing Categories
						src_DisplayCustType=1
                        UseSpecial=1
...
<form method="POST" action="editCustCategories.asp" name="myForm" class="pcForms">
		...
		<tr> 
			<td width="100%"> 
				...
				<% Else
					...
					<tr>
						<td colspan="4">
						<input type="submit" value="<%If strMode="edit" Then %>Add <% Else %>Remove <% End If %>Checked" class="btn btn-primary">
						</td>
					</tr>
	...
	<tr> 
		<td align="center">
		<% If strMode="edit" Then %>
			<input type="button" class="btn btn-default"  value="View Customers of <% Response.Write strPcCC_Name %>" onClick="document.location.href='editCustCategories.asp?OrderBy=<%=request("OrderBy")%>&SortOrd=<%=request("SortOrd")%>&CCID=<%=intCCID%>&mode=view';" class="btn btn-primary">
		<% Else %>
			<input type="button" class="btn btn-default"  value="Add Customers to <% Response.Write strPcCC_Name %>" onClick="document.location.href='editCustCategories.asp?OrderBy=<%=request("OrderBy")%>&SortOrd=<%=request("SortOrd")%>&CCID=<%=intCCID%>&mode=edit';" class="btn btn-primary">
		<% End If %>
		</td>
	</tr>
	<tr>
		<td align="center">
		<input type="button" class="btn btn-default"  value="Edit Customer Category Type" onClick="document.location.href='AdminCustomerCategory.asp?mode=2&id=<%=intCCID%>'" name="button">
		&nbsp;
		<input type="button" class="btn btn-default"  value="Manage Customer Category Types" onClick="document.location.href='AdminCustomerCategory.asp';" name="button">
		</td>
	</tr>
</table>
</form>
