...
Dim pIdProduct, pDescription, intIdCategory, pcvPageType
...
Sub SaveSiteMap()
	...
	call pcs_SaveUTF8(SavedFile, SavedFile, SiteHeader1 & pcv_strSMBody & SiteFooter1)
...
Sub SaveSiteMapIndex()
	...
	call pcs_SaveUTF8(SavedFile, SavedFile, SiteHeader2 & pcv_strSMBody1 & SiteFooter2)
...
Sub GenContent(incURL)
	...
	query="SELECT pcCont_IDPage, pcCont_PageName FROM pcContents WHERE pcCont_IDPage=" & incURL
	set rsSideCatObj=Server.CreateObject("ADODB.RecordSet")					
	set rsSideCatObj=connTemp.execute(query)
	...
	'// Change SEO URL if this is a parent page
	query="SELECT pcCont_idPage FROM pcContents WHERE pcCont_Parent=" & pcIntContentPageID
...
Sub GenSMBody()
	...
	IF not rs.eof then
		...
		For i=0 to intCount		
			...
			LOCALtmpBody=LOCALtmpBody & "|$|" &  pcStrPrdLink & "|-|"
		...
		IF not rs.eof then
			...
			For i=0 to intCount
				if pcArr(0,i)<>"1" then
					intIdCategory=pcArr(0,i)
					pcStrCategoryDesc=pcArr(1,i)
					
					'// Call SEO Routine
					pcGenerateSeoLinks
					'//
					
					LOCALtmpBody=LOCALtmpBody & "|%|" & pcStrCatLink & "|-|"
				end if
				URLCount=URLCount+1
				CheckSiteMap()
			Next
		...
		<td>
            ...
            <div>You have two ways to notify <span style="font-weight: bold">Google</span> of the existence of this site map (and you can do so whenever you update the site map). The first method requires a few more clicks, but allows you to keep track of your site map submissions.
             ...
             <div style="padding-top: 10px;">
                ...
                <form class="pcForms">
                <input type="button" class="btn btn-default"  name="btnSubmitFeed" value="Notify Yahoo!" onclick="javascript:openWin('https://siteexplorer.search.yahoo.com/submit?txtFeedUrl=<%=scSiteURL%>');"/>&nbsp;
                <input type="button" class="btn btn-default"  name="btnSubmitFeed1" value="Notify Bing" onclick="javascript:openWin('http://www.bing.com/webmaster/Crawl/Sitemaps/?url=<%=scStoreURL%>');"/>
