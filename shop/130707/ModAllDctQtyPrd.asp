	...
	<table class="pcCPcontent">
		<tr> 
			<td colspan="5">
                ...
                <h2>
        		<% if btConfigOnly <> 0 then%>Configurable-Only Item:<%else%>Product Name:<%end if%> <%=strDescription%> - SKU: <%=strSKU%>
                </h2>
...
response.redirect "ModAllDctQtyPrd.asp?idproduct="&idproduct&"&msg="&msg
		...
		<tr> 
			...
			<input type="button" class="btn btn-default"  name="Delete" value="Delete discount" onClick="javascript:if (confirm('You are about to permanantly delete this discount from the database. Are you sure you want to complete this action?')) location='modAllDctQtyPrd.asp?Delete=Yes&idproduct=<%=idProduct%>'">&nbsp;
