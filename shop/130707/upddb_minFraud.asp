<% 
PmAdmin=19
pageTitle = "ProductCart MinFraud - Database Upgrade" 
Section = "" 
%>
...
IF request("action")="sql" then
	if request("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			call closeDb()
			response.redirect "upddb_minFraud.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			call closeDb()
			response.redirect "upddb_minFraud.asp?mode=1"
			response.End
		end if
	...
	'========================================================================
	'// START OF DB UPDATES FOR MinFraud
	'========================================================================
	...
	if not TableExists("pcWebServiceFraud") then
		...
		query=query&"pcPay_FA_RiskScore [tinyint] NOT NULL DEFAULT(0),"
		...
		query=query&"pcPay_FA_RiskScoreEmail [int] NOT NULL DEFAULT(0),"
		query=query&"pcPay_FA_Emails [nvarchar] (200),"
		query=query&"pcPay_FA_RiskScoreLock [int] NOT NULL DEFAULT(0)"
		query=query&");"
		err.clear
		on error resume next
	...
	'========================================================================
	'// END OF DB UPDATES FOR MinFraud
	'========================================================================
...
<form action="upddb_minFraud.asp" method="post" name="form1" id="form1" class="pcForms">
<%
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
                ...
                <h1 class="page-header">MinFraud Database Update</h1>
                <p class="lead">
