...
if request("action")="run" then
	...
	if (pcv_ID<>"") and (pcv_ID<>"0") and IsNumeric(pcv_ID) then
		query="UPDATE pcCustomerFields SET pcCField_Name=N'" & pcv_FName & "',pcCField_Description=N'" & pcv_FDesc & "',pcCField_FieldType=" & pcv_FType & ",pcCField_Length=" & pcv_FLen & ",pcCField_Maximum=" & pcv_FMax & ",pcCField_Required=" & pcv_FReq & ",pcCField_ShowOnReg=" & pcv_ShowReg & ",pcCField_ShowOnCheckout=" & pcv_ShowCheckout & ",pcCField_Value=N'" & pcv_FValue & "',pcCField_PricingCategories=" & pcv_PricingCats & " WHERE pcCField_ID=" & pcv_ID & ";"
	else
		query="INSERT INTO pcCustomerFields (pcCField_Name,pcCField_Description,pcCField_FieldType,pcCField_Length,pcCField_Maximum,pcCField_Required,pcCField_ShowOnReg,pcCField_ShowOnCheckout,pcCField_Value,pcCField_PricingCategories) VALUES (N'" & pcv_FName & "',N'" & pcv_FDesc & "'," & pcv_FType & "," & pcv_FLen & "," & pcv_FMax & "," & pcv_FReq & "," & pcv_ShowReg & "," & pcv_ShowCheckout & ",N'" & pcv_FValue & "'," & pcv_PricingCats & ");"
	end if
...
<form name=form1 action="addmodCustField.asp?action=run" method="post" class="pcForms" onSubmit="return Form1_Validator(this)">
	<table class="pcCPcontent">
		...
		<tr>
			<td>Field Value:</td>
			<td><input type="text" size="40" name="pcv_FValue" value="<%=pcv_FValue%>">&nbsp;<em>E.g. &quot;<strong>Yes</strong>&quot;</em>&nbsp;&nbsp;<span class="pcCPnotes">NOTE: Required if the Field Type is &quot;Checkbox&quot;</span>
			</td>
		</tr>
		<tr>
			<td>Field Length:</td>
		...
		<tr> 
			...
			<input type="submit" name="submit" value="<%if (pcv_ID<>"") and (pcv_ID<>"0") and IsNumeric(pcv_ID) then%>Update Special Customer Field<%else%>Add New Special Customer Field<%end if%>" class="btn btn-primary">
			&nbsp;
			<input type="button" class="btn btn-default"  name="back" value="Back" onClick="javascript:location='manageCustFields.asp';">
