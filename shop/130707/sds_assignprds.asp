			...
			if pcv_PageType="0" then
				if pcv_IsDropShipper="1" then
					...
					If statusAPP="1" OR scAPP=1 Then
						query="UPDATE Products SET pcProd_IsDropShipped=1,pcDropShipper_ID=" & pcv_IDSDS & " WHERE pcProd_ParentPrd=" & pcArr(i)
						set rs=connTemp.execute(query)
						set rs=nothing
					End If
				end if
				If statusAPP="1" OR scAPP=1 Then
					query="UPDATE Products SET pcSupplier_ID=" & pcv_IDSDS & " WHERE pcProd_ParentPrd=" & pcArr(i)
					set rs=connTemp.execute(query)
					set rs=nothing
				End If
				...
				If statusAPP="1" OR scAPP=1 Then
					...
					if not rs.eof then
						...
						For j=0 to tmpCount
							...
							set rs=connTemp.execute(query)
							set rs=nothing
							query="INSERT INTO pcDropShippersSuppliers (idproduct,pcDS_IsDropShipper) VALUES (" & tmpArr(0,j) & "," & pcv_IsDropShipper & ");"
							set rs=connTemp.execute(query)
							set rs=nothing
						Next
					end if
