...
response.redirect "AddPayPalPaymentOpt.asp?gwchoice=PayPal&ppv=0&msg="&Server.URLEncode("You must enter your PayPal email address to activate PayPal Standard.")
				...
				if PayPalSelection="PayPalDP" then
					...
					PayPalName="PayPal Payments Pro (Direct Payments)"
					ppGwcode=46
...
response.redirect "techErr.asp?err="&pcStrCustRefID
			...
			query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName, pcPayTypes_ppab) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'PayPal-PayFlow-Pro','gwpfp.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","& percentageToAdd &",2,N'"&paymentNickName&"',"&ppAB&")"
			set rs=Server.CreateObject("ADODB.Recordset")     
			set rs=connTemp.execute(query)
			if err.number<>0 then
				call LogErrorToDatabase()
...
response.redirect "techErr.asp?err="&pcStrCustRefID
			...
			query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName,pcPayTypes_ppab) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'PayPal-PayFlow-Link','gwpfl.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",9,N'"&paymentNickName&"',"&ppAB&")"
			set rs=Server.CreateObject("ADODB.Recordset")     
			set rs=connTemp.execute(query)
		...
		<tr> 
					...
					<tr> 
						<td colspan="2">
                            ...
                            <div class="pcCPmessage">
                                    ...
                                    <br /><br />                            
                            		To use <strong>PayPal Payments Standard</strong> you must modify settings inside your PayPal account. <a href="http://wiki.productcart.com/productcart/paypal_standard" target="_blank">See the documentation</a> for instructions.
                            </div>         
						  	<% End If %>
                            ...
                            <div class="pcCPmessage">
                                    ...
                                    <br /><br />
                          			Before activating <strong>PayPal Website Payments Pro</strong> please <a href="http://wiki.productcart.com/productcart/paypal_pro" target="_blank">review the documentation</a>.
                          	</div> 
							<% End If %>   
                            
                               <% If (Request("ppv") = "1") Then %>
                            <div class="pcCPmessage">
                          			Before activating <strong>PayPal Express Checkout</strong> please <a href="http://wiki.productcart.com/productcart/paypal_pro" target="_blank">review the documentation</a>.
                          	</div> 
							<% End If %> 
          ...
          <script type=text/javascript>
					function whatPayPalSelected(){
						...
						document.getElementById('PayPalDP_table').style.display='';
						//document.getElementById('chpaytype_table').style.display='';
						}else{
						document.getElementById('PayPalDP_table').style.display='none';
						}
					...
					<tr> 
						<td valign="top">
                        	<div align="right"> 
							<input type="radio" class="clearBorder" name="PayPalSelection" value="PayPalDP" onClick="whatPayPalSelected();" checked>
							</div>
                        </td>
					...
					%>
                    <tr><td style="height: 1px; padding: 0px;"><div style="display: none;"><input type="radio" name="PayPalSelection" value="PayPalDP"></div></td></tr>
                    <%
					end if
					'// PAYPAL WPP - END
					...
					<tr> 
						<td colspan="2" valign="top"> 							
							...
							<table ID="PayPalDP_table" width="100%" border="0" cellspacing="0" cellpadding="4" style="display:none">
								<tr> 
