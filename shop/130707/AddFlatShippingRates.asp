...
If sMode="Save" Then
	...
	Session("adminFlatShipTypeDesc")=replace(Session("adminFlatShipTypeDesc"), "'", "''")
	Session("adminFlatShipTypeDesc")=replace(Session("adminFlatShipTypeDesc"), """", "&quot;")
...
response.redirect "AddFlatShippingRates.asp?Type="&trim(Request("FlatShipTypePref"))&"&msg="&Server.Urlencode(msg)
	...
	Session("adminFlatShipTypeDelivery")=replace(Session("adminFlatShipTypeDelivery"), "'", "''")
	Session("adminFlatShipTypeDelivery")=replace(Session("adminFlatShipTypeDelivery"), """", "&quot;")
...
response.redirect "AddFlatShippingRates.asp?Type="&trim(Request("FlatShipTypePref"))&"&msg="&Server.Urlencode(msg)
	...
	query="INSERT INTO FlatShipTypes (FlatShipTypeDesc, WQP, FlatShipTypeDelivery,startIncrement) VALUES (N'"& Session("adminFlatShipTypeDesc") &"', '"& Session("adminFlatShipTypePref") &"','"& Session("adminFlatShipTypeDelivery") &"',"&Session("adminstartIncrement")&");"
	...
	if strCreateErrMsgNum=0 then
		query="SELECT idFlatShipType FROM FlatShipTypes WHERE FlatShipTypeDesc='"& Session("adminFlatShipTypeDesc") &"'"
		set rstemp=connTemp.execute(query)
		do until rstemp.eof
		...
		for i=1 to iCnt
			...
			If ShippingPrice <> "" AND quantityfrom <> "" AND quantityto <> "" Then
				...
				query="INSERT INTO FlatShipTypeRules (idFlatShipType, quantityFrom, quantityTo, ShippingPrice, Num) VALUES ("& idFlatShipType &", "& quantityfrom &", "& quantityto &", "& ShippingPrice &", "& i &")"
				set rstemp=connTemp.execute(query)
				Set rstemp=Nothing
			End If
		next
		...
		query="INSERT INTO shipService  (serviceCode, serviceActive, servicePriority, serviceDescription, serviceFree, serviceFreeOverAmt, serviceHandlingFee,serviceShowHandlingFee, serviceLimitation, idShipment) VALUES ('C"&idFlatShipType&"', -1, 0, N'"&Session("adminFlatShipTypeDesc")&"', "&serviceFree&", "&freeAmt&", "&thandling&","&shfee&","&serviceLimitation&",8);"
		set rstemp=connTemp.execute(query)
		connTemp.Close
		Set rstemp=Nothing
...
<table class="pcCPcontent">
	...
	<tr> 
        ...
        <td colspan="2"><input type="text" name="FlatShipTypeDesc" value="<%=Session("adminFlatShipTypeDesc")%>"> <span class="pcSmallText">Note: Do not enter commas.</span>
		</td>
	</tr>                
    <tr> 
...
%>
    ...
    <tr> 
        ...
        <input type="button" class="btn btn-default"  name="Button" value="Back" onClick="javascript:document.location.href='viewShippingOptions.asp'">
        </td>
    </tr>
</table>
</form>
