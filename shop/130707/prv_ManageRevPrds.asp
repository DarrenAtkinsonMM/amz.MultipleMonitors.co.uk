...
<form method="POST" action="prv_ManageRevPrds.asp?action=update" name="checkboxform" class="pcForms">
	<table class="pcCPcontent">
		...
		<td colspan="5">
		<%if pcv_RevName="1" then%>
		<input type="button" class="btn btn-default"  value="View Live Reviews" onClick="location.href='prv_ManageRevPrds.asp?nav=2'" class="btn btn-primary">
		&nbsp;
		<% else %>
		<input type="button" class="btn btn-default"  value="View Pending Reviews" onClick="location.href='prv_ManageRevPrds.asp?nav=1'" class="btn btn-primary">
		&nbsp;
		<% end if %>
