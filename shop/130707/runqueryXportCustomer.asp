...
Function GenHTML()%>
	...
	<% else
			...
			For i=0 to intCount
			pidcustomer=pcArr(0,i)
			pcname=pcArr(1,i)
			plastName=pcArr(2,i)
			pcustomerCompany=pcArr(3,i)
			pphone=pcArr(4,i)
			pemail=pcArr(5,i)
			paddress=pcArr(6,i)
			paddress2=pcArr(7,i)
			pcity=pcArr(8,i)
			pstateCode=pcArr(9,i)
			pstate=pcArr(10,i)
			if pstateCode="" then
				pstateCode=pstate
			end if
			pzip=pcArr(11,i)
			pCountryCode=pcArr(12,i)
			pcustomerType=pcArr(13,i)
			piRewardPointsAccrued=pcArr(14,i)
			piRewardPointsUsed=pcArr(15,i)
			pRecvNews=pcArr(16,i)
			pIDRefer=pcArr(17,i)
			if pIDRefer<>"" then
			else
				pIDRefer=0
			...
			if FieldList(j,0)<>"" Then
				query="SELECT pcCFV_Value FROM pcCustomerFieldsValues WHERE idcustomer=" & pidcustomer & " AND pcCField_ID=" & FieldList(j,0) & ";"
				set rs=connTemp.execute(query)
				fieldValue=""
				if not rs.eof then
