...
FOR dd=1 TO Count
	...
	IF (geID<>"") AND (clng(geadd)>0) then
		...
		query=query&",products.description,products.sku, products.weight,products.pcprod_QtyToPound,products.emailText, products.deliveringTime, products.idSupplier, products.cost, products.stock, products.notax, products.noshipping, products.iRewardPoints FROM products,pcEvProducts WHERE pcEvProducts.pcEP_ID=" & geID & " AND pcEvProducts.pcEP_IDEvent=" & gIDEvent & " AND products.idproduct=pcEvProducts.pcEP_idproduct AND ((products.active<>0) OR ((products.active=0) AND (products.pcProd_ParentPrd>0))) AND products.removed=0"
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
		...
		'-----------------------------
		'ReCalculate Configurable Items Discounts
		...
		'End ReCulculate Configurable Items Discounts		
		'------------------------------------
		...
		if not rs1.eof then
			...
			query="insert into ProductsOrdered (IDOrder,IDProduct,Quantity,pcPrdOrd_SelectedOptions,pcPrdOrd_OptionsPriceArray,pcPrdOrd_OptionsArray,unitPrice,xfdetails,IDConfigSession,QDiscounts,ItemsDiscounts,pcPO_EPID) values (" & pIDOrder & "," & pIDProduct & "," & pQuantity & ",'" & pcv_strSelectedOptions & "','" & pcv_strOptionsPriceArray & "','" & pcv_strOptionsArray & "'," & punitPrice & ",N'" & pxfdetails & "'," & pIDConfigSession & "," & QDiscounts & "," & ItemsDiscounts & "," & geID & ")"
			set rs1=connTemp.execute(query)
			set rs1=nothing
		end if
