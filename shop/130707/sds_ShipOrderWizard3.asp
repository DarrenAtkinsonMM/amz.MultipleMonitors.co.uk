...
response.redirect "menu.asp"
	...
	if intUSPSLabelFlag="1" then
		query="UPDATE pcPackageInfo SET pcPackageInfo_ShipMethod=N'" & pcv_method & "', pcPackageInfo_ShippedDate='" & dtShippedDate & "', pcPackageInfo_TrackingNumber='" & pcv_tracking & "', pcPackageInfo_Comments=N'" & pcv_AdmComments & "' WHERE idOrder="&pcv_IdOrder&" AND pcPackageInfo_ID="&pcv_PackageID&";"
	else
		if pcv_shippedDate<>"" then
	...
	query="INSERT INTO pcAdminComments (idorder,pcACom_ComType,pcACom_Comments,pcDropShipper_ID,pcACom_IsSupplier,pcPackageInfo_ID) VALUES (" & qry_ID & ",2,N'" & pcv_AdmComments & "',0,0," & pcv_PackageID & ");"
	set rstemp=connTemp.execute(query)
	...
	'DA EDIT TO Block Emails Being Sent, to resume simply include the file below.
