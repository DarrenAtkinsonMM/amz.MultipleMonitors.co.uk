...
If request.Form("submitRemoveCustom")<>"" then
	If request.Form("removecustom")="1" then
		...
		query="DELETE FROM pcPrdXFields WHERE IdXField="&idxfield&";"
...
If removaltype="2" then
	...
	query="DELETE FROM pcPrdXFields WHERE IdProduct=" & idproduct & " AND IdXField=" & idxfield & ";"
	set rs=conntemp.execute(query)
	set rs=nothing
				...
				<tr>
					<td>
						...
						<form name="removeALL" method="post" action="removecustomfields.asp" class="pcForms">
							...
							<input type="submit" name="submitRemoveCustom" value="Remove &quot;<%=fieldname%>&quot; From All Products" class="btn btn-primary">
							</div>
						</form>
						</div>
					</td>
