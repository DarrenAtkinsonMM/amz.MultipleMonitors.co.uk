<%
		...
		if layTopId<>-1 then
			layTop=trim(rsExcel.Fields.Item(int(layTopId)).Value)
			if layTop<>"" then
				layTop=replace(layTop,"'","''")
			end if
		end if
		if layTopLId<>-1 then
			layTopL=trim(rsExcel.Fields.Item(int(layTopLId)).Value)
			if layTopL<>"" then
				layTopL=replace(layTopL,"'","''")
			end if
		end if
		if layTopRId<>-1 then
			layTopR=trim(rsExcel.Fields.Item(int(layTopRId)).Value)
			if layTopR<>"" then
				layTopR=replace(layTopR,"'","''")
			end if
		end if
		if layMiddleId<>-1 then
			layMiddle=trim(rsExcel.Fields.Item(int(layMiddleId)).Value)
			if layMiddle<>"" then
				layMiddle=replace(layMiddle,"'","''")
			end if
		end if
		if layTabsId<>-1 then
			layTabs=trim(rsExcel.Fields.Item(int(layTabsId)).Value)
			if layTabs<>"" then
				layTabs=replace(layTop,"'","''")
			end if
		end if
		if layBottomId<>-1 then
			layBottom=trim(rsExcel.Fields.Item(int(layBottomId)).Value)
			if layBottom<>"" then
				layBottom=replace(layBottom,"'","''")
			end if
		end if
		'APP-S
		app_Apparel=0
		...
		if ApparelID<>-1 then
			app_Apparel=trim(rsExcel.Fields.Item(int(ApparelID)).Value)
			if app_Apparel<>"" then
			else
				app_Apparel=0
			end if
		...
		if ShowoutID<>-1 then
			app_Showout=trim(rsExcel.Fields.Item(int(ShowoutID)).Value)
			if app_Showout<>"" then
			else
				app_Showout=0
			end if
		...
		if OutMsgID<>-1 then
			app_OutMsg=trim(rsExcel.Fields.Item(int(OutMsgID)).Value)
		end if
		
		if app_OutMsg<>"" then
			app_OutMsg=replace(app_OutMsg,"'","''")
		else
			if (app_Showout="1") then
				app_OutMsg="Out of stock"
			end if
		end if
		
		if DTypeID<>-1 then
			app_DType=trim(rsExcel.Fields.Item(int(DTypeID)).Value)
			if app_DType<>"" then
			else
				app_DType=0
			end if
		...
		if ChartTextID<>-1 then
			app_ChartText=trim(rsExcel.Fields.Item(int(ChartTextID)).Value)
			if app_ChartText<>"" then
				app_ChartText=replace(app_ChartText,"'","''")
			end if
		end if
		
		if ChartDescID<>-1 then
			app_ChartDesc=trim(rsExcel.Fields.Item(int(ChartDescID)).Value)
			if app_ChartDesc<>"" then
				app_ChartDesc=replace(app_ChartDesc,"'","''")
			end if
		end if
		
		if ChartImgID<>-1 then
			app_ChartImg=trim(rsExcel.Fields.Item(int(ChartImgID)).Value)
		end if
		
		if ChartIURLID<>-1 then
			app_ChartIURL=trim(rsExcel.Fields.Item(int(ChartIURLID)).Value)
		end if
		'APP-E
		...
		if hideaddimagesid<>-1 then
			hideaddimages=trim(rsExcel.Fields.Item(int(hideaddimagesid)).Value)
			if (not (hideaddimages<>"")) or (isNumeric(hideaddimages)=false) then
				hideaddimages="0"
			end if
		end if
		...
		if alttagtextid>-1 then
			palttagtext=trim(rsExcel.Fields.Item(int(alttagtextid)).Value)
		end if
		if activeid>-1 then
		...
		if pricecatsid(0)>-1 then
			pricecats(0)=trim(rsExcel.Fields.Item(int(pricecatsid(0))).Value)
		end if
		if pricecatsid(1)>-1 then
			pricecats(1)=trim(rsExcel.Fields.Item(int(pricecatsid(1))).Value)
		end if
		if pricecatsid(2)>-1 then
			pricecats(2)=trim(rsExcel.Fields.Item(int(pricecatsid(2))).Value)
		end if
		if pricecatsid(3)>-1 then
			pricecats(3)=trim(rsExcel.Fields.Item(int(pricecatsid(3))).Value)
		end if
		if pricecatsid(4)>-1 then
			pricecats(4)=trim(rsExcel.Fields.Item(int(pricecatsid(4))).Value)
		end if
