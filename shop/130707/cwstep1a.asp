...
if ((right(ucase(filepathname),4) = ".CSV") or (right(ucase(filepathname),4)=".XLS")) AND IsUploadAllowed(filepathname) then
	...
	if not filename = "" then
		...
		call closeDb()
		response.redirect "custindex_import.asp?s=1&nextstep=1&msg=" & Server.URLEncode("Customer data file: " & ucase(lgCSV) & " was uploaded successfully.")
	end if
else
...
Else
	call closeDb()
	response.redirect "custindex_import1.asp?s=0&msg=" & Server.URLEncode("You did not select a file to upload.")
end if
%>