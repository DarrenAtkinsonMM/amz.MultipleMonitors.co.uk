				...
				if rstemp.eof then	
					...
					if updatedup="1" then
						If statusAPP="1" OR scAPP=1 Then
							'// Do not update Option Group order because it can create issues with existing Apparel sub-products
							strSQL="UPDATE pcProductsOptions SET pcProdOpt_Required="&intdupRequired&" WHERE idproduct="& idProduct &" AND idoptionGroup="&intidOptionGroup&";"
						Else
