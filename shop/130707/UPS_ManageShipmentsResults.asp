...
%>
	<table class="pcCPcontent">
		...
		<tr>
			<th colspan="3">UPS&reg; Developer Kit - Manage Shipments for Order Number <%=(scpre+int(pcv_intOrderID))%><a name="top"></a></th>
		</tr>
		...
		<td colspan="11"> 
			<% if pResults<>"0" Then %>
				<table width="100%" border="0" cellspacing="0" cellpadding="4">
					<tr> 
						<td> 
							<form method="post" action="UPS_ManageShipmentsResults.asp" name="" class="pcForms">
							<b> 
		...
		<tr>
			...
			<% else 
				if request("del")="YES" then
					...
					<input type="button" class="btn btn-default"  name="Button" value="There are no Packages to Display >>> Go To Order Details" onClick="document.location.href='<%=pcv_strPreviousPage%>'">
				<% else %>
						<input type="button" class="btn btn-default"  name="Button" value="There are no Packages to Display >>> Go Back" onClick="javascript:history.back()">
				<% end if
			end if %>		</td>
	</tr>
		<tr>
