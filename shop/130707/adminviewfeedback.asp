...
if (request("action")="add") and (request("rewrite")="0") then
	...
	query="Insert Into pcComments (pcComm_IDOrder,pcComm_IDParent,pcComm_IDUser,pcComm_CreatedDate,pcComm_EditedDate,pcComm_FType,pcComm_FStatus,pcComm_Priority,pcComm_Description,pcComm_Details) values (" & intIdOrder & "," & intIdFeedback & ",0,'" & dtComDate & "','" & dtComDate & "',0,0,0,'',N'" & strNewComments & "')"
	set rs=connTemp.execute(query)
...
<table width="90%" border="0" align="center" cellpadding="6" cellspacing="0">
  ...
  <tr>
    ...
    <td class="pcCPHtmlContent"><%=FDetails%></td>
  </tr>
  <tr>
...
<table class="pcCPcontent" style="border: 1px solid #999; width: 600px;">
	...
	<tr><td valign="top">
    <p align="right">
      <%response.write dictLanguage.Item(Session("language")&"_viewFeedback_9")%>
		</p>
  	</td>
		<td valign="top">				
  	<input type=hidden name=IDOrder value="<%=intIdOrder%>">
  	<input type=hidden name=IDFeedback value="<%=intIdFeedback%>">
    <textarea class="htmleditor" name="comments" id="comments" cols="50" rows="8"><%=request("comments")%></textarea>
		</td></tr>
	<tr><td colspan="2">&nbsp;</td></tr>
	<tr><td colspan="2" align="center">
		<input type="submit" name="Submit" value=" Add Comment " class="btn btn-primary" onClick="document.hForm.rewrite.value='0';">
