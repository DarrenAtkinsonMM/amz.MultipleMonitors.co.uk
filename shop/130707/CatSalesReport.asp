...
if not validNum(pIDCategory) or pIDCategory="0" then
	call closeDb()
	Session("message") = "You did not select a valid category."
...
query="SELECT DISTINCT orders.idorder,orders.idcustomer,orders.total,orders.shipmentDetails,orders.paymentDetails,orders.taxAmount,orders.discountDetails,orders.orderDate,orders.processDate,orders.shipDate FROM ProductsOrdered,Orders,Products,Categories_Products WHERE Categories_Products.IDCategory=" & pIDCategory & " AND ProductsOrdered.idOrder=Orders.idOrder AND Products.idProduct=ProductsOrdered.idProduct AND ((Categories_Products.idProduct=ProductsOrdered.idProduct) OR (Categories_Products.idProduct=Products.pcProd_ParentPrd)) AND ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) " & TempSQL1 & TempSQL2 & TempSpecial & " ORDER BY " & tmpD & " DESC;"
'APP-E
...
do until rs.EOF
	...
	query="SELECT Products.idproduct,ProductsOrdered.quantity,(ProductsOrdered.unitPrice*ProductsOrdered.quantity) AS SubPTotal,Products.Description FROM ProductsOrdered,Products,Categories_Products WHERE Categories_Products.idcategory=" & pIDCategory & " AND ProductsOrdered.idProduct=Products.idProduct AND ((Categories_Products.idProduct=ProductsOrdered.idProduct) OR (Categories_Products.idProduct=Products.pcProd_ParentPrd)) AND ProductsOrdered.idOrder=" &  pc_idorder & ";"
