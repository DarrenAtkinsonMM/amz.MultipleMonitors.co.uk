...
if request("action")="rmv" then
	...
	if statusAPP="1" then
		...
		session("sm_PrdCountSub")=tmpPrdCountSub
	else
		tmpPrdCountSub=0
		session("sm_PrdCountSub")=tmpPrdCountSub
	end if
	...
	If statusAPP="1" OR scAPP=1 Then
		src_FormTips2="Note: The Apparel sub-product(s) will not displayed in this list (if any). They are included in the sale with their parent product(s)."
	Else
		src_FormTips2=""
	End If
	src_IncNormal=0
