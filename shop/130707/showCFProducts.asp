...
if pcv_strAction = "del" then
	if pcv_strCustomType = "S" then
		...
		set rs=connTemp.execute(query)
		msg="Custom input field deleted successfully!"
		set rs = nothing
	end if %>
...
<tr>
<td align="right"><input type="button" class="btn btn-default"  name="back" value="Close window" onClick="opener.location.href='ManageCFields.asp#1'; window.close();">
</td>
</tr>
</table>
...
IF pcv_strAction <> "del" THEN ' If not deleting the field, load the main page
	...
	if Left(request("idcustom"),1)="C" then
		query=" AND (products.IdProduct IN (SELECT DISTINCT pcPrdXFields.IdProduct FROM pcPrdXFields WHERE IdXField=" & idcustom & "))"
    ...
    <% If rsInv.eof Then %>
        ...
        <table class="pcCPcontent" style="width: 100%;">
            ...
            <tr>
                <td align="right"><input type="button" class="btn btn-default"  name="back" value="Close window" onClick="window.close();"></td>
            </tr>
        </table>
	...
	<% Else %>
        ...
        <table class="pcCPcontent" style="width: 100%;">                                   
            ...
            <tr>
                <td colspan="4" align="right"><input type="button" class="btn btn-default"  name="back" value="Close Window" onClick="window.close();"></td>
            </tr>
        </table>
