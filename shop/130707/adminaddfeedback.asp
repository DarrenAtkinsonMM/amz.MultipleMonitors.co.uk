...
if (request("action")="add") and (request("rewrite")="0") then
	...
	query="Insert Into pcComments (pcComm_IDOrder,pcComm_IDParent,pcComm_IDUser,pcComm_CreatedDate,pcComm_EditedDate,pcComm_FType,pcComm_FStatus,pcComm_Priority,pcComm_Description,pcComm_Details) values (" & intIdOrder & ",0,0,'" & dtComDate & "','" & dtComDate & "'," & FType & "," & intFStatus & "," & intPriority & ",N'" & strFDesc & "',N'" & strFDetails & "')"
	set rs=Server.CreateObject("ADODB.Recordset")
	set rs=connTemp.execute(query)
	...
	if rs.eof then
		...
		call pcs_hookHelpDeskEmailSent(pcstrCustomerEmail)
...
function newWindow(file,window) {
		...
		if (msgWindow.opener == null) msgWindow.opener = self;
			...
			<table class="pcCPcontent" style="width:auto;">
				...
				<tr>
					<td align="right" valign="top"><%response.write dictLanguage.Item(Session("language")&"_addFB_r")%></td>
					<td>
            <textarea class="htmleditor" name="Details" id="Details" cols="60" rows="7" id="bugLongDsc"><%=request("Details")%></textarea>
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					%>
						<input type="submit" name="Submit" value="Add Feedback" class="btn btn-primary" onClick="document.hForm.rewrite.value='0';">
						&nbsp;<input type="button" class="btn btn-default"  name="back" value=" View all Postings" onClick="location='adminviewallmsgs.asp';">
					 	<%if session("admin_IDOrder")>0 then%>
					 		<input type="button" class="btn btn-default"  name="go" value="View Postings" onClick="location='adminviewallmsgs.asp?IDOrder=<%=session("admin_IDOrder")%>';">
						<%end if%>
