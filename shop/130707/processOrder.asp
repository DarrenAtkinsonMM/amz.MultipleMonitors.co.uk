...
'****************************************************
' START - Payflow API processing codes
'****************************************************

IF request("SubmitPFApi")<>"" THEN
	pcv_strAdminPrefix="1"
	
	ActionType=Ucase(trim(request("SubmitPFApi")))
    GWCode=trim(request("SubmitPFApiGWCode"))
	
	Function PayPal_UpdateStatus(paymentCode, authcode, captured, transType)
        If GWCode = 2 Then
            query="UPDATE pfporders SET captured = " & captured & ", trxtype = '" & transType & "', origid = '" & authcode & "' WHERE idOrder = " & qry_ID & ";"
        Else
		    '// Update pcPay_PFL_Authorize to captured
		    query="UPDATE pcPay_PFL_Authorize SET captured = " & captured & ", transtype = '" & transType & "', authcode = '" & authcode & "' WHERE idOrder = " & qry_ID
        End If
    ...
    If GWCode = 2 Then
        query="SELECT v_Partner, v_Vendor, v_User, v_Password,pfl_testmode,pfl_transtype FROM verisign_pfp WHERE id=1;"
	    set rs=Server.CreateObject("ADODB.RecordSet")
	    set rs=connTemp.execute(query)
	    if err.number<>0 then
    ...
    Else
	    query="SELECT pcPay_PayPal_Partner, pcPay_PayPal_Vendor, pcPay_PayPal_Username, pcPay_PayPal_Password, pcPay_PayPal_TransType, pcPay_PayPal_Sandbox FROM pcPay_PayPal WHERE pcPay_PayPal_ID=1;"
	    set rs=server.CreateObject("ADODB.RecordSet")
	    set rs=connTemp.execute(query)
	...
	'get original auth code and payment code
	query = "SELECT gwAuthCode,paymentCode,total FROM orders WHERE idOrder = " & qry_ID & ";"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if not rs.eof then
		origAuthCode = rs("gwAuthCode")
		paymentCode = rs("paymentCode")
		origTotal = rs("total")
	end if
	set rs = nothing
	
	'get PayFlow transaction data
    If GWCode = 2 Then
        query="SELECT amt, tender, origid FROM pfporders WHERE idOrder = " & qry_ID & ";"
        set rs=server.CreateObject("ADODB.RecordSet")
	    set rs=conntemp.execute(query)
	    if not rs.eof then
		    pAmount=rs("amt")
		...
		'Check Final Amount of this Order
		if Cdbl(origTotal)<>Cdbl(pAmount) then
			query="UPDATE pfporders SET amt=" & origTotal & " WHERE idOrder = " & qry_ID & ";"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=conntemp.execute(query)
			set rs=nothing
			pAmount=origTotal
    ...
    Else
	    query="SELECT idPFL_Authorize, amount, authcode, paymentMethod FROM pcPay_PFL_Authorize WHERE idOrder = " & qry_ID & ";"
	    set rs=server.CreateObject("ADODB.RecordSet")
	    set rs=conntemp.execute(query)
		
	    if not rs.eof then
		    pAmount = rs("amount")
		    pAuthCode = rs("authcode")
		    pPaymentMethod = rs("paymentmethod")
		...
		'Check Final Amount of this Order
		if Cdbl(origTotal)<>Cdbl(pAmount) then
			query="UPDATE pcPay_PFL_Authorize SET amount=" & origTotal & " WHERE idOrder = " & qry_ID & ";"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=conntemp.execute(query)
			set rs=nothing
			pAmount=origTotal
	...
	If pcPay_PayPal_Sandbox = "1" Then
		pcv_PayPalPostURL = "https://pilot-payflowpro.paypal.com"
	Else
		pcv_PayPalPostURL = "https://payflowpro.paypal.com"
	End If
	...
	If ActionType = "CAPTURE" Then
		...
		if cdbl(pAmount)>0 then
			if not IsEmpty(pAuthCode) then
				...
				nvpstr = nvpstr &"&ORIGID="& pAuthCode
				nvpstr = nvpstr &"&AMT="&money(pAmount)
				...
				set xml = Server.CreateObject("Msxml2.serverXmlHttp"&scXML)
				'SB S
				xml.open "POST", pcv_PayPalPostURL, false
				...
				If ack="APPROVED" Then
					...
					pcv_PayPalErrMessage = "<div align=""left"">" & _
								"<ul>" &_
								"<li>" & "<u>Error " & tmpRESULT & "</u></li>" &_
								"<li>" & "Error Description: " & tmpRESPMSG & "</li>"
					pcv_PayPalErrMessage = pcv_PayPalErrMessage & "</ul></div>"
					'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
					' End: Error Reporting
					'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ 
				end if			
				...
				if response_code = 1 then
					...
					'// Update Payment Status - PAID
					query="UPDATE Orders SET pcOrd_PaymentStatus=2, gwTransId='"& pAuthCode &"' WHERE idorder=" & qry_ID & ";"
					set rstemp=server.CreateObject("ADODB.RecordSet")
					set rstemp=connTemp.execute(query)
					set rstemp=nothing
					...
					PayPal_UpdateStatus paymentCode, pAuthCode, 1, transType
					...
					pcv_PayPalErrMessage = "<div>The transaction was declined.</div>"
				end if
			end if
		else
	...
	ElseIf ActionType = "REFUND" Then
		if not IsEmpty(pAuthCode) then
			...
			'--------------------------------------------------------------------------- 
			' Make the call to PayPal to REFUND the transaction.
			'---------------------------------------------------------------------------
			'Send the transaction info as part of the querystring
			set xml = Server.CreateObject("Msxml2.serverXmlHttp"&scXML)
			'SB S
			...
			if response_code = 1 then			
				...
				query="UPDATE Orders SET pcOrd_PaymentStatus=6, gwTransId='"& pAuthCode &"' WHERE idorder=" & qry_ID & ";"
				set rstemp=server.CreateObject("ADODB.RecordSet")
				set rstemp=connTemp.execute(query)
				set rstemp=nothing
				...
				PayPal_UpdateStatus paymentCode, pAuthCode, 0, transType
	...
	ElseIf ActionType = "VOID" Then
		...
		if not IsEmpty(pAuthCode) then
			...
			'--------------------------------------------------------------------------- 
			' Make the call to PayPal to VOID the transaction.
			'---------------------------------------------------------------------------
			'Send the transaction info as part of the querystring
			set xml = Server.CreateObject("Msxml2.serverXmlHttp"&scXML)
			'SB S
			...
			if response_code = 1 then			
				'// Update Payment Status - VOID
				query="UPDATE Orders SET pcOrd_PaymentStatus=8, gwTransId='"& pAuthCode &"' WHERE idorder=" & qry_ID & ";"
				set rstemp=server.CreateObject("ADODB.RecordSet")
				set rstemp=connTemp.execute(query)
				set rstemp=nothing
...
if pcv_PayPalErrMessage <> "" and len(pcv_PayPalErrMessage) > 5 then
	call closeDb()
response.redirect "Orddetails.asp?id="&qry_ID&"&ActiveTab=5&msg1=" & Server.URLEncode(pcv_PayPalErrMessage)
end if
'****************************************************
' END - Payflow API processing codes
'****************************************************


if request("Submit1")<>"" then
	'update customer information and return to order information
	fname=replace(request("name"),"'","''")
	lastname=replace(request("lastName"),"'","''")
	company=replace(request("customerCompany"),"'","''")
	address=replace(request("address"),"'","''")
	address2=replace(request("address2"),"'","''")
	city=replace(request("city"),"'","''")
	stateCode=request("stateCode")
	pstate=request("state")
	zip=request("zip")
	country=request("CountryCode")
	phone=request("phone")
	pcs_ValidateEmailField "email", false, 0
	email=Session("pcAdminemail")
	...
	query="UPDATE customers SET [name]=N'"&fname&"',lastName=N'"&lastname&"',email='"&email&"',customerCompany=N'"&company&"', address=N'"&address&"', address2=N'"&address2&"', city=N'"&city&"', zip='"&zip&"', stateCode='"&stateCode&"', state=N'"&pstate&"', CountryCode='"&country&"', phone='"&phone&"' WHERE idCustomer="& idcustomer
	Set rs=Server.CreateObject("ADODB.Recordset")
	Set rs=conntemp.execute(query)

	'insert updated information into Order Table
	query="UPDATE orders SET address=N'"&address&"', address2=N'"&address2&"', city=N'"&city&"', zip='"&zip&"', stateCode='"&stateCode&"', state=N'"&pstate&"', CountryCode='"&country&"' WHERE idOrder="& qry_ID
	Set rs=Server.CreateObject("ADODB.Recordset")
	Set rs=conntemp.execute(query)
...
if request("Submit2")<>"" then
	...
	if request("custcardtype")="1" then
		
		ccCnt=request("ccCnt")
		for i=1 to ccCnt
			pIdCCOrder=request("CCID"&i)
			pFormValue=request("CCO"&pIdCCOrder)
	...
	if ccp="Y" then
		...
		CCType=request("CCT")
		cardNumber=request("CCNum")

		pcv_SecurityPass = pcs_GetSecureKey
		pcv_SecurityKeyID = pcs_GetKeyID

		cardNumber=enDeCrypt(cardNumber, pcv_SecurityPass)
		cardComments=replace(request("CCcomments"),"'","''")
		if SQL_Format="1" then
			expiration="1/" & request("CCexpM") & "/" & request("CCexpY")
		else
			expiration=request("CCexpM") & "/1/" & request("CCexpY")
		end if
		' validates expiration
		if DateDiff("d", Month(Now)&"/"&Year(now), request("CCexpM")&"/"&request("CCexpY"))<=-1 then
			 conntemp.Close
			 call closeDb()
...
if request("Submit3")<>"" then
	...
	shippingFullName=replace(request("shippingFullName"),"'","''")
	shippingCompany=replace(request("shippingCompany"),"'","''")
	shippingAddress=replace(request("shippingAddress"),"'","''")
	shippingAddress2=replace(request("shippingAddress2"),"'","''")
	shippingcity=replace(request("shippingcity"),"'","''")
	shippingStateCode=request("shippingStatecode")
	shippingState=request("ShippingState")
	shippingZip=request("shippingZip")
	shippingPhone=request("shippingPhone")
	pcs_ValidateEmailField "shippingEmail", false, 0
	shippingEmail=Session("pcAdminshippingEmail")
	pOrdPackageNum=request("ordPackageNum")
	shippingcountry=request("shippingCountryCode")
	idcustomer=request("idcustomer")
	...
	query="UPDATE orders SET shippingCompany=N'"&shippingCompany&"', shippingAddress=N'"&shippingAddress&"', shippingAddress2=N'"&shippingAddress2&"', shippingcity=N'"&shippingcity&"', shippingZip='"&shippingZip&"', pcOrd_shippingPhone='"&shippingPhone&"', pcOrd_shippingEmail='"&shippingEmail&"', shippingState=N'"&shippingstate&"', shippingStateCode='"&shippingStateCode&"', shippingCountryCode='"&shippingcountry&"',shippingFullName=N'"&shippingFullName&"',ordPackageNum="&pOrdPackageNum&",pcOrd_ShowShipAddr=1 WHERE idOrder="& qry_ID
	Set rs=Server.CreateObject("ADODB.Recordset")
	Set rs=conntemp.execute(query)
	'redirect back
...
IF (PayPalProcessOrder=1) OR (request("Submit4")<>"") OR (request("Submit4A")<>"") OR (request("Submit4B")<>"") THEN
	pIdOrder=qry_ID
...
if (request("Submit5")<>"") OR (request("Submit5A")<>"") then

	

	'Start SDBA
	if request("Submit5A")<>"" then
		pcv_SubmitType=1
	...
	set rstemp=connTemp.execute(query)
	query="INSERT INTO pcAdminComments (idorder,pcACom_ComType,pcACom_Comments) VALUES (" & qry_ID & ",3,N'" & pcv_AdmComments & "');"
	set rstemp=connTemp.execute(query)
	'End SDBA
	...
	IF pcv_SubmitType=0 THEN
		...
		shipVia=replace(request("shipVia"),"'","''")
		TrackingNum=request("TrackingNum")
		if shipDate<>"" then
	...
	IF pcv_SubmitType=0 THEN
		...
		if request("sendEmailShip")="YES" then
...
if request("SubmitA3")<>"" then
	...
	set rstemp=connTemp.execute(query)
	query="INSERT INTO pcAdminComments (idorder,pcACom_ComType,pcACom_Comments,pcDropShipper_ID,pcACom_IsSupplier) VALUES (" & qry_ID & ",4,N'" & pcv_AdmComments & "'," & pcv_DropShipperID & "," & pcv_IsSupplier & ");"
	set rstemp=connTemp.execute(query)
	if pcv_AdmComments<>"" then
		pcv_AdmComments=replace(pcv_AdmComments,"''","'")
	end if
...
if request("SubmitA4")<>"" then
	...
	set rstemp=connTemp.execute(query)
	query="INSERT INTO pcAdminComments (idorder,pcACom_ComType,pcACom_Comments,pcDropShipper_ID,pcACom_IsSupplier,pcPackageInfo_ID) VALUES (" & qry_ID & ",2,N'" & pcv_AdmComments & "'," & pcv_DropShipperID & "," & pcv_IsSupplier & "," & pcv_PackageID & ");"
	set rstemp=connTemp.execute(query)
	%>
	<!--#include file="../pc/inc_PartShipEmail.asp"-->
	<%
...
if request("Submit9")<>"" then
	...
	if request("sendEmailShip2")="YES" then
		call sendmail (scCompanyName, scEmail, pEmail, pcv_strSubject, replace(customerShippedEmail, "&quot;", chr(34)))
		'//Send email to shipping email if it is different and exist
		if trim(pShippingEmail)<>"" AND trim(pShippingEmail)<>trim(pEmail) then
			call sendmail (scCompanyName, scEmail, pShippingEmail, pcv_strSubject, replace(customerShippedEmail, "&quot;", chr(34)))
...
if PayPalCancelOrder=1 OR request("Submit6")<>"" OR request("Submit10")<>"" OR request("Submit10A")<>"" then
	
	'Start SDBA
	if request("Submit10A")<>"" then
		pcv_SubmitType=1
	...
	set rstemp=connTemp.execute(query)
	query="INSERT INTO pcAdminComments (idorder,pcACom_ComType,pcACom_Comments) VALUES (" & qry_ID & ",5,N'" & pcv_AdmComments & "');"
	set rstemp=connTemp.execute(query)
	'End SDBA
	...
	'Void Avalara Tax
	query = "SELECT pcOrd_Avalara FROM orders WHERE idOrder = " & qry_ID
	Set rs = Server.CreateObject("ADODB.Recordset")
	Set rs = conntemp.execute(query)
	if rs("pcOrd_Avalara") = 1 then
		'Get the latest Avalara transaction code
		query = "SELECT TOP 1 idOrder FROM Avalara_orders WHERE idOrder LIKE '" & qry_ID & "%' ORDER BY idOrder DESC"
		Set rs = conntemp.execute(query)
		aIdOrder = rs("idOrder")
		call pcs_VoidAvalara(aIdOrder)
	end if
	...
	select case ucase(pPaymentCode)
		case "PFPRO", "PFLINK"
...
response.redirect "techErr.asp?error=110"&Server.Urlencode("Error in processOrder. Error: "&err.description)
				...
				'Update Configurable Items & Additional Charges stock and sales
			...
			END IF
			'End Update Configurable Items & Additional Charges
...
response.redirect "techErr.asp?error=110"&Server.Urlencode("Error in processOrder. Error: "&err.description)
	...
	If scCancelledEmail<>"" Then
		...
		'PAYPALEXP EMAIL
		pEmail=rsCust("email")
		if request("Submit6")<>"" AND request("sendEmailCanc")="YES" then
			call sendmail (scCompanyName, scEmail, pEmail, pcv_strSubject, replace(customerCancelledEmail, "&quot;", chr(34)))
		end if
		if (request("Submit10")<>"" AND request("sendEmailCanc2")="YES") OR (request("Submit10A")<>"") then
			call sendmail (scCompanyName, scEmail, pEmail, pcv_strSubject, replace(customerCancelledEmail, "&quot;", chr(34)))
		end if
	end if
...
if request("Submit7")<>"" then
	
	'update orderstatus and input form variables
	query="UPDATE orders SET orderstatus="&request("resetstat")&" WHERE idOrder="& qry_ID
	Set rs=Server.CreateObject("ADODB.Recordset")
	Set rs=conntemp.execute(query)
	pcv_intOrdReset=request("resetstat")
...
if request("Submit8")<>"" then
	...
	returnReason=replace(request("returnReason"),"'","''")
...
response.redirect "Orddetails.asp?id="&qry_ID&"&r=0&msg=Your Date/Time must be greater than 24 hours from the current date/time."
	...
	query="update orders set ord_OrderName=N'" & pord_OrderName & "',ord_DeliveryDate="
	if scDB="SQL" then
...
if (request("Submit14")<>"") then
	...
	if request("sendEmailShip14")="YES" then
		call sendmail (scCompanyName, scEmail, pEmail, pcv_strSubject, replace(customerShippedEmail, "&quot;", chr(34)))
		'//Send email to shipping email if it is different and exist
		if trim(pShippingEmail)<>"" AND trim(pShippingEmail)<>trim(pEmail) then
			call sendmail (scCompanyName, scEmail, pShippingEmail, pcv_strSubject, replace(customerShippedEmail, "&quot;", chr(34)))
...
if request("submitReSendGCRec")<>"" OR request("submitReSendGCRecA")<>"" then
	
	GC_ReName=getUserInput(request("GC_RecName"),0)
	GC_ReEmail=getUserInput(request("GC_RecEmail"),0)
	if request("submitReSendGCRecA")<>"" then
		...
		Set rs=nothing
		query="UPDATE orders SET pcOrd_GcReName=N'" & GC_ReName & "',pcOrd_GcReEmail='" & GC_ReEmail & "',pcOrd_GcReMsg=N'" & GC_ReMsg & "' WHERE idOrder="& qry_ID
		Set rs=Server.CreateObject("ADODB.Recordset")
		Set rs=conntemp.execute(query)
		Set rs=nothing
	else
		GC_ReMsg=getUserInput(request("GC_RecMsg"),0)
		query="UPDATE orders SET pcOrd_GcReName=N'" & GC_ReName & "',pcOrd_GcReEmail='" & GC_ReEmail & "' WHERE idOrder="& qry_ID
		Set rs=Server.CreateObject("ADODB.Recordset")
		Set rs=conntemp.execute(query)
		Set rs=nothing
	end if
