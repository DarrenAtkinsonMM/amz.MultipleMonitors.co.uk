...
IF (FCount>0) and (request("action")="add") THEN
	...
	For m=0 to FCount-1
		...
		query="UPDATE pcReviewsData SET pcRD_Feel=" & Rev_Feel & ",pcRD_Rate=" & Rev_Rate & ",pcRD_Comment=N'" & Rev_Com & "' WHERE pcRD_IDReview=" & pcv_IDReview & " and pcRD_IDField=" & Rev_IDField
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=connTemp.execute(query)
		set rs=Nothing
