...
If statusAPP="1" OR scAPP=1 Then
	...
	query="select pcprod_ParentPrd from Products where idproduct=" & pIDProduct
	set rs=connTemp.execute(query)
	pcv_ParentPrd=0
	if not rs.eof then
...
query=query&" FROM products WHERE idproduct=" &pIdProduct& " AND ((active<>0) OR ((active=0) and (pcProd_ParentPrd>0) and (pcProd_SPInActive=0)))"
set rstemp=conntemp.execute(query)
...
If statusAPP="1" OR scAPP=1 Then
	IDpre=request("SubPrd")
	if IDPre<>"" then
		query="Delete from ProductsOrdered where idOrder=" &pIdOrder&" and IdProduct=" & IDPre 
		set rstemp=conntemp.execute(query)
		set rstemp=nothing
	end if
End If
...
query = query & "("&pIdOrder&","&pIdProduct&","&pta_QTY&","&tempVar1&",0,N'"&tmpXFStr&"',0," & p_Const & ",'"&replace(pcv_strSelectedOptions,"'","''")&"','"&replace(pcv_strOptionsPriceArray,"'","''")&"','"&replace(pcv_strOptionsArray,"'","''")&"');"
