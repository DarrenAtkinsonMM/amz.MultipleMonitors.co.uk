...
Dim objFSO, objFolder, aFile
If request("action")="RmvAll" Then
	...
	For Each aFile In objFolder.Files
		...
		If (Left(aFile.Name, 1)) <> pcv_ExcludeFile Then
			If (Ucase(Right(aFile.Name,4))=".TXT") OR (Ucase(Right(aFile.Name,4))=".LOG") Then
				tmpFileName = aFile.Name
				objFSO.DeleteFile(pcv_DirPath & "\" & tmpFileName)
			End If
		End If
	...
	<table class="pcCPcontent">
        ...
        <tr>
            <td >
                <div class="pcCPmessageSuccess">All Control Panel Logs Files have been removed successfully!</div>
            </td>
        </tr>
	</table>
	<%
Else
    ...
    <form name="form1" method="post" action="viewCPLogs.asp" class="pcForms">
        ...
        <tr>
          <td colspan="2">
            <input type="submit" name="submit" id="submit" value="Submit" class="btn btn-primary">&nbsp;<input type="button" name="RemoveL" id="RemoveL" value="Remove All Logs" onclick="javascript:location='viewCPLogs.asp?action=RmvAll';" class="btn btn-default">
          </td>
        </tr>
        </table>
    </form>
