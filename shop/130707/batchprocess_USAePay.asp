...
for r=1 to checkboxCnt
	...
	IF request.Form("checkOrd"&r)="YES" THEN
		...
		if uep_success = 1 then
			...
			'purge cc data
			query="SELECT ccCard, pcSecurityKeyID FROM pcPay_USAePay_Orders WHERE idePayOrder="&pidePayOrder&";"
			set rstemp=connTemp.execute(query)
			if NOT rstemp.EOF then
				cardnumber=rstemp("ccCard")
			...
			query="UPDATE pcPay_USAePay_Orders SET ccCard='*' WHERE idePayOrder="&pidePayOrder&";"
			set rstemp=connTemp.execute(query)
			set rstemp=nothing
