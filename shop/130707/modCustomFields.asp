...
response.redirect "modCustomFields.asp?x="&x&"&idproduct="&idproduct&"&message="&Server.UrlEncode("The maximum number of characters for your field must be a numeric value.")        
	...
	query="UPDATE xfields SET xfield=N'"&xfield&"', textarea="&textareainput&", widthoffield="&length&", maxlength="&maxchars&", rowlength="&rows&" WHERE idxfield="&idxfield&";"
	set rs=Server.CreateObject("ADODB.Recordset") 
	set rs=conntemp.execute(query)
	
	query="UPDATE pcPrdXFields SET IdXField=" & idxfield & ",pcPXF_XReq=" & xreq & " WHERE IdProduct=" & idproduct & " AND IdXField="&idxfield&";"
	set rs=conntemp.execute(query)
	set rs=nothing
...
Else
	...
	xreq=0
	query="SELECT pcPXF_XReq FROM pcPrdXFields WHERE IdProduct=" & idproduct & " AND IdXField=" & idxfield & ";"
	set rs=conntemp.execute(query)
	if not rs.eof then
		xreq=rs("pcPXF_XReq")
	end if
