...
<%
query="select products.sku,products.description,products.smallImageUrl,products.stock,products.nostock,products.ServiceSpec,pcEvProducts.pcEP_ID,pcEvProducts.pcEP_Qty,pcEvProducts.pcEP_HQty,pcEvProducts.pcEP_GC,pcEvProducts.pcEP_Price from products,pcEvProducts where pcEvProducts.pcEP_IDEvent=" & gIDEvent & " AND products.idproduct=pcEvProducts.pcEP_IDProduct AND products.removed=0 AND ((products.active<>0) OR ((products.active=0) AND (products.pcProd_ParentPrd>0))) " & mOrder
set rstemp=Server.CreateObject("ADODB.Recordset")
set rstemp=connTemp.execute(query)
%>
...
<%ELSE%>
	...
	<tr> 
		<td colspan="7"> 
			<p>
				<br><br>
				<input type="submit" name="submit" value=" Add Products to Order " class="btn btn-primary">&nbsp;
