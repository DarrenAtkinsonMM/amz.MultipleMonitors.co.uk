...
<form method="POST" action="Affiliate_action.asp" class="pcForms">
	...
	<table class="pcCPcontent">
		...
		<tr>
			...
			<input type="button" class="btn btn-default"  value="Add New Affiliate" onClick="location.href='instAffa.asp'">&nbsp;
		...
		<tr>
			...
			<form name="form1" action="ReportAffiliateHistory.asp" method="post" target="_blank" class="pcForms">
				...
				<% if not rsAffObj.eof then
					...
					set rsAffObj=nothing
				End If %>
				</select>&nbsp;<input name="submit" type="submit" value="View history" class="btn btn-primary">
			</form>
			</td>
		</tr>
	</table>
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error in retreiving products from database: "&Err.Description) 
			...
			<tr>
                ...
                <td>
				<div id="CategoryList" style="margin-bottom: 6px;"></div>
			...
			<tr class="normal"> 
				...
				<input type="submit" name="submit1" value="Generate Links" class="btn btn-primary">
				</td>
			</tr>
			<% If sMode="1" then
	...
	<script type=text/javascript>
		function LoadProductList(IDCat)
		{
			...
			$pc.ajax({
				type: "GET",
				data: "mode=4&x_idCategory="+IDCat,
			...
			{
				$pc("#ProductList").html("<select id=\"product\" name=\"product\">"+data+"</select>");
			}
