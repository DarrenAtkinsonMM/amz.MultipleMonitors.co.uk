...
if pcv_refAction = "update" then
	...
	if newrefer<>"" then
		...
		query="insert into Referrer (Name,SortOrder) values (N'" & newrefer & "',0);"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		set rs=nothing
...
if request("Submit1")="Update" then
	...
	pcs_ValidateTextField	"SPhoneReq", pcv_isSPhoneReqRequired, 2
...
response.redirect pcStrPageName&"?msg="&pcv_strGenericPageError& "&lmode=" & pcLoginMode
    ...
    If Session("pcAdminDFShow") = "1" Then
        If Session("pcAdminDFLabel") = "" Then
		    call closeDb()
            response.redirect pcStrPageName & "?msg=Date Field description is required." & "&lmode=" & pcLoginMode
        End If
    End IF
    
    If Session("pcAdminTFShow") = "1" Then
        If Session("pcAdminTFLabel") = "" Then
		    call closeDb()
            response.redirect pcStrPageName & "?msg=Time Field description is required." & "&lmode=" & pcLoginMode
        End If
    End IF 
    
    If Session("pcAdminAllowNews") = "1" Then
        If Session("pcAdminNewsLabel") = "" Then
		    call closeDb()
            response.redirect pcStrPageName & "?msg=News description is required." & "&lmode=" & pcLoginMode
        End If
    End IF 
    
    If Session("pcAdminTerms") = "1" Then
        If Session("pcAdminTermsLabel") = "" Then
		    call closeDb()
            response.redirect pcStrPageName & "?msg=Terms description is required." & "&lmode=" & pcLoginMode
        End If
    End IF 
	...
	pcStrSPhoneReq = Session("pcAdminSPhoneReq")
						...
						<tr>
							<td width="33%" valign="top">
								<ul>
                                    ...
                                    <li><a href="#sphonereq">Require Shipping Phone Number</a></li>
								</ul>
							</td>
							<td width="33%" valign="top">
								<ul>
	...
	<table class="pcCPcontent">
		...
		<tr> 
			...
			<td width="75%">
				...
				<input name="Submit2" type="submit" value="Add New" class="btn btn-primary"> 
			</td>
		</tr>
		<tr> 
	...
	<table class="pcCPcontent">
		...
		<tr> 
			<th colspan="2"><a name="guest"></a>Guest Checkout Options&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=466"></a></th>
		</tr>
		<tr> 
			<td colspan="2" class="pcCPspacer"></td>
		</tr>
        ...
        <tr>
			<th colspan="2"><a name="sphonereq"></a>Shipping Phone Number</th>
		</tr>
		<tr>
			<td colspan="2" class="pcCPspacer"></td>
		</tr>
        <tr>
			<td align="right">Field required?</td>
			<td>
				<input type="radio" name="SPhoneReq" value="1" <%if SPhoneReq="1" then%>checked<%end if%> class="clearBorder">Yes 
				<input type="radio" name="SPhoneReq" value="0" <%if SPhoneReq<>"1" then%>checked<%end if%> class="clearBorder">No
            </td>
		</tr>
        <tr>
			<td colspan="2" class="pcCPspacer"></td>
		...
		<tr> 
			...
			<td align="left">
				<input name="TermsLabel" type="text" value="<%=pcStrTermsLabel%>" size="35" <% If scUpgrade = 1 Then Response.Write "disabled" %>>
		...
		<tr> 
			...
			<td>
                <textarea name="TermsCopy" cols="60" rows="6" <% If scUpgrade = 1 Then Response.Write "disabled" %>><%=pcStrTermsCopy%></textarea>
                <% If scUpgrade = 1 Then %>
                    <div class="pcSmallText">NOTE: You cannot update the terms and conditions while in upgrade mode.</div>
                <% End If %>
            </td> 
		</tr>
		<tr> 
		...
		<tr> 
			<td colspan="2">You can ask your customers whether or not they would like to receive information from the store. Use the Newsletter Wizard to send a message to selected customers.</td>
		</tr>
		<tr> 
			<td align="right">Field Description:</td>
			<td align="left">
