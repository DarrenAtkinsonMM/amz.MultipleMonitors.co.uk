...
<% 
Dim idproduct, discountdesc, percentage, baseproductonly, discountPerUnit1, discountPerWUnit1, quantityfrom1
...
If statusAPP="1" OR scAPP=1 Then
	If pIDProduct>"0" then
		...
		query="SELECT pcProd_ParentPrd FROM Products INNER JOIN discountsPerQuantity ON Products.idproduct=discountsPerQuantity.idproduct WHERE Products.pcProd_ParentPrd="& pIDProduct & ";"
		Set rs=Server.CreateObject("ADODB.Recordset")
		set rs=conntemp.execute(query)
		if not rs.eof then
			set rs=nothing
			call closedb()
			response.redirect "app-viewDisca.asp?idParent=" & pIDProduct
		end if
		set rs=nothing
...
<form method="POST" action="AdminDctQtyPrd.asp" class="pcForms">
	<table class="pcCPcontent">
		<tr>
            ...
            <% 	' END show message %>
            The discount can be a dollar amount off the &quot;online price&quot; or a percentage off the same price. Select the type of discount below, then enter the &quot;from&quot; and &quot;to&quot; quantity values, and specify the discount value. Refer to the ProductCart <a href="http://www.productcart.com/technical-support.asp" target="_blank">User Guide</a> for more information.
			
			<%
			If statusAPP="1" OR scAPP=1 Then
				query="SELECT pcProd_Apparel FROM Products WHERE idproduct="& idproduct & " AND pcProd_Apparel<>0;"
				Set rs=Server.CreateObject("ADODB.Recordset")
				set rs=conntemp.execute(query)
				if not rs.eof then
				%>
					<div class="pcCPmessageWarning">
						You are setting quantity discounts at the parent product level. <a href="app-viewDisca.asp?idParent=<%=idproduct%>">Click here</a> to set quantity discounts at the sub-product level.
                	</div>
				<%
				end if
				set rs=nothing
	...
	<tr> 
		<td colspan="5" align="center">
			<input type="submit" name="Submit" value="Add First Tier" class="btn btn-primary">&nbsp;
		</td>
	</tr>
</table>
</form>
