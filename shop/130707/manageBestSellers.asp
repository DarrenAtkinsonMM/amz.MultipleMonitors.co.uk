...
if request("action")="upd" then
	...
	if not rs.eof then
		dim rsUpdObj
		query="UPDATE pcBestSellerSettings SET pcBSS_BestSellCount=" & pcIntBestSellCount & ",pcBSS_Style='" & pcStrPageStyle & "',pcBSS_PageDesc=N'" & pcStrBestSellDesc & "',pcBSS_NSold=" & pcIntNSold & ",pcBSS_NotForSale=" & pcIntBestSellNFS & ",pcBSS_OutOfStock=" & pcIntBestSellInStock & ",pcBSS_SKU=" & pcIntShowSKU & ",pcBSS_ShowImg=" & pcIntShowSmallImg & ";"
		set rsUpdObj=Server.CreateObject("ADODB.RecordSet")
		set rsUpdObj=connTemp.execute(query)
		set rsUpdObj=nothing
	else
		dim rsInsObj
		query="INSERT INTO pcBestSellerSettings (pcBSS_BestSellCount, pcBSS_Style, pcBSS_PageDesc, pcBSS_NSold, pcBSS_NotForSale, pcBSS_OutOfStock, pcBSS_SKU, pcBSS_ShowImg) VALUES (" & pcIntBestSellCount & ", '" & pcStrPageStyle & "', N'" & pcStrBestSellDesc & "', " & pcIntNSold & ", " & pcIntBestSellNFS & ", " & pcIntBestSellInStock & ", " & pcIntShowSKU & ", " & pcIntShowSmallImg & ");"
...
if not rs.eof then
	...
	if pcStrBestSellDesc<>"" then
		pcStrBestSellDesc=replace(pcStrBestSellDesc,"""","&quot;")
		pcStrBestSellDesc=replace(pcStrBestSellDesc,"<","&lt;")
		pcStrBestSellDesc=replace(pcStrBestSellDesc,">","&gt;")
	end if
...
<form action="manageBestSellers.asp?action=upd" method="post" name="hForm" class="pcForms" onSubmit="return Form1_Validator(this)">
	<table class="pcCPcontent">
		...
		<tr valign="top">
			...
			<td width="75%">
				<textarea class="htmleditor" id="pcStrBestSellDesc" name="pcStrBestSellDesc" cols="50" rows="6"><%=pcStrBestSellDesc%></textarea>
			</td>
		</tr>	
		<tr> 
			<td colspan="2" class="pcCPspacer"></td>
		...
		<tr> 
			...
			&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=202"></a>
			</td>
		</tr>
		<tr> 
			<td colspan="2"> 
			...
			&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=201"></a>
			</td>
		</tr>
