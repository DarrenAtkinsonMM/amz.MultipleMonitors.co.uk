...
pcv_tmpQuery=""
if pcv_Query<>"" then
	pcv_tmpQArr=split(pcv_Query,"&")
	For iC=lbound(pcv_tmpQArr) to ubound(pcv_tmpQArr)
		if (pcv_tmpQArr(iC)<>"") AND (InStr(ucase(pcv_tmpQArr(iC)),"PASSWORD=")=0) AND (InStr(ucase(pcv_tmpQArr(iC)),"PASS=")=0) then
			if pcv_tmpQuery<>"" then
				pcv_tmpQuery=pcv_tmpQuery & "&"
			end if
			pcv_tmpQuery=pcv_tmpQuery & pcv_tmpQArr(iC)
		end if
	Next
end if

if pcv_tmpQuery<>"" then
	pcv_filePath = pcv_filePath & "?" & pcv_tmpQuery
end if

'// Log current activity

Set objCPFSO = Server.CreateObject ("Scripting.FileSystemObject")
pcStrFileName = Server.Mappath ("CPLogs/"&replace(Date,"/","")&".log")
