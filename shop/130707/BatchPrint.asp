...
'///////////////////////////////
	...
	For k=lbound(A) to ubound(A)
		IF A(k)<>"" then
			...
			Dim pidcustomer, porderdate, pAddress, pAddress2, pcity, pstate, pstateCode, pzip, pCountryCode, ppaymentDetails, pshipmentDetails, pshippingCompany, pshippingAddress, pshippingAddress2, pshippingCity, pshippingStateCode, pshippingState, pshippingZip, pshippingCountryCode, pshippingPhone, pidAffiliate, paffiliatePay, pdiscountDetails, ptaxAmount, ptotal, pcomments, porderStatus, pprocessDate, pshipDate, pshipvia, ptrackingNum, preturnDate, preturnReason,ptaxDetails,pSRF, pord_DeliveryDate, pord_OrderName, pOrdShipType, pOrdPackageNum, pOffPayDescription, pOffPayInfo, pAdminComments, pnotax, intPackingSlip, ptaxcode, pcSCID
			...
			While Not rs.EOF %>
				<table border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
					<tr> 
					<td width="18%" height="71" valign="top">
						<% If Len(scCompanyLogo) > 0 Then %>
							<img src="../pc/catalog/<%=scCompanyLogo%>">
						<% End If %>
					</td>
					...
					<td class="nobreak">
                		...
                		<% query="SELECT ProductsOrdered.idProduct, ProductsOrdered.quantity, ProductsOrdered.pcPrdOrd_SelectedOptions, ProductsOrdered.pcPrdOrd_OptionsPriceArray, ProductsOrdered.pcPrdOrd_OptionsArray, ProductsOrdered.unitPrice, ProductsOrdered.xfdetails"
										...
										Do until rsTemp.eof
											...
											IF (intPackingSlip<>1) OR (request("Ord" & qry_ID & "_" & pidProduct)="1") THEN
...
response.redirect "menu.asp"
	...
	<form action="BatchPrint.asp" method="post" name="invoice" class="pcForms">
		<table class="pcCPcontent">
			...
			<%For m=1 to Count
			if (request("check" & m)="1") and (request("idord" & m)<>"") then
			qry_ID=request("idord" & m) %>
			<%query="SELECT Products.idProduct, Products.Description FROM Products INNER JOIN ProductsOrdered ON Products.idProduct=ProductsOrdered.idProduct WHERE ProductsOrdered.idOrder=" & qry_ID & " ORDER BY Products.Description;"
			Set rs=Server.CreateObject("ADODB.Recordset")
			Set rs=connTemp.execute(query)
			If not rs.eof then
				pcArr=rs.getRows()
			...
			<tr>
				<td><input type="submit" name="GO" value="Submit" class="btn btn-primary"></td>
			</tr>
		</table>
		<script type="text/javascript">
			showPPick();
