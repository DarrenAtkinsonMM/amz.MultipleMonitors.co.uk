<% 
PmAdmin=19
pageTitle = "ProductCart Canada Post Services - Database Upgrade" 
Section = "" 
%>
...
IF request("action")="sql" then
	if request("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			call closeDb()
			response.redirect "upddb_CPServices.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			call closeDb()
			response.redirect "upddb_CPServices.asp?mode=1"
			response.End
		end if
	...
	query="UPDATE shipService SET serviceDescription = 'Canada Post Expedited US Business Contract' WHERE serviceCode = '2020'"
	conntemp.execute(query)
	query="UPDATE shipService SET serviceDescription = 'Canada Post Priority Worldwide US' WHERE serviceCode = '2040'"
	conntemp.execute(query)
	query="UPDATE shipService SET serviceDescription = 'Canada Post Priority Worldwide PAK US' WHERE serviceCode = '2050'"
	conntemp.execute(query)
	query="UPDATE shipService SET serviceDescription = 'Canada Post Priority Worldwide International' WHERE serviceCode = '3040'"
	conntemp.execute(query)
	query="UPDATE shipService SET serviceDescription = 'Canada Post Priority Worldwide PAK International' WHERE serviceCode = '3050'"
	conntemp.execute(query)
	query="UPDATE shipService SET serviceCode = '2000', serviceDescription = 'Canada Post Tracked Packet US' WHERE serviceCode = '2010'"
	conntemp.execute(query)
	query="UPDATE shipService SET serviceCode = '3000', serviceDescription = 'Canada Post Tracked Packet International' WHERE serviceCode = '2025'"
	conntemp.execute(query)
	query="DELETE FROM shipService WHERE serviceCode IN ('1120', '1130', '1220', '1230', '2005')"
	conntemp.execute(query)
...
<form action="upddb_CPServices.asp" method="post" name="form1" id="form1" class="pcForms">
<%
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
                ...
                <h1 class="page-header">Canada Post Services Database Update</h1>
                <p class="lead">
