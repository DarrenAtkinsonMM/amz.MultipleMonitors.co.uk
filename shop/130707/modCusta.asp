...
Session("pcAdminAvalaraExemptionNo")=pcf_ResetFormField(Session("pcAdminAvalaraExemptionNo"), rs("pcCust_AvalaraExemptionNo"))
set rs=nothing
...
IF request("modCustVal")<>"" THEN
	...
	'// Avalara Tax Exemption Number
	pcs_ValidateTextField "AvalaraExemptionNo", false, 0
	...
	Else
		...
		if Session("pcAdminResetPass")="1" AND Session("pcAdminpassword")<>"" then
			Session("pcAdminpassword")	= pcf_PasswordHash(Session("pcAdminpassword"))
		end if
...
response.redirect pcStrPageName&"?msg=" & Server.URLEncode("The email you have chosen is already in use by another customer. If you still wish to use this e-mail for this customer account, <a href='viewcustb.asp?key4=" & Session("pcAdminpcBillingEmail") & "'>search</a> for all customers with the same e-mail address, consolidate their accounts into one using the corresponding feature (orders are moved to the consolidated account), and then remove the accounts that are no longer needed.") & "&idcustomer=" & pidcustomer
		...
		query="UPDATE customers SET pcCust_VATID='" &Session("pcAdminpcBillingVATID")& "', pcCust_SSN='" &Session("pcAdminpcBillingSSN")& "', name=N'" &Session("pcAdminpcBillingFirstName")& "', lastName=N'" &Session("pcAdminpcBillingLastName")& "', email='" &Session("pcAdminpcBillingEmail")& "', fax='" &Session("pcAdminpcBillingFax")& "', city=N'" &Session("pcAdminpcBillingCity")& "', zip='" &Session("pcAdminpcBillingPostalCode")& "', countryCode='" &Session("pcAdminpcBillingCountryCode")& "', state=N'" &Session("pcAdminpcBillingProvince")& "', stateCode='" &Session("pcAdminpcBillingStateCode")& "', shippingcity=N'" &Session("pcAdminShipCity")& "', shippingzip='" &Session("pcAdminShipZip")& "', shippingcountryCode='" &Session("pcAdminShipCountryCode")& "', shippingstate=N'" &Session("pcAdminShipState")& "', shippingstateCode='" &Session("pcAdminShipStateCode")& "', phone='" &Session("pcAdminpcBillingPhone")& "', address=N'" &Session("pcAdminpcBillingAddress")& "', address2=N'" &Session("pcAdminpcBillingAddress2")& "', shippingCompany=N'" &Session("pcAdminShipCompany")& "', shippingaddress=N'" &Session("pcAdminShipAddress")& "', shippingaddress2=N'" &Session("pcAdminShipAddress2")& "', customercompany=N'" &Session("pcAdminpcBillingCompany")& "', customerType=" &Session("pcAdminCustomerType")& ", shippingEmail='" &Session("pcAdminShipEmail")& "', shippingPhone='" &Session("pcAdminShipPhone")& "', pcCust_AvalaraExemptionNo='" &Session("pcAdminAvalaraExemptionNo")& "'"
	    ...
	    ' PRV41 begin
		query=query&", RecvNews="&Session("pcAdminCRecvNews")&", suspend="&Session("pcAdminsuspend")&", idcustomerCategory=" & intidcustomerCategory&", pcCust_Locked=" & Session("pcAdminCustLocked") & ", pcCust_Notes=N'" & Session("pcAdminCustNotes") & "', pcCust_AllowReviewEmails=" & Session("pcAllowReviewEmails") & " WHERE idCustomer="&Session("adminidcustomer")
		' PRV41 end
		...
		if Session("pcAdminResetPass")="1" AND Session("pcAdminpassword")<>"" then
			call pcs_SaveUsedPass(Session("adminidcustomer"),Session("pcAdminpassword"))
		end if

		' Reset Failed Payment Count
		If Session("pcAdminCustLocked") = "0" Then
			call pcs_clearFailedPaymentAttempt(Session("adminidcustomer"))
		End If		
		...
		'// Redirect
		call closeDb()
        response.redirect pcStrPageName&"?idcustomer="&Session("adminidcustomer") & "&s=1&msg=Customer data has been updated successfully!"
...
Public Sub pcs_JavaCheckedBox_2(FieldName, isRequiredField, ErrorMessage, tab)
	  ...
	  <div id="TabbedPanels2" class="tabbable-left">
		...
		<div class="col-xs-3">
            <ul class="nav nav-tabs tabs-left">
				<li class="active"><a href="#tabs-1" data-toggle="tab">General Information</a></li>
                <li><a href="#tabs-2" data-toggle="tab">Default Billing Address</a></li>
                <li><a href="#tabs-3" data-toggle="tab">Default Shipping Address</a></li>
                <li><a href="#tabs-4" data-toggle="tab">Other Settings</a></li>
				<li>
					<div style="margin-top:10px; margin-bottom:10px; margin-left: 5px;">
                    <input type="submit" name="Modify" value=" Save " class="btn btn-primary"> 
                    ...
                    <div style="padding-top: 5px;">
                    <input type="button" class="btn btn-default"  name="Search" value="Locate Another" onClick="document.location.href='viewcusta.asp';">
					</div>
					<div style="padding-top: 5px;">
                    <input type="button" class="btn btn-default"  name="AddNew" value="Add" onClick="document.location.href='instcusta.asp';">
                    </div>
					</div>
                </li>
			</ul>
					...
					<table class="pcCPcontent">				
                        ...
                        <tr>
                            ...
                            <td  valign="top">
								...
								<% if pcf_GetCustType(pidcustomer)=0 then %>Reset Password<% else %>Add Password<%end if%>:&nbsp;<input type="password" id="password" name="password" <% if Session("pcAdminResetPass")<>"1" then%>disabled<% end if %> value="<% if Session("pcAdminResetPass")="1" then%><% =pcf_FillFormField ("password", pcv_ispasswordRequired) %><%end if%>" size="25" maxlength="50">
                                </p>
                            </td>
                        </tr>
                        <tr>
			...
			%>
				<div id="tabs-2" class="tab-pane">
                    ...
                    <%
                        ...
                        <%
						If USPS_AddressValidation = "1" OR ptaxAvalaraAddressValidation = 1 Then %>
                        <tr>
                        	<td><input type="button" class="btn-default" value="Validate Address" onClick="checkAddress('B');"/></td>
                        </tr>
                        <% End If %>
					</table>
			...
			%>
				<div id="tabs-3" class="tab-pane">
					...
					<table class="pcCPcontent">
						...
						If USPS_AddressValidation = "1" OR ptaxAvalaraAddressValidation = 1 Then %>
                        <tr>
                        	<td><input type="button" class="btn-default" value="Validate Address" onClick="checkAddress('S');"/></td>
                        </tr>
                        <% End If %>
					</table>
				...
				<div id="tabs-4" class="tab-pane">
					...
					<table class="pcCPcontent">
                        ...
                        <tr> 
                            <td colspan="2">
                                <%
                                If scGWSecurity = 1 And (scGWLockAttempts > 0) Then
                                    pcv_FailedPaymentCount = 0
                                    query="SELECT pcCust_FailedPaymentCount FROM customers WHERE idCustomer = " & pidcustomer
                                    Set rs = server.CreateObject("ADODB.RecordSet")
                                    Set rs = conntemp.execute(query)
                                    If Not rs.Eof Then
                                        pcv_FailedPaymentCount = rs("pcCust_FailedPaymentCount")
                                        If sdf < scGWLockAttempts Then
                                        
                                        End If
                                    End If
                                    Set rs = Nothing
                                    If pcv_FailedPaymentCount>0 Then
                                        %>
                                        <div class="help-block">
                                            Failed payment attempts: <span class="badge"><%=pcv_FailedPaymentCount%></span>
                                        </div>
                                        <%
                                    End If
                                End If
                                %>
                                <input type="checkbox" name="lock" value="1" <% if Session("pcAdmincustomertype")="3" OR Session("pcAdminCustLocked")="1" then %>checked<% end if %> class="clearBorder pcAutoCompleteOff"> <strong>Lock</strong> this customer&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=456"></a>
                            </td>
                        </tr>
                        <tr> 
                            <td colspan="2">
                                <input type="checkbox" name="suspend" value="1" <% if Session("pcAdminsuspend")="1" then%>checked<% end if %> class="clearBorder"> <strong>Suspend</strong> this customer&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=457"></a>
                            </td>
                        </tr>
						<% 
						'// REWARD POINTS - Start
                        ...
                        <tr>
                        	<td colspan="2">Tax Exemption Number : <input name="AvalaraExemptionNo" type="text" value="<%=Session("pcAdminAvalaraExemptionNo")%>"></td>
                        </tr>
                        <% End If %>
                        <%
						' PRV41 start 
...
<script type=text/javascript>
    ...
    var validator0 = $pc("#modCust").validate({
        rules: {
            password: {
                required: false,
                remote: function() {  
        ...
        if ($pc('#modCust').validate().form()) {
            ...
            if session("cp_nc_custfields_exists") = "YES" then
                ...
                For k = 0 to ubound(pcArr,2)
                    if pcArr(6,k) = "1" then
                        if pcArr(2,k) = "1" then
                            pcs_JavaCheckedBox_2 "custfield_" & pcArr(0,k), "true", "Please select the option", "3"
                        else
                            pcs_JavaTextField "custfield_" & pcArr(0,k), "true", "Please enter a value for this field", "3"
                        end if
                    end if
                Next
            end if
            ...
            %>
            var tmpPass = $pc("#password").val();
            if (confirmedSubmit || tmpPass == '') {
                return (true);
            } else {
                return (false);
            }
	...
	function checkAddress(type)
	{
		...
		if(type == "B") {
			formdata = {
				country : $("select[name='pcBillingCountryCode'] option:selected").val(),
				address : $("input[name='pcBillingAddress']").val(),
				address2 : $("input[name='pcBillingAddress2']").val(),
				city : $("input[name='pcBillingCity']").val(),
				state : $("select[name='pcBillingStateCode'] option:selected").val(),
				zip : $("input[name='pcBillingPostalCode']").val(),
				type : type
			}
		} else {
			formdata = {
				country : $("select[name='ShipCountryCode'] option:selected").val(),
				address : $("input[name='ShipAddress']").val(),
				address2 : $("input[name='ShipAddress2']").val(),
				city : $("input[name='ShipCity']").val(),
				state : $("select[name='ShipStateCode'] option:selected").val(),
				zip : $("input[name='ShipZip']").val(),
				type : type
			}
		}
		$.post('checkAddress.asp', formdata, function(response) {
				WinId = window.open('', 'newwin', 'height=500');
