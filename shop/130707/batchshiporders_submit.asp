...
for r=1 to checkboxCnt	
	if request.Form("checkOrd"&r)="YES" then		
		...
		if pcv_shippedDate<>"" then
			query="INSERT INTO pcPackageInfo (idOrder,pcPackageInfo_ShipMethod,pcPackageInfo_ShippedDate,pcPackageInfo_TrackingNumber,pcPackageInfo_Comments, pcPackageInfo_MethodFlag) VALUES (" & pcv_IdOrder & ",N'" & pcv_method & "','" & dtShippedDate & "','" & pcv_tracking & "',N'" & pcv_AdmComments & "', 4);"
		else					
			query="INSERT INTO pcPackageInfo (idOrder,pcPackageInfo_ShipMethod,pcPackageInfo_TrackingNumber,pcPackageInfo_Comments, pcPackageInfo_MethodFlag) VALUES (" & pcv_IdOrder & ",N'" & pcv_method & "','" & pcv_tracking & "',N'" & pcv_AdmComments & "', 4);"
		end if
		...
		if pCheckEmail="YES" then
			...
			call pcs_hookOrderShippedEmailSent(pTmpEmail)
			...
			if trim(pShippingEmail)<>"" AND trim(pShippingEmail)<>trim(pTmpEmail) then
				...
				call pcs_hookOrderShippedEmailSent(pShippingEmail)
			end if
		end if
