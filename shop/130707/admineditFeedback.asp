...
if (request("action")="update") and (request("rewrite")="0") then
	...
	query="UPDATE pcComments SET pcComm_IDOrder=" & intIdOrder & ",pcComm_EditedDate='" & dtComDate & "',pcComm_FType=" & intFType & ",pcComm_FStatus=" & intFStatus & ",pcComm_Priority=" & intPriority & ",pcComm_Description=N'" & strFDesc & "',pcComm_Details=N'" & strFDetails & "' WHERE pcComm_IDOrder=" & intIdOrder & " and pcComm_IDFeedback=" & IDFeedback
	set rs=connTemp.execute(query)
		...
		<td>
			<table width="100%" bgcolor="#FFFFFF" cellpadding="5" cellspacing="0">
          	...
          	<option value=""></option>
				...
				<tr>
					<td width="25%" align="right" valign="top"><font face="<%=FFont%>" size="<%=FSize%>" color="<%=FColor%>">Long Description:</font>
					<td width="75%"><font face="<%=FFont%>" size="<%=FSize%>" color="<%=FColor%>"><textarea class="htmleditor" name="Details" id="Details" cols="40" rows="7" id="bugLongDsc"><%if request("Details")<>"" then%><%=request("Details")%><%else%><%=strDetails%><%end if%></textarea></font>
					</td>
				</tr>
  			<%if AllowUpload="1" then%>
				...
				<tr>
					<td width="25%" align="right"><font face="<%=FFont%>" size="<%=FSize%>" color="<%=FColor%>">&nbsp;</font></td>
					<td width="75%"><font face="<%=FFont%>" size="<%=FSize%>" color="<%=FColor%>"><input type="submit" name="Submit" value=" Update " class="btn btn-primary" onClick="document.hForm.rewrite.value='0';">&nbsp;<input type="button" class="btn btn-default"  value="Back" onClick="location='adminviewfeedback.asp?IDOrder=<%=intIdOrder%>&IDFeedback=<%=IDFeedback%>'">&nbsp;
