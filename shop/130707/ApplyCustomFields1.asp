			...
			Select the custom field that you would like to copy to other products:
				...
				<table>
					...
					<%else
						...
						For i=0 to intCount%>
						<tr> 
							<td><input type="radio" name="CustField" value="xfield<%=pcArr(0,i)%>" <%if (session("CustFieldCopy") &"")=("xfield" & pcArr(0,i)) then%>checked<%end if%> class="clearBorder"></td>
							<td width="100%"><%=pcArr(1,i)%></td>
						</tr>
						<%Next
					end if %>
				</table>
				...
				<%end if%>
                	<input type="button" class="btn btn-default"  name="back" value="Back" onClick="location='AdminCustom.asp?idproduct=<%=idproduct%>';">
				</p>
				</form>
		</td>
	</tr>
