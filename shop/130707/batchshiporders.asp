...
<table class="pcCPcontent">
	<tr>
		<td colspan="7">
		The following orders have been <strong>processed</strong>.&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=316"></a><br>
	...
	ELSE
		...
		For i=0 to intCount
			...
			If instr(shipmentdetails,",") > 0 Then
				...
				shipmethod=pcf_GetShipServiceName(shipmethod,1)
			Else
				shipmethod = ""
			End If
			...
			If pcv_available=1 AND NOT (trim(paymentCode)="Google Checkout") Then	
				...
				<tr onMouseOver="this.className='activeRow'" onMouseOut="this.className='cpItemlist'" class="cpItemlist"> 
					<td>
						<a href="ordDetails.asp?id=<%=int(idOrder)%>" target="_blank" style="text-decoration: none;" title="View Order Details"><%=int(idOrder)+scpre%></a>&nbsp;|&nbsp;<a href="ordDetails.asp?id=<%=int(idOrder)%>&activetab=2" target="_blank" style="text-decoration: none;" title="View Order Details"><%=scCurSign&money(total)%></a>&nbsp;|&nbsp;<%if pcv_custName <> "NA" then%><a href="modcusta.asp?idcustomer=<%=idCustomer%>" target="_blank" style="text-decoration:none;" title="View Customer Details"><%=pcv_custName%></a><%else%><%=pcv_custName%><%end if%>
					</td>
	...
	<% if noORDRec=0  then %>
	<tr>
		<td colspan="9"><input type="submit" name="Submit" value="Ship Selected Orders" class="btn btn-primary"></td>
	</tr>
	<% end if %>
</table>
<table class="pcCPcontent">
