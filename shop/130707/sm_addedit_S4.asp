...
IF request("a")="post" THEN
	...
	session("sm_TechDetails1")="Products:<div style=""padding-top: 5px; font-weight:bold;"">The sale will affect " & session("sm_PrdCount") & " product(s)"
	if session("sm_PrdCountSub")>"0" then
		session("sm_TechDetails1")=session("sm_TechDetails1") & " and " & session("sm_PrdCountSub") & " sub-product(s)"
	end if
	session("sm_TechDetails1")=session("sm_TechDetails1") & " in your store.</div><div style=""padding-top: 5px;"" class=""pcSmallText"">If you just updated the product included in the sale, this number may be incorrect. it will be updated when you save the sale.</div>"
...
<tr> 
	<td colspan="4">
    	<div class="pcCPmessageInfo">
            ...
            <div style="padding-top: 5px;">The sale will affect <b><%=session("sm_PrdCount")%></b> product(s)<%if session("sm_PrdCountSub")>"0" then%> and <%=session("sm_PrdCountSub")%> sub-product(s)<%end if%> in your store. You can add/remove products by editing the sale.</div>
...
<%
query="SELECT TOP 5 Products.IDProduct,products.price,products.listprice,products.btoBprice,products.cost,products.sku,products.description,products.pcProd_Apparel FROM " & session("sm_Param1") & " WHERE " & session("sm_Param2") & ";"
	...
	<tr>
		...
		<td>
			...
			<% if pcv_apparel="1" then%>
				<br /><i>included its sub-product(s)</i>
			<% end if %>
		</td>
...
<tr align="center">
	<td colspan="4">
		...
		<input type="button" class="btn btn-default"  name="Go" value="Continue" onClick="location='sm_addedit_S5.asp';" class="btn btn-primary">&nbsp;
		<%end if%>
		<input type="button" class="btn btn-default"  name="Back" value="Change Sale Settings" onClick="location='sm_addedit_S3.asp?a=back';">
	</td>
</tr>
</table>
<% 
