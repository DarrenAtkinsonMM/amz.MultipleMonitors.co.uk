...
response.redirect "sm_sales.asp"
	...
	if request("action")="update" then
		...
		query="UPDATE pcSales_Completed SET pcSC_SaveName=N'" & salename & "',pcSC_SaveDesc=N'" & saledesc & "',pcSC_SaveIcon='" & saleicon & "' WHERE pcSC_ID=" & pcSCID & ";"
		set rs=connTemp.execute(query)
		set rs=nothing
		
		query="UPDATE pcSales SET pcSales_Name=N'" & salename & "',pcSales_Desc=N'" & saledesc & "',pcSales_ImgURL='" & saleicon & "' WHERE pcSales_ID=" & pcSaleID & ";"
		set rs=connTemp.execute(query)
		set rs=nothing
...
response.redirect "sm_sales.asp"
	...
	<td>
		<input type="text" name="saleicon" id="saleicon" value="<%=tmpSaleIcon%>" size="50"> <a href="javascript:;" onClick="chgWin('../pc/imageDir.asp?ffid=saleicon&fid=UpdateForm','window2')"><img src="images/search.gif" alt="locate images previously uploaded" width="16" height="16" border=0 hspace="3"></a>&nbsp;
	...
	<tr>
		<td colspan="2">
			...
			<% If statusAPP="1" OR scAPP=1 Then %>
				It affected <b><%=tmpUPrds%></b> product(s)/sub-product(s) in your store.
			<% Else %>
				It affected <b><%=tmpUPrds%></b> product(s) in your store.
			<% End If %>
	...
	<tr>
		<td colspan="2" align="center">
			<%if request("e")="1" then%><input type="submit" name="submit1" value=" Save " class="btn btn-primary">&nbsp;<%end if%><input type="button" class="btn btn-default"  name="Go" value=" View Current & Completed Sales " onClick="location='sm_sales.asp';" <%if request("e")="1" then%>class="btn btn-default"<%else%>class="btn btn-primary"<%end if%>>
		</td>
	</tr>
	</table>
	<%if request("e")="1" then%>
