<%	
	...
	if (pcv_SubmitType=0) or (pcv_SubmitType=1) or (pcv_SubmitType=3) then
		...
		set rstemp=nothing
		query="INSERT INTO pcAdminComments (idorder,pcACom_ComType,pcACom_Comments) VALUES (" & qry_ID & ",1,N'" & pcv_AdmComments & "');"
		set rstemp=server.CreateObject("ADODB.RecordSet")
		set rstemp=connTemp.execute(query)
		set rstemp=nothing
	else
		...
		set rstemp=connTemp.execute(query)
		set rstemp=nothing
		query="INSERT INTO pcAdminComments (idorder,pcACom_ComType,pcACom_Comments) VALUES (" & qry_ID & ",0,N'" & pcv_AdmComments & "');"
		set rstemp=server.CreateObject("ADODB.RecordSet")
		set rstemp=connTemp.execute(query)
		set rstemp=nothing
	end if
	...
	do while not rs.eof
		...
		'GGG Add-on start
		if ((request("Submit4")<>"") or (pcv_SubmitType=3)) AND (pGCs="0") then
...
'// START: Submit 4
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
if (request("Submit4")<>"") or (pcv_SubmitType=3) then
	...
	call pcs_hookOrderProcessed(qry_ID)
	...
	'Commit Avalara tax
	If ptaxAvalara = 1 AND ptaxAvalaraEnabled = 1 AND ptaxAvalaraCommit = 1 Then
...
'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	...
	if (request("sendEmailConf")="YES") OR (request("Submit4A")<>"") OR (request("Submit4B")<>"") OR (pcv_SubmitType=3) then
		if (pcv_SubmitType=0) or (pcv_SubmitType=1) or (pcv_SubmitType=3) then
...
If statusAPP="1" OR scAPP=1 Then
	'// START - Order is processed -> Update Apparel Parent Product Sales
	pIdOrder=qry_ID
	%>
	<!--#include file="../pc/app-updsales.asp"-->
	<%
End If
