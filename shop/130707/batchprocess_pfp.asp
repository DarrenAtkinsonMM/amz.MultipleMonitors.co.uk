...
dim rstemp2, rsEmailInfo, rsCust, pTempProductId, pTodaysDate
...
for r=1 to checkboxCnt
	IF request.Form("checkOrd"&r)="YES" THEN
		...
		Else
			...
			query="UPDATE pfporders SET captured=1, trxtype = 'D',origid = '"&pfp_pnref&"' WHERE idpfporder="&pfpidorder&";"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp.execute(query)
			...
			successData=successData&"Order Number "& (Clng(pidOrder)+Clng(scpre)) &" was successfully updated<BR>"
...
Function generateRequestID()
	pcKeys_Key_ID= Year(Now) & Month(Now) & Day(Now) & Hour(Now) & Minute(Now) & Second(Now) & Session.SessionID & pIdOrder & pfpidorder
    generateRequestID = Left(pcKeys_Key_ID,30)
End Function
