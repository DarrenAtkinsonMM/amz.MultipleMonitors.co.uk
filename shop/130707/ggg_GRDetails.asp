...
<%
'APP-S
query="SELECT products.pcProd_Apparel,products.pcProd_ParentPrd,products.pcProd_Relationship,products.idProduct,products.sku,products.description,pcEvProducts.pcEP_ID,pcEvProducts.pcEP_Qty,pcEvProducts.pcEP_HQty,pcEvProducts.pcEP_GC,pcEvProducts.pcEP_OptionsArray FROM products,pcEvProducts WHERE pcEvProducts.pcEP_IDEvent=" & gIDEvent & " AND products.idproduct=pcEvProducts.pcEP_IDProduct AND products.removed=0 AND ((products.active<>0) OR ((products.active=0) AND (products.pcProd_ParentPrd>0))) ORDER BY products.Description ASC,pcEvProducts.pcEP_GC ASC"
'APP-E
set rstemp=Server.CreateObject("ADODB.Recordset")
set rstemp=connTemp.execute(query)
if err.number<>0 then
...
<tr>
	<td colspan="6">
		...
		<br>
		<input type="button" class="btn btn-default" name="back1" value=" Back " onclick="javascript:location='ggg_manageGRs.asp?idcustomer=<%=pcv_IdCustomer%>';">
		<br>
	</td>
</tr>
<%ELSE%>
...
do while not rstemp.eof
	...
	pcv_ParentPrd=rstemp("pcProd_ParentPrd")
	if IsNull(pcv_ParentPrd) or pcv_ParentPrd="" then
		pcv_ParentPrd=0
	end if
	...
	if pcv_Relationship<>"" then
		pcv_Relationship=mid(pcv_Relationship,len(pcv_ParentPrd & "_")+1,len(pcv_Relationship))
		pcv_Relationship=replace(pcv_Relationship,"_","|")
	end if
	...
	if gGC<>"1" then
		...
		<tr onMouseOver="this.className='activeRow'" onMouseOut="this.className='cpItemlist'" class="cpItemlist"> 
			...
			<% else %>
				<a href="FindProductType.asp?id=<%=pcv_ParentPrd%>" target="_blank"><%=gname%></a>
			<% end if %>
				...
				if pcv_ParentPrd>0 and pcv_Relationship<>"" then
					pcv_intOptionsExist = 1
					pcv_Opts=pcv_Relationship
				end if
			
				If pcv_intOptionsExist = 1 Then
					%>
					(<a href="#" onClick="newWindow2('ggg_options_popup.asp?IDEvent=<%=gIDEvent%>&Apparel=<%=pcv_Apparel%>&ParentPrd=<%=pcv_ParentPrd%>&idOptionArray=<%=pcv_Opts%>&idproduct=<%=pidProduct%>&epID=<%=geID%>','window2')">Add/Edit Options</a>)
					<%
				End If
				%>
				<%'Show Product Options
			
				if pcv_Opts<>"" and pcv_ParentPrd=0 then
				%>
				<br>
				<%
...
<tr> 
	<td colspan="5"> 
		<input type="submit" class="btn btn-primary" name="submit1" value=" Update Registry ">&nbsp;
		<input type="submit" class="btn btn-primary" name="submit2" value=" Remove Selected Products ">&nbsp;
		<input type="button" class="btn btn-default"  name="modify" value=" Modify Details " onclick="javascript:location='ggg_EditGR.asp?IDEvent=<%=gIDEvent%>&idcustomer=<%=pcv_IdCustomer%>';">&nbsp;
		<input type="button" class="btn btn-default"  name="addnew" value=" Add New Products " onclick="javascript:location='ggg_addtoGR.asp?IDEvent=<%=gIDEvent%>&idcustomer=<%=pcv_IdCustomer%>';">&nbsp;
		<input type="button" class="btn btn-default"  name="back" value=" Back " onclick="javascript:location='ggg_manageGRs.asp?idcustomer=<%=pcv_IdCustomer%>';">
