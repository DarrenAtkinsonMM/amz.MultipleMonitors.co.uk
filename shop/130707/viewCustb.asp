...
<form name="form1" method="post" action="viewCustb.asp" class="pcForms">
		...
		<% If iPageCount>1 Then %>
			...
			<p class="pcPageNav">
				<%if iPageCurrent > 1 then %>
					<a href="viewCustb.asp?customerType=<%=request("customerType")%>&key1=<%=request("key1")%>&key2=<%=request("key2")%>&key3=<%=request("key3")%>&key4=<%=request("key4")%>&key5=<%=request("key5")%>&key6=<%=request("key6")%>&key7=<%=request("key7")%>&key8=<%=request("key8")%>&key9=<%=request("key9")%>&key10=<%=request("key10")%>&key11=<%=request("key11")%>&mode=<%=pMode%>&iPageCurrent=<%=iPageCurrent-1%>&order=<%=strORD%>&sort=<%=strSort%>"><img src="../pc/images/prev.gif" width="10" height="10" border="0"></a>
				<% end If
				For I=1 To iPageCount
					If Cint(I)=Cint(iPageCurrent) Then %>
						<%=I%> 
					<% Else %>
						<a href="viewCustb.asp?customerType=<%=request("customerType")%>&key1=<%=request("key1")%>&key2=<%=request("key2")%>&key3=<%=request("key3")%>&key4=<%=request("key4")%>&key5=<%=request("key5")%>&key6=<%=request("key6")%>&key7=<%=request("key7")%>&key8=<%=request("key8")%>&key9=<%=request("key9")%>&key10=<%=request("key10")%>&key11=<%=request("key11")%>&mode=<%=pMode%>&iPageCurrent=<%=I%>&order=<%=strORD%>&sort=<%=strSort%>"><%=I%></a> 
					<% End If %>
				<% Next %>
				<% if CInt(iPageCurrent) < CInt(iPageCount) then %>
					<a href="viewCustb.asp?customerType=<%=request("customerType")%>&key1=<%=request("key1")%>&key2=<%=request("key2")%>&key3=<%=request("key3")%>&key4=<%=request("key4")%>&key5=<%=request("key5")%>&key6=<%=request("key6")%>&key7=<%=request("key7")%>&key8=<%=request("key8")%>&key9=<%=request("key9")%>&key10=<%=request("key10")%>&key11=<%=request("key11")%>&mode=<%=pMode%>&iPageCurrent=<%=iPageCurrent+1%>&order=<%=strORD%>&sort=<%=strSort%>"><img src="../pc/images/next.gif" border="0" width="10" height="10"></a>
				<% end If %>
			</p>
		<% End If %>
	</td>
...
<tr>
	<td align="center">
		...
		<input type="button" class="btn btn-default"  value="New Search" onClick="location.href='viewCusta.asp'">
	</td>
</tr>
</table>
</form>
