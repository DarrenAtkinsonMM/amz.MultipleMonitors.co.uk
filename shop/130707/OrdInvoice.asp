...
IF request.Form("GO")="Generate" THEN
	...
	'DA Edit - Do VAT Charge Check
	boolVAT = CheckVAT(pcv_IsEUMemberState, pshippingCountryCode, CustomerVATID)
	
	While Not rs.EOF %>
		<table border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
			<tr> 
				<td width="57%" colspan="2" height="71" valign="top">
					<img src="/images/logo-inv.jpg" height="120px" width="410px" class="inv-logo">
				</td>
				...
				<td rowspan="2" width="50%" valign="top">
					<table align="right" width="95%" cellpadding="5" cellspacing="0" class="invoice">
						...
						if CustomerVATID <> "" then
							response.write "<tr><td class=""invoice"">VAT #: " & CustomerVATID & "</td></tr>"
						end if
                        %>
		...
		'GGG Add-on end%>
			<tr>           
			<td class="invoice">DELIVERY SERVICE:
				<% 
				Shipper=""
				Service=""
				Postage=""
				...
				If pSRF="1" then
					...
					if ubound(shipping)>1 then
						...
						if len(Service)>0 then
							Service=pcf_GetShipServiceName(Service,1)
  ...
  '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		for k=0 to intArryCnt
			...
			if instr(pTempDiscountDetails,"- ||") then
				...
				if discountType<>"" then %>
					<tr>
						<td class="invoice">DISCOUNT: <%=discountType%></td>
					</tr>
				<% end if
			end if 
		Next %>
	...
	<br>
	<table id="tablePrds" width="100%" border="0" cellpadding="0" cellspacing="0" align="center">
		<tr class="nobreak"> 
			<td class="nobreak">
			<table width="100%" cellpadding="5" cellspacing="0" border="1" class="invoice">
					...
					<% query="SELECT ProductsOrdered.idProductOrdered,ProductsOrdered.idProduct, ProductsOrdered.quantity, ProductsOrdered.pcPrdOrd_SelectedOptions, ProductsOrdered.pcPrdOrd_OptionsPriceArray, ProductsOrdered.pcPrdOrd_OptionsArray, ProductsOrdered.unitPrice, ProductsOrdered.xfdetails"
					...
					Do until rsTemp.eof
						...
						IF (intPackingSlip<>1) OR (request("Ord" & qry_ID & "_" & pidProductOrdered)="1") THEN
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error in OrdDetails: "&err.description) 
		...
		%>
					...
					<tr> 
						...
						<% If intPackingSlip<>1	Then 'Hide this information if this is a packing slip %>
							<div align="right"><%=scCurSign&money(punitPrice1/1.2)%></div>
						<% End if %>
							</td>
						<td width="12%" class="invoice">
						<% If intPackingSlip<>1	Then 'Hide this information if this is a packing slip %>
							<div align="right"><%=scCurSign&money(pRowPrice1/1.2)%></div>
						<% End if %>
						</td>
					</tr>
					<% 'CONFIGURATOR ADDON-S
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error in OrdDetails: "&err.description) 
				...
				For pcv_intOptionLoopCounter = 0 to pcv_intOptionLoopSize %>
					...
					<tr> 
						...
						if tempPrice="" or tempPrice=0 then
							...
							%>
							<td width="16%" class="invoice"><% if intPackingSlip <>1 then %><div align="right"><%=scCurSign&money(tempPrice/1.2)%></div><% end if %></td>	
							<td width="12%" class="invoice">
								<% if intPackingSlip <>1 then %>
									<div align="right">			 
									<%
									tAprice=(tempPrice*Cdbl(pquantity))
									response.write scCurSign&money(tAprice/1.2) 
									%>
									</div>
								<% end if %>
								</td>
	...
	set rstemp=nothing%>
					...
					<tr>
						...
						<td width="16%" class="invoice"><div align="right"><b>DELIVERY:</b></div></td>
						<td width="12%" class="invoice"><div align="right">
						<%
						'if boolVAT = 1 then 
							response.write(scCurSign&money(postage/1.2))
						'else
							'response.write(scCurSign&money(postage))
						'end if
						%>
                        </div></td>
					</tr>
	...
	<% end if %>
					...
					<% if instr(pdiscountDetails,"- ||") or (pcv_CatDiscounts>"0") then
						...
						Next %>
					<tr> 
						<td width="16%" class="invoice"><div align="right"><b>DISCOUNTS:</b></div></td>
						<td width="12%" class="invoice"><div align="right">-<%=scCurSign&money((discount+pcv_CatDiscounts)/1.2)%></div></td>
					</tr>
					<% end if %>
					<%if GCAmount>"0" then%>
					<tr> 
					...
					<%end if%>
					<tr> 
						<td width="16%" class="invoice"><div align="right"><b>SUBTOTAL:</b></div></td>
						<td width="12%" class="invoice"><div align="right">
						<%
						if boolVAT = 1 then 
							response.write(scCurSign&money(ptotal/1.2))
						else
							response.write(scCurSign&money(ptotal))
						end if
						%>
                        </div></td>
					</tr>
					<tr> 
						<td width="16%" class="invoice"><div align="right"><b>VAT:</b></div></td>
						<td width="12%" class="invoice"><div align="right">
						<%
						if boolVAT = 1 then 
							response.write(scCurSign&money(ptotal-(ptotal/1.2)))
						else
							response.write(scCurSign&money(0))
						end if
	...
	<form action="OrdInvoice.asp" method="post" name="invoice" class="pcForms" onSubmit="if (this.oformat[1].checked) this.action='OrdInvoicePDF.asp'; return(true);">
		<table class="pcCPcontent">
			...
			<tr>
				...
				<tr>
					<td>
					<input type="radio" name="packingSlip" id="packingSlip1" value="1" onClick="javascript:showPPick();" class="clearBorder"> Packing Slip<br>
					<input type="radio" name="packingSlip" id="packingSlip2" value="0" onClick="javascript:showPPick();" checked class="clearBorder"> Invoice
					<script type="text/javascript">
					function showPPick()
					{
						if (document.getElementById("packingSlip1").checked)
						{
							document.getElementById("PPick").style.display='';
						}
						else
						{
							document.getElementById("PPick").style.display='none';
						}
					}
					</script>
					</td>
			...
			<%query="SELECT ProductsOrdered.idProductOrdered, Products.Description FROM Products INNER JOIN ProductsOrdered ON Products.idProduct=ProductsOrdered.idProduct WHERE ProductsOrdered.idOrder=" & qry_ID & " ORDER BY Products.Description;"
			Set rs=Server.CreateObject("ADODB.Recordset")
			Set rs=connTemp.execute(query)
			If not rs.eof then
				pcArr=rs.getRows()
			...
			<tr>
			<td><table name="PPick" id="PPick" style="display:none">
			<tr>
				<td class="pcCPspacer"></td>
			</tr>
				<tr>
					<td><b>Select Product(s) for Packing Slip</b></td>
				</tr>
			<%For i=0 to intCount%>
				<tr>
					<td>&nbsp;&nbsp;<input type="checkbox" name="Ord<%=qry_ID%>_<%=pcArr(0,i)%>" value="1" checked>&nbsp;<%=pcArr(1,i)%></td>
				</tr>
			<%Next%>
			<%End if
			set rs=nothing%>
			...
			<tr>
				...
				<tr>
					<td align="center"><input type="submit" name="GO" value="Generate" class="btn btn-primary"></td>
				</tr>
			</table>
			<script type="text/javascript">
				showPPick();
