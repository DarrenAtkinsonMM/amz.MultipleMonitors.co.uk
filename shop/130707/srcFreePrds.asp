...
response.redirect "techErr.asp?error="& Server.Urlencode("Error deleting a product in QueryE") 
							...
							call pcs_hookProductRemoved(pcv_ID, "")
					end if
				end if
			Next
		end if
...
if request("action")="deleteall" then
	query="SELECT products.idProduct FROM products WHERE ((products.idProduct) NOT IN (SELECT categories_products.idProduct FROM categories_products)) AND products.removed=0 AND pcprod_ParentPrd=0"
	set rs=Server.CreateObject("ADODB.Recordset")
	set rs=conntemp.execute(query)
	totalDeleted=0
	Do while not rs.eof
		pcv_IDs = pcv_IDs & rs("idProduct") & ","
		totalDeleted=totalDeleted+1
		rs.moveNext
	Loop
	
	pcv_IDs = left(pcv_IDs, Len(pcv_IDs)-1)
	
	' delete from taxPrd - QueryA
	query="DELETE FROM taxPrd WHERE idProduct IN (" & pcv_IDs & ")"
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		set rs=nothing
		call closeDb()
response.redirect "techErr.asp?error="& Server.Urlencode("Error deleting products in QueryA")
	end If
	
	' delete product from configSpec_products - QueryB
	query="DELETE FROM configSpec_products WHERE configProduct IN (" & pcv_IDs & ")"
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		set rs=nothing
		call closeDb()
response.redirect "techErr.asp?error="& Server.Urlencode("Error deleting products in QueryB")
	end If
	
	' delete product from cs_relationships - QueryC
	query="DELETE FROM cs_relationships WHERE idProduct IN (" & pcv_IDs & ")"
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		set rs=nothing
		call closeDb()
response.redirect "techErr.asp?error="& Server.Urlencode("Error deleting products in QueryC")
	end If
	
	' delete product from categories_products - QueryD
	query="DELETE FROM categories_products WHERE idProduct IN (" & pcv_IDs & ")"
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		set rs=nothing
		call closeDb()
response.redirect "techErr.asp?error="& Server.Urlencode("Error deleting products in QueryD")
	end If
	
	' delete product from products table - QueryE
	query="UPDATE products SET active=0, removed=-1 WHERE idproduct IN (" & pcv_IDs & ")"
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		set rs=nothing
		call closeDb()
response.redirect "techErr.asp?error="& Server.Urlencode("Error deleting products in QueryE")
	end If
...
query="SELECT DISTINCT products.idProduct, products.description, products.sku, products.active FROM products WHERE ((products.idProduct) Not in (SELECT categories_products.idProduct FROM categories_products)) AND products.removed=0 AND pcprod_ParentPrd=0 ORDER BY products.description"
...
%>
<form name="freePrds" action="srcFreePrds.asp" method="post" class="pcForms">
...
<% ELSE 
    ...
    <script type=text/javascript>
		function checkAll()
		{
			...
			{
				box = eval("document.freePrds.C" + j); 
...
%>
	<tr>
        ...
        <input type="button" name="DeleteAll" value="Delete ALL Orphaned Products" onclick="JavaScript:if(confirm('You are about to remove ALL orphaned products from your Control Panel. This action cannot be undone. Are you sure you want to complete this action?')) location='srcFreePrds.asp?action=deleteall'" class="btn btn-primary">&nbsp;
        <%
		end if
		%>
        <input type="button" class="btn btn-default"  value="Manage Categories" onClick="document.location.href='manageCategories.asp'">
        </td>
	</tr>
</table>
