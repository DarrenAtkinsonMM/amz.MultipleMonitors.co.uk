...
<table class="pcCPcontent">
	<tr>
		<th colspan="2">UPS&reg; Developer Kit Shipment Confirmation</th>
	</tr>
	<tr>
...
<%
	...
	if UPS_TESTMODE="1" then
		...
		UPS_URL="https://onlinetools.ups.com/ups.app/xml/ShipAccept"
	end if
	...
	if NOT len(pcv_strErrorMsg)>0 then
		...
		For Each Node In Nodes
			...
			query="SELECT idcustomer,orderDate FROM orders WHERE idOrder="& pcv_intOrderID
			Set rs=Server.CreateObject("ADODB.Recordset")
			Set rs=conntemp.execute(query)
			...
			personalmessage=replace(personalmessage,"<ORDER_ID>",(scpre + int(pcv_intOrderID)))
