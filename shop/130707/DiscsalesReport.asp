...
<html>
<head>
	<title>Online Sales Report - Sales by Discount Code (Electronic Coupon)</title>
...
do while not rs1.eof
	...
	%>
	<tr> 
		<td colspan="8"><h2>Total Sales recorded <% if strTDateVar <> "" and strTDateVar2 <> "" then %>from <%=strTDateVar%> to <%=strTDateVar2%><% end if %><%if DiscountName<>"" then%>
<br>Discount code: <a href="modDiscounts.asp?mode=Edit&iddiscount=<%=IDDiscountTemp%>" target="_blank" title="View discout details">
<strong><%=DiscountName%></strong></a> - <%=DiscountCode%><%end if%></h2></td>
</tr>
<tr> 
...
<tr>         
	<td colspan="7" align="right"><i>Note: VAT is included in the order total</i></td>
</tr>
<%end if%>
<tr>         
	<td colspan="7">&nbsp;</td>
