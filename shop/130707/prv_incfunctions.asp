...
if (svalue="0") or (svalue=null) or (svalue="") then
	...
	For l=1 to tmp3 %>
		<img class="pcReviewRateIcon" src="../pc/catalog/<%=pcv_Img3%>" alt="Empty Star">
	<%
	Next
		if (Round(Rev_rating)-Rev_Rating<0) or (Round(Rev_rating)-Rev_Rating=0.5) then
		%>
			<img class="pcReviewRateIcon" src="../pc/catalog/<%=pcv_Img4%>" alt="Half Star">
		<%
		tmp3=tmp3+1
		else
			if Round(Rev_rating)-Rev_Rating>0 then
		...
		For l=tmp3+1 to 5
		%>
			<img class="pcReviewRateIcon" src="../pc/catalog/<%=pcv_Img5%>" alt="Full Star">
		<%
		Next
