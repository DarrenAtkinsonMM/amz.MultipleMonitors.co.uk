...
<%
Dim tempfrom, tempfromSub

idFlatShipType = getUserInput(request("idFlatShipType"), 0)
If len(idFlatShipType)>0 Then
    Session("adminidFlatShipType") = idFlatShipType
End If
...
If sMode="Save" Then
	...
	Set rstemp=Server.CreateObject("ADODB.Recordset")	
	mySQL="UPDATE FlatShipTypes SET FlatShipTypeDesc=N'"& Session("adminFlatShipTypeDesc") &"', WQP='"& Session("adminFlatShipTypePref") &"', FlatShipTypeDelivery='"& Session("adminFlatShipTypeDelivery") &"', startIncrement="&Session("adminstartIncrement")&" WHERE idFlatShipType="& Session("adminidFlatShipType")
    rstemp.Open mySQL, connTemp
	
	mySQL="UPDATE shipService SET serviceDescription=N'"& replace(Session("adminFlatShipTypeDesc"),"'","''")&"' WHERE serviceCode='C"& Session("adminidFlatShipType")&"';"
	rstemp.Open mySQL, connTemp
	Set rstemp=Nothing
	...
	i=i+1
	loop
	referURL = request("refer") & "?idFlatShipType=" & request("idFlatShipType") & "&panel=4"
	if referURL="" then
...
<table class="pcCPcontent">
	...
	<tr> 
        ...
        <td colspan="2"><input type="text" name="FlatShipTypeDesc" value="<%=rstemp("FlatShipTypeDesc")%>"> <span class="pcSmallText">Note: Do not enter commas.</span>
		</td>
	</tr>                
    <tr> 
    ...
    <tr> 
        <td colspan="4">
            Based on: <b><%=VarBased%></b> <%=VarInput%> 
            <input type="hidden" name="idFlatShipType" size="40" value="<%=idFlatShipType %>"> 
        </td>
    </tr>
	  <% if VarIncrem=1 then %>
      <tr> 
