...
Dim strTDateVar, strTDateVar2, DateVar, DateVar2, dateRange, pIDProduct
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error calculating order total: "&Err.Description) 
     ...
     'retrieve product name
      if pIDProduct<>"" then
        ...
        'serviceSpec=rs1("serviceSpec")
			...
			pageTitle = pageTitle&" for "& PrdName
		' Count total orders except for incomplete
			Dim pcvIntTotalOrders
			query = "SELECT Count(*) AS intTotal FROM orders, ProductsOrdered WHERE ProductsOrdered.idProduct = "& pIDProduct &" AND ProductsOrdered.idOrder = orders.idorder AND ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<=9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) AND " & dateRange
			Set rsTemp = Server.CreateObject("ADODB.Recordset")
			rsTemp.Open query, conntemp
    ...
    ' Get shipment Info
			query="SELECT orders.shipmentDetails, ProductsOrdered.unitPrice, ProductsOrdered.quantity, orders.idorder FROM orders, customers, ProductsOrdered WHERE ProductsOrdered.idProduct = "& pIDProduct &" AND ProductsOrdered.idOrder = orders.idOrder AND orders.idcustomer=customers.idcustomer AND ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<=9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) AND" & dateRange & " ORDER BY "& strORD &" "& strSort
			set totalTemp=Server.CreateObject("ADODB.Recordset")     
			totalTemp.Open query, conntemp

		' Get orders placed by this customer
			query="SELECT ProductsOrdered.unitPrice, ProductsOrdered.quantity, orders.idorder, orderDate, total, orderstatus,orders.pcOrd_PaymentStatus,orders.comments,orders.admincomments,orders.details,orders.rmaCredit, customers.name, customers.lastName, orders.idcustomer, orders.shipmentDetails FROM orders, customers, ProductsOrdered WHERE ProductsOrdered.idProduct = "& pIDProduct &" AND ProductsOrdered.idOrder = orders.idorder AND ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<=9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) AND orders.idcustomer=customers.idcustomer AND " & dateRange & " ORDER BY "& strORD &" "& strSort
			set rstemp=Server.CreateObject("ADODB.Recordset")     
...
%>
  ...
  gTotal=0
  gTotalQuantity=0
  do while not totalTemp.eof
    pUnitPrice = totalTemp("unitPrice")
    pQuantity = totalTemp("quantity")
	  gTotal=gTotal + (pUnitPrice*pQuantity)
	  gTotalQuantity=gTotalQuantity + pQuantity
    totalTemp.movenext
  loop
  set totalTemp = nothing
    ...
    pCustomerID = rstemp("idcustomer")
		...
		ptotal=rstemp("unitPrice")*rstemp("quantity")
    pQty=rstemp("quantity")
    ...
    <td valign="top" width="5%" align="center"><%=pQty%></td>
    <td valign="top" ><%response.write(scCurSign & money(ptotal))%></td>
	...
	<tr>
	<td colspan="6">
	<form method="post" action="viewPrdDateOrders.asp" name="" class="pcForms">
	...
	If iPageCount <> 1 then
		For I=1 To iPageCount
			If I=iPageCurrent Then %>
				<%=I%> 
			<% Else %>
				<a href="viewPrdDateOrders.asp?IDProduct=<%=pIDProduct%>&FromDate=<%=strTDateVar%>&ToDate=<%=strTDateVar2%>&status=<%=pOrderStatus%>&iPageCurrent=<%=I%>&order=<%=strORD%>&sort=<%=strSort%>"><%=I%></a> 
			<% End If %>
		<% Next %>
	<% end if %>
	<% if CInt(iPageCurrent) <> CInt(iPageCount) then
	'We are not at the end, show a next link %>
		<a href="viewPrdDateOrders.asp?IDProduct=<%=pIDProduct%>&FromDate=<%=strTDateVar%>&ToDate=<%=strTDateVar2%>&status=<%=pOrderStatus%>&iPageCurrent=<%=iPageCurrent+1%>&order=<%=strORD%>&sort=<%=strSort%>"><img src="../pc/images/next.gif" border="0" width="10" height="10"></a> 
	<% end If %>
	</form>
	</td>
	</tr>
	...
	<table class="pcCPcontent">
		<tr>
			<td width="33%">Number of orders:<strong>&nbsp;&nbsp;<%=pcvIntTotalOrders%></strong></td>
      <td width="33%">Qty. Ordered:<strong>&nbsp;&nbsp;<%=gTotalQuantity%></strong></td>
			<td width="33%">Totals:<strong>&nbsp;&nbsp;<%=scCurSign & money(gTotal)%></strong></td>
		</tr>
</table>
<% 
END IF
