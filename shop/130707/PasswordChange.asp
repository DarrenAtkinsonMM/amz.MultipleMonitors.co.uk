...
response.redirect "PasswordChange.asp?message="&server.URLEncode("An error occurred while updating your administrative User ID and Password. Your current User ID appeared to be incorrect.")
	...
	npassword=request("pass2")
	cpassword=request("Cpass")
	
	
	set rs=server.createobject("adodb.recordset")
	query="SELECT * FROM admins WHERE idadmin="& cidAdmin &" AND ID=" & session("IDAdmin")
	set rs=conntemp.execute(query)
	tmpResult="false"
	if not rs.eof then
		tmpHash=rs("adminpassword")
		tmpResult=pcf_CheckPassH(cpassword,tmpHash)
	end if
	...
	query="UPDATE admins SET idadmin="&nidAdmin&", adminpassword='"& pcf_PasswordHash(npassword) &"' WHERE idadmin="& cidAdmin
	set rs=conntemp.execute(query)
...
ELSE %>
		...
		<tr>
            ...
            <td>
                ...
                <input name="back" type="button" class="btn btn-default"  value="Start Page" onClick="document.location.href='menu.asp'">
            </td>
		</tr>
	</table>
</form>
