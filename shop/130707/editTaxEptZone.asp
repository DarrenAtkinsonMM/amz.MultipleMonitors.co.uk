...
<td><%if Count<>0 then%>
  ...
  &nbsp;
  <input type="submit" name=submit2 value=" Delete " onclick="javascript:if (confirm('You are about to remove selected tax rules from your database. Are you sure you want to complete this action?')) {return(true)} else {return(false)}" class="btn btn-default">&nbsp;
  <%end if%>
  <input type="button" class="btn btn-default" name=addnew value=" Add New Exemption Rules " onclick="location='AddTaxEptZone.asp';">
  &nbsp;
  <input type="button" class="btn btn-default" name=goback value=" Back " onclick="location='manageTaxEptZone.asp';">
