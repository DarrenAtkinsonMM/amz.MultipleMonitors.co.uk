...
query="SELECT StateCode,StateName,pcCountryCode FROM States ORDER BY "& strORD &" "& strSort
set rs=Server.CreateObject("ADODB.Recordset")
Set rs=connTemp.execute(query)
...
<table class="pcCPcontent">
	...
	<tr>
		<td colspan="5">Uncheck the checkboxes of the states that you would like to remove, then click on &quot;Update&quot; &nbsp;|&nbsp;<a href="AddStates.asp">Add New State</a>&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=437"></a></td>
	</tr>
	<tr>
...
<%else
	...
	For i=0 to intCount
	Count=Count + 1
	StateCode=tmpArr(0,i)
	StateName=tmpArr(1,i)
	CountryCode=tmpArr(2,i)
%>
...
<tr>
	<td colspan="4" class="cpLinksList">
		<%if Count>0 then%><a href="javascript:checkAll();">Check All</a> | <a href="javascript:uncheckAll();">Uncheck All</a> | <%end if%><a href="AddStates.asp">Add New State</a>
	</td>
</tr>
<tr>
	<td colspan="4"><hr></td>
...
<tr>
	<td colspan="4">
		...
		<%if Count>0 then%><input name="submit" type="submit" class="btn btn-primary" value="Remove unchecked">
		&nbsp;<%end if%>
		<input type="button" class="btn btn-default"  name="Button" value="Back" onClick="javascript:history.back()">
		&nbsp;
		<input type="button" class="btn btn-default"  name="Restore" value="Restore Default Settings" onClick="javascript:location='restoreStates.asp?action=update'">
	</td>
</tr>   
</table>
</form>
