...
response.redirect "sm_manage.asp"
		...
		if statusAPP="1" then
			tmpPrdCountSub=cmd.Parameters("@SMCOUNTSUB")
		else
			tmpPrdCountSub=0
		end if
		...
		if statusAPP="1" then
			tmpPrdCountORG=cmd.Parameters("@SMCOUNT")+cmd.Parameters("@SMCOUNTSUB")
		else
	...
	<td>
		...
		<%if tmpPrdCount>0 then%>
			<% If statusAPP="1" OR scAPP=1 Then %>
				The sale will affect <b><%=tmpPrdCount%></b> product(s)<%If tmpPrdCountSub>"0" then%> and <b><%=tmpPrdCountSub%></b> sub-product(s)<%end if%> in your store.<br><br>
			<% Else %>
	...
	<% if (clng(tmpPrdCountORG)>clng(tmpPrdCount)+clng(tmpPrdCountSub)) AND (clng(tmpPrdCount)+clng(tmpPrdCountSub)>0) then %>
	<tr>
		<td>
			<div class="pcCPmessageInfo">
				...
				Of the <strong><%=tmpPrdCountORG%></strong> product(s) that you had selected, only <strong><%=tmpPrdCount+tmpPrdCountSub%></strong> can be placed on sale at this time.
			</div>
		</td>
	</tr>
	<%else%>
	<% if clng(tmpPrdCount)+clng(tmpPrdCountSub)=0 then %>
	<tr>
		<td>
	...
	<tr>
		<td>
			<% if clng(tmpPrdCount)+clng(tmpPrdCountSub)>0 then %>
				<input type="button" class="btn btn-default" name="Run" value="Start Sale Now" onclick="javascript:if (confirm('You are about to start this sale. The price changes you have configured will now be applied to the selected products, and will immediately be live in your storefront. Do you want to continue?')) location='sm_start.asp?a=run&id=<%=pcSaleID%>';" class="btn btn-primary">
			<%else%>
				<input type="button" class="btn btn-default" name="Back" value="View/Edit Pending Sales" onclick="location='sm_manage.asp';" class="btn btn-primary">
			<%end if%>
		</td>
	</tr>
	</table>	
...
IF request("a")="run" THEN
	...
	query="INSERT INTO pcSales_Completed (pcSales_ID,pcSC_Status,pcSC_StartedDate,pcSC_SaveName,pcSC_SaveDesc,pcSC_SaveTech,pcSC_SaveIcon) VALUES (" & pcSaleID & ",1,'" & dtTodaysDate & "',N'" & replace(tmpSaleName,"'","''") & "',N'" & replace(tmpSaleDesc,"'","''") & "','" & replace(tmpSaleDetails,"'","''") & "','" & tmpImgURL & "');"
	set rs=connTemp.execute(query)
	set rs=nothing
	...
	%>
	<img src="images/pc_checkmark_sm.gif" width="16" height="16" alt="Task performed"> Preparing to update <b><%=tmpPrdCount+tmpPrdCountSub%></b> product(s)...<br><br>
	<%
	tmpPrdCount=tmpPrdCountSub+tmpPrdCount
...
<tr>
	<td>
		<input type="submit" name="Preview" value=" Continue " class="btn btn-primary">
	</td>
</tr>
</table>
</form>
