...
form_CustType=getUserInput(request("CustType"),0)
...
<% ELSE%>
	...
	<input type="hidden" name="CustType" value="<%=form_CustType%>">
...
<p style="text-align: center;">
	<%if (src_ToPage<>"") and (pcv_HaveResults="1") and (src_DisplayType>"0") then
		...
		<input type="button" class="btn btn-default"  value="<%=src_Button2%>" onClick="javascript:if (savelist=='xml,') {alert('Please select a customer before clicking on this button');} else {location.href='<%=src_ToPage%>custlist='+getcustlist(savelist);}">
	<%end if%>
