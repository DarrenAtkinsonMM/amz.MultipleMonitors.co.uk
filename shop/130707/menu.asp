...
'// START Check Tax File
If (ptaxfile=1) AND (ptaxfilename<>"") then
	'get the file name
	dim Filename
	...
	Dim Filepath
	Filepath=Server.MapPath("..\pc\") & "\tax\" & Filename
	if NOT FSO.FileExists(Filepath) then
		msg="<li>The Tax file: <b>" & ptaxfilename & "</b> does not exist on the store's folder: ""pc/tax/"". Please check and resolve this issue.</li>"
	end if
End if
'// END
...
if not rs.eof then
	...
	if tmpTotal>10000 then
		msg=msg & "<li>The Saved Shopping Carts table exceeds 10,000 records<br><a href='PurgeSavedCarts.asp'>Click here</a> if you want to clear it.</li>"
	end if
end if
set rs=nothing
'// END
...
if (not isNull(findUser(pcUserArr,2,pcUserArrCount))) or (session("PmAdmin")="19") then
	...
	set rs=connTemp.execute(query)
	if not rs.eof then
		msg=msg & "<li>There are pending <strong>product reviews</strong> awaiting approval. <a href=prv_ManageRevPrds.asp?nav=1>Manage reviews &gt;&gt;</a></li>"
	end if
	set rs=nothing
end if
'// END

call pcs_hookPreCPanelMenu() '// Pre Menu Event Hook
...
IF (session("PmAdmin")="19") OR (tmpRights=1) THEN %> 
	<table class="pcCPcontent">
		<tr>
            ...
            <div class="panel panel-default" style="background-image:url(images/pcv4_icon_sales.gif); background-position: 10px -10px; background-repeat:no-repeat; min-height: 180px;">
			...
			<td width="45%" valign="top">
            	<div class="panel panel-default" style="border: 1px solid #e1e1e1; padding: 5px; background-image: url(images/pcv4_icon_people.png); background-position: 10px -10px; background-repeat:no-repeat; min-height: 180px;">
	...
	<table class="pcCPcontent">    
		...
		<tr>
			...
			<td valign="top" width="50%">
				...
				<div class="panel panel-default" style="background-image:url(images/pcv4_icon_inventoryAdded.gif); background-position: 10px -10px; background-repeat:no-repeat; min-height: 200px;">
        ...
        <tr>
            ...
            <div class="panel panel-default" style="background-image:url(images/pcv4_icon_helpDesk.png); background-position: 10px -10px; background-repeat:no-repeat; min-height: 200px;">
