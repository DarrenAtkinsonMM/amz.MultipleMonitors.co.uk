...
if (request("action")="upd") and (pcv_PackageID<>"0") then
	...
	if tmp_ShippedDate<>"" then
		query="UPDATE pcPackageInfo SET pcPackageInfo_ShipMethod=N'" & tmp_ShipMethod & "',pcPackageInfo_TrackingNumber='" & tmp_TrackingNumber & "',pcPackageInfo_ShippedDate='" & dtShippedDate & "',pcPackageInfo_Comments=N'" & tmp_Comments & "' WHERE pcPackageInfo_ID=" & pcv_PackageID & ";"
	else
		query="UPDATE pcPackageInfo SET pcPackageInfo_ShipMethod=N'" & tmp_ShipMethod & "',pcPackageInfo_TrackingNumber='" & tmp_TrackingNumber & "',pcPackageInfo_Comments=N'" & tmp_Comments & "' WHERE pcPackageInfo_ID=" & pcv_PackageID & ";"
	end if
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=connTemp.execute(query)
...
<%if msg<>"" then%>
	...
	<tr>
		<td>
		<p align="center">
			<input type="button" class="btn btn-default" name="Close Window" value="Close Window" onClick="javascript:window.close();">
		</p>
		</td>
	</tr>
	</table>
...
response.redirect "techErr.asp?err="&pcStrCustRefID
				...
				if not rs.eof then
					...
					<tr>
						<td>&nbsp;</td>
						<td>
							<input type="submit" name="submit1" value="Update Shipment Information" class="btn btn-primary">
							&nbsp;<input type="button" class="btn btn-default" name="Close Window" value="Close Window" onClick="javascript:window.close();">
