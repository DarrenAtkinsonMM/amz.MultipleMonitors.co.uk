...
Sub DELALL(tmpIdProduct)
	' delete from taxPrd
	query="DELETE FROM taxPrd WHERE idProduct=" &tmpIdProduct
	...
	query="DELETE FROM configSpec_products WHERE configProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	...
	query="DELETE FROM configSpec_categories WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	...
	query="DELETE FROM cs_relationships WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	...
	query="DELETE FROM categories_products WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	...
	query="DELETE FROM wishList WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	...
	query="DELETE FROM options_optionsGroups WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	...
	query="DELETE FROM configSessions WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	...
	query="DELETE FROM discountsPerQuantity WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	...
	query="SELECT idOrder FROM ProductsOrdered WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		set rs=nothing
		call closeDb()
	...
	query="DELETE FROM products WHERE idProduct=" &tmpIdProduct
	set rs=conntemp.execute(query)
...
If request("purgeproduct")<>"" then
	...
	call DELALL(pIdProduct)
	
	query="SELECT idproduct FROM products WHERE pcprod_ParentPrd=" & pIdProduct
	set rs=conntemp.execute(query)
...
<p style="margin-top: 8px;">To completely remove products from the database, enter the product ID or select the product by name using the drop down list below. Only products that have been <strong>deleted</strong> from the Control Panel are shown. If you wish to permanently remove all products, you can use the <a href="purgeAllProducts.asp"><strong>Purge All Products</strong></a> feature.</p>
	...
	<tr> 
		<td align="center">
		<input name="purgeproduct" type="submit" id="purgeproduct" value="Permanently Remove Selected Product" class="btn btn-primary">
		&nbsp;
