	...
	if pcv_Values<>"" then
		...
		For k=lbound(pcArray) to ubound(pcArray)
			if trim(pcArray(k))<>"" then
				...
				query="INSERT INTO pcRevBadWords (pcRBW_word) VALUES (N'" & pcArray(k) & "');"
				set rs=server.CreateObject("ADODB.RecordSet")
				set rs=connTemp.execute(query)
			end if
		Next
...
<form method="POST" action="prv_ManageBadWords.asp?action=add" name="checkboxform" class="pcForms">
    ...
    <table class="pcCPcontent">
        ...
        <tr>
            <td>
            <input type="button" class="btn btn-default"  value="Product Reviews Settings" onClick="location.href='PrvSettings.asp'">&nbsp;
