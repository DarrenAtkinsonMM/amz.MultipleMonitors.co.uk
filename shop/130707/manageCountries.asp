...
query="SELECT CountryName,CountryCode FROM Countries ORDER BY "& strORD &" "& strSort
Set rs=Server.CreateObject("ADODB.Recordset")
Set rs=connTemp.execute(query)
%>
...
<table class="pcCPcontent">
	...
	<tr>
		<td colspan="3">Uncheck the checkboxes of the coutries that you would like to remove, then click on &quot;Update&quot; &nbsp;|&nbsp;<a href="AddCountries.asp">Add New Country</a>&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=437"></a></td>
	</tr>
	<tr>
...
<%else
	...
	For i=0 to intCount
	Count=Count + 1
	CountryName=tmpArr(0,i)
	CountryCode=tmpArr(1,i)%>
...
set rs=nothing%>
	...
	<tr>
		<td colspan="3" class="cpLinksList">
			...
			<%if Count>0 then%><a href="javascript:checkAll();">Check All</a> | <a href="javascript:uncheckAll();">Uncheck All</a> | <%end if%><a href="AddCountries.asp">Add New Country</a> 
		</td>
	</tr> 
	<tr>
	...
	<tr>
		<td colspan="3">
			<%if Count>0 then%><input name="submit" type="submit" value="Update" class="btn btn-primary">&nbsp;<%end if%>
			<input type="button" class="btn btn-default"  name="Button" value="Back" onClick="javascript:history.back()">&nbsp;
			<input type="button" class="btn btn-default"  name="Restore" value="Restore Default Country List" onClick="javascript:location='restoreCountries.asp?action=update'">
		</td>
	</tr> 
</table>
</form>
