		...
		For k=1 to Count
			if request("C" & k)="1" then
				...
				if request("submit1")<>"" then
					query="UPDATE pcRevFields SET pcRF_Name=N'" & pcv_Name & "',pcRF_Type=" & pcv_Type & ",pcRF_Active=" & pcv_Active & ",pcRF_Required=" & pcv_Required & ",pcRF_Order=" & pcv_Order & " WHERE pcRF_IDField=" & pcv_ID
...
<form method="POST" action="prv_FieldManager.asp?action=update" name="checkboxform" class="pcForms">
	<table class="pcCPcontent">
		...
		<tr>
            <td colspan="8">
                <input type="button" class="btn btn-default"  value="Add New" onClick="location.href='prv_AddField.asp'">
                <%if count>0 then%>
                <input type="submit" value="Update" name="submit1" class="btn btn-primary">&nbsp;
