...
if err.number <> 0 then
  	call closeDb()
    response.redirect "techErr.asp?error="&Server.UrlEncode("Error in page srcCATs.asp Error: "&err.description)
end If
iPageCount=0
...
<p style="text-align: center;">
	<%if (src_ToPage<>"") and (pcv_HaveResults="1") and (src_DisplayType>"0") then
		...
		<input type="button" class="btn btn-default"  value="<%=src_Button2%>" onClick="javascript:if (savelist=='xml,') {alert('Please select a category before clicking on this button');} else {location.href='<%=src_ToPage%>catlist='+getcatlist(savelist);}">
	<%end if%>
