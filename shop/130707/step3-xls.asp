...
For i=1 to validfields
	...
	session("importT" & i)=request("T" & i)
Next

if (session("append")="1") then
	if (R1=0) then
		session("ImportMsg")="Please make sure that the SKU field is mapped."
		call closeDb()
		response.redirect "step2-xls.asp"
	end if
else
	if r1+r2+r3+r4<>4 then
		session("ImportMsg")="Please make sure that the following fields are mapped:<br>1. SKU<br>2. Name<br>3. Description<br>4. Online Price"
		call closeDb()
		response.redirect "step2-xls.asp"
	end if
end if
...
if herror=true then
	session("ImportMsg")="Some of the mapping instructions are overlapping. Please make sure that the database fields are mapped uniquely."
	call closeDb()
	response.redirect "step2-xls.asp"
end if
if (D2=1) and (D1<>1) then
	session("ImportMsg")="Please map the 'Product Type' field before import/append any Downloadable Product Fields."
	call closeDb()
	response.redirect "step2-xls.asp"
end if
if (S2=1) and (S1<>1) then
	session("ImportMsg")="Please map the 'Drop-Shipper ID' field before map the 'Drop-Shipper is also a Supplier' field."
	call closeDb()
	response.redirect "step2-xls.asp"
end if
...
<tr>
	<td colspan="2">
		...
		<input type="button" class="btn btn-default"  name="backstep" value="<< Back to Step 2" onClick="location='step2-xls.asp?a=1<%=mtemp%>';">&nbsp; 
		<input type="submit" name="submit" value="Go to Step 4 >>" onClick="pcf_Open_Import();" class="btn btn-primary">
		<%
        '// Loading Window
