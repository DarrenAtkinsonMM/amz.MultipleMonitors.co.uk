...
if request("submit4")<>"" then
	...
	if rstemp.eof then
		query="INSERT INTO pcGWSettings (pcGWSet_Show,pcGWSet_Overview,pcGWSet_HTML,pcGWSet_OverviewCart,pcGWSet_HTMLCart) VALUES (" & pcv_intGGGShow & "," & pcv_intGGGshowtext & ",N'" & pcv_details & "'," & pcv_intGGGshowtextCart & ",N'" & pcv_detailsCart & "')"
		set rstemp=connTemp.execute(query)
	else
		query="UPDATE pcGWSettings set pcGWSet_Show=" & pcv_intGGGShow & ",pcGWSet_Overview=" & pcv_intGGGshowtext & ",pcGWSet_HTML=N'" & pcv_details & "',pcGWSet_OverviewCart=" & pcv_intGGGshowtextCart & ",pcGWSet_HTMLCart=N'" & pcv_detailsCart & "';"
		set rstemp=connTemp.execute(query)
	end if
	set rstemp=nothing
end if
...
if request("submit1")<>"" then
	...
	For i=1 to Count
		if request("Opt" & i)="1" then
			...
			query="update pcGWOptions set pcGW_OptName=N'" & OptName & "',pcGW_OptImg='" & OptImg & "',pcGW_OptPrice=" & OptPrice & ",pcGW_OptActive=" & OptActive & ",pcGW_OptOrder=" & OptOrder & " where pcGW_IDOpt=" & IDOpt
			set rstemp=connTemp.execute(query)
			set rstemp=nothing
		end if
	Next
...
<table class="pcCPcontent">
  ...
  <tr>
    	<td colspan="2">
			<textarea class="htmleditor" name="detailsCart" id="detailsCart" cols="80" rows="5"><%=pcv_detailsCart%></textarea>
		</td>
	</tr>
	<tr> 
		<td colspan="2">
  ...
  <tr>
    	<td colspan="2">
			<textarea class="htmleditor" name="details" id="details" cols="80" rows="5"><%=pcv_details%></textarea>
		</td>
	</tr>
	<tr>
		<td colspan="2" class="pcCPspacer"></td>
	...
	<tr> 
		<td colspan="2">
			<input name="submit4" type="submit" class="btn btn-primary" value="Update Settings">
		</td>
	</tr>
	<tr>
		<td colspan="2" class="pcCPspacer"></td>
	...
	<td colspan="2" valign="top">
		...
		<tr>
			<td colspan="6" style="padding: 10px;">
				<%if Count>0 then%>
					<input name="submit1" type="submit" value="Updated Selected" class="btn btn-primary">
					&nbsp;
					<input name="submit2" type="submit" value="Remove Selected" class="btn btn-primary">
					&nbsp;
				<%end if%>
				<input name="New" type="button" class="btn btn-default"  value="Add New" onClick="location='ggg_AddGWOpt.asp';">
	...
	<tr>
		<td colspan="2" style="padding: 10px;">
			<%if Count1>0 then%>
				<input name="submit3" type="submit" value="Remove Selected" class="btn btn-primary">
				&nbsp;
			<%end if%>
			<input name="newpro" type="button" class="btn btn-default"  value="Add New" onClick="location='ggg_addPrdExc.asp';">
