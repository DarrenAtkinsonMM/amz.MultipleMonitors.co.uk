...
<table class="pcCPcontent">
    ...
    <tr>
		...
		if iStart > 0 then
			'Show Prev link
			Response.Write "<A HREF=""AdminDiscounts.asp?list="&request("list")&"&order="&request("order")&"&sort="&request("sort")&"&Start=" & iStart-iOffset &"&Offset=" & iOffset & """>Previous " & iOffset & "</A>"
			showPipe = 1
		  end if
		  ...
		  if iStop < iRows then
			...
			'Show Next link
			Response.Write " <A HREF=""AdminDiscounts.asp?list="&request("list")&"&order="&request("order")&"&sort="&request("sort")&"&Start=" & iStart+iOffset &"&Offset=" & iOffset & """>Next " & iOffset & "</A>"
		  end if
		  %>
	</td>
    </tr>
	...
	<tr>
		<td colspan="10">
    	<form class="pcForms">
			<input type="button" class="btn btn-default"  value="Add New Discount" onClick="location.href='AddDiscounts.asp'" class="btn btn-primary">
			<%
			...
			if not rs.eof then
				if request("list")="archive" then%>
					&nbsp;<input type="button" class="btn btn-default"  name="archive" value="View Current Discount Codes" onClick="location.href='AdminDiscounts.asp'">
				<%else%>
					&nbsp;<input type="button" class="btn btn-default"  name="archive" value="View Archived Discount Codes" onClick="location.href='AdminDiscounts.asp?list=archive'">
				<%end if
			end if
			set rs=nothing
