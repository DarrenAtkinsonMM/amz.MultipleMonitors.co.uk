...
dim CSVRecord(100),f, rstemp1,TopRecord(100)
...
End Function
	...
	Do While TotalFilelines+1<=ubound(ReadLine)
...
if RecordError=false then 'STEP 1
	...
	IF session("append")="1" then
		IF testCat=1 then 'EXISTING Category
			...
			if catNameID>-1 then
				temp4=temp4 & ",categoryDesc=N'" & newCatName & "'"
			end if
			...
			if catSDescID>-1 then
				temp4=temp4 & ",SDesc=N'" & catSDesc & "'"
			end if
			
			if catLDescID>-1 then
				temp4=temp4 & ",LDesc=N'" & catLDesc & "'"
			end if
			...
			if catMTTitleID>-1 then
				temp4=temp4 & ",pcCats_MetaTitle=N'" & catMTTitle & "'"
			end if
			
			if catMTDescID>-1 then
				temp4=temp4 & ",pcCats_MetaDesc=N'" & catMTDesc & "'"
			end if
			
			if catMTKeyID>-1 then
				temp4=temp4 & ",pcCats_MetaKeywords=N'" & catMTKey & "'"
			end if
			...
			if temp4<>"" then
				CanUpdate=1
				if catParentNameID>-1 OR catParentIDID>-1 then
					CanUpdate=CheckUpdateParent(catID,tmpIDParent)
					IF CanUpdate=0 THEN
						if catName<>"" then
							ErrorsReport=ErrorsReport & "Record " & TotalXLSlines & ": The Category Name: '" & catName & "' cannot be sub-category of its own sub-category." & vbcrlf
						else
							ErrorsReport=ErrorsReport & "Record " & TotalXLSlines & ": The Category ID: " & catID & " cannot be sub-category of its own sub-category." & vbcrlf
						end if
						RecordError=true
					END IF
				end if
	...
	ELSE 'Append=0
		...
		if RecordError=false then
			...
			query=query & "(" & tmpIDParent & ",N'" & newCatName & "'," & catOrder & ",'" & catSImg & "','" & catLImg & "'," & catHideCat & ",N'" & catSDesc & "',N'" & catLDesc & "'," & catNotShowDesc & "," & catHideCatRetail & "," & catDisplayCats & "," & catCatsPerRow & "," & catCatRows & ",'" & catDisplayPrds & "'," & catPrdsPerRow & "," & catPrdRows & "," & catFeaturedID & "," & catUseCatImg & ",'" & catPrdDisplayOpt & "',N'" & catMTTitle & "',N'" & catMTDesc & "',N'" & catMTKey & "');"
			'query=replace(query,chr(34),"&quot;")
			'query=replace(query,"**DD**",chr(34))
			set rstemp=conntemp.execute(query)
...
<tr>
	<td valign="top">
		...
		<% end if %>
		<br /><br />
		<input type="button" class="btn btn-default"  name="mainmenu" value="Manage Categories" onClick="location='manageCategories.asp';">
	</td>
</tr>
</table>
<%session("append")=0%>
