...
Function GenHTML()
	...
	if(DataEmpty=1) then
		...
		do until rstemp.eof
			...
			'***** Column 7 *****
			query="select name,lastname,customerCompany,phone,email FROM customers where idcustomer=" & pcv_idCustomer
			...
			if pcv_CustPhone<>"" then
				Response.Write("Phone: " & pcv_CustPhone & "<br>")
			end if
			if pcv_CustEmail<>"" then
				Response.Write("Email: " & pcv_CustEmail)
			end if
			Response.Write("&nbsp;</TD>")
...
Function CreateCSVFile()
	...
	if (DataEmpty=0) then
		...
		do until rstemp.eof
			...
			if pcv_CustPhone<>"" then
				tmpS1=tmpS1 & "Phone: " & pcv_CustPhone & vbcrlf
			end if
			if trim(pcv_CustEmail)<>"" then
...
pcv_pcOrd_CatDiscounts="0"
			...
			if pcv_CustPhone<>"" then
				StringBuilderObj.append  "Phone: " & pcv_CustPhone & vbcrlf
			end if
			if trim(pcv_CustEmail)<>"" then
				StringBuilderObj.append  "Email: " & pcv_CustEmail
			end if
