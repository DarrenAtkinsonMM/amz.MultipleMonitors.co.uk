...
%>
<form action="srcPrds.asp" name="formreturn" method="post" class="pcForms">
<input type="hidden" name="prdlist" value="">
<p style="text-align: center;">
	<%Select Case session("sm_ShowNext")
	Case "1":%>
		<input type="button" class="btn btn-default"  name="Go" value="Continue" onClick="location='sm_addedit_S3.asp';" class="btn btn-primary">&nbsp;
	<%Case "2":%>
		<input type="button" class="btn btn-default"  name="Go" value=" Save Sale " onClick="location='sm_addedit_S5.asp';" class="btn btn-primary">&nbsp;
	<%Case "3":%>
		<input type="button" class="btn btn-default"  name="Go" value=" Back " onClick="location='sm_manage.asp';" class="btn btn-primary">&nbsp;
	<%Case "4":%>
		<input type="button" class="btn btn-default"  name="Go" value=" Back " onClick="location='sm_start.asp?a=start&id=<%=session("sm_pcSaleID")%>';" class="btn btn-primary">&nbsp;
		
	<%End Select%>
	<%if (src_ToPage<>"") and (pcv_HaveResults="1") and (src_DisplayType>"0") then%>
		<input type="button" class="btn btn-default"  value="<%=src_Button2%>" onClick="javascript:if (savelist=='xml,') {alert('Please select a product before clicking on this button');} else {if (confirm('Are you sure you want to complete this action?')) {document.formreturn.action='<%=src_ToPage%>';document.formreturn.prdlist.value=getprdlist(savelist);document.formreturn.submit();}}" class="btn btn-primary">
	<%end if%>
	<%if src_FromPage<>"" and ((src_IDSDS="") or (src_IDSDS="0")) then%>
	&nbsp;<input type="button" class="btn btn-default"  value="<%=src_Button3%>" onClick="location.href='<%=src_FromPage%>'">
	<%end if%>
</p>
</form>
<%if (src_IDSDS<>"") and (src_IDSDS<>"0") then%>
	...
	<table class="pcCPcontent">
		<tr>
			<td align="center">
            <form class="pcForms">
            	<input type="button" class="btn btn-default"  onClick="javascript:AssignPrds();" value="Assign products to the <%if src_PageType="0" then%>supplier<%else%>drop-shipper<%end if%>">&nbsp;
                <input type="button" class="btn btn-default"  onClick="location.href='globalChanges.asp'" value="Move products">&nbsp;
                <input type="button" class="btn btn-default"  onClick="javascript:ExportPrds();" value="Export products">
             </form>
             </td>
		</tr>
	</table>
