...
IF request("submit2")<>"" THEN
	...
	if not rsQ.eof then
		...
		query="INSERT INTO pcSearchData (idSearchField,pcSearchDataName,pcSearchDataOrder) VALUES (" & idSearchField & ",N'" & SFV_name & "'," & SFV_order & ");"
		set rsQ=connTemp.execute(query)
		set rsQ=nothing
	...
	IF request("Count")<>"" then
		For k=1 to clng(Count)
			if request("C"&k)="1" then
				...
				query="UPDATE pcSearchData SET pcSearchDataName=N'" & SFV_name & "',pcSearchDataOrder=" & SFV_order & " WHERE idSearchData=" & SFV_id & ";"
				set rsQ=connTemp.execute(query)
				set rsQ=nothing
			...
			<% Else
              ...
              <a href="javascript: if (confirm('Are you sure you want to permanently delete this custom field value from the database? <%if pcv_HavePrds=1 then%>It will also be removed from all products to which it had been assigned.<%end if%>?')) {location='delSearchFieldValue.asp?idSearchField=<%=idSearchField%>&idSearchData=<%=SFV_id%>';}" title="Delete"><img src="images/pcIconDelete.jpg" alt="Delete"></a>
                ...
                <td colspan="5">
                    ...
                    <%if count<>"" then%>
                    <input name="submit1" type="submit" value="Update Selected" size="20" class="btn btn-primary">
                    <%end if%> 
                    <input type="button" class="btn btn-default"  value="Back" onClick="location='ManageSearchFields.asp';">
                </td>
                </tr>
				<tr>
					<td colspan="5" class="pcCPspacer"><hr></td>
			...
			<tr>
				<td colspan="5">
					...
					<tr>       
						<td>&nbsp;</td>
						<td>
							<input type="submit" name="submit2" value="Add New" class="btn btn-primary" onClick="javascript: if (document.form1.new_valuename.value=='') {alert('Please enter a value in the New Value field'); document.form1.new_valuename.focus();return(false)} else {return(true)};"> 
                            <input type="button" class="btn btn-default"  value="Back" onClick="location='ManageSearchFields.asp';">
						</td>
					</tr>
					</table>
