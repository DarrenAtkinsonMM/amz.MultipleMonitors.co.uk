	...
	<form name="rating" method="post" action="prv_EditReviewB.asp?action=add" onSubmit="return Form1_Validator(this)" class="pcForms">
		...
		<table class="pcCPcontent">
			...
			<%For i=0 to FCount-1
				if (Fi(i)<>"1") and (Fi(i)<>"2") then%>
					<tr>
						...
						<td>
						<%IF FType(i)="0" THEN%>
							<input type="text" size="45" name="Field<%=Fi(i)%>" value="<%=Server.HTMLEncode(FValue(i))%>" onchange="javascript:TestURL(this);" onfocus="javascript:TestURL(this);" onblur="javascript:TestURL(this);" onclick="javascript:TestURL(this);" onmouseout="javascript:TestURL(this);" onmouseover="javascript:TestURL(this);" onkeypress="javascript:TestURL(this);"> <span id="URL_Field<%=Fi(i)%>" <%if instr(ucase(FValue(i)),"HTTP://")<>1 and instr(ucase(FValue(i)),"HTTPS://")<>1 then%>style="display:none"<%end if%>><input type="button" class="btn btn-default"  name="genField<%=Fi(i)%>" value="Generate URL" onclick="javascript:genURL(document.rating.Field<%=Fi(i)%>);"></span>
						<%END IF%>
						<%IF FType(i)="1" THEN%>
							<textarea class="htmleditor" cols="50" rows="5" name="Field<%=Fi(i)%>" id="Field<%=Fi(i)%>"><%=Server.HTMLEncode(FValue(i))%></textarea>
						<%END IF%>
						<%IF FType(i)="2" THEN%>
			...
			<tr>
				...
				<td>
				<input type="submit" name="submit" value="Update Review" class="btn btn-primary">&nbsp;<input type="button" class="btn btn-default"  name="Del" value="Delete Review" onclick="javascript:if (confirm('You are about to remove this product review from your database. <% = Replace(strPrvPrompt,"'","\'") %>\nAre you sure you want to complete this action?')) location='prv_EditReviewB.asp?action=delete&IDReview=<%=pcv_IDReview%>&IDProduct=<%=pcv_IDProduct%>&nav=<%=pcv_RevName%>'">&nbsp;<input type="button" class="btn btn-default"  name="Back" value="Back" onclick="location='prv_ManageReviews.asp?IDProduct=<%=pcv_IDProduct%>&nav=<%=pcv_RevName%><%
				' PRV41 begin
