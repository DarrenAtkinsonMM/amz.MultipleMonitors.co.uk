...
if request("action")="update" then
   ...
   if CP="2" then
	...
	if CP="11" then
		...
		pcv_tmp1=0
		' Standard = yes | Configurable = no | Configurable-Only Item = no -> STANDARD ONLY
		...
		' Standard = no | Configurable = yes | Configurable-Only Item = no -> CONFIGURABLE ONLY
		if (request("pcv_prdtype1")="") and (request("pcv_prdtype2")<>"") and (request("pcv_prdtype3")="") then
			query=query & " AND products.serviceSpec<>0"
			pcv_tmp1=2
		end if
		' Standard = no | Configurable = no | Configurable-Only Item = yes -> CONFIGURABLE ITEM ONLY
		if (request("pcv_prdtype1")="") and (request("pcv_prdtype2")="") and (request("pcv_prdtype3")<>"") then
			query=query & " AND products.configOnly<>0"
			pcv_tmp1=3
		end if
		' Standard = yes | Configurable = yes | Configurable-Only Item = no -> STANDARD and CONFIGURABLE
		if (request("pcv_prdtype1")<>"") and (request("pcv_prdtype2")<>"") and (request("pcv_prdtype3")="") then
			query=query & " AND products.configOnly=0"
			pcv_tmp1=4
		end if
		' Standard = yes | Configurable = no | Configurable-Only Item = yes -> STANDARD and CONFIGURABLE ITEM
		if (request("pcv_prdtype1")<>"") and (request("pcv_prdtype2")="") and (request("pcv_prdtype3")<>"") then
			query=query & " AND products.serviceSpec=0"
			pcv_tmp1=5
		end if
		' Standard = no | Configurable = yes | Configurable-Only Item = yes -> CONFIGURABLE and CONFIGURABLE ITEM
		if (request("pcv_prdtype1")="") and (request("pcv_prdtype2")<>"") and (request("pcv_prdtype3")<>"") then
			query=query & " AND (products.serviceSpec<>0 OR products.configOnly<>0)"
			pcv_tmp1=6
		end if
		' Standard = no | Configurable = no | Configurable-Only Item = no
		if (request("pcv_prdtype1")="") and (request("pcv_prdtype2")="") and (request("pcv_prdtype3")="") then
			pcv_tmp1=7
		end if
		' Standard = yes | Configurable = yes | Configurable-Only Item = yes
		if (request("pcv_prdtype1")<>"") and (request("pcv_prdtype2")<>"") and (request("pcv_prdtype3")<>"") then
			pcv_tmp1=8
		end if
		if scBTO<>1 then
			...
			Select Case pcv_tmp1
				...
				Case 2: TempStr1=TempStr1 & "configurable products "
				Case 3: TempStr1=TempStr1 & "configurable-only items "
				Case 4: TempStr1=TempStr1 & "standard products or configurable products "
				Case 5: TempStr1=TempStr1 & "standard products or configurable-only items "
				Case 6: TempStr1=TempStr1 & "configurable products or configurable-only items "
				Case 8: TempStr1=TempStr1 & "standard products or configurable products or configurable-only items "
			End Select
		end if
	...
	if ((UP="1") and (instr(request("priceSelect"),"CC_")>0)) OR ((UP="2") and (instr(request("priceSelect1"),"CC_")>0)) then
		...
		query=query & smSalesQuery & " UNION (SELECT DISTINCT products.idProduct,products.price,products.listprice,products.btoBprice FROM Products WHERE products.pcProd_ParentPrd IN (SELECT DISTINCT Products.idproduct FROM " & query1(1) & ") AND products.removed=0 " & smSalesQuery & ")"
	else
	...
	If statusAPP="1" OR scAPP=1 Then
		if request("pcv_incSPs")="1" then
			TempStr1=TempStr1 & "<br>" & "Parent Products and Sub-products"
		end if		
		if request("pcv_incSPs")="2" then
			TempStr1=TempStr1 & "<br>" & "Sub-products only"
		end if
	End If
	...
	if UP="1" then
		...
		Case "4": TempStr2="Change Online Price Differential"
		Case "5": TempStr2="Change Wholesale Price Differential"
		Case Else:
	...
	if UP="2" then
		...
		Case "4": TempStr2="Make the online price differential " & request("wprice") & "% "
		Case "5": TempStr2="Make the wholesale price differential " & request("wprice") & "% "
		Case Else:
		...
		'End SDBA
		Case "5": TempStr2=TempStr2 & "of the online price differential."
		Case "6": TempStr2=TempStr2 & "of the wholesale price differential."
		Case Else:
	...
	if UP="3" then
		...
		Case "5":
			TempStr2="The 'Non Shipping Product' option will be assigned to the selected products."	
		Case "6":
		...
		Case "22":
			...
			TempStr2=TempStr2 & "<i>* Note: This option affects Gift Certificates only</i>"
		Case "23":
			TempStr2="The 'Hide Price' option will be assigned to the selected products.<br>"
			TempStr2=TempStr2 & "<i>* Note: This option affects Configurable Products only</i>"
		Case "24":
		...
		Case "31":
			TempStr2="The 'Show Configurator+ conflict alert messages' option will be assigned to the selected products."
		Case "32":
			TempStr2="The option 'Hide Additional Images' will be assigned to the selected products."
		End Select
	end if
	if UP="4" then
		...
		Case "5":
			TempStr2="The 'Non Shipping Product' option will be removed from the selected products."	
		Case "6":
		...
		Case "17":
			...
			TempStr2=TempStr2 & "<i>* Note: This option affects Gift Certificates only</i>"
		Case "18":
			TempStr2="The 'Hide Price' option will be removed from the selected products.<br>"
			TempStr2=TempStr2 & "<i>* Note: This option affects Configurable Products only</i>"
		Case "19":
		...
		Case "31":
			TempStr2="The 'Show Configurator+ conflict alert messages' option will be removed from the selected products."
		Case "32":
			TempStr2="The option 'Hide Additional Images' will be removed to the selected products."
		End Select
	end if
	if UP="5" then
		...
		Case "2":
			TempStr2="The type of selected products will be changed to 'Configurable Product'."
		Case "3":
			TempStr2="The type of selected products will be changed to 'Configurable-Only Item'."
		End Select
	end if
	if UP="6" then
...
<table class="pcCPcontent">
		...
		<input type=hidden name="pcv_incSPs" value="<%=request("pcv_incSPs")%>">
		<tr>
		...
		<tr>
			<td align="center">
				<input type="submit" name="submit" value="Yes, apply these changes" onClick="pcf_Open_globalChanges();" class="btn btn-primary">&nbsp;
				<input type="button" class="btn btn-default"  name="back" value="No, return to the previous page" onClick="javascript:history.back()">
				<%
                '// Loading Window
	...
	if (theForm.UP1.value == "1")
  	{
			...
			<% If statusAPP="1" OR scAPP=1 Then %>
			var tmpValue=theForm.priceSelect.value;
			if (tmpValue.indexOf("CC_")==-1)
			{
			if ((eval(theForm.priceSelect.value)>3) && (eval(theForm.pcv_incSPs.value)<2))
		   {
				alert("You can change Price Differentials for sub-products only.");
				theForm.priceSelect.focus();
				return (false);
		   }
		   if ((eval(theForm.priceSelect.value)<=3) && (eval(theForm.pcv_incSPs.value)==2) && (eval(theForm.priceSelect.value)!=2))
		   {
				alert("You can change Price Differentials and List Price for sub-products only.");
				theForm.pcv_incSPs.focus();
				return (false);
		   }
		   }
			<% End If %>
	...
	if (theForm.UP1.value == "2")
  	{
		    ...
		    <% If statusAPP="1" OR scAPP=1 Then %>
		    var tmpValue=theForm.priceSelect1.value;
		    var tmpValue1=theForm.priceSelect2.value;
			if (tmpValue.indexOf("CC_")==-1)
			{
		    if ((eval(theForm.priceSelect1.value)>3) && (eval(theForm.pcv_incSPs.value)<2))
		   {
				alert("You can change Price Differentials for sub-products only.");
				theForm.priceSelect1.focus();
				return (false);
		   }
		   if ((eval(theForm.priceSelect1.value)<=3) && (eval(theForm.pcv_incSPs.value)==2) && (eval(theForm.priceSelect1.value)!=2))
		   {
				alert("You can change Price Differentials and List Price for sub-products only.");
				theForm.pcv_incSPs.focus();
				return (false);
		   ...
		   //if ((tmpValue.indexOf("CC_")==-1) && (tmpValue1.indexOf("CC_")==-1))
			//{
		   //if ((eval(theForm.priceSelect1.value)>3) && (eval(theForm.priceSelect2.value)==2))
		   //{
			//	alert("Sub-products don't have List Price.");
			//	theForm.priceSelect2.focus();
			//	return (false);
		   //}
		   //}
		   if (tmpValue1.indexOf("CC_")==-1)
			{
		   if ((eval(theForm.priceSelect2.value)>4) && (eval(theForm.pcv_incSPs.value)<2))
		   {
				alert("Parent Products don't have Price Differentials.");
				theForm.priceSelect2.focus();
				return (false);
		   }
		   }
		   if ((tmpValue.indexOf("CC_")==-1) && (tmpValue1.indexOf("CC_")==-1))
			{
		   if ((eval(theForm.priceSelect1.value)<=3) && (eval(theForm.priceSelect2.value)>4))
		   {
				alert("You can change Price Differentials with Price Differentials only.");
				theForm.priceSelect2.focus();
				return (false);
		   }
		   }
...
<tr>
	...
	<td>
		...
		<%if scBTO=1 then%>
		&nbsp;&nbsp;&nbsp;<input name="pcv_prdtype2" type="checkbox" value="1" <% if section="services" then %> checked<% end if %> class="clearBorder">Configurable Products
		&nbsp;&nbsp;&nbsp;<input name="pcv_prdtype3" type="checkbox" value="1" class="clearBorder">Configurable-Only Items
		<%end if%>
...
<tr>
	<td colspan="2">Include: 
		<select name="pcv_incSPs" size="1">
			<option value="1" selected>Parent Products & sub-products</option>
			<option value="2">Sub-products Only</option>
		</select>
	</td>
</tr>
<tr>
...
<tr>
	...
	<td>Change the&nbsp;            
		<select name="priceSelect" id="priceSelect" size="1">
			...
			<% If statusAPP="1" OR scAPP=1 Then %>
			<option value="4">Online Price Differential</option>
			<option value="5">Wholesale Price Differential</option>
			<% End IF %>
		...
		&nbsp;&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=403"></a>
	</td>
</tr>
<tr>
	<td>&nbsp;</td>
...
<tr>
	...
	<td valign="top">Recalculate the&nbsp; 
		...
		<select name="priceSelect2" size="1">
			...
			<%'End SDBA%>
			<% If statusAPP="1" OR scAPP=1 Then %>
			<option value="5">Online Price Differential</option>
			<option value="6">Wholesale Price Differential</option>
			<% End If %>
			<%tmp_HavePricingCAT=0
			query="Select idcustomerCategory, pcCC_Name FROM pcCustomerCategories WHERE pcCC_CategoryType<>'ATB' ORDER BY pcCC_Name asc"
			set rstemp4=connTemp.execute(query)
		...
		&nbsp;&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=402"></a>
	</td>
</tr>
<tr>
	<td>&nbsp;</td>
...
<tr>
	...
	<td>Assign the following option:
		<select name="coption" id="optionSelect" size="1">
			...
			<option value="5">Non Shipping Product</option>
			...
			<%if scBTO=1 then%>
			<option value="23">Hide Price (Configurable Product)</option>
			<option value="24">Hide Default Configuration (Configurable Product)</option>
			<option value="25">Disallow purchasing - Show Prices (Configurable Product)</option>
			<option value="26">Disallow purchasing - Hide Prices (Configurable Product)</option>
			<option value="27">Skip Product Details Page (Configurable Product)</option>
			<%end if%>
			<option value="30">Image Magnifier (MojoZoom)</option>
            <option value="32">Hide Additional Images</option>
			<% If statusCM="0" OR scCM=1 Then %>            
			<option value="31">Show Configurator+ conflict alert messages</option> 
			<% End If %>           
		</select>
	</td>
</tr>
...
<tr>
	...
	<td>Unassign the following option:
		<select name="roption" id="optionSelect" size="1">
			...
			<%if scBTO=1 then%>
			<option value="18">Hide Price (Configurable Product)</option>
			<option value="19">Hide Default Configuration (Configurable Product)</option>
			<option value="20">Disallow purchasing (Configurable Product)</option>
			<option value="21">Skip Product Details Page (Configurable Product)</option>
			<%end if%>
			<option value="24">Image Magnifier (MojoZoom)</option>
            <option value="32">Hide Additional Images</option>
			<% If statusCM="0" OR scCM=1 Then %>
			<option value="31">Hide Configurator+ conflict alert messages</option>
			<% End If %>           
		</select>
	</td>
</tr>
...
<tr>
	...
	<td>Change Product Type:
		<select name="ptoption" id="optionSelect" size="1">
			...
			<option value="3">Configurable-Only Item</option>
		</select>
	</td>
</tr>
<%end if%>
