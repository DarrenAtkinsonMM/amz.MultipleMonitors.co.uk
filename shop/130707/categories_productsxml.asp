...
If nTotalRecs > 0 Then
	...
	Do While (Not rs.Eof) And (nRecCount < nStopRec)
		...
		If CLng(nRecCount) >= CLng(nStartRec) Then
			x_idProduct = rs("idProduct")
			x_Description = replace(replace(replace(rs("description"),"""","&quot;"),"<","&lt;"),">","&gt;")
			tmpPrdCatList=tmpPrdCatList & "<option value=""" & x_idProduct & """>" & x_Description & "</option>" & VbCrLf
		End If
		rs.MoveNext
	Loop
End If
