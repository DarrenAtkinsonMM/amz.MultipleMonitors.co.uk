...
%>
        ...
        <tr> 
            <td colspan="3">
            <%if Count<>0 then%>
            <input type="submit" name="submit2" value="Delete Selected" onclick="javascript:if (confirm('You are about to remove selected tax rules from your database. Are you sure you want to complete this action?')) {return(true)} else {return(false)}" class="btn btn-primary">&nbsp;
            <%end if%>
            <input type="button" class="btn btn-default"  name="addnew" value="Add New Tax Exemption Rule" onclick="location='AddTaxEpt.asp';">&nbsp;
			...
			%>
            <input type="button" class="btn btn-default"  name="back" value="Manage Taxes" onClick="document.location.href='<%=pcvBackURL%>';">
