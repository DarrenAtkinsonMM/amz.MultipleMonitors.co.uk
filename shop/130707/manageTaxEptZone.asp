...
<tr> 
<td colspan="3">
<input type="button" class="btn btn-default"  name="addnew" value="Add New Zone-based Tax Exemption" onclick="location='AddTaxEptZone.asp';" class="btn btn-primary">&nbsp;
<%if Count<>0 then%>
<input type="submit" name=submit2 value=" Delete Selected " onclick="javascript:if (confirm('You are about to remove selected tax rules from your database. Are you sure you want to complete this action?')) {return(true)} else {return(false)}" class="btn btn-default">&nbsp;
<%end if%>
<input type="button" class="btn btn-default"  name="goback" value="Back" onclick="location='viewTax.asp'">
