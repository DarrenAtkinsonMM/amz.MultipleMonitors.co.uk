...
response.redirect "AddCustomCardPaymentOpt.asp?msg=5"
		...
		'insert new type
		query="INSERT INTO customCardTypes (customCardDesc) VALUES (N'"&replace(customCardDesc,"'","''")&"');"
		set rstemp=conntemp.execute(query)
		if err.number <> 0 then
			set rstemp=nothing
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error in AddCustomCardPaymentOpt: "&Err.Description) 
		...
		query="INSERT INTO customCardRules (idCustomCardType,ruleName,intRuleRequired,intLengthOfField,intMaxInput,intOrder) VALUES ("&idCustomCardType&",N'"&replace(ruleName,"'","''")&"',"&Creq&","&lengthOfField&","&maxInput&",0);"
		set rstemp=conntemp.execute(query)
		if err.number <> 0 then
			set rstemp=nothing
...
<form name="form" method="post" action="AddCustomCardPaymentOpt.asp" class="pcForms">
	...
	<tr>
		<td colspan="2">Process orders when they are placed: <input type="checkbox" name="pcv_processOrder" value="1">&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=301"></a></td>
	</tr>
	<tr>
