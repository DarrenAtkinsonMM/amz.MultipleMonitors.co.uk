...
query="SELECT x_Login,x_Password,x_testmode FROM pcPay_AuthorizeDPM Where id=1;"
set rs=server.CreateObject("ADODB.RecordSet")
set rs=conntemp.execute(query)

dim x_Login, x_Password, x_testmode
x_Login=rs("x_Login")
...
For r=1 to checkboxCnt
	...
	IF request.Form("checkOrd"&r)="YES" THEN
		...
		'===========================================
		query="SELECT authorders.idOrder, authorders.idauthorder, authorders.amount, authorders.transtype, authorders.authcode, authorders.ccnum,  authorders.ccexp, authorders.idCustomer, authorders.fname, authorders.lname, authorders.address, authorders.zip, authorders.pcSecurityKeyID, orders.orderDate, orders.orderStatus, orders.gwTransID, orders.stateCode, orders.state, orders.city, orders.countryCode, orders.shippingAddress, orders.shippingStateCode, orders.shippingState, orders.shippingCity, orders.shippingCountryCode, orders.shippingZip, orders.ShippingFullName, orders.address2, orders.shippingCompany, orders.shippingAddress2, customers.name, customers.lastName, customers.customerCompany, customers.phone, customers.email FROM customers INNER JOIN authorders INNER JOIN orders ON authorders.idOrder = orders.idOrder ON authorders.idCustomer = customers.idcustomer AND customers.idcustomer = orders.idCustomer WHERE (authorders.idauthorder = "&pidAuthOrder&");"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		...
		'send pre-auth-capture
			...
			if Dev_Testmode = 1 Then
				xml.open "POST", "https://test.authorize.net/gateway/transact.dll?"& data & "", false
			else
