...
response.redirect "addCFtoPrds.asp?nav="&nav&"&x="&x&"&idproduct="&idproduct&"&message="&Server.UrlEncode("The maximum number of characters for your field must be a numeric value.")        
	...
	query="INSERT INTO xfields (xfield, textarea, widthoffield, maxlength, rowlength, randnum) VALUES (N'"&xfield&"',"&textareainput&","&length&","&maxchars&","&rows&","&randnum&");"
	...
	if idproduct<>"" then
		call closeDb()
response.redirect "addCFtoPrds2.asp?action=apply&prdlist=" & idproduct & ","
	else
		call closeDb()
