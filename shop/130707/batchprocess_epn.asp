...
dim rsEmailInfo, rsCust, pTempProductId

query="SELECT pcPay_EPN_Account, pcPay_EPN_RestrictKey, pcPay_EPN_TestMode, pcPay_EPN_TranType FROM pcPay_EPN WHERE pcPay_EPN_ID=1"
...
'how many checkboxes?
dim checkboxCnt
checkboxCnt=request.Form("EPNcheckboxCnt")
...
For r=1 to checkboxCnt
	...
	IF request.Form("checkOrd"&r)="YES" THEN
		...
		if cdbl(authamount)<cdbl(curamount) then
			...
			stext=stext & "&TransID="&Request.Form("transid"&r)
			stext=stext & "&RestrictKey="&pcPay_EPN_RestrictKey
			stext=stext & "&TranType=Auth2Sale"
			stext=stext & "&Total="&Request.Form("curamount"&r)
			stext=stext & "&Inv="&pIdOrder
			stext=stext & "&HTML=No"
...
'			'store the response
			...
			'sResponse = """YSUCCESSFUL"","""","""",""35"",""20140103144646-080880-35"""
			...
			if sApproval = "Y" then
				...
				query="UPDATE pcPay_EPN_Authorize SET authcode='"& sTransID &"', transtype='Auth2Sale', captured=1 WHERE idauthorder="& pidAuthOrder &";"
			...
			' Get order information from the database
					query="SELECT orders.idcustomer,orders.address,orders.City,orders.StateCode,orders.zip,orders.CountryCode,orders.shippingAddress,orders.shippingCity,orders.shippingStateCode,orders.shippingZip,orders.shippingCountryCode,orders.ShipmentDetails,orders.PaymentDetails,orders.discountDetails,orders.taxAmount,orders.total,orders.comments,orders.ShippingFullName,orders.address2,orders.ShippingCompany,orders.ShippingAddress2,orders.taxDetails,orders.iRewardValue, orders.iRewardPointsCustAccrued, customers.phone, ord_DeliveryDate, ord_VAT, pcOrd_CatDiscounts FROM orders, customers WHERE orders.idcustomer=customers.idcustomer AND orders.idOrder=" &qry_ID
				...
				successData=successData&"Order Number "& idOrder &" was updated successfully<BR>"

			else
				if sApproval = "N" then
					strResultText = "transaction has been declined with the following response: <b>" & mid(sResponse, 3, 16) & "</b><br>"
				else
					strResultText = "the processor was unable to handle your transaction: <b>" & sResponse & "</b><br>"
				end if

				failedCnt=failedCnt+1
				failedData=failedData&"Order Number: "& idOrder &" was NOT updated, " & strResultText & "<BR>"
