...
if request("action")="upd" then
	...
	if not rs.eof then
		...
		query="UPDATE pcHomePageSettings SET pcHPS_FeaturedCount=" & pcIntHPFeaturedCount & ",pcHPS_Style='" & pcStrHPStyle & "'," &_
		    "pcHPS_PageDesc=N'" & pcStrHPDesc & "',pcHPS_ShowSKU=" & pcIntHPShowSKU & ",pcHPS_ShowImg=" & pcIntHPShowImg & "," &_
		...
		query="INSERT INTO pcHomePageSettings (pcHPS_FeaturedCount,pcHPS_Style,pcHPS_PageDesc,pcHPS_First,pcHPS_ShowSKU,pcHPS_ShowImg," &_
		    ...
		    "VALUES (" & pcIntHPFeaturedCount & ",'" & pcStrHPStyle & "',N'" & pcStrHPDesc &"'," & pcIntHPFirst &_
...
if not rs.eof then
	...
	pcStrHPDesc=rs("pcHPS_PageDesc")
	if pcStrHPDesc<>"" then
		pcStrHPDesc=replace(pcStrHPDesc,"""","&quot;")
		pcStrHPDesc=replace(pcStrHPDesc,"<","&lt;")
		pcStrHPDesc=replace(pcStrHPDesc,">","&gt;")
	end if
...
<form action="manageHomePage.asp?action=upd" method="post" name="hForm" class="pcForms" onSubmit="return Form1_Validator(this)">
	<table class="pcCPcontent">
		...
		<tr valign="top">
			...
			<td width="75%">
				<textarea class="htmleditor" id="pcStrHPDesc" name="pcStrHPDesc" cols="50" rows="6"><%=pcStrHPDesc%></textarea>
			</td>
		</tr>	
		<tr> 
			<td colspan="2"><hr></td>
		...
		<tr>
			<td colspan="2">
				<input type="checkbox" name="pcIntHPFirst" <%if pcIntHPFirst<>0 then%>checked<%end if%> value="0" class="clearBorder"> Highlight the first featured product. &nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=203"></a>
			</td>
		</tr>
		<tr> 
			<td colspan="2"><hr></td>
