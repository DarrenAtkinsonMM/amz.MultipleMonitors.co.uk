...
%>								
	...
	<tr> 
		...
		<input type="submit" value="Delete selected" name="submit1" class="btn btn-primary" onclick="return(confirm('You are about to completely remove the selected subscription packages. If you only want to remove one or more products from a specific relationship, click CANCEL and then select EDIT for that subscription package. Click OK to confirm the removal.'));">&nbsp;
		<input type="button" class="btn btn-default"  value="Add New Package" onClick="location.href='sb_CreatePackages.asp'">&nbsp;
        <% If pcv_strHasOrders = "1" Then %>
        <input type="button" class="btn btn-default"  value="Subscription Report" onClick="location.href='sb_ViewSubs.asp?idmain=0'">&nbsp;
        <% End If %>
		<input type="button" class="btn btn-default"  value="Main Menu" onClick="location.href='sb_Default.asp'">&nbsp;
        <input type="button" class="btn btn-default"  value="Edit Settings" onClick="location.href='sb_Settings.asp'">&nbsp;
		</td>
	</tr>
