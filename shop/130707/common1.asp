<%
		...
		if layTopId<>-1 then
			layTop=trim(CSVRecord(layTopId))
		end if
		if layTopLId<>-1 then
			layTopL=trim(CSVRecord(layTopLId))
		end if
		if layTopRId<>-1 then
			layTopR=trim(CSVRecord(layTopRId))
		end if
		if layMiddleId<>-1 then
			layMiddle=trim(CSVRecord(layMiddleId))
		end if
		if layTabsId<>-1 then
			layTabs=trim(CSVRecord(layTabsId))
		end if
		if layBottomId<>-1 then
			layBottom=trim(CSVRecord(layBottomId))
		end if
		'APP-S
		app_Apparel=0
		app_Showout=0
		...
		if ApparelID<>-1 then
			app_Apparel=trim(CSVRecord(ApparelID))
			if app_Apparel<>"" then
			else
				app_Apparel=0
			end if
		...
		if ShowoutID<>-1 then
			app_Showout=trim(CSVRecord(ShowoutID))
			if app_Showout<>"" then
			else
				app_Showout=0
			end if
		...
		if OutMsgID<>-1 then
			app_OutMsg=trim(CSVRecord(OutMsgID))
		end if
		...
		if DTypeID<>-1 then
			app_DType=trim(CSVRecord(DTypeID))
			if app_DType<>"" then
			else
				app_DType=0
			end if
		...
		if ChartTextID<>-1 then
			app_ChartText=trim(CSVRecord(ChartTextID))
		end if
		
		if ChartDescID<>-1 then
			app_ChartDesc=trim(CSVRecord(ChartDescID))
		end if
		
		if ChartImgID<>-1 then
			app_ChartImg=trim(CSVRecord(ChartImgID))
		end if
		
		if ChartIURLID<>-1 then
			app_ChartIURL=trim(CSVRecord(ChartIURLID))
		end if
		'APP-E
		...
		if hideaddimagesid<>-1 then
			hideaddimages=trim(CSVRecord(hideaddimagesid))
		...
		if alttagtextid>-1 then
			palttagtext=trim(CSVRecord(alttagtextid))
		end if
		if activeid>-1 then
		...
		if pricecatsid(0)>-1 then
			pricecats(0)=trim(CSVRecord(pricecatsid(0)))
		end if
		if pricecatsid(1)>-1 then
			pricecats(1)=trim(CSVRecord(pricecatsid(1)))
		end if
		if pricecatsid(2)>-1 then
			pricecats(2)=trim(CSVRecord(pricecatsid(2)))
		end if
		if pricecatsid(3)>-1 then
			pricecats(3)=trim(CSVRecord(pricecatsid(3)))
		end if
		if pricecatsid(4)>-1 then
			pricecats(4)=trim(CSVRecord(pricecatsid(4)))
		end if
