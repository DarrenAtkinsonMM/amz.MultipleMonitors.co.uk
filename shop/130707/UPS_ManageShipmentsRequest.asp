...
function whatPayorTypeSelected(){
	var selectValDom = document.forms['form1'].elements['PayorType'];
	var selectedVal = selectValDom.options[selectValDom.selectedIndex].value;
	if (selectedVal == 'FreightCollect' || selectedVal == 'ConsigneeBilled') {
		document.getElementById('PayorType_table').style.display='none';
	}else{
		document.getElementById('PayorType_table').style.display='';
		if (selectedVal == 'BillThirdParty') {
...
if NOT rs.eof then
	...
	If Request.Form("PayorType") = "" then
		Session("pcAdminUPSPayorType")=rs("pcUPSPref_PaymentMethod")
	Else
		Session("pcAdminUPSPayorType")=Request.Form("PayorType")
	End If
	...
	<tr>
		<td>
			...
			if request.form("submit")<>"" then
				...
				if Session("pcAdminUPSPayorType")="PrePaid" OR Session("pcAdminUPSPayorType")="BillThirdParty" then
					pcs_ValidateTextField	"PayorAccountNumber", true, 0
				else
					pcs_ValidateTextField	"PayorAccountNumber", false, 0
				end if
				if Session("pcAdminUPSPayorType")="BillThirdParty" then
					pcs_ValidateTextField	"PayorCountryCode", true, 0
				else
				...
				Else
					...
					objUPSClass.WriteParent "Shipment", ""
						...
						objUPSClass.WriteParent "PaymentInformation", ""
							...
							if Session("pcAdminPayorCountryCode") = "" then
								if session("pcAdminShipToDiffLocation")="Y" then
									Session("pcAdminPayorCountryCode") = Session("pcAdminShipFromCountryCode")
								else
									Session("pcAdminPayorCountryCode") = UPS_COUNTRY
								end if
							end if
							
							'// Prepaid
							if lcase(Session("pcAdminUPSPayorType"))="prepaid" then
								objUPSClass.WriteParent "Prepaid", ""
									objUPSClass.WriteParent "BillShipper", ""
										objUPSClass.AddNewNode "AccountNumber", Session("pcAdminUPSAccountNumber")
									objUPSClass.WriteParent "BillShipper", "/"
								objUPSClass.WriteParent "Prepaid", "/"
								
							'// Bill Third Party
							elseif lcase(Session("pcAdminUPSPayorType"))="billthirdparty" then
								objUPSClass.WriteParent "BillThirdParty", ""
									objUPSClass.WriteParent "BillThirdPartyShipper", ""
										objUPSClass.AddNewNode "AccountNumber", Session("pcAdminPayorAccountNumber")
										objUPSClass.WriteParent "ThirdParty", ""
											objUPSClass.WriteParent "Address", ""
												objUPSClass.AddNewNode "CountryCode", Session("pcAdminPayorCountryCode")
											objUPSClass.WriteParent "Address", "/"
										objUPSClass.WriteParent "ThirdParty", "/"
									objUPSClass.WriteParent "BillThirdPartyShipper", "/"
								objUPSClass.WriteParent "BillThirdParty", "/"
							...
							'// Consignee Billed
							elseif lcase(Session("pcAdminUPSPayorType"))="consigneebilled" then
								objUPSClass.WriteParent "ConsigneeBilled", ""
								objUPSClass.WriteParent "ConsigneeBilled", "/"
								
							'// Freight Collect
							elseif lcase(Session("pcAdminUPSPayorType"))="freightcollect" then
								objUPSClass.WriteParent "FreightCollect", ""
								objUPSClass.WriteParent "FreightCollect", "/"
...
'					response.End()
					...
					if UPS_TESTMODE="1" then
						...
						UPS_URL="https://onlinetools.ups.com/ups.app/xml/ShipConfirm"
					end if

					call objUPSClass.SendXMLRequest(UPS_AccessRequest & UPS_postdata, UPS_URL)
					...
					if NOT len(pcv_strErrorMsg)>0 then
						...
						<form name="form1" action="pcUPSConfirmLabel.asp" method="post" class="pcForms">
							...
							<p>
								...
								<input name="Continue" type="submit" value="Confirm Shipment" class="btn btn-primary">
							</p>
							<p>&nbsp;</p>
							<p>&nbsp;</p>
				...
				if request("err")<>"" then %>
					<div class="pcCPmessage">
						<img src="images/pcadmin_note.gif" alt="Alert" width="20" height="20"> <%=Session("ErrMsg")%>
					</div>
				<% end if %>
				...
				<table class="pcCPcontent">
					<tr>
						...
						<table class="pcCPcontent">
							...
							<tr>
								<td align="right" valign="top"><b>Bill Shipper:</b></td>
								<td align="left">
							...
							<tr>
							  <td align="left" valign="top" colspan="2">
								<table id="PayorType_table" <% if Session("pcAdminUPSPayorType")="ConsigneeBilled" OR Session("pcAdminUPSPayorType")="FreightCollect" then %>style="display:none"<%end if %>>
									<tr>
                                        <td align="right" valign="top"><b>Billing Account Number:</b></td>
                                        <td align="left">
                                        	<input name="PayorAccountNumber" type="text" id="PayorAccountNumber" value="<%=pcf_FillFormField("PayorAccountNumber", true)%>">
											<%pcs_UPSRequiredImageTag "PayorAccountNumber", true%>
                                        </td>
									</tr>
									<tr id="countryCode" <% if Session("pcAdminUPSPayorType")<>"BillThirdParty" then %>style="display:none"<%end if %>>
                                        <td align="right" valign="top"><b>Billing Country Code:</b></td>
                                        <td align="left">
                                        	<input name="PayorCountryCode" type="text" id="PayorCountryCode" value="<%=pcf_FillFormField("PayorCountryCode", true)%>">
                                        	<%pcs_UPSRequiredImageTag "PayorCountryCode", true%>
                                        </td>
									</tr>
								</table>
							  </td>
...
%>checked<%end if%>>
							...
							<br>
							<a href="javascript:;" onclick="newWindow('UPS_AVPopup.asp?State=<%=strShipToStateCode%>&City=<%=strShipToCity%>&PC=<%=strShipToPostalCode%>&Country=<%=strShipToCountry%>','ProductWindow')">UPS&reg; Developer Kit Address Validation</a><br>
...
may result in additional charges.<br>
						...
						for k=1 to pcPackageCount
							...
							<div id="tab<%=4+int(k)%>" class="panes">
								<table class="pcCPcontent">
									...
									<tr>
                    <td>
                      <label for="Service<%=k%>">Package Type:</label>
                    </td>
                    <td>
                      ...
                      &nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=320"></a>
                    </td>
									</tr>
									<tr>
									<td colspan="2" class="pcCPspacer"></td>
                  ...
                  <td colspan="2">
									...
									<p style="padding-top: 5px;">Weight Units:
                  ...
                  &nbsp;
                  <a class="pcCPhelp" href="helpOnline.asp?ref=321"></a>
                  </p>
									</td>
                  </tr>
                  <tr>
                  ...
                  <a class="pcCPhelp" href="helpOnline.asp?ref=322"></a>
                  
                  <!--<a href="http://www.ups.com/content/us/en/resources/prepare/oversize.html" target="_blank">Click for more information on UPS oversized packages.</a>-->
                  <br><br>
									<p style="padding-top: 5px;">
							...
							<tr>
							<th colspan="2">COD - Collect On Delivery</th>
							</tr>
							<tr>
									<td colspan="2" class="pcCPspacer"></td>
								  </tr>
				...
				<td align="right"><input type="checkbox" name="CODPackage<%=k%>" value="1" class="clearBorder" <%=pcf_CheckOption("CODPackage"&k, "1")%>></td>
							  <td><strong>C.O.D. is required for this package.</strong></td>
							  </tr>
							<tr>
								<td width="23%" align="right">Collection Amount:</td>
								<td width="77%" align="left">
							...
							<tr>
								<td align="right">Collection Currency:</td>
								<td align="left">
							...
							<tr>
								<td align="right">Collection Fund Type:</td>
								<td align="left">
                  ...
                  <td align="right">Insured Value:</td>
									...
									<tr>
										<td align="right">Delivery Confirmation: </td>
                    <td align="left">
