<%
if pcv_PayPalSpecialFeatures=1 AND (pcgwTransId<>"" AND isNULL(pcgwTransId)=False) then
...
response.redirect("Orddetails.asp?id="&qry_ID)
		...
		<tr>
			...
			<div style="padding-bottom:4px;">						
				<a href="http://wiki.productcart.com/productcart/paypal" target="_blank">Read this KB article for more information about Authorization & Capture.</a>			</div>
			</td>
		</tr> 		
		<tr>
			<td colspan="2">
		...
		<%
		PayPalbtns=0
		if (pcv_isPaid=-1 AND pcv_PaymentStatus<>6 AND pcv_PaymentStatus<>8) OR pcv_isAuthorizePending=-1 OR pcv_isAuthorizeNeeded=-1 then %>
		<tr>
		...
		<tr>
			<td colspan="2">
				...
				PayPalbtns=1 %>
					<input type="submit" name="SubmitPayPal1" value=" Capture "  class="btn btn-primary">&nbsp;&nbsp;
				<% end if %>
				...
				PayPalbtns=1 %>
					<input type="submit" name="SubmitPayPal1" value=" Reauthorize "  class="btn btn-primary">&nbsp;&nbsp;
				<% end if %>
				<% if pcv_isPaid=-1 AND pcv_PaymentStatus<>6  AND pcv_PaymentStatus<>8 then
				PayPalbtns=1 %>					
					<input type="submit" name="SubmitPayPal1" value=" Refund "  onClick="javascript: if (confirm('This action will NOT cancel the order, it will refund the payment via PayPal. Are you sure you want to continue?')) return true ; else return false ;" class="btn btn-primary">&nbsp;&nbsp;					
				<% end if %>
				<% if pcv_isAuthorizePending=-1 then
				PayPalbtns=1 %>												
					<input type="submit" name="SubmitPayPal1" value="  Void  " onClick="javascript: if (confirm('This action will cancel the PayPal authorization and mark the order status as canceled.  Are you sure you want to continue?')) return true ; else return false ;" class="btn btn-primary">					
				<% end if %>
      </td>
    </tr>
    <tr>
      <td colspan="2">
		...
		<tr>
			<td colspan="2"><a href="javascript:openwin('popup_PayPalTransSearch.asp?transID=<%= pcv_strTransactionID %>');">Search PayPal Transactions</a></td>
		</tr> 
...
%>
<% 
if (pcv_strAVSRespond<>"" OR pcv_strCVNResponse<>"" OR pcv_strPayerStatus<>"" OR pcv_strAddressStatus<>"") AND pcv_PayPalSpecialFeatures=1 then  %>
<tr>
	<td colspan="2" class="pcCPspacer"></td>
</tr>
<tr>
...
<% end if %>
<% if pcv_strAVSRespond<>"" AND pcv_PayPalSpecialFeatures=1 then %>
<tr>
	<td colspan="2">
		<%
...
<% if pcv_strCVNResponse<>"" AND pcv_PayPalSpecialFeatures=1 then %>
<tr>
	<td colspan="2">
		<%
...
<% if pcv_strPayerStatus<>"" AND pcv_PayPalSpecialFeatures=1 then %>
<tr>
	<td colspan="2">
...
<% if pcv_strAddressStatus<>"" AND pcv_PayPalSpecialFeatures=1 then %>
<tr>
	<td colspan="2">
