...
dim rstemp2, strtext, strtext1, File1, pcv_rootCat, strParent, intIdProduct, pcIntIncludeWeight
                ...
                <td>
                    ...
                    <% Else %>
                        <table class="pcCPcontent">
                            ...
                            <tr>
                                <td colspan="4">
                                    <input type="submit" name="Submit1" value="Generate Bing Shopping Data Feed" onClick="pcf_Open_BingShopping();" class="btn btn-primary">
                                    &nbsp;
                                    <input type="button" class="btn btn-default"  name="Back" value="Back" onClick="document.location.href='exportBing.asp'">
									<%
									'// Loading Window
		...
		if Count1>-1 then
			...
			For k=0 to Count1
				...
				'IF session("tmp" & intIdProduct)<>"1" THEN
					...
					'If it's a configurable product, check if there is a Default Price and use it
	...
	<table class="pcCPcontent">
		...
		<tr>
			<td>
				...
				<%
				query="SELECT Count(*) As tmpTotal FROM products WHERE products.active = 0 AND products.pcProd_ParentPrd=0 AND products.removed=0;"
				...
				If statusAPP="1" OR scAPP=1 Then
					...
					query="SELECT Count(*) As tmpTotal FROM products WHERE products.pcProd_ParentPrd>0 AND products.removed=0;"
					set rsQ=connTemp.execute(query)
					if not rsQ.eof then
						if rsQ("tmpTotal")>"0" then
							Count=Count+Clng(rsQ("tmpTotal"))
							response.write rsQ("tmpTotal") & " products are apparel sub-products and not included in the Google Shopping feed<br />"
						end if
					end if
					set rsQ=nothing
				...
				if not rsQ.eof then
					if rsQ("tmpTotal")>"0" then
					Count=Count+Clng(rsQ("tmpTotal"))
					response.write rsQ("tmpTotal") & " products are configurable items and not included in the Bing Shopping data feed<br />"
					end if
				end if
				set rsQ=nothing
...
IF request("action")<>"add" AND request("action")<>"gen" THEN
    ...
    <form name="form1" method="post" action="exportBing.asp?action=add" onSubmit="return Form1_Validator(this)" class="pcForms">
        ...
        <tr>
            <td colspan="2">
                <ul>
                <li>Review the User Guide for <a href="http://wiki.productcart.com/productcart/marketing-generate_bing_shopping_file" target="_blank">information about this feature</a>.</li>
				<li>Take advantage of Custom Search Fields to pre-fill the MPN, UPC, and ISBN codes (if applicable). <a href="http://wiki.productcart.com/productcart/managing_search_fields#mapping_custom_search_fields_to_export_fields" target="_blank">More information</a>.</li>
                </ul>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            <td colspan="2" align="center">
                <input type="submit" name="Submit" value=" Generate Bing Shopping Data Feed" onClick="pcf_Open_BingShopping2();" class="btn btn-primary">
                <%
                '// Loading Window
                '	>> Call Method with OpenHS();
