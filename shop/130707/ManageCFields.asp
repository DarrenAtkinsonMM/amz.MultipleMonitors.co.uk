...
if request("action")="updfield" then
	...
	if Left(request("idcustom"),1)="S" then
		query="UPDATE pcSearchFields SET pcSearchFieldName=N'" & newvalue & "' WHERE idSearchField=" & idcustom
		set rs=Server.CreateObject("ADODB.Recordset")
		set rs=connTemp.execute(query)
	else
		query="UPDATE xfields SET xfield=N'" & newvalue & "' WHERE idxfield=" & idcustom
		set rs=Server.CreateObject("ADODB.Recordset")
		set rs=connTemp.execute(query)
	end if
  ...
  <tr>
			...
			<Form action="ManageCFields.asp?action=updfield" method="post" name="Form1" class="pcForms">
				<table class="pcCPcontent" style="width:auto">
					...
					<tr>
						<td>&nbsp;</td>
						<td> 
						<input type="submit" name="submit" value="Update" class="btn btn-primary" onClick="javascript: if (document.Form1.newvalue.value=='') {alert('Please enter a value for New Name'); document.Form1.newvalue.focus();return(false)} else {return(true)};">
						&nbsp;
						<input type="button" class="btn btn-default"  name="show" value="Show Products" onClick="javascript: newWindow('showCFProducts.asp?idcustom='+document.Form1.idcustom.value,'products');">
						</td>
					</tr>
          <tr>
	...
	<Form action="delCFfromPrds1.asp?action=delfield" method="post" name="Form3" class="pcForms">
		...
		<tr>
			<td>&nbsp;</td>
			<td> 
			<input type="submit" name="submit" value="Select Products" class="btn btn-primary">
			&nbsp;
			<input type="button" class="btn btn-default"  name="show" value="Show Products" onClick="javascript: newWindow('showCFProducts.asp?idcustom='+document.Form3.idcustom.value,'products');"></td>
		</tr>
		</table>
	</Form>
	<%end if%>
	...
	<tr>
		<td align="center">
			<form class="pcForms">
			<input type="button" class="btn btn-default"  onClick="JavaScript: location.href='LocateProducts.asp?cptype=0'" value="Locate a Product">&nbsp;
			<input type="button" class="btn btn-default"  onClick="JavaScript: location.href='menu.asp'" value="Start Page">
			</form>
		</td>
	</tr>
	</table>
