...
if src_DiscType="1" then
	tmpStrEx=" AND ((products.idproduct IN (SELECT DISTINCT idproduct FROM discountsPerQuantity)) OR (products.idproduct IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN discountsPerQuantity ON products.idproduct=discountsPerQuantity.idproduct))) AND ((products.idproduct NOT IN (SELECT DISTINCT idproduct FROM pcPrdPromotions)) AND (products.idproduct NOT IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN pcPrdPromotions ON products.idproduct=pcPrdPromotions.idproduct)))"
else
	if src_DiscType="2" then
		tmpStrEx=" AND ((products.idproduct NOT IN (SELECT DISTINCT idproduct FROM discountsPerQuantity)) AND (products.idproduct NOT IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN discountsPerQuantity ON products.idproduct=discountsPerQuantity.idproduct)) AND (products.idproduct NOT IN (SELECT DISTINCT idproduct FROM pcPrdPromotions)) AND (products.idproduct NOT IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN pcPrdPromotions ON products.idproduct=pcPrdPromotions.idproduct)))"
	else
		if session("srcprd_DiscArea")="1" AND (src_PromoType="" OR src_PromoType="-1") then
			tmpStrEx=" AND ((products.idproduct NOT IN (SELECT DISTINCT idproduct FROM pcPrdPromotions)) AND (products.idproduct NOT IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN pcPrdPromotions ON products.idproduct=pcPrdPromotions.idproduct)))"
		end if
	end if
end if

if src_PromoType="1" then
	tmpStrEx=" AND ((products.idproduct NOT IN (SELECT DISTINCT idproduct FROM discountsPerQuantity)) AND (products.idproduct NOT IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN discountsPerQuantity ON products.idproduct=discountsPerQuantity.idproduct))) AND ((products.idproduct IN (SELECT DISTINCT idproduct FROM pcPrdPromotions)) OR (products.idproduct IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN pcPrdPromotions ON products.idproduct=pcPrdPromotions.idproduct)))"
else
	if src_PromoType="2" then
		tmpStrEx=" AND ((products.idproduct NOT IN (SELECT DISTINCT idproduct FROM discountsPerQuantity)) AND (products.idproduct NOT IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN discountsPerQuantity ON products.idproduct=discountsPerQuantity.idproduct)) AND (products.idproduct NOT IN (SELECT DISTINCT idproduct FROM pcPrdPromotions)) AND (products.idproduct NOT IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN pcPrdPromotions ON products.idproduct=pcPrdPromotions.idproduct)))"
	else
		if src_PromoType="0" then
			tmpStrEx=" AND ((products.idproduct NOT IN (SELECT DISTINCT idproduct FROM discountsPerQuantity)) AND (products.idproduct NOT IN (SELECT DISTINCT products.pcprod_ParentPrd FROM Products INNER JOIN discountsPerQuantity ON products.idproduct=discountsPerQuantity.idproduct)))"
		end if
	end if
end if
...
if statusAPP="1" then
	tmpQ1=",products.pcprod_Apparel,products.pcprod_ParentPrd"
else
	tmpQ1=",0,0"
end if

strSQL="SELECT DISTINCT " & pcv_strLimitPhrase & " products.idProduct, products.description, products.active, products.smallImageUrl, products.sku, products.serviceSpec,products.configonly,products.stock,products.pcProd_ReorderLevel,products.price,products.cost" & tmpQ1 & " FROM products " & tmpSQL1 & session("srcprd_from") & tmp_addFrom & " WHERE products.price>="&pPriceFrom&" And products.price<=" &pPriceUntil&" AND products.removed=0"
if session("srcprd_where")="" then
	if statusAPP="1" then
		strSQL=strSQL & " AND products.pcprod_ParentPrd=0 "
	end if
else
	if instr(ucase(session("srcprd_where"))," AND PRODUCTS.PCPROD_PARENTPRD=")=0 then
		if statusAPP="1" then
			strSQL=strSQL & " AND products.pcprod_ParentPrd=0 "
		end if
	end if
