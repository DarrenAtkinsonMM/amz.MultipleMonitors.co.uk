...
if not validNum(pidRMA) then 
	call closeDb()
	response.redirect("resultsAdvancedAll.asp?B1=View+All&dd=1")
end if
...
if request.form("Modify")<>"" then
	...
	for i=1 to iCnt
		...
		'Get products for Avalara
		arrProducts = arrProducts & Request.Form("idProductOrdered"&i) & ","
	next
	set rs=nothing
	...
	if genNumber="YES" then
		...
		if pRmaNumber<>"" then
			...
			msg=msg & "New RMA number was generated"
		end if
	else
		pRmaNumber=request("pRmaNumber1")
		pRmaNumber=replace(pRmaNumber,"'","''")
	...
	query="UPDATE PCReturns set rmaNumber='"&pRmaNumber&"', rmaReturnReason=N'"&rmaReturnReason&"', rmaReturnStatus='"&rmaReturnStatus&"', rmaApproved="&rmaApproved&" WHERE idRMA="&pidRMA&";"
	set rs=Server.CreateObject("ADODB.Recordset")
	set rs=conntemp.execute(query)
	...
	if rmaApproved = 1 then
		call pcs_ReturnAvalara(idOrder, arrProducts)
	end if
	
	if msg<>"" then
		msg=msg & " and "
	end if
	msg=msg & "RMA details have been updated"
...
<%IF (request.form("Modify")<>"") AND (msgType=1) THEN%>
	<% ' START show message, if any %>
		<!--#include file="pcv4_showMessage.asp"-->
	<% 	' END show message %>
	<br>
	<br>
	<input type="button" class="btn btn-default"  name="OrderDetails" value="Back to Order Details" onClick="location.href='Orddetails.asp?id=<%=pIdOrder%>'">
	<br>
	<br>
<%ELSE%>
	<form method="post" name="modRma" action="modRmaa.asp" class="pcForms">
		...
		<tr>
			<td align="right" nowrap="nowrap">Create a random <strong>RMA Number</strong>?</td>
		...
		<td colspan="2" align="center">
			<input type="submit" name="Modify" class="btn btn-primary" value="Create/Update">
			&nbsp;
			<% if pIdOrder<>"" then %>
			<input type="button" class="btn btn-default"  name="OrderDetails" value="Back to Order Details" onClick="location.href='Orddetails.asp?id=<%=pIdOrder%>'">
			<% end if %>
