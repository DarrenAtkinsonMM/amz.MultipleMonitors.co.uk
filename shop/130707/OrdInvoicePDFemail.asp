...
'///////////////////////////////
	...
	boolVAT = CheckVAT(pcv_IsEUMemberState, pshippingCountryCode, CustomerVATID)
...
Set pdf=CreateJsObject("FPDF")
		...
		pdf.SetCreator("ProductCart software - Early Impact, Inc. (www.earlyimpact.com)")
		...
		pdf.Open()
			...
			pdf.AddPage()
			pdf.Image "/images/logo-inv.jpg",10,10,100
			...
			pdf.Row vbcrlf,"",dictLanguage.Item(Session("language")&"_custOrdInvoice_1") & tmpDate
			...
			pdf.Row vbcrlf & vbcrlf & vbcrlf & vbcrlf & vbcrlf 
			...
			pdf.Row dictLanguage.Item(Session("language")&"_custOrdInvoice_5") & strInvoiceNum & tmpPDF
			pdf.SetFont "Arial","",9
			pdf.SetCustomStyle ""
			...
			if CustomerVATID <> "" then
				CurY=pdf.GetY()
				pdf.SetXY curX+100,curY
				pdf.Row "VAT #: " & CustomerVATID
			end if
  ...
  '~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
			...
			pdf.SetCustomStyle "","","",""
					...
					Do until rsTemp.eof
						...
						psku=rsTemp2("sku")
						pDescription=replace(rsTemp2("description"),"&quot;","''")
		...
		If intPackingSlip<>1 Then
			...
			if (pcSCID>"0") AND (pcSCIcon="") then
				...
				pdf.Row pquantity,psku & " - " & pDescription,scCurSign&money(punitPrice1/1.2),scCurSign&money(pRowPrice1/1.2)
			end if
			if pcSCIcon<>"" then
		...
		if len(pcv_strSelectedOptions)>0 then %>
				...
				For pcv_intOptionLoopCounter = 0 to pcv_intOptionLoopSize
					...
					if tempPrice="" or tempPrice=0 then
						...
						if intPackingSlip <>1 then
							tmpPDF2=scCurSign&money(tempPrice/1.2)
						end if
						
						if intPackingSlip <>1 then
							tAprice=(tempPrice*Cdbl(pquantity))
							tmpPDF3=scCurSign&money(tAprice/1.2) 
						end if
					end if
					if intPackingSlip <>1 then
	...
	if (QDiscounts<>"") and (CDbl(QDiscounts)<>"0") and intPackingSlip<>1 then
		...
		pdf.SetAligns "L", "R", "R"
		pdf.SetBorders 0,0,0
		pdf.Row "","Quantity Discounts:",scCurSign&money((-1*QDiscounts)/1.2)
	...
	if pExtRowPrice<>pRowPrice then
		if intPackingSlip<>1 then
			...
			pdf.SetAligns "L", "L", "R", "R"
			pdf.SetBorders 0,0,0,0
			pdf.Row "","","PRODUCT SUBTOTAL:",scCurSign&money(pRowPrice/1.2)
		end if
	end if
...
<%'START - Fees & Order Total%>
	...
	if intPackingSlip<>1 then
						...
						pdf.SetAligns "L", "R", "R"
						pdf.SetBorders 0,0,0
						pdf.Row "","DELIVERY:",scCurSign&money(postage/1.2)
					%>
					...
					<% if instr(pdiscountDetails,"- ||") or (pcv_CatDiscounts>"0") then
						...
						pdf.SetAligns "L", "R", "R"
						pdf.SetBorders "TB",0,0
						pdf.Row "","DISCOUNTS:","-" & scCurSign & money((discount+pcv_CatDiscounts)/1.2)
					end if %>
					<%if GCAmount>"0" then
						pdf.SetCustomStyle "","",""
						pdf.SetColumns 115,40,35
					...
					If boolVAT = 0 then
						pdf.Row "","SUB TOTAL:",scCurSign&money(ptotal)
					else
						pdf.Row "","SUB TOTAL:",scCurSign&money(ptotal/1.2)
					end if
					...
					pdf.SetBorders "T",0,0
						...
						If boolVAT = 0 then
							pdf.Row "","VAT:",scCurSign&money(ptotal-(ptotal))
						else
							pdf.Row "","VAT:",scCurSign&money(ptotal-(ptotal/1.2))
						end if
	...
	if intPackingSlip <>1 then
		tmpPDFFile="MM-Invoice-" & strInvoiceNum & ".pdf"
	else
		tmpPDFFile="Packing-Slip-ID" & strInvoiceNum & tmpExt & ".pdf"
	end if
	ListPDFFiles=ListPDFFiles & "<li>" & "<a href='pdf-invoices/" & tmpPDFFile & "'>" & tmpPDFFile & "</a></li>"
	pdf.Output server.mappath("./pdf-invoices/" & tmpPDFFile),1
	%>	
	...
	queryTNum="SELECT pcPackageInfo_TrackingNumber FROM pcPackageInfo WHERE idOrder=" & request.QueryString("id") 
	set rsTNum=server.CreateObject("ADODB.RecordSet") 
	set rsTNum=conntemp.execute(queryTNum)
	'daTrackingNumber="123"
	daTrackingNumber=rsTNum("pcPackageInfo_TrackingNumber")
	set rsTNum=nothing							  
...
'==================================
	...
	Select Case Request.QueryString("guide")
		Case "pc"
			...
			strEmailH1 = "Your Delivery Info, Invoice, & Quick Start Guide"
			strEmailP = "<p>Dear " & CustomerName & ",<br /><br />We have put together a short email which will help you get ready for your delivery and then get up and running quickly, please take a look through.<br /><br />"
			
			strEmailP = strEmailP & "<strong>Delivery Details</strong><br />We are pleased to let you know that your order has been released for shipping and it should be delivered on the next working day unless otherwise advised.<br /><br />Your tracking link is below and the couriers will contact you directly with further delivery updates and a 1 hour delivery window.<br /><br />Tracking Link: <a href=""https://www.dpdlocal.co.uk/apps/tracking/?reference=" & daTrackingNumber & """>DPD Consignment: " & daTrackingNumber & "</a><br /><br />"
			
			strEmailP = strEmailP & "<strong>Your Invoice</strong><br />A copy of your invoice is attached to this email, please save it for your records.<br /><br />"
			
			strEmailP = strEmailP & "<strong>PC Setup</strong><br />To help you get up and running quickly we have attached a quick start guide for your new PC, please <strong>read before you start assembling</strong> your new equipment as the guide contains helpful pointers.<br /><br />In addition to this you can view a more detailed <strong>Getting Started Guide</strong> on our website here: <a href=""http://www.multiplemonitors.co.uk/pages/getting-started/"">www.multiplemonitors.co.uk/pages/getting-started/</a>.<br /><br />"
			
			strEmailP = strEmailP & "Thank you again for your order and if you do have any setup questions or problems just let us know, we are here to help!<br /><br />Best Regards,<br /><br />Multiple Monitors</p>"
		Case "pcstand"
			attachement2 = "c:\inetpub\wwwroot\MultipleMonitors.co.uk\guides\PC-Quickstart.pdf"
			...
			strEmailP = strEmailP & "<strong>Stand Assembly</strong><br />An assembly guide for your Synergy Stand is also attached, there will be a hard copy supplied in with the stand as well, we strongly advise you review it in full and watch the stand assembly videos (link below) before beginning assembly of your stand.<br /><br />Stand Assembly Videos: <a href=""https://www.synergy-stand.com/assembly-videos.html"">www.synergy-stand.com/assembly-videos.html</a>.<br /><br />"
