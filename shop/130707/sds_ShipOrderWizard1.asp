...
if pcv_strPageAction = "FedExWs" OR pcv_strPageAction = "UPS" OR pcv_strPageAction = "USPS" then
...
<table class="pcCPcontent">
	...
	<%
	if pcv_strPageAction="UPS" or pcv_strPageAction="USPS" then
		select case pcv_strPageAction
			case "UPS"
				pcv_DescriptionString="UPS&reg; Developer Kit Shipping"
			case "FedExWs"
				...
				<td colspan="2">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<td width="96%">UPS, the UPS Shield trademark, the UPS Ready mark, <br />the UPS Developer Kit mark and the Color Brown are trademarks of <br />United Parcel Service of America, Inc. All Rights Reserved.</td>
						</tr>
					</table>
					<br />
...
<br>
		...
		<% end if
		IF pcv_strPageAction="FedExWs" then  %>
			<tr>
...
<br>
	...
	<%
	query="SELECT Products.idproduct, Products.Description, Products.sku, Products.Stock, Products.noStock, Products.pcProd_IsDropShipped, Products.pcDropShipper_ID, Products.pcProd_ParentPrd, ProductsOrdered.idProductOrdered, ProductsOrdered.quantity, ProductsOrdered.pcPrdOrd_BackOrder, ProductsOrdered.pcPrdOrd_Shipped FROM Products INNER JOIN ProductsOrdered ON Products.idproduct=ProductsOrdered.idproduct WHERE ProductsOrdered.idorder=" & pcv_IdOrder
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	...
	<% ELSE %>
			...
			<table class="pcCPcontent">
				...
				Do while not rs.eof
					...
					if (pcv_ParentPrd<>"") and (pcv_ParentPrd>"0") then
						...
						pcv_TargetProduct = pcv_IDProduct
					end if
					...
					if (NOT pcf_ItemAvailable(pcv_IDProductOrdered)) AND (pcv_strPageAction = "FedExWs" OR pcv_strPageAction = "UPS" OR pcv_strPageAction = "USPS") then
						pcv_cancheck=0
					end if
					%>
					<tr>
						<td align="center" width="5%">
							<% dim pcv_showLink
							pcv_showLink = 0
							if (clng(pcv_Stock)>=clng(pcv_Qty)) OR (pcv_cancheck=1) then
								pcv_showLink = 1
							end if %>
						...
						<td align="left" width="70%"><%if pcv_cancheck=0 then%><font color="#666666"><i><%end if%><a href="FindProductType.asp?id=<%=pcv_TargetProduct%>" target="_blank"><%=pcv_Description%></a> (<%=pcv_Sku%>)<%if pcv_cancheck=0 then%></i></font><%end if%></td>
		...
		<tr>
			<td colspan="4" align="left">
				...
				<%end if%>
			<%if pcv_available>0 then%>
				<input type="submit" name="submit1" value="<%=pcv_strButtonText%>" class="btn btn-primary" <%if EndiciaReg=1 then%>onclick="javascript:pcf_Open_EndiciaPop();"<%end if%>>
			<%else%>
			<span id="showSubmit" style="display:none"><input type="submit" name="submit1" value="<%=pcv_strButtonText%>" class="btn btn-primary" <%if EndiciaReg=1 then%>onclick="javascript:pcf_Open_EndiciaPop();"<%end if%>></span>
			<%end if%>
