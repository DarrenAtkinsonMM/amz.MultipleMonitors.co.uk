...
if request.Form("submitflag")="1" then
	...
	if intMode="2" then
	'// MODIFY - Start
		query="UPDATE pcCustomerCategories SET pcCC_Name=N'"&strpcCC_Name&"', pcCC_Description=N'"&strpcCC_Description&"', pcCC_WholesalePriv="&intpcCC_WholesalePriv&" ,pcCC_CategoryType='"& strpcCC_CategoryType&"', pcCC_ATB_Percentage="&dblpcCC_ATB_Percentage&", pcCC_ATB_off='"&strpcCC_ATB_off&"', pcCC_NFSoverride="&strpcCC_NFSO&" WHERE idcustomerCategory="&intModeID&";"
		SET rs=server.CreateObject("ADODB.RecordSet")
		SET rs=conntemp.execute(query)
	...
	'// ADD NEW - Start
		...
		SET rs=conntemp.execute(query)
		if rs.eof then
			query="INSERT INTO pcCustomerCategories (pcCC_Name, pcCC_Description, pcCC_WholesalePriv, pcCC_CategoryType, pcCC_ATB_Percentage, pcCC_ATB_off, pcCC_NFSoverride) VALUES (N'"&strpcCC_Name&"', N'"&strpcCC_Description&"', "&intpcCC_WholesalePriv&", '"& strpcCC_CategoryType&"', "&dblpcCC_ATB_Percentage&", '"&strpcCC_ATB_off&"',"&strpcCC_NFSO&");"
...
{
	if (theForm.pcCC_ATB_Percentage.value!="")
	if (!isNumeric(theForm.pcCC_ATB_Percentage.value))
	{
		alert("Please enter an integer for Percentage field");
		theForm.pcCC_ATB_Percentage.focus();
		return(false);
	}
	
	if (theForm.GlobalPercentOff.value!="")
	if (!isNumeric(theForm.GlobalPercentOff.value))
	{
		alert("Please enter an integer for Percentage field");
		theForm.GlobalPercentOff.focus();
		return(false);
	}
...
<form name="CustTypeForm" action="AdminCustomerCategory.asp" method="post" onSubmit="return Form1_Validator(this);" class="pcForms">
  ...
  <input type="hidden" name="id" value="<%=intModeID%>">
	<% if intMode<2 then
  	...
  	<table class="pcCPcontent">
		...
		<tr>
			<td align="right"></td>
			<td>Apply Wholesale Customer Privileges: <input type="checkbox" name="pcCC_WholesalePriv" value="1" <% if intpcCC_WholesalePriv=1 then %>checked<% end if %> class="clearBorder">&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=305"></a></td>
		</tr>
		<tr>
			<td align="right"></td>
			<td>Override &quot;Not For Sale&quot; setting: <input type="checkbox" name="NFSO" value="1" <% if strpcCC_NFSO=1 then %>checked<% end if %> class="clearBorder">&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=318"></a></td>
		</tr>
		<tr>
			<td>&nbsp;</td>
			<td>Apply custom pricing: <input name="pcCC_CategoryType" type="radio" value="ATB" <% if strpcCC_CategoryType="ATB" then %>checked<% end if %> class="clearBorder"> Across the board &nbsp;<input name="pcCC_CategoryType" type="radio" value="PBP" <% if strpcCC_CategoryType="PBP" then %>checked<% end if %> class="clearBorder"> On a product by product basis &nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=306"></a></td>
		</tr>
		<tr>
		...
		<% end if %>
		<tr>
		  <td colspan="2">Assign price based on <input name="GlobalPercentOff" type="text" value="0" size="4"> % off the current online price.&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=307"></a></td>
		</tr>
		<tr>
			<td colspan="2" class="pcCPspacer"></td>
		</tr>
		...
		<tr>
		  <td colspan="2" align="center">
				...
				<%if intMode<>"1" then%>
				&nbsp;<input type="button" class="btn btn-default"  value="Assign/Remove Customers" onClick="location.href='EditCustCategories.asp?CCID=<%=intModeID%>&move=view'">
				<%end if%>
				&nbsp;<input type="button" class="btn btn-default"  value="Reset Prices" onClick="location.href='resetPricingCatPrices.asp'">
				&nbsp;<input type="button" class="btn btn-default"  value="Other Pricing Categories" onClick="location.href='AdminCustomerCategory.asp'">
			</td>
		</tr>
	</table>
</form>
...
if pcCC_EXIST=1 and pcCC_INPROCESS=0 then %>
	...
	<table class="pcCPcontent">
		<tr> 
			<td colspan="3">
                <div class="cpOtherLinks"><a href="AdminCustomerCategory.asp?mode=1">Add New Pricing Category</a> | <a href="resetPricingCatPrices.asp">Reset Prices for a Pricing Category</a> | <a href="http://wiki.productcart.com/productcart/customer-prices" target="_blank">Help</a></div>
		  </td>
		</tr>
