...
<p style="text-align: center;">
	<%if (src_ToPage<>"") and (pcv_HaveResults="1") and (src_DisplayType>"0") then
		...
		<input type="button" class="btn btn-default"  value="<%=src_Button2%>" onClick="javascript:if (savelist=='xml,') {alert('Please select a product before clicking on this button');} else {location.href='<%=src_ToPage%>sdslist='+getsdslist(savelist);}">
	<%end if%>
