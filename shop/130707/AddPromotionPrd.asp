...
<%
Dim pcvStrProductName, pidProduct
...
if (intRequestSubmit1="Save") and (msg="") then
	...
	query=query & " VALUES (" & pIdProduct & "," & qtytrigger & "," & discountType & "," & discountvalue & "," & applyunits & ",N'" & promomsg & "',N'" & confirmmsg & "',N'" & descmsg & "'," & prinactive & "," & pcIncExcCust & "," & pcIncExcCPrice & "," & pcRetail & "," & pcWholesale & ");"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	set rs=nothing
	...
	<table class="pcCPcontent">
       <tr>
		...
		<tr>
			<td>
				<input type="button" class="btn btn-default"  name="back" value=" Product Promotions " onClick="location='PromotionPrdSrc.asp';">
				&nbsp;&nbsp;<input type="button" class="btn btn-default"  name="back1" value=" View/Edit the Promotion " onClick="location='ModPromotionPrd.asp?idproduct=<%=pidproduct%>&iMode=start';">
				&nbsp;&nbsp;<input type="button" class="btn btn-default"  name="back2" value=" Back to Main menu " onClick="location='menu.asp';">
			</td>
		</tr>		
	</table>	
