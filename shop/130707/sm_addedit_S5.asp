...
if (UP="") OR (UP="0") OR (request("a")="new") then
	...
	if pcSaleID<>"" then
		...
		if not rs.eof then
			...
			if statusAPP="1" then
				tmpPrdCount1=cmd.Parameters("@SMCOUNTSUB")
				session("sm_PrdCountSub")=tmpPrdCount1
			else
				tmpPrdCount1=0
				session("sm_PrdCountSub")=tmpPrdCount1
			end if
			...
			session("sm_TechDetails1")="Products:<div style=""padding-top: 5px; font-weight:bold;"">The sale will affect " & session("sm_PrdCount") & " product(s) in your store."
			if session("sm_PrdCountSub")>"0" then
				session("sm_TechDetails1")=session("sm_TechDetails1") & " and " & session("sm_PrdCountSub") & " sub-product(s)"
			end if
			session("sm_TechDetails1")=session("sm_TechDetails1") & "</div><div style=""padding-top: 5px;"" class=""pcSmallText"">If you just updated the product included in the sale, this number may be incorrect. it will be updated when you save the sale.</div>"
...
if request("Go")<>"" then
	...
	if UpdatedSale=0 then
		query="INSERT INTO pcSales (pcSales_TargetPrice,pcSales_Type,pcSales_Relative,pcSales_Amount,pcSales_Round,pcSales_Name,pcSales_Desc,pcSales_ImgURL,pcSales_CreatedDate,pcSales_Param1,pcSales_Param2,pcSales_Tech) VALUES (" & tmpTargetPrice & "," & tmpSalesType & "," & tmpRelative & "," & tmpSalesAmount & "," & tmpRound & ",N'" & pcf_ReplaceCharacters(replace(session("sm_SaleName"),"'","''")) & "',N'" & pcf_ReplaceCharacters(replace(session("sm_SaleDesc"),"'","''")) & "','" & session("sm_SaleIcon") & "','" & dtTodaysDate & "','" & replace(session("sm_Param1"),"'","''") & "','" & replace(session("sm_Param2"),"'","''") & "','" & replace(session("sm_TechDetails"),"'","''") & "');"
		set rs=connTemp.execute(query)
		set rs=nothing
...
<tr>
	...
	<td>
		<textarea class="htmleditor" name="saledesc" id="saledesc" cols="50" rows="10"><%=session("sm_SaleDesc")%></textarea>
	</td>
</tr>
<tr>
	...
	<td>
		<input type="text" name="saleicon" id="saleicon" value="<%=session("sm_SaleIcon")%>" size="50"> <a href="javascript:;" onClick="chgWin('../pc/imageDir.asp?ffid=saleicon&fid=UpdateForm','window2')"><img src="images/search.gif" alt="locate images previously uploaded" width="16" height="16" border=0 hspace="3"></a>&nbsp;
...
<tr align="center">
	<td colspan="2">
		...
		<input type="submit" name="Go" onClick="document.UpdateForm.what.value='0';" value="Save Sale" class="btn btn-primary">&nbsp;
		<%end if%>
		<input type="submit" name="Back" onClick="document.UpdateForm.what.value='1';" value="Edit Sale">
