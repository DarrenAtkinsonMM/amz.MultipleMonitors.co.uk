...
<form method="POST" action="manageTaxEptCust.asp" name="myForm" class="pcForms">
	...
	<tr> 
		<td colspan="4">
		<% If strMode="edit" Then %>
			<input type="button" class="btn btn-default"  value="View Customers" onClick="document.location.href='manageTaxEptCust.asp?OrderBy=<%=request("OrderBy")%>&SortOrd=<%=request("SortOrd")%>&ZoneRateID=<%=intZoneRateID%>&mode=view';" class="btn btn-primary">
		<% Else %>
			<input type="button" class="btn btn-default"  value="Add Customers" onClick="document.location.href='manageTaxEptCust.asp?OrderBy=<%=request("OrderBy")%>&SortOrd=<%=request("SortOrd")%>&ZoneRateID=<%=intZoneRateID%>&mode=edit';" class="btn btn-primary">
		<% End If %>
			<input type="button" class="btn btn-default"  value="Return to Tax Rule Details" onClick="document.location.href='AddTaxPerZone.asp?idTaxZonesGroupID=<%=intZoneRateID%>&mode=MOD';">
		</td>
	</tr>
</table>
</form>
