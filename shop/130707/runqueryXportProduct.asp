...
<%End Function
	Function CreateCSVFile()
		...
		Do Until rstemp.EOF
			...
			If ptype="1" then
				if rstemp("serviceSpec")=-1 then
					StringBuilderObj.append chr(34) & "CONFIGURABLE" & chr(34) & ","
				else
					if rstemp("configOnly")=-1 then
	...
	call pcs_SaveUTF8(strFile & ".csv",strFile & ".csv",StringBuilderObj.toString)
	...
	If Not rstemp.EOF Then
		Do Until rstemp.EOF
			...
			If ptype="1" then
				t=t+1
				if rstemp("ServiceSpec")=-1 then
					xlWorksheet.Cells(iRow,i + t).Value="'" & "CONFIGURABLE"
				else
					if rstemp("ConfigOnly")=-1 then
