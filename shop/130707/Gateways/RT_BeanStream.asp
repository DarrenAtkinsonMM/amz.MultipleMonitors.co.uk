...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName=N'"&paymentNickName&"' WHERE gwCode=57"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	set rs=nothing
	if err.number <> 0 then
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'Beanstream','gwBeanStream.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",57,N'"&paymentNickName&"')"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=connTemp.execute(query)
	if err.number<>0 then
		call LogErrorToDatabase()
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    <h4>Beanstream electronic payment processing - Direct post API</h4>    
                    <p>
                        Are you looking for a simple, cost-effective way to streamline credit card acceptance?  Beanstream provides multiple, flexible options to help you simplify billing processes, manage online payments and more.
                    </p>
                    <p>
                        <a class="btn btn-info btn-xs" href="http://www.beanstream.com/site/ca/index.html" target="_blank">Learn More</a>                      
                    </p>
                </div>
                ...
                You can create a login and password for ProductCart to use, by going to your Beanstream Order Settings page, beneath the heading &ldquo;Use username/password validation against transaction&rdquo;.
                                ...
                                <tr> 
                                    ...
                                    <td> <select name="pcPay_BS_TransType" id="tranType" onChange="checkInterac(this)" >
                                            <option value="P" <% if pcPay_BS_TransType="P" then%>selected<% end if %>>Sale</option>
