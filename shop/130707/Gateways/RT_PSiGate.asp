...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",sslURL='"&psiFile&"',priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName=N'"&paymentNickName&"' WHERE gwCode=4"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	set rs=nothing
	if err.number <> 0 then
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'PSIGate','"&psiFile&"',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",4,N'"&paymentNickName&"')"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=connTemp.execute(query)
	if err.number<>0 then
		call LogErrorToDatabase()
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    <h4>PSiGate</h4>    
                    <p>PSiGate makes it easy to get your business online and accepting credit card and debit transactions quickly and securely.</p>
                    <p>
                        <a class="btn btn-info btn-xs" href="http://www.psigate.com" target="_blank">Learn More</a>        
                    </p>
                </div>
                ...
                <tr> 
                    ...
                    <td>
                    	<select name="psi_XMLTransType">
                            <option value="0" <% if psi_TransType="0" then %>selected<% end if %>>Sale</option>
