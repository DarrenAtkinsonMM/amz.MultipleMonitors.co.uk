...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ", priceToAdd="&priceToAdd&" ,percentageToAdd="&percentageToAdd&" ,paymentNickName=N'"&paymentNickName&"' WHERE gwCode=37"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		strErrorDescription=err.description
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	if pcPay_FAC_Checking="1" then
		...
		if not rs.eof then
			query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ", priceToAdd="&priceToAdd&" ,percentageToAdd="&percentageToAdd&" ,paymentNickName=N'"&paymentNickName2&"' WHERE gwCode=38"
			set rs=Server.CreateObject("ADODB.Recordset")     
			set rs=conntemp.execute(query)
		else
			query="INSERT INTO payTypes (paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES ('FastCharge eCheck','gwFastChargeCheck.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",38,N'"&paymentNickName2&"')"
			set rs=Server.CreateObject("ADODB.Recordset")     
			set rs=conntemp.execute(query)
		end if
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'FastCharge','gwfastcharge.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",37,N'"&paymentNickName&"')"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=connTemp.execute(query)
	if err.number<>0 then
		call LogErrorToDatabase()
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    ...
                    <p>
                        <a class="btn btn-info btn-xs" href="http://www.fastcharge.com" target="_blank">Learn More</a>        
                    </p>
                </div>
            ...
            <td>
                ...
                <tr> 
                    ...
                    <td> <select name="pcPay_FAC_TransType">
                            <option value="1" <%if pcPay_FAC_TransType="1" then%>selected<%end if%>>Sale</option>
