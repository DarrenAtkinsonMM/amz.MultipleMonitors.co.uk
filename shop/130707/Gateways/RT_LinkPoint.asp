...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription)
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus &",sslURL='"&sslURL& "', priceToAdd="&priceToAdd&" ,percentageToAdd="&percentageToAdd&" ,paymentNickName=N'"&paymentNickName&"' WHERE gwCode=8"
	set rs=Server.CreateObject("ADODB.Recordset")
	set rs=conntemp.execute(query)
	set rs=nothing
	if err.number <> 0 then
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'LinkPoint','"&sslURL&"',-1,0,0,9999,0,9999,0,9999,-1,"&priceToAdd&","&percentageToAdd&",8,N'"&paymentNickName&"')"
	set rs=Server.CreateObject("ADODB.Recordset")
	set rs=connTemp.execute(query)
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    <h4>First Data</h4>    
                    <p>First Data online payment processing solutions deliver the innovation and security you and your customers demand to keep pace in this ever-changing marketplace.</p>
                    <p>
                        <a class="btn btn-info btn-xs" href="http://www.linkpoint.com" target="_blank">Learn More</a>        
                    </p>
                </div>
        ...
        <tr>
			...
			&nbsp;<img src="images/red_x.png" width="12" height="12" /><strong> &nbsp;FDGG API (LinkPoint API) with Batch Post Authorizations</strong> cannot be enabled. Errors were found while testing for the required components. These library files are available for download directly from First Data and need to be installed directly on the server.<br />
						...
						<% else %>
							...
							<BR>
							API Keys Directory: store/pcadmin/ folder (Copy your sercurity keys file *.pem to this folder before activating this payment gateway)
						<% End If %>
						</td>
				</tr>
				<% if request("mode")="Edit" then %>
				...
				<tr>
					...
					<td>
						<select name="lp_transType">
							<option value="sale" <% if lp_transType="sale" then %>selected<% end if %>>Sale</option>
