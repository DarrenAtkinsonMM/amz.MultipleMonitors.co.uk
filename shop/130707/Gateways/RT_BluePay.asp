...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName=N'"&paymentNickName&"' WHERE gwCode=29"
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'BluePay','gwBluePay.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",29,N'"&paymentNickName&"')"
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    <h4>BluePay - Integrated using BluePay 2.0 Pos</h4>    
                    <p>
                        An All-In-One Credit Card Processing Company
                    </p>
                    <p>
                        <a class="btn btn-info btn-xs" href="http://www.BluePay.com" target="_blank">Learn More</a>                      
                    </p>
                </div>
                               ...
                               <% end if %>
                                <tr> 
                                    ...
                                    <td> <select name="BPTransType">
                                            <option value="AUTH" <% if UCase(Trim(BPTransType))="AUTH" then%>selected<% end if %>>Authorize Only</option>
                                            <option value="SALE" <% if UCase(Trim(BPTransType))="SALE" then%>selected<% end if %>>Sale</option>
                                        </select></td>
                                </tr>
                                <tr> 
                                    <td> <div align="right"> 
