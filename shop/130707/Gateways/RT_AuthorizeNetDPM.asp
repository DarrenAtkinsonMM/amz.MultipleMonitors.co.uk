...
Function gwAuthorizeDPMEdit()
	'request gateway variables and insert them into the pcPay_AuthorizeDPM table
	query= "SELECT x_Login,x_Password FROM pcPay_AuthorizeDPM where id=1"
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	x_URLMethod="gwAuthorizeDPM.asp"
	x_DPMType=request.Form("x_DPMType")
...
'	end if
	...
	query="UPDATE pcPay_AuthorizeDPM SET x_Type='"&x_Type&"||"&cardTypes&"',x_Login='"&x_Login&"',x_Password='"&x_Password&"',x_Version='3.1',x_Curcode='"&x_Curcode&"',x_Method='DPM',x_DPMType='"&x_DPMType&"',x_CVV="&x_CVV&",x_testmode="&x_testmode&",x_eCheck="&x_eCheck&",x_secureSource="&x_secureSource&",x_eCheckPending="&x_eCheckPending&" where id=1"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	set rs=nothing
	if err.number <> 0 then
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",sslURL='"&x_URLMethod&"', priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName=N'"&paymentNickName&"' WHERE gwCode = 1103"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	set rs=nothing
	if err.number <> 0 then
...
<% if request("gwchoice")="1103" then
	if request("mode")="Edit" then
		query= "SELECT x_Type,x_Login,x_Password,x_Curcode,x_DPMType,x_CVV,x_testmode,x_eCheck,x_secureSource,x_eCheckPending FROM pcPay_AuthorizeDPM where id=1"
		set rs=Server.CreateObject("ADODB.Recordset")
		set rs=conntemp.execute(query)
		if err.number <> 0 then
			strErrorDescription=err.description
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
		...
		query= "SELECT pcPayTypes_processOrder, pcPayTypes_setPayStatus, priceToAdd, percentageToAdd, paymentNickName FROM payTypes WHERE gwCode=" & request("gwchoice")
		set rs=Server.CreateObject("ADODB.Recordset")     
		set rs=conntemp.execute(query)
		if rs.eof then
			paymentNickName="Credit Card"
...
'		set rs=nothing
    ...
    <% end if %>
	<input type="hidden" name="addGw" value="1103">
    <input type="hidden" name="x_DPMType" value="KEY">
    <input type="hidden" name="x_Method" value="DPM">
	<% If x_eCheck=1 Then %>
    <% Else %>
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    <h4>Authorize.Net DPM</h4>    
                    <p>
                        ProductCart uses Authorize.Net DPM (Direct Post Method) to communicate with the payment gateway. Customers never leave your Web store and credit cards are validated in real-time according to the settings you have set in the Authorize.Net Merchant Center (e.g. address verification on or off). Please note that you must have SSL enabled to process transactions through DPM.
                    </p>
                    <p>
                    ...
                    <div class="panel panel-default">
                        ...
                        <div id="collapseOne" class="panel-collapse collapse in">
                            <div class="panel-body">
                                ...
                                <% if request("mode")="Edit" then %>
                                    <% dim ADPM_LoginCnt,ADPM_LoginEnd,ADPM_LoginStart
                                    ADPM_LoginCnt=(len(x_Login)-2)
                                    ADPM_LoginEnd=right(x_Login,2)
                                    ADPM_LoginStart=""
                                    for c=1 to ADPM_LoginCnt
                                        ADPM_LoginStart=ADPM_LoginStart&"*"
                                    next %>
                                    <tr> 
                                        <td colspan="2">Current Login ID:&nbsp;<%=ADPM_LoginStart&ADPM_LoginEnd%></td>
                                    </tr>
                                    <tr> 
