...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName=N'"&paymentNickName&"' WHERE gwCode=64"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	set rs=nothing
	if err.number <> 0 then
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'PayJunction','gwPayJunction.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",64,N'"&paymentNickName&"')"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=connTemp.execute(query)
	if err.number<>0 then
		call LogErrorToDatabase()
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    <h4>Payjunction</h4>    
                    <p>One stop shop. PayJunction will setup both a merchant account and gateway   service for you. 24/7 customer support for accounts at no additional charge. PayJunction is also a PCI Level 1 compliant provider.</p>
                    <p>
                        <a class="btn btn-info btn-xs" href="#" target="_blank">Learn More</a>        
                    </p>
                </div>
