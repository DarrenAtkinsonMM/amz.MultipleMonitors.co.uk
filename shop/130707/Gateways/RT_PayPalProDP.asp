...
Function gwPPPDPGetName()
	gwPPPDPGetName = "PayPal Payments Pro (Direct Payments)"
	If myCountry = "UK" Then
		gwPPPDPGetName = "Website Payments Pro (Direct Payments)"
	End If
End Function

Function gwPPPDPGetURL()
	url = ""
	...
	Case "UK"
		url = "https://www.paypal.com/uk/webapps/mpp/paypal-payments-pro"
	Case Else
...
'--- Start PayPal Payments Pro (Direct Payments) ---
Function gwPPPDPEdit()	
	PayPalName = gwPPPDPGetName()
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName, pcPayTypes_ppab) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'" & PayPalName & "','gwPayPal.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","& percentageToAdd &",46,N'"&paymentNickName&"',0)"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=connTemp.execute(query)
	if err.number<>0 then
		call LogErrorToDatabase()
...
<% if request("gwchoice")="46" then
	...
	payPal_SupportedCardTypes = "V, M, A, D"
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
    ...
    <% else
		'//Check if any other PayPal Services are activated.
				query= "SELECT idPayment, gwCode FROM payTypes WHERE gwCode IN (3, 53, 80, 9, 99)"
		set rs=Server.CreateObject("ADODB.Recordset")     
		set rs=conntemp.execute(query)
		if NOT rs.eof then
        	...
        	<div class="pcCPmessage">
        	  <p>You currently have <strong><%=pcConflictDesc%></strong> active for this store. In order to use <strong><%= PayPalName %></strong> you will need to first disable <strong><%=pcConflictDesc%></strong>.<br />
        	    <br />
        	  </p>
        	  <p><a href="pcConfigurePayment.asp?mode=Del&id=<%=pcConflictIdPayment%>&gwchoice=<%=pcConflictID%>&activate=46">Disable <%=pcConflictDesc%> and enable <%= gwPPPDPGetName() %></a></p>
        	  <br />
...
<% if pcConflictIdPayment = 0 then %>
    ...
    <div class="pcCPmessageSuccess">
        <% if request("mode")="Edit" then %>
            <p><strong>You're editing </strong><strong><%= gwPPPDPGetName() %></strong>
            <br />
            <br />
            <p><strong><a href="pcPaymentSelection.asp">Change Payment Option</a></strong></p>
            <br /></p>
        <% else %>
            <p><strong>You've selected <%= gwPPPDPGetName() %></strong><br />
            ...
            <br>
            <a href="<%= gwPPPDPGetURL() %>" target="_blank">Sign Up and Learn More</a></strong>
            </p>
            <br />
                        ...
                        <div class="panel-heading">
                            <h4 class="panel-title">
                                <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
                                    Step 1: Configure Account - <%= gwPPPDPGetName() %>
                                </a>
                            </h4>
                        </div>
                                ...
                                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                                    ...
                                    <tr>
                                     	<td colspan="2" valign="top">
                                            <p>
                                                <strong>NOTE:</strong> 
                                                We recommend you use the latest <strong><a href=""><%= gwPPPGetName() %></a></strong> solution (also known as <strong><%= gwPPPGetName() %> Payflow Edition</strong>) if you use <a target="_blank" href="https://manager.paypal.com/">PayPal Manager</a> to manage your transactions. 
                                                However, if you only have your <strong>API Signature</strong> available for your account, use this method instead.
                                            </p>
                                            <br />
                                            ...
                                            <p><a href="https://www.paypal.com/us/cgi-bin/webscr?cmd=_get-api-signature&amp;generic-flow=true" target="_blank">Get your API Credentials.</a></p><br />
    
                                     	</td>
                                 	 </tr>
                                    <tr> 
                                        <td width="127" valign="top" nowrap>
                                            <div align="right">API Credentials: <br /></div>
                                        </td>
                                        <td>
                        			...
                        			<% 
                                            ...
                                            for i=0 to UBound(cardTypeArray)
                                                select case Trim(cardTypeArray(i))
                                                    case "V"
                                                        V=true
                                                    case "M"
                                                        M=true
                                            ...
                                            SA=false
                                            SD=false
                                            supportedCardTypes=split(payPal_SupportedCardTypes,",")
                                            for i=0 to UBound(supportedCardTypes)
                                                select case Trim(supportedCardTypes(i))
                                                    case "V"
                                                        SV=true
                                                    case "M"
                                                        SM=true
                                    ...
                                    <tr> 
                                        ...
                                        <td>
                                            <% If SV Then %>
                                                <input name="pcPay_PayPal_CardTypes" type="checkbox" class="clearBorder" value="V" <% if V then response.write "checked" %>> Visa 
                                            <% End If %>
                                            <% If SM Then %>
                                                <input name="pcPay_PayPal_CardTypes" type="checkbox" class="clearBorder" value="M" <% if M then response.write "checked" %>> MasterCard 
                                            <% End If %>
                                            <% If SA Then %>
                                                <input name="pcPay_PayPal_CardTypes" type="checkbox" class="clearBorder" value="A" <% if A then response.write "checked" %>> American Express
                                            <% End If %>
                                            <% If SD Then %>
                                                <input name="pcPay_PayPal_CardTypes" type="checkbox" class="clearBorder" value="D" <% if D then response.write "checked" %>> Discover
                                            <% End If %>
                                        </td>
                                    </tr>
                                    <tr> 
                                    ...
                                    <tr>
                                        <td>&nbsp;</td>
                                        <td>
                                             <a class="pcCPhelp" href="helpOnline.asp?ref=805">More information on <%= gwPPPDPGetName() %></a>
                                        </td>
                                    </tr>
                                    <tr>
                                    	<td colspan="2">&nbsp;</td>
