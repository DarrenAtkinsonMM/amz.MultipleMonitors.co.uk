...
Function gwPPAGetName()
	gwPPAGetName = "PayPal Payments Advanced"
End Function

Function gwPPAGetURL()
	gwPPAGetURL = "https://www.paypal.com/webapps/mpp/paypal-payments-advanced"
End Function

'--- Start PayPal Payments Advanced ---
Function gwPPAEdit()
		
	paymentDesc = gwPPAGetName()
	
	'request gateway variables and insert them into the pcPay_PayPalvanced table
	...
	pcPay_PayPal_Username=pcPay_PayPal_Username
	...
	pcPay_PayPal_Password=pcPay_PayPal_Password
	...
	query="UPDATE pcPay_PayPal SET pcPay_PayPal_Partner='"&pcPay_PayPal_Partner&"',pcPay_PayPal_Vendor='"&pcPay_PayPal_Vendor&"',pcPay_PayPal_Username='"&pcPay_PayPal_Username&"',pcPay_PayPal_Password='"&pcPay_PayPal_Password &"',pcPay_PayPal_TransType="&pcPay_PayPal_TransType&",pcPay_PayPal_CVC="&pcPay_PayPal_CVC&",pcPay_PayPal_Sandbox="&pcPay_PayPal_Sandbox&", pcPay_PayPal_Layout='"&pcPay_PayPal_Layout&"', pcPay_PayPal_Shape='"&pcPay_PayPal_Shape&"', pcPay_PayPal_Size='"&pcPay_PayPal_Size&"', pcPay_PayPal_Color='"&pcPay_PayPal_Color&"' WHERE pcPay_PayPal_ID=1"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	set rs=nothing
	if err.number <> 0 then
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",paymentDesc='" & paymentDesc & "',priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName='PayPal',pcPayTypes_ppab=0 WHERE gwCode=80"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		strErrorDescription=err.description
...
Function gwPPA()
	...
	query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName, pcPayTypes_ppab) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'" & paymentDesc & "','gwPPA.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","& percentageToAdd &",80,N'"&paymentNickName&"',0)"
	set rs=Server.CreateObject("ADODB.Recordset")
	set rs=connTemp.execute(query)
	if err.number<>0 then
		call LogErrorToDatabase()
...
<% if request("gwchoice")="80" then
	...
	if request("mode")="Edit" then
		...
		query= "SELECT pcPay_PayPal_ID, pcPay_PayPal_Partner, pcPay_PayPal_Vendor, pcPay_PayPal_Username, pcPay_PayPal_Password, pcPay_PayPal_TransType, pcPay_PayPal_CVC, pcPay_PayPal_Sandbox, pcPay_PayPal.pcPay_PayPal_Layout, pcPay_PayPal.pcPay_PayPal_Shape, pcPay_PayPal.pcPay_PayPal_Size, pcPay_PayPal.pcPay_PayPal_Color FROM pcPay_PayPal where pcPay_PayPal_ID=1"
		set rs=Server.CreateObject("ADODB.Recordset")
		set rs=conntemp.execute(query)
		if err.number <> 0 then
			strErrorDescription=err.description
		...
		ppa_UserCnt=(len(pcPay_PayPal_Username)-2)
		ppa_UserEnd=right(pcPay_PayPal_Username,2)
		ppa_UserStart=""
    ...
    <% else
		'//Check if any other PayPal Services are activated.
				query= "SELECT idPayment, gwCode FROM payTypes WHERE gwCode IN (3, 46, 53, 9, 99, 999999)"
		set rs=Server.CreateObject("ADODB.Recordset")     
		set rs=conntemp.execute(query)
		if NOT rs.eof then
        	...
        	<div class="pcCPmessage">
        	  <p>You currently have <strong><%=pcConflictDesc%></strong> active for this store. In order to use <strong><%= gwPPAGetName() %></strong> you will need to first disable <strong><%=pcConflictDesc%></strong>.<br />
        	    <br />
        	  </p>
        	  <p><a href="pcConfigurePayment.asp?mode=Del&id=<%=pcConflictIdPayment%>&gwchoice=<%=pcConflictID%>&activate=80">Disable <%=pcConflictDesc%> and enable <%= gwPPAGetName() %></a></p>
        	  <br />
...
<% if pcConflictIdPayment = 0 then %>
    ...
    <div class="pcCPmessageSuccess">
        <% if request("mode")="Edit" then %>
            <p><strong>You're editing <%= gwPPAGetName() %></strong>
            <br />
            <br />
            <p><strong><a href="pcPaymentSelection.asp">Change Payment Option</a></strong></p>
            <br /></p>
        <% else %>
            <p><strong>You've selected <%= gwPPAGetName() %></strong><br>
            <br>
            ...
            <br>
            <a href="<%= gwPPAGetURL() %>" target="_blank">Sign Up and Learn More</a></strong>
      </p>
            <br />
                        ...
                        <div class="panel-heading">
                            <h4 class="panel-title">
                                <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">
                                    Step 1: Configure Account - <%= gwPPAGetName() %>
                                </a>
                            </h4>
                        </div>
    ...
    so that payments can be sent to your account.<br /></td>
                                    ...
                                    <tr> 
                                        ...
                                        <td><input type="text" value="" name="pcPay_PayPal_Vendor" class="pcAutoCompleteOff" size="24" /></td>
                                    </tr>
                                    <tr>
                                      <td align="right">User:</td>
                                      <td><input type="text" value="" name="pcPay_PayPal_Username" class="pcAutoCompleteOff" size="24" /></td>
                                    </tr>
                                    <tr>
                                      <td align="right">Password:</td>
                                      <td><input type="password" value="" name="pcPay_PayPal_Password" class="pcAutoCompleteOff" size="24" /></td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr> 
                                        ...
                                        <td><input type="checkbox" class="clearBorder" name="pcPay_PayPal_Sandbox" value="1" <% If pcPay_PayPal_Sandbox="1" Then Response.Write "Checked" %>></td>
                                    </tr>
                                    <tr>
                                    	<td colspan="2" class="pcCPspacer"></td>
                                    </tr>
                                    <tr>
                                    	<td colspan="2">&nbsp;&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=800">More info on <%= gwPPAGetName() %></a></td>
                                    </tr>
                                    <tr>
                                        <td colspan="2">&nbsp;</td>
                                    </tr>
