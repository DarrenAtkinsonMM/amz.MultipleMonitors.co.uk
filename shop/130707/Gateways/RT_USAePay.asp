...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	query="UPDATE payTypes SET  pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ", priceToAdd="&priceToAdd&" ,percentageToAdd="&percentageToAdd&" ,paymentNickName=N'"&paymentNickName&"' WHERE gwCode=35"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		strErrorDescription=err.description
...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	if pcPay_Uep_Checking="1" then
		...
		if not rs.eof then
			query="UPDATE payTypes SET priceToAdd="&priceToAdd&" ,percentageToAdd="&percentageToAdd&" ,paymentNickName=N'"&paymentNickName2&"' WHERE gwCode=36"
			set rs=Server.CreateObject("ADODB.Recordset")     
			set rs=conntemp.execute(query)
		else
			query="INSERT INTO payTypes (paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES ('USAePay eCheck','gwUSAePayCheck.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",36,N'"&paymentNickName2&"')"
			set rs=Server.CreateObject("ADODB.Recordset")     
			set rs=conntemp.execute(query)
		end if
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	if pcPay_Uep_Checking=1 then
		query="INSERT INTO payTypes (pcPayTypes_processOrder,pcPayTypes_setPayStatus,paymentDesc, sslURL, active, Cbtob, quantityFrom, quantityUntil, weightFrom, weightUntil, priceFrom, priceUntil,ssl,priceToAdd, percentageToAdd,gwCode,paymentNickName) VALUES (" & pcv_processOrder & "," & pcv_setPayStatus & ",'USAePay eCheck','gwUSAePayCheck.asp',-1,0,0,9999,0,9999,0,9999,-1,"& priceToAdd &","&percentageToAdd&",36,N'"&paymentNickName2&"')"
		set rs=Server.CreateObject("ADODB.Recordset")     
		set rs=connTemp.execute(query)
		if err.number<>0 then
			call LogErrorToDatabase()
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    <h4>USAePay</h4>    
                    <p>Since 1998 the USA ePay team has been making it possible for businesses to   accept credit cards easily, safely and securely.</p>
                    <p>
                        <a class="btn btn-info btn-xs" href="http://www.usaepay.com" target="_blank">Learn More</a>        
                    </p>
                </div>
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                        ...
                        <tr>
                            <td colspan="2">
                              ...
                              <br /><br /><input type="button" class="btn btn-default"  value="Back" onclick="javascript:history.back()"></center>
                              </td>
                        </tr>
					<% else %>
						<% if request("mode")="Edit" then %>
                        ...
                        <tr> 
                            ...
                            <td valign="top"> <select name="pcPay_Uep_TransType">
                                    <option value="1" <%if pcPay_Uep_TransType="1" then%>selected<%end if%>>Sale</option>
