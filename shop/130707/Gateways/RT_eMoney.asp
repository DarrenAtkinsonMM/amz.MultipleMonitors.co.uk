...
response.redirect "techErr.asp?error="& Server.Urlencode("Error: "&strErrorDescription) 
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName=N'"&paymentNickName&"' WHERE gwCode=54"
	set rs=Server.CreateObject("ADODB.Recordset")     
	set rs=conntemp.execute(query)
	set rs=nothing
	if err.number <> 0 then
        ...
        <tr>
            <td>
                <div class="bs-callout bs-callout-info">
                    <h4>ETS EMoney Transaction Platform</h4>    
                    <p>ETS developed the EMoney transaction platform to provide merchants with a &quot;one-stop&quot; turnkey solution for merchant processing.</p>
                    <p>
                        <a class="btn btn-info btn-xs" href="https://www.etsms.com/emoney.aspx" target="_blank">Learn More</a>        
                    </p>
                </div>
