...
Function OffLineCCEdit()
	...
	query="UPDATE payTypes SET pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus & ",priceToAdd="& priceToAdd &" , percentageToAdd="& percentageToAdd &", paymentNickName=N'"&paymentNickName&"', Cbtob="&Cbtob&" WHERE gwCode= 6"
...
Function OffLineCustomEdit()
	...
	query="UPDATE PayTypes SET paymentDesc='"&paymentDesc&"', terms='"& terms &"', Creq="& Creq &", Cbtob="& Cbtob &", Cprompt='"& Cprompt &"',priceToAdd="& priceToAdd &",percentageToAdd="& percentageToAdd &", paymentNickName=N'"&paymentNickName&"', pcPayTypes_processOrder=" & pcv_processOrder & ",pcPayTypes_setPayStatus=" & pcv_setPayStatus &"  WHERE idPayment="&idPayment
	set rstemp=Server.CreateObject("ADODB.Recordset")     
	set rstemp=conntemp.execute(query)
    ...
    <tr> 
        <td>
            ...
            <table width="100%" border="0" cellspacing="0" cellpadding="4">
				...
				<tr> 
					<td colspan="2">
					Process orders when they are placed: <input type="checkbox" name="pcv_processOrder" value="1" <%if pcv_processOrder="1" then%>checked<%end if%> class="clearBorder"></td>
				</tr>
				<tr> 
					<td colspan="2">When orders are placed, set payment status is:
						<select name="pcv_setPayStatus">
							<option value="0" <%if (pcv_setPayStatus="0") OR (pcv_setPayStatus="") OR (pcv_setPayStatus>"2") then%>selected<%end if%>>Pending</option>
