...
response.redirect "addSFtoPrds.asp?nav="&nav&"&idproduct="&idproduct&"&x="&x&"&message="&server.URLEncode("You must specify a field name.")
	...
	if CP_showSEARCH="" then
		...
		if rs.eof then
			query="INSERT INTO pcSearchFields (pcSearchFieldName,pcSearchFieldShow,pcSearchFieldOrder,pcSearchFieldCPShow,pcSearchFieldSearch,pcSearchFieldCPSearch) VALUES (N'"&customname&"'," & tmpFieldShow & "," & tmpFieldOrder & "," & CP_show & "," & SF_showSEARCH & "," & CP_showSEARCH & ");"
			set rs=Server.CreateObject("ADODB.Recordset") 
			set rs=conntemp.execute(query)
		end if
		set rs=nothing
		...
		if not rs.eof then
			...
			if keyword<>"" then
				query="INSERT INTO pcSearchData (idSearchField,pcSearchDataName,pcSearchDataOrder) VALUES (" & idcustom & ",N'" & keyword & "'," & tmpValueOrder & ");"
				set rs=connTemp.execute(query)
				query="SELECT idSearchData FROM pcSearchData WHERE idSearchField=" & idcustom & " AND pcSearchDataName like '" & keyword & "';"
				set rs=connTemp.execute(query)
				idkeyword=rs("idSearchData")
