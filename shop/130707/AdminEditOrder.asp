...
if request.QueryString("del")="YES" then
	...
	For j=1 to mCount
		if clng(delPrd)=clng(session("admin_cp_" & qryID & "_idprd" & j)) then
			...
			set rs=conntemp.execute(query)
			set rs=nothing
			call pcs_hookStockChanged(session("admin_cp_" & qryID & "_id" & j), "")
			exit for
		end if
	Next
...
if request.QueryString("action")="upd" OR request.Form("SubmitUPD")<>"" then
	...
	IF pcv_ordStatus="3" THEN
		...
		'Call License Generator for Standard & Configurable Products
		if pcv_OrdDPs="1" then
			...
			do while not rstemp.eof
				...
				if not rs.eof then
					...
					'Insert Standard & Configurable Products Download Requests into DPRequests Table
...
if request.QueryString("action")="upd" then
	...
	If statusAPP="1" OR scAPP=1 Then
		%>
		<!--#include file="app-cp-UpdSPDiscounts.asp"-->
		<%
	End If
	...
	do until rs.eof
		...
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		'// Start: ReCalculate Configurable Items Discounts
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		itemsDiscounts=0
		if scBTO=1 then
		if pIdConfigSession<>"0" then
			...
			if ArrProduct(0)<>"na" then
				...
				next
			end if 'Have Configurable Items
		...
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		'// END: ReCalculate Configurable Items Discounts
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

		'Configurator Additional Charges
		Charges=0
		if scBTO=1 then
			if pIdConfigSession<>"0" then
		...
		end if 'Configurator Additional Charges
	...
	if varShip="0" then
		...
		if shipServiceCode<>"" then
			tShippingString=tShippingString & ",0," & shipServiceCode
		end if
	end if
	...
	'Update PayPal Payments Advanced Amounts
	query="SELECT amt FROM pfporders WHERE idOrder = " & qryID & ";"
    set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if not rs.eof then
		query="UPDATE pfporders SET amt=" & tTotal & " WHERE idOrder = " & qryID & ";"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		set rs=nothing
	end if
	...
	query="SELECT amount FROM pcPay_PFL_Authorize WHERE idOrder = " & qryID & ";"
    set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if not rs.eof then
		query="UPDATE pcPay_PFL_Authorize SET amount=" & tTotal & " WHERE idOrder = " & qryID & ";"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		set rs=nothing
	end if
...
if request.Form("SubmitUPD")<>"" then
	...
	If statusAPP="1" OR scAPP=1 Then
		for i=1 to tCnt
			...
			tunitPrice=replacecommaToCal(request.Form("unitPrice"&i))
			query="UPDATE ProductsOrdered SET quantity="&tQTY&", unitPrice="&replacecommaToDB(tunitPrice)&" WHERE idProductOrdered="&tIdProductOrdered&" AND idOrder="&tIdOrder&";"
			set rs=server.CreateObject("ADODB.RecordSet")
			set rs=connTemp.execute(query)
			set rs=nothing
		next	
	...
	for i=1 to tCnt
		...
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		'// Start: Configurator Additional Charges
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		Charges=0
		if pIdConfigSession<>"0" then
			query="SELECT stringCProducts, stringCValues, stringCCategories FROM configSessions WHERE idconfigSession=" & pIdConfigSession
		...
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		'// END: Configurator Additional Charges
		'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		query="SELECT products.pcProd_ParentPrd FROM products WHERE products.idProduct="&tIdProduct&";"
		set rsParent=server.CreateObject("ADODB.RecordSet")
		set rsParent=conntemp.execute(query)
		pcv_intParentId=rsParent("pcProd_ParentPrd")
		set rsParent=nothing

		tmpHaveParentDisCount=0
		IF pcv_intParentId>"0" then
			query="SELECT * FROM discountsPerQuantity WHERE idProduct=" &pcv_intParentId& ";"
			set rstemp=server.CreateObject("ADODB.RecordSet")
			set rstemp=conntemp.execute(query)
			if not rstemp.eof then
				tmpHaveParentDisCount=1
			end if
		end if
		set rstemp=nothing
		...
		IF tmpHaveParentDisCount=1 THEN

			query="SELECT QDiscounts FROM ProductsOrdered  WHERE idProductOrdered="&tIdProductOrdered&" AND idOrder="&tIdOrder&";"
			response.write query & "<br>"
			set rstemp=server.CreateObject("ADODB.RecordSet")
			set rstemp=conntemp.execute(query)
			if not rstemp.eof then
				tempNum=rstemp("QDiscounts")
			end if
			set rstemp=nothing
	...
	'*****************************************************
		...
		Do While not rs.eof
			...
			for o=1 to request.form("tCnt")
                ...
                pcv_intId = request.Form("IdProduct" & o)

                pcv_intParentId = ""
				query="SELECT products.pcProd_ParentPrd FROM products WHERE products.idProduct=" &pcv_intId
				set rsParent=server.CreateObject("ADODB.RecordSet")
				set rsParent=conntemp.execute(query)
				...
				if pcv_intParentId>0 then
					query="select idproduct from categories_products where idcategory=" & rs("IDCat") & " and idproduct=" & pcv_intParentId
				else
					query="select idproduct from categories_products where idcategory=" & rs("IDCat") & " and idproduct=" & pcv_intId
				end if
				set rstemp=server.CreateObject("ADODB.RecordSet")
				set rstemp=connTemp.execute(query)
				if not rstemp.eof then
					...
					CatSubTotal = CatSubTotal + (request.Form("unitPrice" & o) * request.Form("QTY"&o)) '- itemsDiscounts - tempQDiscounts

					pcv_strApplicableProducts = pcv_strApplicableProducts & pcv_intId & chr(124) &  ApplicableCategoryID & ","
				end if
				set rstemp=nothing
	...
	'Check if the order is using Avalara tax
	query = "SELECT pcOrd_Avalara FROM orders WHERE idOrder = " & qryID
	set rs = ConnTemp.execute(query)
	ptaxAvalaraOrder = rs("pcOrd_Avalara")
	...
	if ptaxAvalaraOrder = 1 then
		...
		query = "SELECT TOP 1 idOrderCounter, status FROM Avalara_orders WHERE idOrder = " & qryID & " ORDER BY idOrderCounter DESC"
		Set rs = conntemp.execute(query)
		aIdOrder = rs("idOrderCounter")
        aStatus = rs("status")
        Set rs = Nothing
		
        '// Void Avalara transaction if the current status is Committed
		If lcase(aStatus) = "committed" OR lcase(aStatus) = "voided" then			
			call pcs_VoidAvalara(aIdOrder)
			if pcv_ErrorMessage <> "" then
				response.redirect "AdminEditOrder.asp?ido="&request("ido")&"&error=" & Server.HTMLEncode(pcv_ErrorMessage)
			end if
		end if
		
        '// After Void, we need to resubmit...
		call pcs_CommitAvalara(qryID, "edit")
		if pcv_ErrorMessage <> "" then
			response.redirect "AdminEditOrder.asp?ido="&request("ido")&"&error=" & Server.HTMLEncode(pcv_ErrorMessage)
		end if
		...
		if intOrderStatus = 3 then
			call pcs_CommitAvalara(qryID, "commit")
			if pcv_ErrorMessage <> "" then
				response.redirect "AdminEditOrder.asp?ido="&request("ido")&"&error=" & Server.HTMLEncode(pcv_ErrorMessage)
			end if
		end if
	...
	'Update PayPal Payments Advanced Amounts
	query="SELECT amt FROM pfporders WHERE idOrder = " & tIdOrder & ";"
    set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if not rs.eof then
		query="UPDATE pfporders SET amt=" & replacecommaToDB(tTotal) & " WHERE idOrder = " & tIdOrder & ";"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		set rs=nothing
	end if
	...
	query="SELECT amount FROM pcPay_PFL_Authorize WHERE idOrder = " & tIdOrder & ";"
    set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if not rs.eof then
		query="UPDATE pcPay_PFL_Authorize SET amount=" & replacecommaToDB(tTotal) & " WHERE idOrder = " & tIdOrder & ";"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		set rs=nothing
	end if
...
<form action="AdminEditOrder.asp" method="post" name="EditOrder" class="pcForms" onSubmit="return validate()">
	...
	<% if wrngID=0 then %>
	  ...
	  <tr>
				...
				<td>
					<table class="pcCPcontent">
						<tr>
							...
							<th>Product</th>
							<th>Qty</th>
							<th>Unit Price&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=312"></a></th>
							<th colspan="2">Price&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=312"></a></th>
						</tr>
						...
						query=query&", products.description, products.weight, products.sku, products.notax, products.stock, products.nostock, products.pcProd_BackOrder, products.pcprod_QtyToPound, products.pcProd_ParentPrd FROM products INNER JOIN ProductsOrdered ON products.idProduct = ProductsOrdered.idProduct WHERE (((ProductsOrdered.idOrder)="&qryID&"));"
						set rs=server.CreateObject("ADODB.RecordSet")
						...
						do until rs.eof
							...
							Dim pcv_TargetProduct
							if (pcv_ParentPrd<>"") and (pcv_ParentPrd>"0") then
								pcv_TargetProduct = pcv_ParentPrd
							else
								pcv_TargetProduct = pIdProduct
							end if
							%>

							<tr>
								<td><%=pSKU%></td>
								<td><a href="FindProductType.asp?idproduct=<%=pcv_TargetProduct%>" target="_blank"><%=pProductName%></a>
								<%if pcSCID>"0" then
								...
								<%'GGG Add-on
								if gIDEvent="0" then%><a href="AdminEditOrder.asp?action=restore&Cnt=<%=pCnt%>&ido=<%=pIdOrder%>&idprd=<%=pIdProduct%>&idprdO=<%=pIdProductOrdered%>" title="Restore unit price to current online price for this product">Restore</a>&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=313"></a><%end if 'GGG Add-on end%>
								&nbsp;&nbsp;
						...
						<%
						Dim pcv_intParentId
						query="SELECT products.pcProd_ParentPrd FROM products WHERE products.idProduct="&pIdProduct&";"
						set rsParent=server.CreateObject("ADODB.RecordSet")
						set rsParent=conntemp.execute(query)
						pcv_intParentId=rsParent("pcProd_ParentPrd")
						set rsParent=nothing
						...
						if pcv_intParentId>"0" then
							...
							if NOT rstemp.eof then %>
								<tr bgcolor="#FFFFFF">
									...
									<td>
										<%
										Dim pcv_intQDProduct
										if pcv_intParentId>"0" then
											pcv_intQDProduct=pcv_intParentId
										else
											pcv_intQDProduct=pIdProduct
										end if
										%>
										<a href="javascript:win2('../pc/priceBreaks.asp?type=<%=Session("customerType")%>&idproduct=<%=pcv_intQDProduct%>')">View Quantity Discounts</a>
									</td>
									<td></td>
						  ...
						  if (pcv_ParentPrd<>"") and (pcv_ParentPrd>"0") then%>
							  <tr>
								  ...
								  <td colspan="5">
									  <% 'APP-S %>
									  <a href="addPrdtoOrd.asp?ido=<%=qryID%>&SubPrd=<%=pidProduct%>&idproduct=<%=pcv_ParentPrd%>&prename=<%=Server.HTMLEncode(pProductName)%>&pIdPrdOrd=<%=pIdProductOrdered%>">Edit Options</a>
									  <% 'APP-E %>
								  </td>
							  </tr>
						  <%end if
		...
		<% query="SELECT orderDate, idCustomer, total, taxAmount, ord_VAT, shipmentDetails, paymentDetails, discountDetails, iRewardPoints,iRewardValue,iRewardPointsCustAccrued,taxDetails,pcOrd_GcCode,pcOrd_GcUsed,pcOrd_Avalara FROM orders WHERE idOrder="&qryID&";"
	...
	<table class="pcCPcontent">
		<tr>
			<th colspan="4">Other Settings&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=314"></a></th>
		</tr>
		<tr>
			...
			<% else
				...
				for i=0 to (ubound(taxArray)-1)
					...
					<tr bgcolor="#e1e1e1">
                        ...
                        &nbsp;
                        <% else %>
                        Calculate&nbsp; <input name="calculateTaxPercentage<%=pTaxTypeCnt%>" type="text" value="<%=session("adminTaxPercentage"&pTaxTypeCnt)%>" size="6">
						...
						<td>
							...
							<input name="TaxTotal<%=pTaxTypeCnt%>" type="text" value="<%=money(taxDesc(1))%>" size="10" <% if ptaxAvalaraOrder = 1 then %>disabled<% end if %>>
						</td>
...
<table class="pcCPcontent">
	...
	<tr>
		<td align="center">
		<input name="SubmitUPD" type="submit" class="btn btn-primary" id="SubmitUPD" value="Update Order">
			&nbsp;&nbsp;
		<input type="button" class="btn btn-default"  name="Button" value="Back to Order Details" onClick="location.href='Orddetails.asp?id=<%=qryID%>'">
		</td>
	</tr>
</table>
</form>
