...
tmpquery=""
Select Case request("incApp")
Case "1": tmpquery="WHERE pcprod_ParentPrd>0"
Case "2": tmpquery=""
Case Else:
	tmpquery="WHERE pcprod_ParentPrd=0"
End Select

if strSQL2<>"" and tmpquery<>"" then
	tmpquery=Replace(tmpquery,"WHERE"," AND")
end if

query="SELECT idproduct,sku,description,sdesc,details,price,listprice,btoBprice,serviceSpec, configOnly,weight,stock,pcProd_ReorderLevel,imageURL,smallImageUrl,largeImageURL,idbrand,active,listHidden,hotDeal,notax,noshipping,formQuantity,emailtext,OverSizeSpec,removed FROM products " & strSQL2 & tmpquery & " order by idproduct"
		...
		Do Until rstemp.EOF
			...
			If ptype="1" then
				if rstemp("serviceSpec")=-1 then
					StringBuilderObj.append "<td>" & "CONFIGURABLE"& "</td>"
				else
