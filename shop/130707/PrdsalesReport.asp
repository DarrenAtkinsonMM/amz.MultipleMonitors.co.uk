...
if pIDProduct<>"" then
	...
	queryT="SELECT idproduct FROM Products WHERE pcProd_ParentPrd=" & pidproduct & ";"
	set rsQ=connTemp.execute(queryT)
	tmpT=pidproduct
	if not rsQ.eof then
		pcArrT=rsQ.getRows()
		intCountT=ubound(pcArrT,2)		
		For mk=0 to intCountT
			if tmpT<>"" then
				tmpT=tmpT & ","
			end if
			tmpT=tmpT & pcArrT(0,mk)
		Next
	end if
	set rsQ=nothing
	
	SQLCC=" (ProductsOrdered.IDproduct IN (" & tmpT & ")) and "
...
<%
	...
	ELSE 'View Report for only 1 product%>
		...
		tmpQ=" (ProductsOrdered.IDproduct IN (" & tmpT & ")) "
				
		query="SELECT products.description,orders.idorder,orders.idcustomer,orders.total,orders.shipmentDetails,orders.paymentDetails,orders.taxAmount,orders.discountDetails,orders.orderDate,orders.processDate,orders.shipDate,ProductsOrdered.quantity,ProductsOrdered.unitPrice,ProductsOrdered.QDiscounts,ProductsOrdered.ItemsDiscounts FROM Orders,ProductsOrdered,Products WHERE Products.idproduct=ProductsOrdered.idproduct AND " & tmpQ & " and orders.idorder=ProductsOrdered.idorder and ((orders.orderStatus>2 AND orders.orderStatus<5) OR (orders.orderStatus>6 AND orders.orderStatus<9) OR (orders.orderStatus=10 OR orders.orderStatus=12)) " & TempSQL1 & TempSQL2 & TempSpecial & SQLCC1 & " ORDER BY " & tmpD & " DESC;"
...
&nbsp;for <%=PrdName%><%end if%><%if pcSaleName<>"" then%><br>The Sale Name: <%=pcSaleName%><%end if%></h2></td>
	...
	<tr> 
		...
		<%if pcv_isAPP=1 then%>
			<th colspan="2" nowrap>Product Name</th>
		<%else%>
...
do until rs.EOF
	...
	<tr valign="top">  
		...
		<%if pcv_isAPP=1 then%>
			<td colspan="2" nowrap><%=pc_PrdName%></td>
		<%else%>
	...
	<tr bgcolor="#e1e1e1">
		...
		<td><%if pcv_isAPP=1 then%><%else%><b><%=gTotalNumberOrders%></b><%end if%></td>
		<td><b><%=gTotalQty%></b></td>
