<% 
PmAdmin=19
pageTitle = "ProductCart Order Events - Database Upgrade" 
Section = "" 
%>
...
IF request("action")="sql" then
	if request("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			call closeDb()
			response.redirect "upddb_orderEvents.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			call closeDb()
			response.redirect "upddb_orderEvents.asp?mode=1"
			response.End
		end if
	...
	'========================================================================
	'// START OF DB UPDATES FOR Order Events
	'========================================================================

	'// CREATE NEW TABLES
	'***************************************************************
	'// Create table pcOrderEvents
	if not TableExists("pcOrderEvents") then
		query="CREATE TABLE pcOrderEvents ("
		query=query&"idEvent [int] IDENTITY (1,1) PRIMARY KEY NOT NULL,"
		query=query&"idOrder [int] NOT NULL DEFAULT(0),"
		query=query&"status [tinyint] NOT NULL DEFAULT(0),"
		query=query&"date [datetime],"
		query=query&"handle [nvarchar](100)"
		query=query&");"
		err.clear
		on error resume next
    ...
    call AlterTableSQL("orders", "ADD", "pcOrd_Exported", "[tinyint]", 1, "0", "1")
	
	set rs=nothing
	
	'========================================================================
	'// END OF DB UPDATES FOR Order Events
	'========================================================================
...
<form action="upddb_orderEvents.asp" method="post" name="form1" id="form1" class="pcForms">
<%
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
                ...
                <h1 class="page-header">Order Events Database Update</h1>
                <p class="lead">
