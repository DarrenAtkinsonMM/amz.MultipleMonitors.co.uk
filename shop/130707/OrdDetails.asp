...
'// FedEx Web Services Config
set objFedExWSClass = New pcFedExWSClass
pcv_strFedExWSEnabled = objFedExWSClass.pcf_FedExEnabled
pcv_strFedExWSPackagesExist = objFedExWSClass.pcf_FedExPackages(qry_ID)
...
pcs_JavaTextField	"name", true, dictLanguage.Item(Session("language")&"_NewCust_3"), "5"
pcs_JavaTextField	"email", true, dictLanguage.Item(Session("language")&"_NewCust_3"), "5"
pcs_JavaTextField	"address", true, dictLanguage.Item(Session("language")&"_NewCust_3"), "5"
pcs_JavaTextField	"city", true, dictLanguage.Item(Session("language")&"_NewCust_3"), "5"
pcs_JavaTextField	"zip", true, dictLanguage.Item(Session("language")&"_NewCust_3"), "5"
...
<table class="pcCPcontent">
	...
	<%
    query="SELECT SB_GUID, SB_Terms, idOrder FROM SB_Orders WHERE idOrder=" & qry_ID & ";"    
    ...
    If NOT rsSB.eof Then
		...
		pcv_idOrder = rsSB("idOrder")
		pcv_ParentID = GetParentOrderID(pcv_strGUID)
        %>
        <tr>
            <td colspan="2" class="pcCPspacer">
            	<% If pcv_ParentID = pcv_idOrder Then %>
                    <div class="pcCPSBInfo">
                        This is order contains a subscription. <a href="https://www.subscriptionbridge.com/merchantcenter/" target="_blank">Log into SubscriptionBridge</a> to view subscription details. <a href="sb_ViewSubs.asp?idmain=&Guid=<%=pcv_strGUID%>&idOrder=<%=pcv_idOrder%>">Locate additional orders</a> related to this subscription.
                    </div>
                <% Else %>
                    <div class="pcCPSBInfo">
                        This order is part of a subscription. <a href="Orddetails.asp?id=<%=pcv_ParentID%>">View the order</a> that activated this subscription. <a href="sb_ViewSubs.asp?idmain=&Guid=<%=pcv_strGUID%>&idOrder=<%=pcv_idOrder%>">Locate additional orders</a> related to this subscription.
                    </div>
                <% End If %>
            </td>
        </tr>
	...
	<tr>
		<td colspan="2" class="pcCPspacer">
			...
			<h2>Order #: <%=(scpre+int(qry_ID))%>&nbsp;|&nbsp;Date: <%=porderdate%><% if pcv_OrderTime<>"" then %>&nbsp;|&nbsp;Time: <%=pcv_OrderTime%><%end if%>&nbsp;|&nbsp;Total: <%=scCurSign&money(ptotalAdj)%>&nbsp;|&nbsp;<a href="OrdInvoiceGuides.asp?id=<%=qry_ID%>">Email Invoice</a>
...
<form id="form2" name="form2" method="POST" action="processOrder.asp" onSubmit="return Form1_Validator(this)" class="pcForms">
	<input type="hidden" name="ActiveTab" value="<%=intActiveTab%>">
	<input type="hidden" name="qry_ID" value="<%=request("id")%>">
		...
		<tr>
            ...
            <div id="TabbedPanels2" class="tabbable">
                ...
                <ul class="nav nav-tabs">
                    ...
                    <li><a href="#tab2" data-toggle="tab">Product Details</a></li>
                    <li><a href="#tab3" data-toggle="tab">Process/Update</a></li>
                    <li><a href="#tab4" data-toggle="tab">Shipping Center</a></li>
                    <li><a href="#tab5" data-toggle="tab">Payment Info</a></li>
                    <li><a href="#tab6" data-toggle="tab">Billing &amp; Shipping</a></li>
				...
				<div id="tab1" class="tab-pane active">
					<table class="pcCPcontent">
						...
						<%if pcOrderKey<>"" then%>
						<tr>
							<td colspan="2">Order Code: <strong><%=pcOrderKey%></strong><%if pcvGuest="1" then%>&nbsp;-&nbsp;Guest Checkout&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=317"></a><%end if%></td>
						</tr>
						<%end if%>
						...
						pcv_PayPalSpecialFeatures = 0

						If getIsPayPalClassicEnabled Then
                        
                            If instr(PaymentType,"PayPal") Then
							    pcv_PayPalSpecialFeatures = 1
                            End If
                            
						ElseIf getIsPFApiEnabled Then
                        
							pcv_PayPalGWCode = getPFApiGatewayCode(qry_id)

							'// Identify which gateway we were using
							If trim(PaymentType)="PayPal Express Checkout" Then
								PaymentType = getPFApiExpressTitle(pcv_PayPalGWCode)
							End If
						End If
						...
						<tr>
							<td colspan="2">
								...
								if porderstatus="7" then %>
									<a href="#" onClick="$pc('#TabbedPanels2 li:eq(1) a').tab('show'); form2.ActiveTab.value = 2">View partial shipment details</a>
									&nbsp;|&nbsp;
								<%
								end if
						...
						<tr>
							<td>This order contains products that will be <strong>drop-shipped</strong> by a third party. See <a href="#" onClick="$pc('#TabbedPanels2 li:eq(1) a').tab('show'); form2.ActiveTab.value = 2">Product Details</a> for more information.</td>
						</tr>
						<% end if %>

						<% if pcv_haveBOPrds <> 0 then %>
						<tr>
							<td>This order contains products that might be <strong>back-ordered</strong>. See <a href="#" onClick="$pc('#TabbedPanels2 li:eq(1) a').tab('show'); form2.ActiveTab.value = 2">Product Details</a> to see a list of back-ordered products.</td>
						</tr>
						<% end if %>
						...
						<tr>
							<td colspan="2">Order Name:
								...
								%>
									<input type="submit" name="Submit11" value="Update Order Name" class="btn btn-primary">
								<%
								end if
								%>
							</td>
						...
						%>
							...
							<tr>
								<td colspan="2">
									...
									<input type="text" id="DF1" name="DF1" class="datepicker" size="30" value="<%=Date1%>">
								</td>
							</tr>
						<%
						...
						%>
							...
							<tr>
								<td colspan="2">
									<input type="submit" name="Submit11" value="Update Order Information" class="btn btn-primary">
								</td>
							</tr>
							<tr>
								<td colspan="2" class="pcCPspacer"></td>
						...
						<tr>
							<td colspan="2">
								<% If pcomments <> "" then %><div><strong>Note</strong> that this order also contains <a href="javascript:;" onClick="$pc('#TabbedPanels2 li:eq(6) a').tab('show'); form2.ActiveTab.value = 7">comments entered by the customer</a> while placing the order.</div><% end if %>
								<%
...
%>
													...
													<td class="invoice" colspan="3">
														<table width="100%" cellspacing="1" cellpadding="0" class="invoiceBto">
															...
															for i=lbound(ArrProduct) to (UBound(ArrProduct)-1)
																...
																btDisplayQF = False
																If Not rsQ.EOF Then btDisplayQF=rsQ("displayQF")
																...
																'APP-S
																query="SELECT categories.categoryDesc, products.description, products.sku, products.weight FROM categories, products WHERE (((categories.idCategory)="&ArrCategory(i)&") AND ((products.idProduct)="&intParentPrd&"))"
																'APP-E
																set rsConfigObj=server.CreateObject("ADODB.RecordSet")
																set rsConfigObj=connTemp.execute(query)
									...
									If trim(pcArrayPayment(1))="" then
										if err.number<>0 then
											PayCharge=0
										end if
										PayCharge=0
									else
										PayCharge=pcArrayPayment(1)
									end If
...
response.redirect "techErr.asp?err="&pcStrCustRefID
															...
															<table id="show_<%=count%>" style="display:none" class="pcCPcontent">
																<tr>
																	<td align="right">&nbsp;Add any comments for the drop-shipper (optional):<br>
																		...
																		<input type="submit" name="SubmitA3" value="Send/Resend Order Notification" onClick="javascript:SetValue(<%=pcv_DropShipperID%>,<%=pcv_IsSupplier%>,'AdmComments_<%=pcv_DropShipperID%>_<%=pcv_IsSupplier%>',0);" class="btn btn-primary">
																	</td>
																</tr>
																<tr>
																	<td class="pcCPspacer"></td>
												...
												if not rs1.eof then
													...
													<tr style="background-color: #ffffff">
														...
														Select Case tmp_ShipMethod
															...
															Case "ParcelPost": tmp_ShipMethod="Retail Ground"
																...
																<table id="show_<%=count%>A" style="display:none">
																	<tr>
																		<td>Comments:<br>
																			...
																			<input type="submit" class="btn btn-default" name="SubmitA4" value="Continue Send/Resend Partially Shipped Email" onClick="javascript:SetValue(<%=pcv_DropShipperID%>,<%=pcv_IsSupplier%>,'AdmComments_<%=pcv_DropShipperID%>_<%=pcv_IsSupplier%>A',<%=pcv_PackageID%>);">
																		</td>
																	</tr>
																</table>
															</td>
										...
										<tr align="top">
											...
											<td>
												...
												<%else%>
													...
													<% else %>
														<span style="color: #00F;">Pending</span>&nbsp;|&nbsp;<a href="#" onClick="$pc('#TabbedPanels2 li:eq(3) a').tab('show'); form2.ActiveTab.value = 4">Ship &gt;&gt;</a>
													<% end if %>
												<%end if%>
											</td>
										</tr>
					...
					<div id="tab3" class="tab-pane">
						<table class="pcCPcontent">
							...
							%>
									...
									<tr>
										...
										<td><strong>Process Order</strong>. &nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=309"></a>
										<br />
									...
									<tr>
										...
										<td>
											...
											<input type="submit" onClick="this.disabled=true;hidden1.name=this.name;hidden1.value=this.value;this.form.submit()" name="Submit4" value="<% if pOrderStatus=2 then %>Process This Order<% else %>Update Order Status<% end if %>" class="btn btn-primary">
											&nbsp;
											<input type="button" class="btn btn-default"  onClick="location.href='batchprocessorders.asp'" value="Batch Process with Other Orders">
										</td>
									</tr>
									<tr>
										<td colspan="2"><hr></td>
									...
									<tr>
										...
										<td valign="top">
										<strong>Cancel Order</strong>. &nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=310"></a>
										<br />
									...
									<tr>
										<td>&nbsp;</td>
										<td><input type="submit" name="Submit6" value="Cancel Order" class="btn btn-primary"></td>
									</tr>
								<%
								...
								%>
									<tr>
										<td colspan="2">This order was <b>shipped</b>: <a href="#" onClick="$pc('#TabbedPanels2 li:eq(1) a').tab('show'); form2.ActiveTab.value = 2">see shipment(s) details &gt;&gt;</a></td>
									</tr>
								<%
								end if
									...
									<tr>
										<td colspan="2">
											...
											<ul style="list-style:circle; padding: 10px; margin-left: 15px;">
												<li><a href="#" onClick="$pc('#TabbedPanels2 li:eq(1) a').tab('show'); form2.ActiveTab.value = 2">View partial shipment details</a></li>
												<li><a href="#" onClick="$pc('#TabbedPanels2 li:eq(3) a').tab('show'); form2.ActiveTab.value = 4">Ship additional items</a></li>
											</ul>
										</td>
									</tr>
									<tr>
								...
								%>
									...
									<tr>
										...
										<td width="95%">
											...
											<br />
											Ship this order using the <a href="#" onClick="$pc('#TabbedPanels2 li:eq(3) a').tab('show'); form2.ActiveTab.value = 4">Shipping Wizard &gt;&gt;</a></td>
									</tr>
									<tr>
										<td>&nbsp;</td>
									...
									<tr>
										...
										<ul style="list-style:circle; padding: 10px; margin-left: 15px;">
											<li><u>Automatically</u>, using the <a href="#" onClick="$pc('#TabbedPanels2 li:eq(3) a').tab('show'); form2.ActiveTab.value = 4">Shipping Wizard</a> (recommended)</li>
									...
									<tr>
										<td>&nbsp;</td>
										<td>
										<input type="submit" name="SubmitA2" value="Update Order Status" class="btn btn-primary">
										</td>
									</tr>
									<tr>
										<td colspan="2"><hr></td>
									...
									<tr>
										<td>&nbsp; </td>
										<td>
										<input type="submit" name="Submit10" value="Cancel Order"></td>
									</tr>
								<%
								...
								%>
									...
									<tr>
										<td valign="top"><img src="images/quick.gif" width="24" height="18"></td>
										<td><input type="button" class="btn btn-default"  name="RMAInfo" value="Create RMA Number" class="btn btn-primary" onClick="location.href='genRma.asp?idOrder=<%=qry_ID%>'"></td>
									</tr>
									<tr>
										<td colspan="2" class="pcCPspacer"></td>
									</tr>
								...
								if RMAVar=1 AND RMAStatus=0 then %>
									...
									<tr>
										<td>&nbsp;</td>
										<td><input type="button" class="btn btn-default"  name="RMAInfo" value="View RMA Information" onClick="location.href='modRmaa.asp?idOrder=<%=qry_ID%>&idRMA=<%=pIdRMA%>'"></td>
									</tr>
								<% end if %>
							...
							<tr>
								...
								<td>
								<strong>Reset Order Status</strong>&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=311"></a>
								<br />
						...
						<table class="pcCPcontent">
							...
							<tr>
								...
								<input type="submit" name="Submit4B" value="Resend Order Received E-mail" class="btn btn-primary">
								</td>
							</tr>
							<tr>
								<td colspan="2"><hr></td>
							...
							if ((porderStatus>"2") and (porderStatus<"5")) or ((porderStatus>"6") and (porderStatus<"9")) then%>
								...
								<tr>
									...
									<input type="submit" name="Submit4A" value="Resend Order Confirmation e-mail" class="btn btn-primary">
									</td>
								</tr>
								<tr>
									<td colspan="2"><hr></td>
							...
							%>
								...
								<tr>
									...
									<input type="submit" name="Submit5A" value="Resend Order Shipped Email" class="btn btn-primary"></td>
								</tr>
								<tr>
									<td colspan="2"><hr></td>
								</tr>
							...
							%>
								...
								<tr>
									...
									<input type="submit" name="Submit10A" value="Resend Order Cancelled Email" class="btn btn-primary">
									</td>
								</tr>
								<tr>
									<td colspan="2"><hr></td>
				...
				%>
					...
					if porderStatus <= 2 then ' START Order must be processed before it can be shipped
					%>
							<table class="pcCPcontent">
								<tr>
									<td colspan="2" class="pcCPspacer"></td>
								...
								<tr>
									<td colspan="2">
									This order has not yet been processed. An order must be processed before it can be shipped. <a href="#" onClick="$pc('#TabbedPanels2 li:eq(2) a').tab('show'); form2.ActiveTab.value = 3">Process this order &gt;&gt;</a>
									</td>
								</tr>
							</table>
					<%
						...
						if shwOnOff=1 then
							HaveShipPrds=0
							queryQ="SELECT TOP 1 idProduct FROM ProductsOrdered WHERE idOrder=" & qry_ID & " AND pcPrdOrd_Shipped=0;"
							set rsQ=connTemp.execute(queryQ)
							if not rsQ.eof then
								HaveShipPrds=1
							end if
							set rsQ=nothing
							if HaveShipPrds=1 then%>
							<table class="pcCPcontent">
									<tr>
										<th colspan="2">SHIPWIRE Shipping Wizard </th>
									</tr>
									<tr>
										<td colspan="2" class="pcCPspacer"></td>
									</tr>
									<tr>
										<td colspan="2">Use the button below to start the SHIPWIRE Shipping Wizard. This will allow you to send your order to the SHIPWIRE.<br><br></td>
									</tr>
									<tr>
											<td colspan="2"><input type="button" name="shiporderwizardSHW" value="Start SHIPWIRE Shipping Wizard" onClick="location='shw_SendOrder.asp?idorder=<%=qry_ID%>';" class="submit2"></td>
									</tr>
									<tr>
					...
					%>
								<table class="pcCPcontent">
									...
									<tr>
											<td colspan="2"><input type="button" class="btn btn-default"  name="shiporderwizard" value="Start Custom Shipping Options Shipping Wizard" onclick="location='sds_ShipOrderWizard1.asp?idorder=<%=qry_ID%>';" class="btn btn-primary"></td>
									</tr>
									<tr>
										<td colspan="2" class="pcCPspacer"><hr></td>
									</tr>
								...
								if pcv_strFedExWSEnabled then ' START show FedEx shipping wizard
								...
								<table class="pcCPcontent">
									...
									<tr>
										<td colspan="2">Use the following button(s) to access the FedEx Shipping Wizard.</td>
									</tr>
									<tr>
										<td colspan="2">
											<input type="button" class="btn btn-default"  name="ShipManager" value="Start FedEx Shipping Wizard" onclick="location='sds_ShipOrderWizard1.asp?idorder=<%=qry_ID%>&PageAction=<%=pcv_intFedExAction%>';" class="btn btn-primary">
										</td>
									</tr>
									<%
									' // Show Manage Shipped Packages
									if pcv_strFedExWSPackagesExist then
									%>
									<tr>
										<td colspan="2">
											<input type="button" class="btn btn-default"  name="managepackages" value="Manage Shipped Packages" onClick="location='<%=pcv_strFedExShipURL%>?id=<%=qry_ID%>';">
										</td>
									</tr>
									<% end if %>
									<tr>
							...
							if pcv_strUPSEnabled and intShipUPSWizard = 1 then ' START show UPS shipping wizard
							%>
								<a name="UPSshipwizard">&nbsp;</a>
								<a name="managepackages">&nbsp;</a>
								<table class="pcCPcontent">
									<tr>
										<th colspan="2">UPS&reg; Developer Kit Shipping</th>
									</tr>
									<tr>
										<td colspan="2" class="pcCPspacer"></td>
									</tr>
									...
									<tr>
										<td colspan="2">Use the following button(s) to access UPS&reg; Developer Kit Shipping</td>
									</tr>
									<tr>
										<td colspan="2">
											<input type="button" class="btn btn-default"  name="ShipManager" value="Start Shipping Wizard" onclick="location='sds_ShipOrderWizard1.asp?idorder=<%=qry_ID%>&PageAction=UPS&m=new';" class="btn btn-primary">
										</td>
									</tr>
									<%
									' // Show Manage Shipped Packages
									...
									<tr>
										<td colspan="2">
											<input type="button" class="btn btn-default"  name="managepackages" value="Manage Shipped Packages" onClick="location='UPS_ManageShipmentsResults.asp?id=<%=qry_ID%>';">
										</td>
									</tr>
									<% end if %>
									<tr>
							...
							<%
							end if ' END show UPS Shipping Wizard
							...
							%>
								...
								<table class="pcCPcontent">
									...
									<tr>
										<td colspan="2">
											<input type="button" class="btn btn-default"  name="ShipManager" value="Start Shipping Wizard" onclick="location='sds_ShipOrderWizard1.asp?idorder=<%=qry_ID%>&PageAction=USPS&m=new';" class="btn btn-primary">
										</td>
									</tr>
									<% else %>
									<tr>
									...
									<tr>
										<td colspan="2">
											<input type="button" class="btn btn-default"  name="managepackages" value="Manage Shipped Packages" onClick="location='USPS_ManageShipmentsResults.asp?id=<%=qry_ID%>';">
										</td>
									</tr>
									<% end if %>
									<tr>
							...
							<%
							end if ' END show USPS Shipping Wizard
						'// if porderStatus <> 10 then
				...
				<div id="tab5" class="tab-pane">
					<table class="pcCPcontent">
						...
						<tr>
							<td colspan="2">
							To update this order's the payment status select the new status below and click on &quot;Update Payment Status&quot;. Note that the payment gateway involved, if any, is <u>not</u> contacted when you change the payment status.&nbsp;<a class="pcCPhelp" href="helpOnline.asp?ref=302"></a>
							</td>
						</tr>
						<tr>
							<td colspan="2" class="pcCPspacer"></td>
                ...
                'SB E
                '///////////////////////////////////////////////////////
                '/// END: IF PayPal Classic APIs
                '///////////////////////////////////////////////////////
                %>
                
                <% If pcPaymentCode = "PayPalAdvanced" Or pcPaymentCode = "PayPalPro" Or pcPaymentCode = "PFLink" Or pcPaymentCode = "PFPRO" OR pcv_PayPalGWCode = 53 Or pcv_PayPalGWCode = 80 Or pcv_PayPalGWCode = 99 Then %>
                	<!-- #include file="OrdDetails_PFApi.asp" -->
                <% end if %>
								...
								<% end if %>
								<% if varShowCCInfo=1 and (varGWInfo="A" OR varGWInfo="EIG") then
									if varGWInfo="A" then
							...
							<% if intShowBtn=1 then %>
								<tr>
									<td>&nbsp;</td>
									<td><input type="submit" name="Submit2" value="Update Payment Information" class="btn btn-primary"><% If intShowPurgeBtn = 1 Then %>
									<input type="button" class="btn btn-default"  name="PurgeCCNumber" value="Purge Credit Card Number" onClick="location='gwCCPurgeSubmit.asp?POID=<%=qry_ID%>';">
								<% End If %></td>
								</tr>
					...
					<table class="pcCPcontent">
						<tr>
							<td align="left" valign="top" width="50%">
								...
								<table class="pcCPcontent" id="pcCPBillingInfo">
									<tr>
									...
									<tr>
										<td><p>E-mail:</p></td>
										<td><p><input name="email" type="text" value="<%=pemail%>" size="30"></p></td>
									</tr>
									<%
									...
									<tr>
										<td colspan="2" align="center"><input type="submit" name="Submit1" value="Update Customer Information" class="btn btn-primary" onClick="return(Form2_Validator(document.getElementById('form2')));">
										</td>
									</tr>
									<tr>
										<td colspan="2" class="pcCPspacer"></td>
							...
							<td width="50%" align="left" valign="top">
								...
								<table class="pcCPcontent" id="pcCPShippingInfo">
									<tr>
									...
									if varShip<>"0" then
										...
										<tr>
											<td><p>Email:</p></td>
											<td><p>
												<% if pshippingEmail="" then %> <input name="shippingEmail" type="text" size="30">
												<% else %> <input name="shippingEmail" type="text" value="<%=pshippingEmail%>" size="30">
												<% end if %></p>
											</td>
										</tr>
										...
										<tr>
											<td colspan="2" align="center">
												<input type="submit" name="Submit3" value="Update Shipping Information" class="btn btn-primary">
											</td>
										</tr>
									<% end if %>
									<tr>
					...
					<div id="tab7" class="tab-pane">
						<table class="pcCPcontent">
							...
							If pidaffiliate>"1" then
                                ...
                                If pTotal>0 Then                                
								    paffiliatePayActual =  ((pTotalAdj * paffiliatePay)/pTotal) 
                                End If
								...
								<tr>
									<td>Adjust new commission by: <a class="pcCPhelp" href="helpOnline.asp?ref=477"></a></td>
									<td>
								...
								<tr>
									<td nowrap>&nbsp;</td>
									<td><input name="submit12" type="submit" value="Update earned commission" class="btn btn-primary"></td>
								</tr>
							<% end if %>
										...
										<tr>
											...
											<td><a href="<%=pdownloadStr%>" target="_blank"><%=pdownloadStr%></a>
												<% if (pURLExpire<>"") and (pURLExpire="1") then
													...
													<input type="button" class="btn btn-default"  name="resetURL" value=" Reset URL Expiration " onClick="location='resetURL.asp?requestSTR=<%=RequestStr%>&orderid=<%=pidorder%>';">
												<%end if%>
											</td>
										</tr>
							...
							%>
								...
								<tr>
									<td valign="top">&nbsp;</td>
									<td valign="top">
					<%if Gc_ReMsg<>"" then%><input type="submit" name="submitReSendGCRecA" value="Resend with original message" class="btn btn-primary">&nbsp;<%end if%>
					<input type="submit" name="submitReSendGCRec" value="<%if (Gc_ReName<>"") AND (Gc_ReEmail<>"") then%>Resend with new message<%else%>Send message<%end if%>" class="btn btn-primary">
				   </td>
								</tr>
								<%
						END IF
				...
				<script type=text/javascript>
                    ...
                    if request("ActiveTab")<>"" then%>
                        $pc('#TabbedPanels2 li:eq(<%=intActiveTab-1%>) a').tab('show');
					<%end if%>
                </script>
