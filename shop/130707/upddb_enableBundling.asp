<% 
PmAdmin=19
pageTitle = "ProductCart Enable Bundling and Javascript Optimization - Database Upgrade" 
Section = "" 
%>
...
IF request("action")="sql" then
	if request("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			call closeDb()
			response.redirect "upddb_enableBundling.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			call closeDb()
			response.redirect "upddb_enableBundling.asp?mode=1"
			response.End
		end if
	...
	query="ALTER TABLE [pcStoreSettings] ADD [pcStoreSettings_EnableBundling] [int] DEFAULT (0) NOT NULL"
	...
	query="ALTER TABLE [pcStoreSettings] ADD [pcStoreSettings_OptimizeJavascript] [int] DEFAULT (0) NOT NULL"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
...
<form action="upddb_enableBundling.asp" method="post" name="form1" id="form1" class="pcForms">
<%
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
                ...
                <h1 class="page-header">Enable Bundling and Javascript Optimization Database Update</h1>
                <p class="lead">
