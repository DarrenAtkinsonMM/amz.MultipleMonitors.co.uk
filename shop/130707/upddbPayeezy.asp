...
<% '--Updater File-- %>
<% pageTitle = "Payeezy Gateway - Database Update" %>
<% Section = "" %>
<%PmAdmin=19%> 
...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			call closeDb()
response.redirect "upddbPayeezy.asp?mode=3"
			response.End
		end if
...
response.redirect "techErr.asp?error="&Server.Urlencode("Error while opening database")
		...
		if instr(ucase(scDSN),"DSN=") then
			call closeDb()
response.redirect "upddbPayeezy.asp?mode=1"
			response.End
		end if
...
<form action="upddbPayeezy.asp" method="get" name="form1" class="pcForms">
<%
if mode="complete" then 
	call closeDb()
response.redirect "pcConfigurePayment.asp?gwchoice=1101"
	response.end
else %>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
				...
				<span style="font-weight: bold">You must update your database before you can add Payeezy Gateway.<u></u></span><br>
				In order to activate Payeezy real-time payment gateway you will need to update your ProductCart database to add the required table.</div>				<p><strong><br>
				    <br>
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" class="btn btn-default"  name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbPayeezy.asp?action=sql';" class="btn btn-primary">
						<% end if %>
					</td>
			</tr>
			</table>
