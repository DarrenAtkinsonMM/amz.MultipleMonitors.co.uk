...
if request("action")="upd" then
	pcv_Qty1=getUserInput(request("qty1"),8)
	pcv_Qty2=getUserInput(request("qty2"),8)
...
response.redirect "pcSplitPOQty_popup.asp?idProductOrdered="&pcv_IDProductOrdered&"&Qty="&pcv_ProductQty&"&msg="&msg
	...
	if (int(pcv_Qty1)+int(pcv_Qty2))<>int(pcv_ProductQty) then
		...
		query="INSERT INTO ProductsOrdered (idOrder, idProduct, service, quantity, idOptionA, idOptionB, unitPrice, unitCost, xfdetails, idconfigSession, rmaSubmitted, QDiscounts, ItemsDiscounts, pcPackageInfo_ID, pcDropShipper_ID, pcPrdOrd_Shipped, pcPrdOrd_BackOrder, pcPrdOrd_SentNotice, pcPrdOrd_SelectedOptions, pcPrdOrd_OptionsPriceArray, pcPrdOrd_OptionsArray, pcPO_EPID, pcPO_GWOpt, pcPO_GWNote, pcPO_GWPrice) VALUES ("&pcvt_idOrder&", "& pcvt_idProduct&", "& pcvt_service&", "& pcv_Qty2&", "& pcvt_idOptionA&", "& pcvt_idOptionB&", "& pcvt_unitPrice&", "& pcvt_unitCost&", N'"& pcvt_xfdetails&"', "& pcvt_idconfigSession&", "& pcvt_rmaSubmitted&", 0, "& pcvt_ItemsDiscounts&", "& pcvt_pcPackageInfo_ID&", "& pcvt_pcDropShipper_ID&", "& pcvt_pcPrdOrd_Shipped&", "& pcvt_pcPrdOrd_BackOrder&", "& pcvt_pcPrdOrd_SentNotice&", '"& pcvt_pcPrdOrd_SelectedOptions&"', '"& pcvt_pcPrdOrd_OptionsPriceArray&"', '"& pcvt_pcPrdOrd_OptionsArray&"', "& pcvt_pcPO_EPID&", "& pcvt_pcPO_GWOpt&", '"& pcvt_pcPO_GWNote&"', "& pcvt_pcPO_GWPrice &");"
		set rs=server.CreateObject("ADODB.RecordSet")
		set rs=conntemp.execute(query)
		set rs=nothing
