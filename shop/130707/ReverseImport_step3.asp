...
if request("C58")="1" then
	HeaderBuilderObj.append """Product ID#""" & fseparator
end if
...
if request("C48")="1" then
	HeaderBuilderObj.append """Parent Product ID#""" & fseparator
end if
...
'APP-S
if request("C47")="1" then
	HeaderBuilderObj.append """Apparel Product""" & fseparator
end if
'APP-E
...
'APP-S
if request("C6a")="1" then
	HeaderBuilderObj.append """Online Price Difference""" & fseparator
end if
if request("C6b")="1" then
	HeaderBuilderObj.append """Wholesale Price Difference""" & fseparator
end if
'APP-E
...
if request("C68")="1" then
	HeaderBuilderObj.append """Alt Tag Text""" & fseparator
end if
...
if request("C42")="1" then
	HeaderBuilderObj.append """Hide Configurator Prices""" & fseparator
end if
...
if request("C67")="1" then
	HeaderBuilderObj.append """Hide Additional Images""" & fseparator
end if
...
if request("C57")="1" then
	HeaderBuilderObj.append """Google Shopping - Gender""" & fseparator & """Google Shopping - Age""" & fseparator & """Google Shopping - Color""" & fseparator & """Google Shopping - Size""" & fseparator & """Google Shopping - Pattern""" & fseparator & """Google Shopping - Material""" & fseparator
end if
if request("C59")="1" then
	HeaderBuilderObj.append """Product Layout - Top Area""" & fseparator & """Product Layout - Top-Left Area""" & fseparator & """Product Layout - Top-Right Area""" & fseparator & """Product Layout - Middle Area""" & fseparator & """Product Layout - Tabs Area""" & fseparator & """Product Layout - Bottom Area""" & fseparator
end if
if request("C60")="1" then
	HeaderBuilderObj.append """Show out of stock items""" & fseparator
end if
if request("C61")="1" then
	HeaderBuilderObj.append """Out of stock items message""" & fseparator
end if
if request("C62")="1" then
	HeaderBuilderObj.append """Display type""" & fseparator
end if
if request("C63")="1" then
	HeaderBuilderObj.append """Size chart text link""" & fseparator
end if
if request("C64")="1" then
	HeaderBuilderObj.append """Size chart description""" & fseparator
end if
if request("C65")="1" then
	HeaderBuilderObj.append """Size chart image file""" & fseparator
end if
if request("C66")="1" then
	HeaderBuilderObj.append """Size chart image URL""" & fseparator
end if
...
if not rstemp.eof then
	...
	For i=0 to intCount
		if request("PCat" & tmpArr(0,i))="1" then
			if tmpPCat<>"" then
				tmpPCat=tmpPCat & ","
			end if
			tmpPCat=tmpPCat & tmpArr(0,i)
...
'// Remove orphaned sub-products before export
if session("cp_revImport_extype")>"0" then
	
	query="UPDATE Products SET removed=-1 WHERE (pcprod_ParentPrd>0) AND (pcprod_ParentPrd NOT IN (SELECT idProduct FROM Products WHERE removed=0 AND pcprod_Apparel=1));"
...
'***** Create SQL Query *****
query="SELECT sku, description, serviceSpec, configOnly, price, listPrice, bToBPrice, weight, stock, IDBrand, smallImageUrl, imageUrl, largeImageURL, active, listHidden, hotDeal, iRewardPoints, notax, noshipping, formQuantity, emailText, noStock, noshippingtext, pcprod_minimumqty, pcprod_qtyvalidate, OverSizeSpec, cost, pcProd_BackOrder, pcProd_ShipNDays, pcProd_NotifyStock, pcProd_ReorderLevel, pcProd_IsDropShipped, pcSupplier_ID, pcDropShipper_ID, Downloadable, pcprod_GC, pcprod_hidebtoprice, pcprod_HideDefConfig, NoPrices, pcProd_SkipDetailsPage, idproduct, ShowInHome, pcprod_AddPrice, pcprod_AddWPrice, pcProd_SPInActive, pcprod_ParentPrd, pcprod_Apparel, pcprod_QtyToPound, pcProd_Surcharge1, pcProd_Surcharge2, pcPrd_MojoZoom, pcProd_HideSKU, products.pcProd_GoogleCat,products.pcProd_GoogleGender,products.pcProd_GoogleAge,products.pcProd_GoogleColor,products.pcProd_GoogleSize,products.pcProd_GooglePattern,products.pcProd_GoogleMaterial, pcProd_DisplayLayout, pcProd_PrdNotes, pcProd_MetaTitle, pcProd_MetaDesc, pcProd_MetaKeywords, details, sDesc, pcProd_Top, pcProd_TopLeft, pcProd_TopRight, pcProd_Middle, pcProd_Tabs, pcProd_Bottom, pcprod_ShowStockMsg, pcprod_StockMsg, pcprod_ApparelRadio, pcprod_SizeLink, pcprod_SizeInfo, pcprod_SizeImg, pcprod_SizeURL, pcProd_AdditionalImages, pcProd_AltTagText FROM Products WHERE removed=0 "
'Last: 71 - Last changes (1): pcprod_AddPrice, pcprod_AddWPrice, pcProd_SPInActive,pcprod_ParentPrd,pcprod_Apparel
...
if session("cp_revImport_prdlist")<>"ALL" then
	...
	if tmp_query<>"" then
		...
		if session("cp_revImport_extype")=0 then
			...
			if session("cp_revImport_extype")=1 then
				tmp_query=" AND (pcprod_ParentPrd IN (" & tmp_query & "))"
			else
				tmp_query=" AND ((idproduct IN (" & tmp_query & ")) OR (pcprod_ParentPrd IN (" & tmp_query & ")))"
			end if
		end if
	else
		if session("cp_revImport_extype")=0 then
			tmp_query=" AND pcprod_ParentPrd=0"
		else
			if session("cp_revImport_extype")=1 then
				tmp_query=" AND pcprod_ParentPrd>0"
			end if
...
'***** Generate Data Lines *****
if request("C58")="1" then
	DataBuilderObj.append """" & pcArr(40,i) & """" & fseparator
end if
if request("C1")="1" then
...
if request("C5")="1" then
	if pcArr(2,i)<>0 then
		DataBuilderObj.append """CONFIGURABLE""" & fseparator
	else
		if pcArr(3,i)<>0 then
...
if request("C68")="1" then
	DataBuilderObj.append """" & pcArr(80,i) & """" & fseparator
end if
if request("C16")="1" then
	
	if pcArr(45,i)>"0" then
		if pcArr(44,i)="0" then
			DataBuilderObj.append """-1""" & fseparator
		else
			DataBuilderObj.append """0""" & fseparator
		end if
	else
...
if request("C50")="1" then
	DataBuilderObj.append """" & pcArr(48,i) & """" & fseparator
end if
if request("C51")="1" then
	DataBuilderObj.append """" & pcArr(49,i) & """" & fseparator
end if
if request("C52")="1" then
	tmp_PrdNotes=pcArr(60,i)
...
if request("C53")="1" then
	DataBuilderObj.append """" & pcArr(50,i) & """" & fseparator
end if
if request("C67")="1" then
	DataBuilderObj.append """" & pcArr(79,i) & """" & fseparator
end if
if request("C54")="1" then
	DataBuilderObj.append """" & pcArr(59,i) & """" & fseparator
end if
if request("C55")="1" then
	DataBuilderObj.append """" & pcArr(51,i) & """" & fseparator
end if
if request("C56")="1" then
	DataBuilderObj.append """" & pcArr(52,i) & """" & fseparator
end if
if request("C57")="1" then
	DataBuilderObj.append """" & pcArr(53,i) & """" & fseparator & """" & pcArr(54,i) & """" & fseparator & """" & pcArr(55,i) & """" & fseparator & """" & pcArr(56,i) & """" & fseparator & """" & pcArr(57,i) & """" & fseparator & """" & pcArr(58,i) & """" & fseparator
end if
if request("C59")="1" then
	DataBuilderObj.append """" & pcArr(66,i) & """" & fseparator & """" & pcArr(67,i) & """" & fseparator & """" & pcArr(68,i) & """" & fseparator & """" & pcArr(69,i) & """" & fseparator & """" & pcArr(70,i) & """" & fseparator & """" & pcArr(71,i) & """" & fseparator
end if
if request("C60")="1" then
	DataBuilderObj.append """" & pcArr(72,i) & """" & fseparator
end if
if request("C61")="1" then
	DataBuilderObj.append """" & pcArr(73,i) & """" & fseparator
end if
if request("C62")="1" then
	DataBuilderObj.append """" & pcArr(74,i) & """" & fseparator
end if
if request("C63")="1" then
	DataBuilderObj.append """" & pcArr(75,i) & """" & fseparator
end if
if request("C64")="1" then
	DataBuilderObj.append """" & pcArr(76,i) & """" & fseparator
end if
if request("C65")="1" then
	DataBuilderObj.append """" & pcArr(77,i) & """" & fseparator
end if
if request("C66")="1" then
	DataBuilderObj.append """" & pcArr(78,i) & """" & fseparator
end if

if tmpPCat<>"" then
	tmpPriceCat=split(tmpPCat,",")
	tmpPriceCatValue=tmpPriceCat
	For k=lbound(tmpPriceCatValue) to ubound(tmpPriceCatValue)
		tmpPriceCatValue(k)=0
	Next
	query="SELECT idcustomerCategory,pcCC_Price FROM pcCC_Pricing WHERE idproduct=" & pcArr(40,i) & " AND idcustomerCategory IN (" & tmpPCat & ") ORDER BY idcustomerCategory ASC;"
	set rsQ=connTemp.execute(query)
	if not rsQ.eof then
		...
		For k=0 to intCount1
			For ik=lbound(tmpPriceCat) to ubound(tmpPriceCat)
				if tmpPriceCat(ik)<>"" then
					if Clng(tmpPriceCat(ik))=Clng(tmpValue(0,k)) then
						tmpPriceCatValue(ik)=tmpValue(1,k)
					end if
				end if
			Next
		Next
	...
	For ik=lbound(tmpPriceCat) to ubound(tmpPriceCat)
		if tmpPriceCat(ik)<>"" then
			DataBuilderObj.append """" & tmpPriceCatValue(ik) & """" & fseparator
		end if
	Next
end if
...
'***** End of Generate Data Lines *****	
	...
	call pcs_SaveUTF8(strFile & tmpext,strFile & tmpext,HeaderBuilderObj.toString & VBCrLf & DataBuilderObj.toString)
...
<tr>
	<td colspan="2">
		<input type="button" class="btn btn-default"  name="back" value="Start Again" onClick="javasccript:location='ReverseImport_step1.asp';">
	</td>
</tr>
<%ELSE%>
<tr>
...
<tr>
	...
	<td align="center">
		...
		<p style="padding-top:10px;">Also note that to open a *.txt file in MS Excel you should first start Excel, and then open the file by selecting &quot;File > Open&quot;. This will initiate the <u>MS Excel Text Import Wizard</u>, where you can specify the custom separator used in the file, if any.
		</p>
	</td>
</tr>
<%END IF%>
