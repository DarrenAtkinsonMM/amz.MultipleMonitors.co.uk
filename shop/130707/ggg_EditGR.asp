...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	query="Update pcEvents set pcEv_Type='" & getype & "',pcEv_Name=N'" & geName & "',pcEv_Date='" & geDate & "',pcEv_Delivery=" & gedelivery & ",pcEv_MyAddr=" & gemyaddr & ",pcEv_Hide=" & gehide & ",pcEv_Notify=" & genotify & ",pcEv_IncGcs=" & geIncGc & ",pcEv_Active=" & geActive & ",pcEv_HideAddress=" & geHideAddress & " where pcEv_IDCustomer=" & pcv_IdCustomer & " and pcEv_IDEvent=" & gIDEvent
	set rstemp=connTemp.execute(query)
	if err.number<>0 then
		call LogErrorToDatabase()
		set rstemp=nothing
...
<table class="pcCPcontent">
	...
	<tr> 
		...
		<td> 
			<input type="text" name="edate" class="datepicker" size="30" value="<%=gedate%>"> (<i>Format: <%if scDateFrmt="DD/MM/YY" then%>DD/MM/YY<%else%>MM/DD/YY<%end if%></i>)
		</td>
	</tr>
	<tr> 
		<td colspan="2"> 
...
response.redirect "techErr.asp?err="&pcStrCustRefID
	...
	<tr> 
		<td colspan="2" align="center">
			<input type="submit" class="btn btn-primary" name="submit" value="Update Registry" onClick="document.Form1.subdel.value='0';document.Form1.rewrite.value='0';">&nbsp;
			<%if gShowDel=1 then%>
				&nbsp;<input type="submit" class="btn btn-primary" name="delreg" value="Delete Registry" onClick="document.Form1.subdel.value='1'; document.Form1.delregistry.value='ok';">
			<%end if%>
