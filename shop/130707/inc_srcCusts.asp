...
<table class="pcCPsearch">
	...
	<tr>
		<td width="31%" align="left">Customer Type:</td>
		<td>
			<select name="CustType">
				<option value='' selected>All</option>
				<option value='0'>Retail Customer</option>
				<option value='1'>Wholesale Customer</option>
				<%if (src_DisplayCustType="") OR (src_DisplayCustType="0") then%>
				<% 'if there are pricing categories - List them here
				queryQ="SELECT idcustomerCategory, pcCC_Name, pcCC_CategoryType FROM pcCustomerCategories;"
				SET rsQ=Server.CreateObject("ADODB.RecordSet")
				SET rsQ=conntemp.execute(queryQ)
				if NOT rsQ.eof then 
					do until rsQ.eof 
						intIdcustomerCategory=rsQ("idcustomerCategory")
						strpcCC_Name=rsQ("pcCC_Name")
						%>
	...
	<tr>
		<td colspan="2" align="center">
			<br>
			<input name="Submit" type="submit" class="btn btn-default" value="<%=src_Button1%>">
		</td>
	</tr>
</table>
</form>
