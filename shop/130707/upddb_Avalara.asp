<% 
PmAdmin=19
pageTitle = "ProductCart Avalara - Database Upgrade" 
Section = "" 
%>
...
IF request("action")="sql" then
	if request("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			call closeDb()
			response.redirect "upddb_Avalara.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			call closeDb()
			response.redirect "upddb_Avalara.asp?mode=1"
			response.End
		end if
	...
	'========================================================================
	'// START OF DB UPDATES FOR Avalara
	'========================================================================
	...
	'//  Add column pcOrd_Avalara for table "orders"
	query="ALTER TABLE [orders] ADD [pcOrd_Avalara] [int] DEFAULT (0) NOT NULL"
	...
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
			...
			ErrStr=ErrStr&"Unable to update TABLE orders - Error: "&Err.Description&"<BR>"
			err.number=0
			iCnt=iCnt+1
		end if
	end if
	
	'//  Add column pcCust_AvalaraExemptionNo for table "customers"
	query="ALTER TABLE [customers] ADD [pcCust_AvalaraExemptionNo] [nvarchar] (20)"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
			...
			ErrStr=ErrStr&"Unable to update TABLE customers - Error: "&Err.Description&"<BR>"
			err.number=0
			iCnt=iCnt+1
		end if
	end if
	
	'//  Add column pcProd_AvalaraTaxCode for table "products"
	query="ALTER TABLE [products] ADD [pcProd_AvalaraTaxCode] [nvarchar] (20)"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
	...
	'//  Add column pcCats_AvalaraTaxCode for table "categories"
	query="ALTER TABLE [categories] ADD [pcCats_AvalaraTaxCode] [nvarchar] (20)"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
			...
			ErrStr=ErrStr&"Unable to update TABLE categories - Error: "&Err.Description&"<BR>"
			err.number=0
			iCnt=iCnt+1
		end if
	end if
	...
	'//  Add column pcCustSession_Avalara for table "pcCustomerSessions"
	query="ALTER TABLE [pcCustomerSessions] ADD [pcCustSession_Avalara] [int] DEFAULT(0) NOT NULL"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
		if instr(ucase(Err.Description),"COLUMN NAMES IN EACH TABLE MUST BE UNIQUE") then
			...
			ErrStr=ErrStr&"Unable to update TABLE pcCustomerSessions - Error: "&Err.Description&"<BR>"
			err.number=0
			iCnt=iCnt+1
		end if
	end if
...
<form action="upddb_Avalara.asp" method="post" name="form1" id="form1" class="pcForms">
<%
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
                ...
                <h1 class="page-header">Avalara Database Update</h1>
                <p class="lead">
