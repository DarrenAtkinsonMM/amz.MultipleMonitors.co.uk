...
dim rstemp2, strtext, strtext1, File1, pcv_rootCat, strParent, intIdProduct, pcIntIncludeWeight, pcIntUniqueIdent
...
IF request("action")="gen" THEN
	...
	'//Bundle Column
	pcvBundleCol=0
	query="SELECT idSearchFieldMapping FROM pcSearchFields_Mappings WHERE pcSearchFieldsColumn LIKE 'IS BUNDLE' AND pcSearchFieldsFileID LIKE 'f';"
	...
	if pcvBundleCol=1 then
		strtext1 = strtext1 & "is_bundle" & chr(9)
	end if
	...
	if pcIntIncludeApparel=1 then
		...
		strtext1 = strtext1 & "item_group_id" & chr(9)
	end if
	if Right(strtext1,1)=chr(9) then
		strtext1 = Left(strtext1,len(strtext1)-1)
	end if
	strtext1 = strtext1 & vbcrlf
	%>
	<div id="ea">
	...
	'~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		Next
		If tmpCats<>"" then
			query1=" AND C.idcategory IN (" & tmpCats & ") "
			query2=" AND E.idcategory IN (" & tmpCats & ") "
		Else
			query1=""
			query2=""
		End if
		...
		if request("excWCats")="1" then
			ExcWCatsStr=" AND B.pccats_RetailHide<>1 "
		else
			ExcWCatsStr=""
		end if

		if request("excNFSPrds")="1" then
			ExcNFSPrds=" AND A.formQuantity=0 "
		else
			ExcNFSPrds=""
		end if
		...
		query="SELECT B.idcategory,B.categoryDesc,B.pccats_BreadCrumbs,A.idProduct,A.description,A.serviceSpec,A.price,A.imageUrl,A.sku,A.stock,A.hotDeal,A.showInHome,F.BrandName,A.noStock,A.pcProd_BackOrder,A.pcProd_BTODefaultPrice,A.weight,A.pcProd_GoogleCat,A.pcProd_GoogleGender,A.pcProd_GoogleAge,A.pcProd_GoogleSize,A.pcProd_GoogleColor,A.pcProd_GooglePattern,A.pcProd_GoogleMaterial,A.pcProd_GoogleGroup,A.pcProd_Apparel,A.details,A.sDesc FROM ((products A INNER JOIN categories_products C ON A.idproduct=C.idproduct) INNER JOIN categories B ON C.idcategory=B.idcategory) LEFT OUTER JOIN Brands F ON A.idBrand=F.idBrand WHERE ((A.price>0) OR ((A.price=0) AND (A.serviceSpec<>0) AND (A.pcProd_BTODefaultPrice>0)) OR (A.pcProd_Apparel=1)) AND A.active = -1 " & ExcWCatsStr &  ExcNFSPrds & " AND A.removed=0 AND A.configOnly=0 AND A.idproduct=C.idproduct AND B.idcategory=C.idcategory " & query1 & " AND (B.idcategory IN (SELECT TOP 1 D.idcategory FROM categories D,categories_products E WHERE E.idproduct=A.idproduct AND D.idcategory=E.idcategory" & query2 & " ORDER BY D.idcategory ASC)) ORDER BY B.idcategory ASC,A.description ASC;"
		if request("excWCats")="1" then
		querytest="SELECT DISTINCT A.idproduct FROM products A,categories B,categories_products C WHERE ((A.price>0) OR ((A.price=0) AND (A.serviceSpec<>0) AND (A.pcProd_BTODefaultPrice>0))) AND A.active = -1 AND B.pccats_RetailHide=1" &  ExcNFSPrds & " AND A.removed=0 AND A.configOnly=0 AND A.idproduct=C.idproduct AND B.idcategory=C.idcategory " & query1 & ";"
		end if
		...
		' 3 = product ID
		' 4 = product name
		' 5 = whether it is a configurable product
		' 6 = price
		' 7 = image file name
		' 8 = sku
		' 9 = stock
		...
		Call pcs_ValidateFileName(File1) '// Check file for security issues.
		File1 = replace(File1, ".txt", "")
		File1 = File1 & ".txt"
		call pcs_SaveUTF8(File1, File1, strtext1)
		strtext1=""
		...
		if Count1>-1 then
			...
			For k=0 to Count1
				...
				'IF session("tmp" & intIdProduct)<>"1" THEN
					...
					'If it's a configurable product, the price is the Default Price
					...
					IF dblPrice>0 THEN
						...
						if pcvBundleCol=1 then
							tmpBundle=""
							tmpBundle=pcf_FillByName("f","IS BUNDLE")
							if IsNull(tmpBundle) OR tmpBundle="" then
								tmpBundle="FALSE"
							end if
						end if
						...
						strBrandName=pcArray(12,k)
						if IsNull(strBrandName) OR strBrandName="" then
						...
						if (scOutOfStockPurchase=0) OR (Clng(strDisregardStock)<>0) then
						...
						strProductDesc=pcArray(26,k)
						strShortProductDesc=pcArray(27,k)
						...
						Count3=Count3+1
						StringBuilderObj.append ClearHTMLTagsGB("/~/" & strProductName & "/~//*//~/" & strProductDesc & "/~//*/" & strPrdType,0) & "/*/" & strProductURL & "/*/" & strProductImgURL & "/*/" & strSKU & "/*/" & strAvailability & "/*/" & dblPrice & " " & strCurrency & "/*/" & strCondition & "/*//~/" & strBrandName & "/~//*/" & strExpDate
						if pcIntUniqueIdent=1 then
							StringBuilderObj.append "/*/" & tmpMPN & "/*/" & tmpGTIN
						end if
						if pcvBundleCol=1 then
							StringBuilderObj.append "/*/" & tmpBundle
						end if
						if pcIntIncludeWeight=1 then
					...
					END IF 'Product Price > 0

					If statusAPP="1" OR scAPP=1 Then
						IF (pcv_Apparel="1") AND (pcIntIncludeApparel=1) THEN
							'call opendb()
							queryQ="SELECT products.idProduct,products.description,products.price,products.imageUrl,products.sku,products.pcProd_GoogleGender,products.pcProd_GoogleAge,products.pcProd_GoogleSize,products.pcProd_GoogleColor,products.pcProd_GooglePattern,products.pcProd_GoogleMaterial,products.stock,products.noStock,products.pcProd_BackOrder FROM Products WHERE pcProd_ParentPrd=" & intIdProduct & " AND active=0 AND removed=0 AND pcProd_SPInActive=0;"
							set rsQ=connTemp.execute(queryQ)
							if not rsQ.eof then
								tmpAr=rsQ.getRows()
								intCountQ=ubound(tmpAr,2)
								For Q=0 to intCountQ
									SP_name=tmpAr(1,Q)
									SP_price=money(tmpAr(2,Q))
									SP_image=tmpAr(3,Q)
									if (trim(SP_image)<>"") AND (lcase(SP_image) <> "no_image.gif") then
										SP_image=SPathInfo & "pc/catalog/" & SP_image
									else
										SP_image=strProductImgURL
									end if
									SP_sku=tmpAr(4,Q)
									SP_GGen = tmpAr(5,Q)
									SP_GAge = tmpAr(6,Q)
									...
									SP_GMat = tmpAr(10,Q)
									SP_stock= tmpAr(11,Q)
									SPDisregardStock=tmpAr(12,Q)
									if IsNull(SPDisregardStock) or trim(SPDisregardStock)="" then SPDisregardStock=0
									SPBackOrderOK=tmpAr(13,Q)
									if IsNull(SPBackOrderOK) or trim(SPBackOrderOK)="" then SPBackOrderOK=0
									if (scOutOfStockPurchase=0) OR (Clng(SPDisregardStock)<>0) then
										SPAvailability = "in stock"
									else
										if CLng(SP_stock)<1 then
											If (scOutofStockPurchase=-1 AND strBTO=0 AND SPDisregardStock=0 AND SPBackOrderOK=1) OR (strBTO<>0 AND scOutofStockPurchase=-1 AND iBTOOutofstockpurchase=-1 AND SPDisregardStock=0 AND SPBackOrderOK=1) Then
												SPAvailability = "preorder"
											else
												SPAvailability = "out of stock"
											end if
										else
											SPAvailability = "in stock"
										end if
									...
									StringBuilderObj.append ClearHTMLTagsGB("/~/" & SP_name & "/~//*//~/" & strProductDesc & "/~//*/" & strPrdType,0) & "/*/" & strProductURL & "/*/" & SP_image & "/*/" & SP_sku & "/*/" & SPAvailability & "/*/" & SP_price & " " & strCurrency & "/*/" & strCondition & "/*//~/" & strBrandName & "/~//*/" & strExpDate
									if pcIntUniqueIdent=1 then
										StringBuilderObj.append "/*/" & tmpMPN & "/*/" & tmpGTIN
									end if
									if pcvBundleCol=1 then
									...
									StringBuilderObj.append "/*//~/" & pcv_GCat & "/~/"
									if pcIntIncludeApparel=1 then
										StringBuilderObj.append "/*/" & SP_GGen
										StringBuilderObj.append "/*/" & SP_GAge
										StringBuilderObj.append "/*/" & SP_GColor
										StringBuilderObj.append "/*/" & SP_GSize
										StringBuilderObj.append "/*/" & SP_GPat
										StringBuilderObj.append "/*/" & SP_GMat
										StringBuilderObj.append "/*/" & strSKU
									end if
									StringBuilderObj.append "/-/"
									response.write "<script type=text/javascript>document.getElementById('co').innerHTML='" & Count3 & "';</script>"
								Next
							end if
							set rsQ=nothing
						END IF
					...
					if Count3>=250 then
						...
						call pcs_AppendUTF8(File1,File1,strtext)
						Count2=Count2+Count3
						response.write "<script type=text/javascript>document.getElementById('co').innerHTML='" & Count2 & "';</script>"
						Count3=0
						strtext=""
	...
	response.write "<script type=text/javascript>document.getElementById('ea').style.display='none';</script>"%>
	<table class="pcCPcontent">
		<tr>
			...
			The Google Shopping data feed was created successfully.
				<br>
				<br>
				<b><%=Count%></b> products have been exported succefully to the Google Shopping data feed: <a href="<%=File1%>"><%=File1%></a>. To download the file, either right-click on the file name and select '<strong>Save Target As...</strong>' or '<strong>Save Link as...</strong>' or FTP into the <em><%=scAdminFolderName%></em> folder and download it. <a href="http://wiki.productcart.com/productcart/marketing-generate_google_base_file#uploading_or_scheduling" target="_blank">Learn more about submitting your Google Shopping file</a>.
				<br>
				<br>
				<br>
				<form class="pcForms">
					<input type="button" class="btn btn-default" name=back value="Create Another File" onClick="location='exportFroogle.asp'">&nbsp;
					<input type="button" class="btn btn-default" name=back value="Start Page" onClick="location='menu.asp'">
				</form>
				<br>
				<br>
			</td>
		...
		<tr>
			<td>
				...
				If statusAPP="1" OR scAPP=1 Then
					...
					query="SELECT Count(*) As tmpTotal FROM products WHERE (products.pcProd_ParentPrd>0) AND ((products.removed<>0) OR (pcProd_SPInActive<>0));"
					set rsQ=connTemp.execute(query)
					if not rsQ.eof then
						if rsQ("tmpTotal")>"0" then
							Count=Count+Clng(rsQ("tmpTotal"))
							response.write rsQ("tmpTotal") & " products are apparel sub-products and not included in the Google Product Search feed<br />"
						end if
					end if
					set rsQ=nothing
				...
				if not rsQ.eof then
					if rsQ("tmpTotal")>"0" then
					Count=Count+Clng(rsQ("tmpTotal"))
					response.write rsQ("tmpTotal") & " products are configurable items and not included in the Google Shopping feed<br />"
					end if
				end if
				set rsQ=nothing
...
IF request("action")<>"add" AND request("action")<>"gen" THEN
    ...
    <form name="form1" method="post" action="exportFroogle.asp?action=gen" onSubmit="return Form1_Validator(this)" class="pcForms">
        ...
        <tr>
            <td colspan="2">
                <ul>
                    ...
                    <li>Google Shopping now requires <a href="#unique">unique product identifiers</a>. See <a href="http://wiki.productcart.com/productcart/managing_search_fields#mapping_custom_search_fields_to_export_fields" target="_blank">how to leverage Customer Search Fields</a> for this purpose.</li>
                    <li>More: <a href="http://www.google.com/support/merchants/bin/answer.py?hl=en_US&answer=188494" target="_blank">Product Search Feed specifications</a> (Google Web site) | <a href="http://wiki.productcart.com/productcart/marketing-generate_google_base_file" target="_blank">How to use this feature</a> (ProductCart Wiki).</li>
              </ul>
            </td>
        </tr>
        <tr>
        ...
        <tr>
        	...
        	<td>Yes, I have to use unique product identifiers - <a href="http://wiki.productcart.com/productcart/managing_search_fields#mapping_custom_search_fields_to_export_fields" target="_blank" class="pcSmallText">Learn how to associate these values to products</a></td>
        </tr>
        <tr>
        ...
        <tr>
            <td colspan="2" align="center">
                <input type="submit" name="Submit" value=" Generate Google Shopping Data Feed" class="btn btn-primary">
                <%
                '// Loading Window
