...
If (scConfirmEmail<>"" and pcv_CustomerReceived=0) or (scReceivedEmail<>"" and pcv_CustomerReceived=1) Then
	...
	customerEmail=customerEmail & "<br>" & vbcrlf & personalmessage & "<br>" & vbcrlf
End If

if pcv_AdmComments<>"" then
	customerEmail=customerEmail & "<br>" & vbcrlf & replace(pcv_AdmComments,"''","'") & "<br><br>" & vbcrlf
end if
...
if pVATID<>"" AND pcv_strShowVatId=-1 Then
	customerEmail=customerEmail & pVATID & "<br>" & vbcrlf
end if

If pSSN<>"" AND pcv_strShowSSN=-1 Then
	customerEmail=customerEmail & pSSN & "<br>" & vbcrlf
End If
...
customerEmail=customerEmail & pPhone & "<br>" & vbcrlf
...
if geHideAddress=0 then 
	...
	If Trim(pshippingAddress) <> "" Then
		...
		customerEmail=customerEmail & pshippingPhone & "<br>" & vbcrlf
	Else
...
If DFShow = "1" and pord_DeliveryDate <> "//" Then
	customerEmail=customerEmail & "<br>" & vbcrlf & DFLabel & " " & pord_DeliveryDate & "<br>" & vbcrlf
End If
...
'DA-EDIT Check for VAT applicable Status
...
Do While Not rsOrderDetails.EOF
	...
	pPrice=rsOrderDetails("unitPrice")/1.2
...
customerEmail=customerEmail & FixedField(15, "R", scCurSign & money(iSubTotal))
customerEmail=customerEmail & "<br>" & vbcrlf
...
If Postage<>0 Then
	...
	'else
		'customerEmail=customerEmail & FixedField(15, "R", scCurSign & money(Postage))
	'end if
	customerEmail=customerEmail & "<br>" & vbcrlf
End If
...
'DA-Edit VAT ...
If boolVAT=1 Then
	daVATCalc = ptotal-(ptotal/1.2)
	customerEmail=customerEmail & FixedField(65, "R", "VAT: ")
	customerEmail=customerEmail & FixedField(15, "R", scCurSign & money(daVATCalc))
	customerEmail=customerEmail & "<br>" & vbcrlf
Else
	customerEmail=customerEmail & FixedField(65, "R", "VAT: ")
	customerEmail=customerEmail & FixedField(15, "R", scCurSign & money(0))
	customerEmail=customerEmail & "<br>" & vbcrlf
End If
...
IF DPOrder="1" then
	...
	do while not rs11.eof
		...
		If pcv_strShowSection = -1 Then
			...
			CustomerEmail=customerEmail & dictLanguage.Item(Session("language")&"_sendMail_28") & pName & "<br>" & vbcrlf & "<br>" & vbcrlf
...
IF pGCs="1" then
	...
	do while not rs11.eof
		...
		If pcv_strShowSection = -1 Then
			...
			customerEmail=customerEmail & dictLanguage.Item(Session("language")&"_sendMail_43") & pName & "<br>" & vbcrlf & "<br>" & vbcrlf
...
IF pGCs="1" then
	...
	if GcReEmail<>"" then
		...
		if GcReMsg<>"" then
			ReciEmail3=replace(dictLanguage.Item(Session("language")&"_sendMail_72"),"<customer name>",pCustomerFullNamePlusEmail) & "<br>" & vbcrlf & GcReMsg & "<br>" & vbcrlf
		else
		ReciEmail3=""
		end if
