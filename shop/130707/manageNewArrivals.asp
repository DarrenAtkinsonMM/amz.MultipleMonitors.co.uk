...
if request("action")="upd" then
	...
	if not rs.eof then
		dim rsUpdObj
		query="UPDATE pcNewArrivalsSettings SET pcNAS_NewArrCount=" & pcIntNewArrCount & ",pcNAS_Style='" & pcStrPageStyle & "',pcNAS_PageDesc=N'" & pcStrNewArrDesc & "',pcNAS_NDays=" & pcIntNDays & ",pcNAS_NotForSale=" & pcIntNewArrNFS & ",pcNAS_OutOfStock=" & pcIntNewArrInStock & ",pcNAS_SKU=" & pcIntShowSKU & ",pcNAS_ShowImg=" & pcIntShowSmallImg & ";"
		set rsUpdObj=server.CreateObject("ADODB.RecordSet")
		set rsUpdObj=connTemp.execute(query)
		set rsUpdObj=nothing
	else
		dim rsInsObj
		query="INSERT INTO pcNewArrivalsSettings (pcNAS_NewArrCount,pcNAS_Style,pcNAS_PageDesc,pcNAS_NDays,pcNAS_NotForSale,pcNAS_OutOfStock,pcNAS_SKU,pcNAS_ShowImg) VALUES (" & pcIntNewArrCount & ",'" & pcStrPageStyle & "',N'" & pcStrNewArrDesc & "'," & pcIntNDays & "," & pcIntNewArrNFS & "," & pcIntNewArrInStock & "," & pcIntShowSKU & "," & pcIntShowSmallImg & ");"
...
if not rs.eof then
	...
	if pcStrNewArrDesc<>"" then
		pcStrNewArrDesc=replace(pcStrNewArrDesc,"""","&quot;")
		pcStrNewArrDesc=replace(pcStrNewArrDesc,"<","&lt;")
		pcStrNewArrDesc=replace(pcStrNewArrDesc,">","&gt;")
	end if
...
function Form1_Validator(theForm)
{
  ...
  theForm = document.manageNewArrivals;
...
<form action="manageNewArrivals.asp?action=upd" method="post" name="manageNewArrivals" class="pcForms" onSubmit="return Form1_Validator(this)">
	<table class="pcCPcontent">
		...
		<tr valign="top">
			...
			<td width="75%">
				<textarea class="htmleditor" id="pcStrNewArrDesc" name="pcStrNewArrDesc" cols="50" rows="6"><%=pcStrNewArrDesc%></textarea>
			</td>
		</tr>
		<tr> 
			<td colspan="2" class="pcCPspacer"></td>
