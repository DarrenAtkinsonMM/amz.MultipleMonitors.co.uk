<%@LANGUAGE="VBSCRIPT"%>
<% '--Updater File-- %>
<% pageTitle = "Authorize.Net Direct Post Method (DPM) Gateway - Database Update" %>
<% Section="paymntOpt" %>
<%PmAdmin=5%>
...
IF request("action")="sql" then
	if request.querystring("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			call closeDb()
response.redirect "upddbAuthorizeDPM.asp?mode=3"
			response.End
		end if
...
response.redirect "techErr.asp?error="&Server.Urlencode("Error while opening database")
		...
		if instr(ucase(scDSN),"DSN=") then
			call closeDb()
response.redirect "upddbAuthorizeDPM.asp?mode=1"
			response.End
		end if
	...
	'**** Create table pcPay_AuthorizeDPM ***************************************
...
<form action="upddbAuthorizeDPM.asp" method="get" name="form1" class="pcForms">
<%
if mode="complete" then 
	call closeDb()
response.redirect "pcConfigurePayment.asp?gwchoice=1103"
	response.end
else %>
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
				...
				<span style="font-weight: bold">You must update your database before you can add Authorize.Net DPM Gateway.<u></u></span><br>
				In order to activate Authorize.Net DPM real-time payment gateway you will need to update your ProductCart database to add the required table.</div>				<p><strong><br>
				    <br>
			...
			<% end if %>
				<tr>
					...
					<% if request.querystring("mode")="1" OR request.querystring("mode")="3" then %>
						...
						<% else %>
					  <input type="button" class="btn btn-default"  name="access2" value="Update Your ProductCart MS SQL Database" onClick="location='upddbAuthorizeDPM.asp?action=sql';" class="btn btn-primary">
						<% end if %>
					</td>
			</tr>
			</table>
