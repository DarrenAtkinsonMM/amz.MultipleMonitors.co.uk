<% 
PmAdmin=19
pageTitle = "ProductCart Advanced Security - Database Upgrade" 
Section = "" 
%>
...
IF request("action")="sql" then
	if request("hmode")="2" then
		...
		if SSIP="" or UID="" or PWD="" then
			call closeDb()
			response.redirect "upddb_advancedSecurity.asp?mode=3"
			response.End
		end if
		...
		if instr(ucase(scDSN),"DSN=") then
			call closeDb()
			response.redirect "upddb_advancedSecurity.asp?mode=1"
			response.End
		end if
	...
	'========================================================================
	'// START OF DB UPDATES FOR Advanced Security
	'========================================================================
  ...
  if not TableExists("pcTransactionLogs") then
    ...
    query=query&"datetime [datetime] NULL,"
		query=query&"IP [nvarchar] (20) NOT NULL,"
	...
	'//  Add column pcCust_FailedPaymentCount for table "customers"
	query="ALTER TABLE [customers] ADD [pcCust_FailedPaymentCount] [int] DEFAULT (0) NOT NULL"
	set rs=server.CreateObject("ADODB.RecordSet")
	set rs=conntemp.execute(query)
	if err.number <> 0 then
	...
	'========================================================================
	'// END OF DB UPDATES FOR Advanced Security
	'========================================================================
...
<form action="upddb_advancedSecurity.asp" method="post" name="form1" id="form1" class="pcForms">
<%
	...
	<table class="pcCPcontent" style="width:600px;" align="center">
		...
		<tr>
			<td>
                ...
                <h1 class="page-header">Advanced Security Database Update</h1>
                <p class="lead">
