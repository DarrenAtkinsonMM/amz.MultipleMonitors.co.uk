...
if request("a")="clone" then
	...
	if IsNumeric(pcSaleID) AND pcSaleID>"0" then
		...
		if not rs.eof then
			...
			query="INSERT INTO pcSales (pcSales_TargetPrice,pcSales_Type,pcSales_Relative,pcSales_Amount,pcSales_Round,pcSales_Name,pcSales_Desc,pcSales_ImgURL,pcSales_CreatedDate,pcSales_Param1,pcSales_Param2,pcSales_Tech) VALUES (" & pcArr(0,i) & "," & pcArr(1,i) & "," & pcArr(2,i) & "," & pcArr(3,i) & "," & pcArr(4,i) & ",N'" & pcArr(5,i) & " (Cloned)" & "',N'" & pcArr(6,i) & "','" & pcArr(7,i) & "','" & dtTodaysDate & "','" & pcArr(9,i) & "','" & pcArr(10,i) & "','" & pcArr(11,i) & "');"
			set rs=connTemp.execute(query)
			set rs=nothing
...
if rs.eof then
	...
	<tr>
	<td colspan="5">
		<input type="button" class="btn btn-default"  name="Go" value=" Start A New Sale " onclick="location='sm_start.asp';" class="btn btn-primary">	
	</td>
	</tr>
<%else
	pcArr=rs.getRows()
...
<tr align="center">
	<td colspan="5">
		<input type="button" class="btn btn-default"  name="Go" value="Create New Sale" onclick="location='sm_addedit_S1.asp?a=new';" class="btn btn-primary">
        &nbsp;
		<%if SMArchived<>1 then
        ...
        set rs=connTemp.execute(query)
        if not rs.eof then%>
        <input type="button" class="btn btn-default"  value="Show Archived Sales" onClick="location='sm_sales.asp?arc=1'">
        <%end if
        set rs=nothing
        else%>
        <input type="button" class="btn btn-default"  value="Show Running &amp; Completed Sales" onClick="location='sm_sales.asp'">
        <%end if%>
